...
llvm::Error discardTemporaryOutput(llvm::sys::fs::TempFile &Temporary) {
  ...
  if (!CloseFailed)
    return llvm::createStringError(
        llvm::errc::io_error,
        "cannot remove temporary translation output %s: %s",
        TemporaryPath.c_str(), Detail.c_str());

  llvm::Error CloseError = llvm::createStringError(
      llvm::errc::io_error, "cannot close temporary translation output %s: %s",
      TemporaryPath.c_str(), Detail.c_str());
  ...
  return llvm::joinErrors(
      std::move(CloseError),
      llvm::createStringError(RemoveError,
                              "cannot remove temporary translation output %s",
                              TemporaryPath.c_str()));
}
  ...
  if (std::error_code Error = Operations.Rename(TemporaryPath, OutputPath)) {
    return llvm::joinErrors(
        llvm::createStringError(
            Error, "cannot atomically replace translation output %s with %s",
...
llvm::Error closeAndCommitTemporaryOutput(llvm::sys::fs::TempFile &Temporary,
  ...
  auto Close = [&]() -> llvm::Error {
    ...
    if (RenameHandleError) {
      ...
      return llvm::createStringError(
          RenameHandleError,
          "cannot retain temporary translation output %s for atomic "
          "replacement",
          TemporaryPath.c_str());
    }
    ...
    if (CloseError) {
      ...
      CloseError = llvm::createStringError(
          llvm::errc::io_error,
          "cannot close temporary translation output %s: %s",
          TemporaryPath.c_str(), Detail.c_str());
    }
#ifdef _WIN32
    if (CloseError)
  ...
  auto Rename = [&](llvm::StringRef From,
    ...
    const AtomicOutputReplaceOperations ReplaceOperations{Replace};
    return replaceTemporaryOutputWithoutMoveAside(From, To, ReplaceOperations);
#else
  ...
  auto Register = [&](llvm::StringRef Path) -> llvm::Error {
    ...
    // registration in place is what closes the former keep()/register gap.
    (void)Path;
    return llvm::Error::success();
#else
    // If delete-on-close was unavailable, TempFile::create() already installed
    // signal cleanup.  Otherwise add it before clearing the delete disposition,
    // so one cleanup mechanism is active throughout the handoff.
    if (Temporary.RemoveOnClose)
      return llvm::Error::success();
    std::string Detail;
    ...
    return llvm::createStringError(
        ...
        "cannot register temporary translation output for signal cleanup: %s",
        Detail.c_str());
#endif
  };
  ...
  const AtomicOutputCommitOperations Operations{Close, Rename, Discard,
                                                Register, Unregister};
  ...
  // Rename consumed the only temporary pathname.  Mark the TempFile complete
