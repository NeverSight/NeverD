...
- `EVMPrecompiles.def` — словарь адресов, на которых отвечает сам протокол, с
  форком, зарезервировавшим каждый из них. Газ намеренно отсутствует: стоимость
  precompile является функцией её входа и переоценивалась без изменения адреса
  или операции.
