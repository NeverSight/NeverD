...
**الحالة:** تنفيذ P0 موجود لـ PE وELF وMach-O، لكن الإغلاق ينتظر محوّل إعادة تشغيل لمدخلات العملية وملخصات مكتملة لتأثيرات الاستدعاء. تغطية الأحكام والهوية مثبتة بـ [`unittests/safety`](../../unittests/safety) واختبار الطرف إلى الطرف [`SafetyIntegrationTests.cpp`](../../unittests/safety/SafetyIntegrationTests.cpp) الذي يشغّل على كل مضيف مصفوفة PE/ELF/Mach-O × x86-64/AArch64 الإلزامية. انظر [تدقيق وصيد أمان الذاكرة](../memory-safety.ar.md). يتسع P1 إلى فيضان المكدس/العالمي والقراءات غير المهيأة وسلاسل التنسيق.
...
| تدقيق وصيد أمان الذاكرة | قيد التنفيذ — تنفيذ P0 موجود؛ الإغلاق بانتظار إعادة التشغيل وملخصات الاستدعاء |
