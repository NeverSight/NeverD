...
`DriverResources.h` e `DriverResources.def` definiscono assegnazioni fisse `register_bank`, accessi e larghezze dei registri, nomi dei campi e limiti condivisi. `DriverScenario` convalida JSON e C++ prima dell’esecuzione; `DriverResult` registra la configurazione senza duplicare lo stato osservato del banco. `KernelMMIO` gestisce per ogni PDO valori persistenti, intervalli fisici tradotti, generazioni delle risorse, accessibilità hardware e alias indipendenti. `KernelModelResources` gestisce le liste START raw/tradotte impacchettate e di sola lettura e l’integrazione con provider e ciclo di vita. Il successo effettivo di START inferiore pubblica la generazione prima dei callback superiori; il SET di dispositivo del provider aggiorna l’accessibilità D0/D3. La pulizia dopo START fallito o STOP/REMOVE viene verificata prima del completamento terminale dell’IRP; i callback superiori di completamento possono prima rimuovere i mapping, senza rilascio implicito. La rimozione improvvisa vieta subito l’accesso hardware. `GuestMemory` e `UnicornBackend` convalidano l’intero accesso CPU/API originale prima dei callback dei registri, condividendo il primo errore/arresto affinché la suddivisione di Unicorn non produca effetti successivi. Il riavvio conserva i valori dell’assegnazione fissa; gli alias non possiedono copie separate dei byte. Questo bus limitato di registri di memoria non implementa RAM arbitraria, ribilanciamento delle risorse, porte, interrupt o DMA.

`KernelFramework` gestisce i binding KMDF 1.33, l’identità della tabella, gli oggetti e i contesti WDF, gli inizializzatori dei dispositivi di controllo, le code predefinite sequenziali e gli handle delle richieste. Le sue interfacce tipizzate per dispositivi e richieste delegano a `KernelModel` lo spazio dei nomi WDM, la memoria, lo stato dei pacchetti, la mappatura MDL e la validazione del completamento; nessuna delle due parti crea dispositivi o IRP duplicati. L’instradamento delle code mantiene lo stato di dispatch gestito dal framework separato dal ritorno del callback guest di tipo `void`. Le continuazioni del completamento eseguono la pulizia e la distruzione degli oggetti figli prima di rilasciare l’IRP, mentre i riferimenti esterni mantengono soltanto il contesto WDF. L’eliminazione delle richieste pendenti viene rifiutata prima di modificare gli antenati; annullamento automatico e svuotamento durante l’eliminazione restano esclusi. `DriverSession` esegue callback annidati con budget condivisi. `DriverImage` valida i metadati CFG; `GuardControlFlow` gestisce le destinazioni dichiarate immagine/API e l’adattatore CPU preserva lo stato delle chiamate check/dispatch. Dispositivi PnP, pianificazione generale delle code e relativo annullamento, estensioni di classe e UMDF restano non supportati.

Lo scenario imposta con `cancel_after_100ns` una scadenza virtuale per trasferimento. Il record di ogni IRP in `KernelModel` possiede scadenza e istante assoluto effettivo `cancel_requested_at_100ns`; gli scenari pubblici restano seriali. `KernelModel` applica il ritardo zero dopo l’instradamento e prima del callback I/O guest, conserva i completamenti precedenti e include le scadenze positive nell’avanzamento a riposo. `KernelFramework` gestisce marcatura/rimozione, accodamento/consegna e riferimento interno fino al ritorno del callback. L’accodamento non autorizza il completamento; dopo la consegna, un elemento di lavoro può coordinarlo con il callback in attesa. Conservare WDF non rende nuovamente valido l’IRP completato. `KernelScheduler` separa annullamento e lavoro, conserva il tipo in sospensione/ripresa e condivide i budget di capacità e dispatch. I DPC precedono gli annullamenti FIFO, poi vengono lavoro ordinario e ripresa delle attese passive pronte. I callback di annullamento eseguono a `PASSIVE_LEVEL`. Questo contratto dei dispositivi di controllo non fornisce routine di annullamento WDM né uno scheduler generale delle code.
