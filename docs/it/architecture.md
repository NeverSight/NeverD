...
`KernelFramework` gestisce binding KMDF 1.33, identità della tabella, driver/oggetti generici e durata dei contesti tipizzati. `DriverSession` esegue callback guest annidati e riprende l’operazione sospesa con budget condivisi. `DriverImage` valida i metadati CFG; `GuardControlFlow` gestisce le destinazioni dichiarate immagine/API e l’adattatore CPU preserva lo stato check/dispatch. Non aggiunge dispositivi, code, richieste, estensioni di classe KMDF o UMDF.
