...
`KernelModelIRPStack` gestisce cursori limitati, dispatch alla destinazione esatta e svolgimento del completamento sul pacchetto originale. Le scritture inline Copy/Skip/SetCompletion restano autorevoli. Stato di dispatch, controllo di completamento e `IoStatus` finale sono distinti; pending può propagarsi dopo il ritorno. `STATUS_MORE_PROCESSING_REQUIRED` conserva IRP/MDL/buffer fino alla ripresa dello svolgimento finale, anche nel completamento annidato. `KernelGuestCall` contiene sottosistema proprietario e token locale contro collisioni WDM/WDF; `DriverSession` conserva frame CPU e IRQL ereditato. Un driver guest può collegarsi sopra PDO dello scenario con proprietà separata; gli IRP allocati dal driver e gli IRP di alimentazione restano non supportati. Collegamento/inoltro WDF, estensione di stack attivi, scollegamento intermedio, cambio della funzione principale e destinazioni esterne restano esclusi. Ogni posizione inferiore consumata viene azzerata prima del callback di completamento superiore.

`DriverPnp.h` e il file pubblico `DeviceLifecycle.def` definiscono le enumerazioni condivise del ciclo di vita e i dati tipizzati di scenari e rapporti. `KernelModelPnpDevices` gestisce l’identità stabile dei PDO, un inventario separato del driver provider e le osservazioni effettive di AddDevice. `KernelModelPnpRequests` collega le transazioni del ciclo di vita e l’identità immutabile del dispositivo al record IRP esistente; l’ammissione delle operazioni sui file usa tale identità. L’associazione FDO/PDO e l’identità del ciclo di vita del file persistono dopo lo scollegamento. `KernelModelPnpCompletion` gestisce la ricezione e il completamento effettivi del bus e le scadenze virtuali limitate. Riutilizza `KernelModelIRPStack` e continuazioni con proprietario identificato, senza un secondo pacchetto o callback guest artificiali. Solo il completamento superiore definitivo conferma lo stato del ciclo di vita. Un’operazione PnP riuscita deve raggiungere il provider ed essere completata al suo interno; REMOVE e CANCEL_REMOVE richiedono esattamente `STATUS_SUCCESS` (0). START e QUERY_REMOVE possono fallire prima in un livello superiore, lasciando null le osservazioni del bus. Il ritiro del provider e lo scollegamento/eliminazione da parte del guest sono transizioni di proprietà distinte; i dispositivi guest non rilasciati non vengono mai eliminati silenziosamente. L’ambito pubblico comprende Start/QueryRemove/CancelRemove/Remove senza risorse, non PnP generale, alimentazione o KMDF PnP.
