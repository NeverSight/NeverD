...
Un callback Swift di inizializzazione unica di una stringa immutabile può essere proiettato quando il chiamante completo di sei istruzioni, il corpo condiviso di dodici e l’accessore di indirizzo di tre dimostrano un percorso esatto. Restano entrambe le scritture da otto byte, nel loro ordine, e la chiamata reale a `swift_bridgeObjectRetain`. La stringa con tag viene verificata e il suo indirizzo ricostruito nello spazio condiviso dell’intera sezione dei letterali, preservando anche lo spazio condiviso del simbolo di destinazione. Byte correnti, audit completi, conservazione del frame, importazioni forti e corpo finale sono riconvalidati; nessuna ABI generale viene assegnata al corpo condiviso o all’accessore.
