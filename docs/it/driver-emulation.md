...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | MDL delle richieste, mapping di sistema e identità PFN condivise in sola lettura | Interfacce DMA fuori dal sottoinsieme descritto sotto |
| MDL allocati dal driver | Descrittori autonomi o associati a IRP, MDL parziali, mapping utente/sistema condivisi e rilascio al completamento | Quote, MDL manuali, spazi di processo arbitrari |
...
| WDM `METHOD_NEITHER` | Buffer utente separati, sonde, MDL bloccati, identità sintetiche, revoca VA o uscita dopo il dispatch e annullamento limitato | Nessuna tabella delle pagine o gestione generale dei processi; sono supportate viste MDL limitate per PID |
...
| Driver PnP di bus, funzione o filtro | PDO espliciti senza risorse o con banchi di registri, AddDevice guest e otto funzioni minori comuni del ciclo PnP | Altre operazioni PnP, politica generale di alimentazione, hardware/risorse generali e KMDF PnP generale |
...
| Driver hardware, DMA, PCI, di interrupt o di virtualizzazione | Banchi espliciti, MMIO, interrupt latched/di livello esclusivi o condivisi e DMA coerente common/SG/channel | Altri modelli di dispositivi, RAM fisica arbitraria, PCI, porte, altre interfacce DMA e stato CPU privilegiato |
...
`register_bank` accetta `interrupts` da solo o con `resources`; almeno una lista deve essere non vuota. `resource_free` rifiuta il campo anche vuoto. Ogni linea richiede `id`, `raw_vector`, `raw_level`, `raw_affinity`, `translated_vector`, `translated_level`, `translated_affinity`, `mode` (`latched` o `level_sensitive`) e `share` (`device_exclusive` o `shared`). Le sorgenti di livello richiedono `retrigger_after_100ns` positivo fino a INT64_MAX; quelle latched lo rifiutano. Limiti: 8 linee per PDO, 32 totali, ID ASCII limitati e unici per PDO, livello grezzo 0–65535 e DIRQL tradotto 3–12. Entrambe le affinità sono 1 (CPU0/gruppo0); i vettori mantengono 32 bit. Condividere un vettore tradotto richiede uguali livello, affinità, modalità e intervallo e `share: "shared"` per tutti. I descrittori di memoria precedono quelli di interrupt ordinati, con flag coerenti di modalità e condivisione. Sono linee sintetiche dichiarate, non un controller PCI. `DriverInterrupts.h` / `DriverInterrupts.def` definiscono nomi e limiti.

READ/WRITE/IOCTL possono dichiarare `interrupt_events` con `after_100ns`, `device_id` e `interrupt_id`. `action` predefinita è `pulse` per latched; le sorgenti di livello richiedono `assert` o `deassert` e rifiutano `pulse`. Le linee latched richiedono `pulse`. Gli altri tipi rifiutano il campo anche vuoto. Limiti: 64 eventi per richiesta, 1024 dichiarati totali, ritardo non negativo fino a INT64_MAX. L’invio acquisisce connessioni e generazione PDO attive; il completamento IRP non cancella gli eventi. La consegna avviene ai confini dei callback, senza preemption di istruzioni; il tempo avanza quando inattivo. La pubblicazione hardware precede l’ammissione e gli ISR, ordinati per DIRQL assegnato, precedono DPC/worker. Connessione persa, generazione indisponibile o D3 producono `undelivered_reason`, senza riassegnazione implicita.

Un impulso latched condiviso invoca tutti gli ISR acquisiti nell’ordine di registrazione, anche dopo TRUE. La linea di livello è l’OR degli stati per PDO/sorgente/generazione; ripetere `assert` è idempotente. Tutte le modifiche contemporanee si applicano prima del campionamento. Se ancora attiva, la linea viene ricampionata alla fine del giro ISR più `retrigger_after_100ns`. Per il livello, TRUE ferma la ricerca del giro ma non riconosce né disattiva una sorgente; lo fa soltanto `deassert`. Le scritture dei registri non implicano acknowledgement. Sono ammessi 4096 giri di consegna, ciascuno con massimo 32 handler, oltre ai budget di callback e istruzioni. Disconnessione e ritiro del dispositivo rifiutano sorgenti ancora attive.

`IoConnectInterrupt` usa undici argomenti e l’esatta assegnazione tradotta. `IoConnectInterruptEx` supporta FullySpecified (1), LineBased (2, una linea del PDO esplicito) e FullySpecifiedGroup (4, gruppo0); la disconnessione richiede versione e contesto corrispondenti. Registrazione e disconnessione richiedono PASSIVE_LEVEL. Il lock può essere privato o un `KSPIN_LOCK` non paginato inizializzato dal chiamante; più connessioni possono condividerlo con uguale `SynchronizeIrql`, almeno pari a ogni DIRQL assegnato. Zero in LineBased sceglie tale livello; la priorità dello scheduler non cambia. Restano CPU0/gruppo0 e nessun salvataggio dello stato floating point. L’ISR reale riceve il `KINTERRUPT` opaco e ServiceContext e restituisce BOOLEAN in AL. ISR, `KeSynchronizeExecution` e acquire/release manuali condividono il vero lock non ricorsivo. Synchronize opera all’IRQL di sincronizzazione e ripristina IRQL/CR8; proprietà e IRQL salvato sono verificati. Le API di spin lock executive non possono usare lock connessi. preemption di istruzioni restano esclusi. START fallito e STOP/REMOVE riusciti richiedono disconnessione prima del completamento finale; i DPC accodati non vengono eliminati implicitamente.

`configuration.pnp_devices[].interrupts` conserva le risorse e `configuration.interrupt_events` le dichiarazioni. Le osservazioni `interrupts` contengono `source_request_index`, `event_index`, `device_id`, `interrupt_id`, `action`, `epoch`, `due_at_100ns`, `occurred_at_100ns`, `delivered_at_100ns`, `returned_at_100ns`, `interrupt_object`, `return_value`, `claimed` e `undelivered_reason`. `handlers[]` registra ogni ISR realmente invocato: oggetto, istanti, BOOLEAN, `claimed` e `delivery_index`. Un’asserzione può produrre più giri; deassert o assert annullato allo stesso istante non inventano ISR. `occurred_at_100ns` indica l’applicazione della modifica di stato. `claimed` aggrega i ritorni reali; un interrupt non rivendicato resta valido. `delivered_at_100ns` indica la prima consegna; `returned_at_100ns` e `claimed` descrivono l’ultimo ciclo, mentre `handlers[]` conserva l’intera cronologia. Lo [scenario di interrupt](../examples/driver-interrupt-scenario.json) eseguibile usa l’originale `driver_wdm_interrupts.c` compilato con il vero WDK e gli artefatti opzionali `NEVERD_WDM_INTERRUPT_FIXTURE` / `NEVERD_WDM_INTERRUPT_CFG_FIXTURE`: sette richieste includono START ritardato, IOCTL pending completato da ISR→DPC, cleanup/close del file e rimozione. L’ingresso C/Python `scenario_json` e il layout `neverd_driver_options_v1` restano invariati. Gli artefatti mancanti sono saltati esplicitamente; le prove di esecuzione riguardano solo Linux.
...
KMDF 1.33 usa l’ABI esatta 1.33.0: 458 slot di funzione hanno identità guest stabili e le 99 API seguenti hanno semantica di esecuzione. `WdfVersionBind` e `WdfVersionUnbind` gestiscono il binding guest attorno al vero wrapper WDK `FxDriverEntry`. `WdfGetDriver` legge le variabili globali pubbliche del driver. Driver non PnP, oggetti generici, dispositivi di controllo, code e richieste in ingresso condividono contesti tipizzati, conteggi dei riferimenti e callback eseguiti di pulizia/distruzione/scaricamento. Tranne le DDI di interrupt e le operazioni su richieste e oggetti con supporto IRQL esplicito descritte sotto, le chiamate e i callback del framework richiedono `PASSIVE_LEVEL`; aggiungere riferimenti dopo il completamento della pulizia resta fuori da questo profilo. Slot non modellati, `WdfLdrQueryInterface`, estensioni di classe e UMDF arrestano esplicitamente l’esecuzione.

API di interrupt: `WdfInterruptCreate`, `WdfInterruptQueueDpcForIsr`, `WdfInterruptQueueWorkItemForIsr`, `WdfInterruptSynchronize`, `WdfInterruptAcquireLock`, `WdfInterruptReleaseLock`, `WdfInterruptEnable`, `WdfInterruptDisable`, `WdfInterruptWdmGetInterrupt`, `WdfInterruptGetInfo`, `WdfInterruptGetDevice`.

Gli oggetti interrupt condividono risorse di linea o MSI assegnate e blocchi interni. ISR e callback sincronizzati usano il DIRQL assegnato, oppure `PASSIVE_LEVEL` per gestione passiva; `WdfInterruptGetInfo` riporta tale IRQL. I DPC usano `DISPATCH_LEVEL`, i work item `PASSIVE_LEVEL`. Inserimenti ripetuti si uniscono mentre accodati e la sospensione conserva la proprietà fino al ritorno. L’accensione connette e abilita dopo D0Entry; lo spegnimento disabilita, disconnette e attende i callback differiti prima di D0Exit e del rilascio delle risorse. Abilitazione/disabilitazione esplicite eseguono callback reali senza inventare impulsi. Restano esclusi `WDFSPINLOCK`/`WDFWAITLOCK` esterni, serializzazione automatica del padre, interrupt di risveglio e connessioni inactive mantenute, oltre a politiche idle/wake e rienumerazione automatica dei dispositivi guasti.

Le seguenti API per richieste e oggetti supportano IRQL fino a `DISPATCH_LEVEL`: `WdfObjectDereferenceActual`, `WdfRequestComplete`, `WdfRequestCompleteWithInformation`, `WdfRequestRetrieveInputBuffer`, `WdfRequestRetrieveOutputBuffer`, `WdfRequestRetrieveInputMemory`, `WdfRequestRetrieveOutputMemory`, `WdfRequestRetrieveInputWdmMdl`, `WdfRequestRetrieveOutputWdmMdl`, `WdfRequestGetInformation`, `WdfRequestSetInformation`, `WdfRequestGetIoQueue`, `WdfRequestGetFileObject`, `WdfRequestWdmGetIrp`, `WdfRequestGetParameters`, `WdfRequestGetStatus`, `WdfRequestMarkCancelableEx`, `WdfRequestUnmarkCancelable`, `WdfRequestIsCanceled`, `WdfMemoryGetBuffer`.

`WdfObjectGetTypedContextWorker`, `WdfObjectContextGetObject`, `WdfObjectReferenceActual` supportano anche gli IRQL di interrupt modellati; l’accesso al contesto e l’acquisizione di riferimenti non invocano callback passivi.

Il completamento di una richiesta o il rilascio dell’ultimo riferimento a un oggetto sopra `PASSIVE_LEVEL` pianifica la pulizia, la distruzione e le consegne successive effettive a `PASSIVE_LEVEL`. La vecchia API `WdfRequestMarkCancelable` resta limitata a `PASSIVE_LEVEL` per mantenere sincrono il callback quando la richiesta è già annullata. Le altre API mantengono i vincoli IRQL esistenti.
...
La struttura `WDF_IO_QUEUE_CONFIG` di 96 byte supporta code predefinite e non predefinite manuali, sequenziali e parallele con limite finito o illimitato con esecuzione passiva esplicita e senza sincronizzazione del framework. Le code dei dispositivi di controllo non sono gestite dall’alimentazione. I callback specifici READ/WRITE/IOCTL hanno precedenza su quello predefinito. Le richieste accettate nella coda restituiscono `STATUS_PENDING` anche se completate in modo sincrono; il registro di ritorno di un callback void non completa la sua richiesta. Il completamento differito usa lo scheduler esistente. Senza gestore la richiesta si completa con `STATUS_INVALID_DEVICE_REQUEST`; READ/WRITE di lunghezza zero si completa senza consegna, salvo abilitazione di quest’ultima. Il pacchetto di file predefinito completa CREATE/CLEANUP/CLOSE con successo e Information=0. Le code manuali non predefinite ricevono le richieste tramite `WdfRequestForwardToIoQueue`; `WdfIoQueueRetrieveNextRequest` le restituisce in ordine FIFO. Se una richiesta viene annullata prima del recupero, il framework la rimuove e la completa con `STATUS_CANCELLED`. Le code automatiche non predefinite consegnano le richieste inoltrate ai propri callback; la coda manuale predefinita trattiene le richieste in arrivo fino al recupero. `WdfIoQueueRetrieveNextRequest` funziona per code manuali e sequenziali; le code parallele restituiscono `STATUS_INVALID_DEVICE_STATE`. Senza un callback appropriato, la coda automatica completa la richiesta con `STATUS_INVALID_DEVICE_REQUEST` quando si libera uno slot di presentazione. Il comportamento PnP e di alimentazione più generale resta fuori da questo profilo. `WdfRequestRequeue` rimette una richiesta recuperata in testa alla stessa coda manuale. `NumberOfPresentedRequests` limita le richieste presentate in parallelo; le altre attendono il completamento o l’annullamento di una richiesta presentata. Una coda sequenziale predefinita accetta altre richieste mentre una è presentata; le successive attendono in ordine FIFO che si liberi uno slot e possono essere annullate prima della consegna. `WdfIoQueueStop` sospende la consegna ma continua ad accettare richieste. `WdfIoQueueStart` consegna quelle in attesa e `WdfIoQueueGetState` riporta i conteggi delle richieste in coda e consegnate. Il recupero durante l’arresto restituisce `STATUS_WDF_PAUSED`; il callback di completamento dell’arresto riceve il contesto indicato quando tutte le richieste già consegnate sono completate o hanno lasciato la coda; quelle in attesa non lo ritardano. Una seconda registrazione mentre è in attesa viene rifiutata.

`WdfIoQueueReadyNotify` registra un callback `EvtIoQueueState` per una coda manuale. A `PASSIVE_LEVEL` riceve `(WDFQUEUE, WDFCONTEXT)` quando il numero di richieste in coda passa da zero a un valore positivo, anche se il driver possiede ancora richieste già estratte. Una coda già non vuota può notificare subito dopo la registrazione; una coda arrestata attende `WdfIoQueueStart`. Una seconda registrazione o l’annullamento prima dell’arresto restituisce `STATUS_INVALID_DEVICE_REQUEST`. Dopo `WdfIoQueueStop`, NULL annulla la registrazione.

`WdfIoQueueFindRequest` cerca in una coda manuale senza trasferire la proprietà della richiesta e aggiunge un riferimento quando riesce. Il driver lo rilascia con `WdfObjectDereference`. `WdfIoQueueRetrieveFoundRequest` trasferisce la proprietà di una richiesta ancora in coda; una richiesta rimossa per annullamento restituisce `STATUS_NOT_FOUND`. I parametri facoltativi usano lo stesso layout di `WdfRequestGetParameters`. Un oggetto file attivo filtra `WdfIoQueueFindRequest`. `WdfIoQueueRetrieveRequestByFileObject` estrae la successiva richiesta corrispondente da una coda manuale o sequenziale e lascia invariata l’uscita se non trova corrispondenze.

Un `EvtIoCanceledOnQueue` configurato riceve `(WDFQUEUE, WDFREQUEST)` solo per una richiesta già consegnata al driver e poi inoltrata o rimessa in coda, oppure accodata esplicitamente dal callback nel contesto del chiamante. Una richiesta mai consegnata viene completata dal framework con `STATUS_CANCELLED` senza quel callback. La notifica restituisce la richiesta al driver, che deve completarla nel callback o in seguito e non può rimetterla in coda. Svuotamento e notifica dello stato attendono il ritorno del callback e il completamento della richiesta.

`WdfIoQueueDrain` rifiuta le nuove richieste con `STATUS_INVALID_DEVICE_STATE` e consegna quelle già in coda. Il callback viene eseguito quando non restano richieste in coda né detenute dal driver. L’inoltro restituisce `STATUS_WDF_BUSY`; `WdfIoQueueStart` ripristina l’accettazione.

`WdfIoQueuePurge` annulla con `STATUS_CANCELLED` le richieste non ancora consegnate e richiede l’annullamento sul relativo IRP delle richieste già consegnate e marcate come annullabili. La pulizia precede il rilascio dell’IRP. Il callback di stato attende le richieste in coda, quelle detenute dal driver e i callback di annullamento attivi. Il driver completa le richieste non annullabili.

`WdfIoQueueStopSynchronously`, `WdfIoQueueDrainSynchronously` e `WdfIoQueuePurgeSynchronously` sospendono il chiamante guest a `PASSIVE_LEVEL` finché le richieste pertinenti terminano. Stop continua ad accettare ma sospende la consegna e attende le richieste già consegnate. Drain rifiuta nuove richieste, consegna quelle in coda e attende entrambe le categorie. Purge annulla le richieste in coda e quelle marcate annullabili, attendendo anche il ritorno dei callback di annullamento. Senza una fonte di completamento, l’attesa segnala un errore di modello bloccato.

`WdfIoQueueStopAndPurge` e `WdfIoQueueStopAndPurgeSynchronously` annullano le richieste già in coda e quelle del driver marcate come annullabili, poi continuano ad accettare nuove richieste senza consegnarle fino a `WdfIoQueueStart`. Il callback di stato asincrono e l’attesa sincrona terminano dopo le richieste originali e i relativi callback di annullamento; le nuove richieste restano in coda e non ritardano la notifica. Entrambe le forme di Stop riattivano l’accettazione dopo Drain o Purge.
...
Per richieste KMDF buffered, direct e neither, `WdfDeviceInitSetIoInCallerContextCallback` viene eseguito prima della coda nel processo richiedente a `PASSIVE_LEVEL`. Il callback deve completare la richiesta oppure chiamare una volta `WdfDeviceEnqueueRequest` per la coda predefinita. Per IOCTL `METHOD_NEITHER` e READ/WRITE neither, `WdfRequestRetrieveUnsafeUserInputBuffer` e `WdfRequestRetrieveUnsafeUserOutputBuffer` restituiscono gli indirizzi utente originali solo in questo callback. `WdfRequestProbeAndLockUserBufferForRead` e `WdfRequestProbeAndLockUserBufferForWrite` controllano i permessi e bloccano le pagine della richiesta; `WdfMemoryGetBuffer` restituisce un alias di sistema utilizzabile nel callback della coda fuori dal contesto richiedente. Il completamento libera blocchi e alias. Sono supportati puntatori incorporati verso `user_buffers` espliciti; le mappature arbitrarie restano escluse.

Le richieste neither possono dichiarare `user_buffers` (`id`, `size`, `input`/`access` facoltativi) e `user_pointers`. `source`/`target` indicano `buffer` (`input`, `output`, `memory`) e `offset`; solo `memory` usa un `id` locale alla richiesta. Gli slot di puntatore di otto byte non possono sovrapporsi; la destinazione può essere interna al buffer o alla sua fine. Sono ammessi 64 buffer aggiuntivi e 256 puntatori per scenario, con 64 KiB per buffer nel budget totale di 512 KiB. Destinazioni uguali condividono byte reali e le regole esistenti di permessi, blocco, revoca e uscita del processo. `configuration.user_memory` conserva le dichiarazioni. `user_buffers[].backing_hex` è una fotografia RAM diagnostica alla terminazione, anche dopo revoca o errore; non sostituisce `output_hex` né concede accesso. `id`: 1–64 ASCII, `[A-Za-z0-9][A-Za-z0-9_.-]*`.

`WdfRequestRetrieveInputMemory` e `WdfRequestRetrieveOutputMemory` espongono viste WDFMEMORY della richiesta sui buffer esistenti per I/O buffered o direct. Una seconda richiesta nella stessa direzione mantiene l’handle; `WdfMemoryGetBuffer` restituisce il buffer originale e la lunghezza logica. Lunghezza zero o direzione errata produce uno stato WDF di errore; l’I/O neither richiede ancora controllo e blocco delle pagine nel contesto del chiamante. Le viste prese in prestito non aggiungono blocchi MDL e scadono al completamento della richiesta.

Un sottoinsieme KMDF PnP limitato supporta un FDO collegato direttamente al PDO. `EvtDriverDeviceAdd` riceve un inizializzatore del framework; `WdfDeviceCreate` crea il FDO e `WdfFdoInitWdmGetPhysicalDevice` e `WdfDeviceWdmGetPhysicalDevice` conservano l’identità del PDO. Dopo un AddDevice fallito o un Remove completato, il framework esegue i callback di pulizia/distruzione ed elimina il dispositivo. Le code PnP accettano `WdfUseDefault` e `WdfTrue` per la gestione dell’alimentazione; `WdfFalse` resta senza gestione automatica. `WdfDeviceInitSetPnpPowerEventCallbacks` supporta `EvtDevicePrepareHardware`, `EvtDeviceD0Entry`, `EvtDeviceD0Exit`, `EvtDeviceReleaseHardware`, `EvtDeviceQueryStop`, `EvtDeviceQueryRemove` e `EvtDeviceSurpriseRemoval`. `EvtDeviceQueryStop` ed `EvtDeviceQueryRemove` vengono eseguiti prima del bus e possono attendere o rifiutare. Il rifiuto completa l’IRP con il loro stato, senza osservazione del bus o transizione di alimentazione; `STATUS_PENDING` e `STATUS_NOT_SUPPORTED` sono risultati non validi. `EvtDeviceSurpriseRemoval` è una notifica void prima dell’inoltro; il registro di ritorno viene ignorato. PrepareHardware riceve due elenchi di risorse distinti prima di D0Entry, vuoti per i dispositivi senza risorse; Per gli elenchi vuoti, `WdfCmResourceListGetCount` restituisce zero e `WdfCmResourceListGetDescriptor` restituisce NULL. Un errore di PrepareHardware o D0Entry diventa lo stato di START e viene comunque chiamato ReleaseHardware. Durante lo stop o la rimozione da D0, D0Exit precede ReleaseHardware e il completamento dell’IRP. I dispositivi `register_bank` configurati espongono voci `CM_PARTIAL_RESOURCE_DESCRIPTOR` di sola lettura fino a ReleaseHardware; il driver può mappare la memoria tradotta con `MmMapIoSpace` e deve rimuovere la mappatura prima del completamento di STOP o Remove. Gli altri tipi di risorse e la politica generale restano esclusi. `WdfIoQueuePnpHeld` indica la sospensione della consegna prima di D0 e dopo l’uscita da D0; prima di uscire da D0, `EvtIoStop` riceve ogni richiesta posseduta dal driver in una coda gestita dall’alimentazione; il driver può completarla, chiamare `WdfRequestStopAcknowledge` o lasciare che una fonte già pianificata la completi. Senza `EvtIoStop` registrato, il framework attende comunque il completamento di tutte le richieste consegnate. TRUE la rimette in coda per la consegna dopo il riavvio; FALSE mantiene la proprietà del driver e richiama `EvtIoResume` dopo il rientro in D0. Le richieste già in coda attendono il riavvio. Se non rimane alcuna fonte di completamento, l’attesa termina con un `model_error` per stallo. Il fixture WDK `driver_kmdf_pnp.c` usa `NEVERD_KMDF_PNP_FIXTURE` / `NEVERD_KMDF_PNP_CFG_FIXTURE`.
...
Per un FDO PnP, `WdfDeviceInitSetDeviceType` registra il tipo a 32 bit specificato nel `DEVICE_OBJECT` WDM; senza la chiamata, il valore predefinito resta `FILE_DEVICE_UNKNOWN`. Gli incrementi della priorità I/O dipendenti dal tipo non sono modellati.

`WdfDeviceInitSetExclusive` imposta `DO_EXCLUSIVE` sul dispositivo WDM creato dall’inizializzatore. Un dispositivo di controllo con nome rifiuta una seconda apertura indipendente finché il primo file non viene chiuso. Su un FDO PnP, questo flag da solo non rende esclusivi il PDO con nome o l’intero stack; l’esclusività del PDO definita dall’INF è fuori da questo profilo.

`WdfDeviceInitSetFileObjectConfig` registra `EvtDeviceFileCreate`, `EvtFileCleanup` ed `EvtFileClose` prima della creazione del dispositivo e copia gli attributi di contesto opzionali. Il profilo supporta `WdfFileObjectNotRequired`, `WdfFileObjectWdfCanUseFsContext`, `WdfFileObjectWdfCanUseFsContext2` e `WdfFileObjectWdfCannotUseFsContexts`. Lo slot di contesto WDM scelto conserva l’handle WDF fino al CREATE fallito o al CLOSE e deve essere inizialmente vuoto. Con `WdfFileObjectCanBeOptional`, `WdfRequestGetFileObject` restituisce NULL per una richiesta I/O senza un oggetto file WDM corrispondente. CREATE, CLEANUP e CLOSE richiedono ancora un oggetto file WDM; `WdfFdoInitSetFilter` abilita l’inoltro predefinito dei filtri; `WdfTrue` lo abilita esplicitamente e `WdfFalse` lo disabilita. L’inoltro delle richieste file richiede un percorso FDO/PDO diretto e un `bus_completion` esplicito. Un `delay_100ns` positivo è ammesso per l’inoltro automatico di CREATE/CLEANUP/CLOSE e per CREATE sincrono, asincrono con routine di completamento o con `SEND_AND_FORGET`. Le callback CLEANUP e CLOSE precedono l’invio al dispositivo inferiore. Una callback CREATE con `WdfFileObjectNotRequired` può ottenere il target locale appartenente al dispositivo tramite `WdfDeviceGetIoTarget` e inoltrare la richiesta originale con `WdfRequestSend` usando solo `WDF_REQUEST_SEND_OPTION_SEND_AND_FORGET`. Il PDO mantenuto consuma la risposta esplicita del bus e gestisce il completamento finale, anche quando la risposta inferiore è ritardata. Una callback CREATE con un oggetto file del framework può usare `WDF_REQUEST_SEND_OPTION_SYNCHRONOUS`, leggere il risultato inferiore con `WdfRequestGetStatus` e completare la richiesta originale con tale stato. L’invio asincrono predefinito formatta prima la richiesta con `WdfRequestFormatRequestUsingCurrentType` e registra `WdfRequestSetCompletionRoutine`. La callback riceve `WDF_REQUEST_COMPLETION_PARAMS`, può leggere il risultato inferiore tramite `WdfRequestGetCompletionParams` o `WdfRequestGetStatus` e completa la richiesta originale con uno stato scelto indipendentemente da quello inferiore. Per un invio con routine di completamento, la risposta ritardata del PDO mantiene in sospeso la richiesta fino alla scadenza virtuale, poi accoda la routine con lo stato finale inferiore. `WDF_REQUEST_SEND_OPTION_TIMEOUT` accetta un intervallo relativo negativo o una scadenza assoluta positiva dell’orologio virtuale a 100 ns, per CREATE sincrono o asincrono con callback di completamento. Un timeout precedente produce `STATUS_IO_TIMEOUT`; un completamento inferiore immediato o alla stessa scadenza ha precedenza. Una scadenza assoluta passata non ha intervallo residuo. Zero disabilita il timeout; altre opzioni restano escluse. L’handle WDF e il `FILE_OBJECT` WDM restano identità distinte, interrogabili con `WdfRequestGetFileObject`, `WdfFileObjectGetDevice` e `WdfFileObjectWdmGetFileObject`. Un CREATE fallito elimina l’oggetto WDF senza callback di pulizia o chiusura; dopo un’apertura riuscita, CLEANUP e CLOSE precedono la pulizia e la distruzione del contesto.

Un `WdfRequestSend` sincrono ritardato conserva e sospende il vero frame di chiamata guest, poi lo riprende con il risultato booleano dell’invio dopo il completamento. Il NTSTATUS finale si legge con `WdfRequestGetStatus`. `WDF_REQUEST_SEND_OPTION_TIMEOUT` accetta un intervallo relativo negativo o una scadenza assoluta positiva dell’orologio virtuale a 100 ns, per CREATE sincrono o asincrono con callback di completamento. Un timeout precedente produce `STATUS_IO_TIMEOUT`; un completamento inferiore immediato o alla stessa scadenza ha precedenza. Una scadenza assoluta passata non ha intervallo residuo. Zero disabilita il timeout; altre opzioni restano escluse. L’inoltro automatico esegue le callback del file prima dell’invio inferiore e conserva l’IRP e il file WDM durante le callback di pulizia e distruzione richieste dal completamento. I riferimenti WDF esterni conservano il contesto, ma non prolungano la vita del file WDM di una richiesta completata.

I CREATE/CLEANUP/CLOSE WDM inoltrati a un PDO configurato accettano anche un `bus_completion` esplicitamente ritardato. La vera routine di completamento guest viene eseguita dopo la scadenza inferiore, conservando la propagazione dello stato pendente e la responsabilità dello stato finale.
...
| `ExRaiseStatus`, `ExRaiseAccessViolation`, `ExRaiseDatatypeMisalignment` | Eccezioni C `__except`, filtri reali e `__finally` di unwind; nessun ritorno API normale |
...
| `IoConnectInterrupt`, `IoDisconnectInterrupt`, `IoConnectInterruptEx`, `IoDisconnectInterruptEx` | Linee latched/di livello assegnate, handler e lock esclusivi/condivisi; legacy ed Ex 1/2/4 a PASSIVE_LEVEL |
...
Il booleano facoltativo `user_unmap_after_dispatch` revoca gli indirizzi utente originali di un trasferimento WDM neither non vuoto dopo il ritorno del dispatch e prima di lavoro o annullamento programmati. Le pagine bloccate dall’MDL e gli alias di sistema restano utilizzabili fino allo sblocco; i puntatori utente grezzi e nuovi blocchi falliscono. Se l’output è revocato, `output_hex` è vuoto. Gli indirizzi delle allocazioni utente dichiarate inizialmente dallo scenario non vengono riutilizzati; la loro revoca in momenti arbitrari resta fuori dal modello.
...
processo e PFN di MDL non costruiti, gli MDL costruiti manualmente, i mapping utente fuori dal modello MDL per processo descritto sotto e l’accesso diretto
tramite UserBuffer grezzo sono rifiutati. Un buffer diretto di lunghezza zero
ha un MDL nullo.

`IoAllocateMdl` alloca metadati per un buffer non vuoto, senza overflow e di massimo 1 MiB, senza verificarlo né bloccarlo. `Irp` può essere NULL o un IRP modellato attivo. Un descrittore primario sostituisce la testa della catena del driver; i descrittori staccati restano del driver. `SecondaryBuffer` aggiunge in coda o diventa la testa se la catena è vuota. Il MDL direct-I/O originale della richiesta deve restare raggiungibile e non può essere sostituito o liberato dal driver. `ChargeQuota` deve essere FALSE; l’esaurimento dell’arena restituisce NULL. `MmBuildMdlForNonPagedPool` richiede l’intero intervallo in una singola allocazione attiva del pool non paginato. La funzione sicura e le macro WDM riutilizzano indirizzo e permessi originali anche con nuovi flag che vietano scrittura/esecuzione. Mapping di sistema aggiuntivi e smappatura sono rifiutati. `IoFreeMdl` libera solo il descrittore del driver indicato, senza scollegarlo né seguire `Next`. Un MDL utente deve essere sbloccato prima della liberazione manuale. Il buffer del pool ha una vita indipendente; entrambi gli ordini di rilascio sono permessi senza riutilizzare memoria liberata.

Il driver può modificare `MDL.Next` e `IRP.MdlAddress` per inserire o staccare descrittori modellati. Il completamento finale dell’IRP convalida prima tutta la catena corrente, sblocca i MDL utente collegati, revoca i loro alias e libera tutti i descrittori collegati. Descrittori staccati e memoria del pool restano del driver. Cicli, collegamenti sconosciuti o liberati, descrittori condivisi tra IRP attivi e descrittori WDF privati inseriti in catene WDM falliscono esplicitamente. Le dipendenze DMA e dispatcher attive impediscono il rilascio prematuro. Gli altri campi MDL modellati e i PFN costruiti restano di sola lettura; campi di processo, PFN non costruiti, MDL manuali non sono supportati. Prima dello scaricamento occorre liberare tutti i descrittori del driver rimasti.

`IoBuildPartialMdl` descrive un sottointervallo non vuoto di un MDL sorgente costruito; una lunghezza zero seleziona la parte restante. La destinazione deve avere capacità PFN sufficiente. I descrittori parziali condividono le pagine fisiche senza acquisire un altro blocco. Ereditano un mapping di sistema attivo della sorgente o ne creano uno indipendente con `MmGetSystemAddressForMdlSafe`. Liberare un MDL parziale o prepararlo al riutilizzo rilascia solo il mapping che possiede. La vera funzione inline WDK `MmPrepareMdlForReuse` può quindi precedere un’altra chiamata a `IoBuildPartialMdl`. Un descrittore sorgente non paginato può essere liberato indipendentemente; il blocco utente radice o un alias di sistema preso in prestito non possono essere ritirati finché un MDL parziale ne dipende. Il completamento controlla l’intera catena prima di rilasciare le dipendenze, indipendentemente dall’ordine dei descrittori. Il DMA attivo impedisce ricostruzioni e rilasci prematuri.

`MmMapLockedPagesSpecifyCache` accetta `UserMode` nel contesto di un richiedente vivo, incluso un worker collegato, fino ad `APC_LEVEL`. Ogni chiamata crea una vista del processo sulle stesse pagine senza cambiare `MappedSystemVa`. Le viste sono sempre NX; `MdlMappingNoWrite` impedisce scritture. Il pool deve essere non paginato e coprire pagine private complete. Un nuovo blocco con `MmProbeAndLockPages` mantiene l’identità fisica. L’indirizzo richiesto deve conservare l’offset di pagina MDL e rientrare nell’arena limitata; l’esaurimento solleva `STATUS_INSUFFICIENT_RESOURCES` intercettabile. `MmNonCached`, `MmCached` e `MmWriteCombined` ereditano l’attributo cache già assegnato alle pagine; la RAM ordinaria resta cached. Valori riservati e registrazioni fisiche incompatibili falliscono esplicitamente.

`MmUnmapLockedPages` richiede la coppia esatta indirizzo utente/MDL e il PID creatore. Rimuove realmente la vista e libera indirizzo e budget per riuso richiesto o automatico. Rimuovere la VA originale dello scenario conserva le viste MDL indipendenti. Pin fisici e alias di sistema rimangono associati alle pagine originali anche quando la VA viene riusata per altro backing o altri permessi. Viste vive impediscono il rilascio di descrittore, blocco radice, pool e memoria della richiesta.

Le viste sono identificate da `(PID, VA)`: processi diversi possono possedere lo stesso indirizzo numerico contemporaneamente con pagine e permessi distinti. Un cambio di processo sostituisce atomicamente l’intero insieme attivo dopo preflight completo; il ritorno ripristina pagine e permessi originali. Uscita o unmap di un PID non rimuovono viste di altri PID. Un oggetto dispatcher opaco vivo in una vista da ritirare impedisce il cambio finché non viene rimosso. Il modello limitato non implementa allocazione virtuale arbitraria, tabelle delle pagine o un gestore generale dei processi.
...
Il profilo è `wdm-x64-scheduled-v75`. `nt_status` rimane il risultato di DriverEntry,
...
La consegna usa metadati unwind x64 V1 e `__C_specific_handler`. Uno scope costante `EXCEPTION_EXECUTE_HANDLER` seleziona direttamente il proprio handler. Esegue veri filtri guest: zero continua la ricerca, un valore positivo seleziona l’handler e uno negativo chiede la ripresa. Dopo la selezione esegue i `__finally` degli ambiti abbandonati nell’ordine di unwind, prima dell’handler; nessuna pulizia durante ricerca o ripresa. Il codice guest esegue i percorsi normali di `__finally`. I frame ordinari ripristinano i registri generali non volatili entro lo stack originale. `GetExceptionCode()` mantiene il codice sollevato; gli handler possono sollevare verso un ambito esterno supportato.

I filtri ricevono `EXCEPTION_POINTERS`, record di eccezione e `CONTEXT_INTEGER | CONTEXT_CONTROL` stabili su uno stack limitato separato, fuori dallo shadow space Win64. Lo stato CPU originale completo, inclusi SIMD/floating point e flag, viene conservato. Un filtro negativo può riprendere un fault CPU utente di lettura/scrittura dopo verifica di GPR, RIP/RSP e flag aritmetici/di direzione modificati. I campi immutabili sono verificati per qualunque risultato del filtro. Riprendere un’eccezione sollevata da un’API modellata resta escluso. Identità del thread/processo, PreviousMode e autorità utente derivano dal padre senza autorizzare worker indipendenti; un worker collegato conserva processo creatore di sistema e KernelMode.

Sono supportati eccezioni annidate e unwind in collisione, catene V1, prologhi parziali, epiloghi canonici e ripristino completo XMM6–XMM15. I record mantengono i collegamenti e un finally già iniziato non viene ripetuto. C++ e metadati incompleti restano rifiutati. Eccezioni API o utente non gestite producono `model_error`; altri fault di memoria CPU, interrupt e istruzioni non valide restano terminali. A ogni ritorno del filtro, `EXCEPTION_POINTERS`, `EXCEPTION_RECORD` e i campi non supportati di `CONTEXT` devono restare invariati.

`__GSHandlerCheck_SEH` verifica il cookie corrente dell’immagine prima della ricerca e durante l’unwind, anche nei frame senza finally. Gli slot fissi o allineati dinamicamente usano offset con segno verificati e il frame pointer originale per la codifica. I flag del gestore C sono indipendenti dai controlli del cookie. Prologhi ed epiloghi non leggono cookie non ancora stabiliti. Una differenza arresta l’esecuzione prima del filtro, cleanup o gestore interessato. `__GSHandlerCheck` autonomo controlla i cookie anche durante ricerca e unwind senza inventare ambiti C. Richiede identità esatta di simbolo o importazione; il codice anonimo non viene dedotto da pattern di istruzioni. I wrapper GS/C++ e le personalità delle eccezioni C++ restano non supportati. I frame originali in `driver_seh_gs.h` eseguono anche il controllo WDK collegato.
...
`MmAllocatePagesForMdl`, `MmAllocatePagesForMdlEx` e `MmFreePagesFromMdl` gestiscono pagine indipendenti con vincoli fisici, allocazioni parziali e blocchi contigui. Pagine e descrittore vengono liberati separatamente. `MmProtectMdlSystemAddress` modifica i permessi PAGE della vista di sistema privata mantenendo PFN, cache e permessi utente. `MmProbeAndLockPages` accetta pool KernelMode: il pool paginato si blocca fino ad APC_LEVEL e diventa accessibile a DISPATCH_LEVEL; non può essere liberato prima dello sblocco.

`messages` specifica indirizzo, dati, vettore, livello, affinità e polarità MSI. `message_id` è locale al descrittore; il terzo argomento ISR è globale al PDO. `CONNECT_MESSAGE_BASED` (3) restituisce la tabella reale. `CONNECT_MESSAGE_BASED_PASSIVE` (5) e le linee passive permettono attese PASSIVE_LEVEL, serializzano lo stesso interrupt e conservano frame indipendenti. Arrivo e consegna sono distinti, senza inventare protocolli PCI.

`WdfDeviceInitSetPowerPolicyOwnership` imposta il proprietario della politica. Init/Suspend/Restart/Flush/Cleanup e i callback D0 vengono eseguiti; D3/D0 mantiene le risorse. La politica predefinita associa Sleeping3/Working a D3/D0 consumando `requested_device_power` esplicito. Ogni IRP figlio ha `origin: "framework_power_policy"` e `response_index`. S3 attende il figlio; S0 può terminare dopo la richiesta D0. Idle/wake e rienumerazione automatica restano esclusi.
