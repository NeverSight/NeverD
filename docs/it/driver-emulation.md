...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | MDL delle richieste, mapping di sistema e identità PFN condivise in sola lettura | Mapping utente e interfacce DMA fuori dal sottoinsieme descritto sotto |
| MDL allocati dal driver | Descrittori autonomi o associati a IRP, MDL parziali, mapping utente/sistema condivisi e rilascio al completamento | Quote, MDL manuali, spazi di processo arbitrari e riutilizzo delle VA |
...
`MmMapLockedPagesSpecifyCache` accetta `UserMode` e `MmCached` nel contesto di un richiedente attivo, incluso il contesto limitato di un worker collegato, fino a `APC_LEVEL`. Ogni chiamata crea una vista indipendente di proprietà del processo sulla stessa memoria fisica, senza cambiare `MappedSystemVa`. Le viste non sono mai eseguibili (NX); `MdlMappingNoWrite` le rende di sola lettura. La memoria del pool deve essere non paginata e occupare pagine private complete. Bloccare nuovamente una vista utente con `MmProbeAndLockPages` conserva l’identità fisica originale. L’indirizzo richiesto deve mantenere l’offset di pagina del MDL e rientrare nell’arena VA limitata dedicata alle viste utente; il suo esaurimento solleva un `STATUS_INSUFFICIENT_RESOURCES` intercettabile.

`MmUnmapLockedPages` richiede l’esatta coppia indirizzo utente/MDL e il processo creatore (PID). La rimozione della VA originale dello scenario conserva le viste MDL indipendenti e gli alias di sistema. L’uscita di un processo revoca solo le sue viste utente; le pagine fisiche bloccate e gli alias di sistema restano validi. Le viste attive impediscono il rilascio del descrittore, del blocco radice e della memoria del pool o della richiesta. Gli indirizzi utente revocati non vengono riutilizzati. Spazi di processo arbitrari, altre politiche di cache e rimappatura delle VA utente restano fuori da questo modello limitato.
