...
| Driver hardware, DMA, PCI, di interrupt o di virtualizzazione | Banchi espliciti, MMIO, interrupt latched esclusivi e DMA coerente common/SG | Altri modelli di dispositivi, RAM fisica arbitraria, PCI, porte, interrupt condivisi/di livello/MSI, altre interfacce DMA e stato CPU privilegiato |
...
`IoGetDmaAdapter` accetta i campi storici di `DEVICE_DESCRIPTION` versione 0/1 per un bus master Internal e pubblica un `DMA_ADAPTER` di versione uno con la vera tabella `DMA_OPERATIONS` da 104 byte. Le richieste di versione 2/3 restituiscono NULL senza leggere la coda moderna del descrittore. Ogni metodo indiretto è associato all’esatto adattatore attivo, indipendentemente dalle importazioni kernel. Sono implementati `AllocateCommonBuffer`, `FreeCommonBuffer`, `GetDmaAlignment`, `GetScatterGatherList`, `PutScatterGatherList` e `PutDmaAdapter`; le altre sei voci della versione uno restano trap identificate per nome. Allocazione/liberazione di common buffer e interrogazioni dell’allineamento richiedono PASSIVE_LEVEL; Get/PutScatterGatherList richiedono DISPATCH_LEVEL, mentre il rilascio dell’adattatore consente fino a DISPATCH_LEVEL. x64 ignora `CacheEnabled`. Richieste di versioni non supportate, capacità dichiarate incompatibili e carenze di allocazione documentate restituiscono NULL; selezioni d’interfaccia malformate o non modellate ed errori del backend restano errori espliciti. `AllocateAdapterChannel`, `FlushAdapterBuffers`, `FreeAdapterChannel`, `FreeMapRegisters`, `MapTransfer`, `ReadDmaCounter`.
...
Il profilo è `wdm-x64-scheduled-v14`. `nt_status` rimane il risultato di DriverEntry,
