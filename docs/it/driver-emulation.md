...
`WdfDeviceInitSetFileObjectConfig` registra `EvtDeviceFileCreate`, `EvtFileCleanup` ed `EvtFileClose` prima della creazione del dispositivo e copia gli attributi di contesto opzionali. Il profilo supporta `WdfFileObjectWdfCannotUseFsContexts` e `WdfFileObjectNotRequired`; l’inoltro a una destinazione inferiore e le classi che usano FsContext falliscono esplicitamente. L’handle WDF e il `FILE_OBJECT` WDM restano identità distinte, interrogabili con `WdfRequestGetFileObject`, `WdfFileObjectGetDevice` e `WdfFileObjectWdmGetFileObject`. Un CREATE fallito elimina l’oggetto WDF senza callback di pulizia o chiusura; dopo un’apertura riuscita, CLEANUP e CLOSE precedono la pulizia e la distruzione del contesto.
...
Il profilo è `wdm-x64-scheduled-v60`. `nt_status` rimane il risultato di DriverEntry,
