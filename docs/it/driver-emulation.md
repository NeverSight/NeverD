...
| Driver hardware, DMA, PCI, di interrupt o di virtualizzazione | Banchi espliciti, MMIO, interrupt latched/di livello esclusivi o condivisi e DMA coerente common/SG/channel | Altri modelli di dispositivi, RAM fisica arbitraria, PCI, porte, MSI e interrupt passivi, altre interfacce DMA e stato CPU privilegiato |
...
Il profilo è `wdm-x64-scheduled-v72`. `nt_status` rimane il risultato di DriverEntry,
...
Eccezioni annidate o unwind in collisione in un filtro/finally, personalità GS/C++, metadati concatenati/incompleti, prologhi e operazioni unwind XMM falliscono esplicitamente. Eccezioni API o utente non gestite producono `model_error`; altri fault di memoria CPU, interrupt e istruzioni non valide restano terminali. A ogni ritorno del filtro, `EXCEPTION_POINTERS`, `EXCEPTION_RECORD` e i campi non supportati di `CONTEXT` devono restare invariati.
