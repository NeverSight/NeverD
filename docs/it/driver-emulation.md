...
Un sottoinsieme KMDF PnP senza risorse supporta un FDO collegato direttamente al PDO. `EvtDriverDeviceAdd` riceve un inizializzatore del framework; `WdfDeviceCreate` crea il FDO e `WdfFdoInitWdmGetPhysicalDevice` e `WdfDeviceWdmGetPhysicalDevice` conservano l’identità del PDO. Dopo un AddDevice fallito o un Remove completato, il framework esegue i callback di pulizia/distruzione ed elimina il dispositivo. La coda PnP deve impostare esplicitamente `PowerManaged = WdfFalse`. `WdfDeviceInitSetPnpPowerEventCallbacks` supporta `EvtDevicePrepareHardware`, `EvtDeviceD0Entry`, `EvtDeviceD0Exit` e `EvtDeviceReleaseHardware`. PrepareHardware riceve due elenchi di risorse vuoti e distinti prima di D0Entry; `WdfCmResourceListGetCount` restituisce zero e `WdfCmResourceListGetDescriptor` restituisce NULL. Un errore di PrepareHardware o D0Entry diventa lo stato di START e viene comunque chiamato ReleaseHardware. Durante lo stop o la rimozione da D0, D0Exit precede ReleaseHardware e il completamento dell’IRP. Risorse non vuote, code gestite dall’alimentazione e politica generale restano escluse. Il fixture WDK `driver_kmdf_pnp.c` usa `NEVERD_KMDF_PNP_FIXTURE` / `NEVERD_KMDF_PNP_CFG_FIXTURE`.
...
Il profilo è `wdm-x64-scheduled-v52`. `nt_status` rimane il risultato di DriverEntry,
