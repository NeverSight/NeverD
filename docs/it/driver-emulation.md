...
Il modello limitato non offre tutto l’asincronismo Windows. Attese alertable o utente, APC, annullamento generale delle richieste WDM, invii concorrenti di scenari pubblici, API KMDF per contesto chiamante e buffer utente, UMDF, dispositivi PnP KMDF e pianificazione generale delle code, PnP/alimentazione completo, hardware generale, altre interfacce DMA e altre modalità di interrupt restano non supportati. L’inizializzazione sola esegue callback esplicitamente accodati senza creare richieste o scaricamenti impliciti.
...
Il profilo è `wdm-x64-scheduled-v31`. `nt_status` rimane il risultato di DriverEntry,
