...
`WdfDeviceInitSetFileObjectConfig` registra `EvtDeviceFileCreate`, `EvtFileCleanup` ed `EvtFileClose` prima della creazione del dispositivo e copia gli attributi di contesto opzionali. Il profilo supporta `WdfFileObjectNotRequired`, `WdfFileObjectWdfCanUseFsContext`, `WdfFileObjectWdfCanUseFsContext2` e `WdfFileObjectWdfCannotUseFsContexts`. Lo slot di contesto WDM scelto conserva l’handle WDF fino al CREATE fallito o al CLOSE e deve essere inizialmente vuoto. Con `WdfFileObjectCanBeOptional`, `WdfRequestGetFileObject` restituisce NULL per una richiesta I/O senza un oggetto file WDM corrispondente. CREATE, CLEANUP e CLOSE richiedono ancora un oggetto file WDM; l’inoltro a una destinazione inferiore resta escluso. L’handle WDF e il `FILE_OBJECT` WDM restano identità distinte, interrogabili con `WdfRequestGetFileObject`, `WdfFileObjectGetDevice` e `WdfFileObjectWdmGetFileObject`. Un CREATE fallito elimina l’oggetto WDF senza callback di pulizia o chiusura; dopo un’apertura riuscita, CLEANUP e CLOSE precedono la pulizia e la distruzione del contesto.
...
Il profilo è `wdm-x64-scheduled-v62`. `nt_status` rimane il risultato di DriverEntry,
