...
都带有运行时标识符、其存在即代表开启的 account，以及各 cluster 激活它时所在的 slot。
某个 gate 若在某个 cluster 下没有对应行，就表示它在那里尚未激活。`simd-0321` 在每个
...
`sol_alloc_free_` 完全不需要 gate：运行时一直照常兑现它，同时拒绝接受调用它的新程序，
这只是两个 registry 之间的差别，而不是别的什么。
