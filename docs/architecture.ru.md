...
с версионированными cache key запроса и артефакта. Ненулевой бюджет созданных
байтов ограничивает размер объекта; ноль означает отсутствие ограничения со
стороны вызывающей policy. Компилятор останавливается на проверенных relocatable
байтах: он не линкует, не публикует, не передаёт их dispatcher и не исполняет, а
также не предоставляет lowering guest-инструкций.
...
проверенного компилятора объектных файлов и аудита объектов определяют и
проверяют эти границы. По-прежнему отсутствуют полный lowering guest-инструкций,
путь аудита/линковки/загрузки графа JITLink, доверенный dispatcher или его
factory и исполнение. Поэтому доступные границы не образуют полного исполняемого
backend трансляции, полной pipeline межархитектурной трансляции или полного
сквозного переписывания
