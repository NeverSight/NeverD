...
新增测试验证完整 CPU 上下文恢复（寄存器、标志、SIMD、FPU、CR8）、共享内存及故障／跨后端上下文拒绝。工作项样例覆盖标记后的待处理请求、延迟完成、队列与生命周期错误、停滞请求、共享预算，以及公共报告中分开的派发／完成状态。内部定时器／DPC 测试只验证状态转换，不表示支持来宾 API 或完整 Windows 异步环境。
