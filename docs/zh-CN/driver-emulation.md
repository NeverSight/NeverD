...
| KMDF 1.33 非 PnP 驱动 | 版本绑定、对象／上下文、具名控制设备、顺序或无限并行默认队列、非默认手动队列，以及实际执行回调的缓冲／直接／neither 请求 | 不支持 PnP 设备、通用队列调度、类扩展或 UMDF |
