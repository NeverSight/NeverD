...
精确的 selector 专用 Objective-C stub 还可以在恢复出的调用恰好止于声明的固定参数前缀时绑定动态格式对象。因为不存在可变参数尾部，格式内容无法改变生成调用的参数 ABI；生成代码会把原始对象交给框架解析器。任何额外实参、不精确的 stub、声明冲突或物理 ABI 不匹配都会继续保持未解析状态。
