...
Swift 布尔结果的联合验证同时检查当前 Objective-C 入口 ABI、不可变的直接调用指令、精确的强运行时导入，以及完整的 LowIR 消费者证明。只允许一个比较调用点，其他调用的 ABI 全部从当前运行时目录重新建立；拒绝原生签名猜测、动态分派和重复调用点。该证据本身不会发布源码，也不会声明字节返回 ABI。
