...
`DriverResources.h` 与 `DriverResources.def` 定义固定 `register_bank` 分配、寄存器权限、宽度、字段拼写及共享上限。`DriverScenario` 负责 JSON／原生预检，`DriverResult` 记录配置，不复制银行的实际状态。`KernelMMIO` 拥有每个 PDO 的持久寄存器值、转换后物理区间、资源分配身份、硬件可访问性及独立映射别名。`KernelModelResources` 负责紧凑布局的只读原始／转换后 START 清单及提供者／生命周期集成；实际下层 START 成功后，在上层回调前发布资源分配，提供者设备 SET 完成时更新 D0／D3 可访问性。上层完成回调可以先取消映射；失败 START 或 STOP／REMOVE 的拆除检查在最终 IRP 完成前进行，不会隐式释放。突然移除立即拒绝硬件访问。`GuestMemory` 与 `UnicornBackend` 在寄存器回调前验证完整的原始 CPU／API 访问，并共享首次故障／停止锁存，防止 Unicorn 拆分访问后继续产生效果。固定分配的重启保留寄存器值，映射别名不拥有独立字节副本。此有界内存寄存器总线不提供任意 RAM、资源重平衡、端口、中断或 DMA。
