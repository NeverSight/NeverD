...
源码绑定仅在指针参数具有常量地址时构建经过验证的直接类对象身份映射。该映射仅限当前绑定操作；后续每次操作都会重新验证当前镜像，包括类名、元类标志和身份冲突。
...
带编译器格式声明的 NSString 调用，可从已验证的常量格式对象恢复提升后的标量实参。顺序参数与序号参数必须完整且类型一致。精确的 selector stub 只有在尾部为空，或尾部完全由源码指针组成时，才能保留动态格式；若 SSA 仍把值写成标量，则其完整定义链必须终止于已认证的 Objective-C 运行时指针返回调用。整数、浮点、混合定义和其他无类型尾部仍须解析不可变格式文本。Darwin arm64 从八字节栈槽读取可变实参；x86_64 按整数、浮点寄存器组及溢出栈槽传参。生成调用保留省略号与动态派发。未知转换、计数写入、long double 及未支持扩展仍明确拒绝。 同一套格式分析也支持 `NSLog` 等已声明的 C 导入，要求精确的动态库导出证据，并让实参数量不同的调用共用正确的可变参数原型。
...
C 声明库也覆盖 CoreGraphics、CoreServices 和 ImageIO 的导出。不透明的图像、颜色、字符串及 `iovec` 指针、整数计数和浮点返回值保留其声明的 ABI。公共系统框架的路径别名与导出事实一起生成；私有路径、不同的框架版本和未声明的符号不会因此获得绑定。 mobile 声明也使用加载器的类型语法：语法完整的聚合类型指针按不透明指针处理，不推测其布局。固定 `notify.h`、向量 I/O 和 `UTTypeConformsTo` API 也来自同一套四目标编译器交集，并且必须精确匹配对应的系统导出。
...
固定参数 C 调用还会使用编译器提取的声明和 SDK 导出、再导出信息。绑定必须匹配实际 dyld 库、符号及标量 ABI；弱导入、未知提供库和不支持的原型保持未绑定。公开的 `dispatch_once_f` 与 `dispatch_queue_set_specific` 回调原型只在精确的 libdispatch 导出边界替换目录中的不透明 `^?` 回调编码。精确的 `dispatch_once_f` 调用可以重建完整的可写谓词及采用公开回调 ABI 的唯一本地回调；回调仍须拥有完整的源码依赖闭包。生成的 C 使用链接到原符号的独立标识符与新的共享谓词存储。没有语言分派表的普通同步调用保留真实调用及内存副作用。

C 目录包含 `sys/mount.h` 声明，并保留架构专属的链接符号：ARM64 使用 `getmntinfo`，x86-64 使用 `getmntinfo$INODE64`。输出参数保持不透明的二级指针，模式参数和返回值保持有符号 32 位整数，并执行精确的系统导出检查。不会推测文件系统记录布局或返回缓冲区内容。
...
通过 Objective-C 入口 thunk 暴露的 Swift 延迟初始化静态对象 getter，仅在 `vgZTo` 符号、运行时方法签名、谓词测试、`swift_once` 调用、存储加载和 `objc_retainAutoreleaseReturnValue` 结果共同构成精确的编译器模式时才可投影。谓词 `_Wz`、初始化器 `_WZ` 与存储 `vpZ` 符号必须相互吻合；原始第三寄存器只能用作 once 上下文，初始化器必须忽略该上下文且不能存在普通直接调用者。投影会重建谓词和对象存储，将 null 作为无关上下文传入，并把初始化器保留为经过检查的依赖。形状、符号、参数用途或回调有任何偏差时均保持未恢复。

具有已认证 `cfcTo` 符号及运行时 `init` 签名的 Swift Objective-C 构造器 thunk，也可在未声明的第三参数寄存器仅作为一次精确 `swift_once` 调用的上下文出现时将其移除。谓词 `_Wz` 与初始化器 `_WZ` 必须匹配；回调必须忽略该上下文，且不能有普通直接调用者。投影传入 null，只推导谓词的八字节存储范围，并保留其他所有控制流、内存及调用效果；常规源码体与依赖闭合检查仍然适用。

ARM64 once 回调可以将除此之外未使用的 x2 上下文转发给一次嵌套的 `swift_once`，前提是外层 `_WZ` 符号及内层 `_Wz`/`_WZ` 配对精确匹配、两个回调都没有普通直接调用者，且具有独立类型证明的叶回调忽略其上下文。发现与投影使用同一合同。投影传入 null 并保留每条语句；void 回调 ABI 只允许丢弃纯寄存器、临时变量或常量返回值。返回表达式中的栈读取、加载、调用及未闭合依赖仍会被拒绝。
...
对于保存寄存器、使用栈帧并执行普通调用的原生辅助函数，有界 LowIR 分析必须证明每条退出路径都恢复原有的保留寄存器字节、栈指针和链接寄存器，才能生成 void 源码摘要。部分写入、隐式零扩展、重叠存储和调用破坏会使相应事实失效。经证明不含帧派生字节的地址所执行的存储与本次调用的私有栈帧互不相交；部分或完整的帧派生别名、保存或向外传递帧地址都会使证明失败。唯一的栈帧借用例外是已有精确源码绑定的 `objc_msgSendSuper2` 首参数：它可以指向完全位于已分配栈帧内的完整 16 字节 `objc_super` 对象，因为该运行时合同只在调用期间同步读取它。普通消息、不完整指针、越过栈帧边界的对象及其他任何栈帧参数仍会被拒绝。HighIR 字节活跃性把栈槽值视为对该槽准确字节范围的有界读取，而取得或传递其地址仍属于逃逸；格式错误或范围重叠的访问仍按保守方式处理。因此，互不相交的私有存储可以只丢弃未被读取的尾部字节。重新提升后，可以移除在 HighIR 中完全没有出现的辅助寄存器参数，并再次运行流水线。私有存储消除仍由现有 HighIR 清理负责，标准参数和实际输入使用保持不变。

arm64 UIKit 目录还绑定 `UIProgressView` 的 `observedProgress` 对象结果及 `setProgress:animated:`；后者保留独立的 `float` 和布尔参数寄存器。两项声明都要求编译器生成的 iPhoneOS 与 arm64 iPhoneSimulator 证据一致，并且提供库必须是精确的系统 UIKit；其他架构仍不受支持。
...
ARM64 原生栈帧保存分析可读取当前推断入口签名明确描述的、按八字节对齐的完整标量入栈参数槽。LowIR 读取必须与整个槽精确匹配。这些字节仍是未知输入值，不能证明保存的寄存器身份或被调用函数私有栈帧内的地址。写入、部分读取、空隙和未声明的槽不会获得该权限。源码函数体、每个调用方的实参值和依赖闭包仍需验证。
...
不可变 Swift 字符串的 once 回调只有在完整验证六指令调用方、十二指令共享函数和三指令地址访问器所构成的精确路径后才能投影。源码保留两次八字节写入的顺序和真实的 `swift_bridgeObjectRetain` 调用；验证字符串标记后，用整个字面量段的共享存储重建地址，同时保留目标符号的共享存储。当前映像字节、完整流水线审计、栈帧保存、强运行时导入和最终源码函数都会重新校验，不向共享函数或地址访问器授予通用 ABI。

ARM64 Objective-C 源码恢复支持由调用方证明的单个 Swift 字符串比较及其原始单比特结果。清零扩展前会验证当前入口 ABI、强导入、精确调用点和所有消费者。生成的 C 以 `swiftcall` 的 `_Bool` 声明运行时函数，再显式转换为 `uint8_t`，保留原有掩码及周围调用。发布时重新检查 LowIR 和流水线审计，拒绝重复求值及来自其他函数的证据。普通字节返回、原生调用猜测以及 patch、lift 模式不使用这种归一化。
