...
固定参数 C 调用还会使用编译器提取的声明和 SDK 导出、再导出信息。绑定必须匹配实际 dyld 库、符号及标量 ABI；弱导入、未知提供库和不支持的原型保持未绑定。公开的 `dispatch_once_f` 与 `dispatch_queue_set_specific` 回调原型只在精确的 libdispatch 导出边界替换目录中的不透明 `^?` 回调编码。生成的 C 使用独立标识符链接原始符号。没有语言分派表的普通同步调用保留真实调用及内存副作用。
