...
`lib/emulation` مكوّن تنفيذ اختياري يُفعّل بواسطة `NEVERD_ENABLE_DRIVER_EMULATION`. تصل إليه CLI المسماة `emulate-driver` عبر واجهة C API العامة. يملك `DriverSession` مسؤولية التهيئة المحدودة لـx64 WDM واستدعاءات create/IOCTL/read/write/cleanup/close/unload المتسلسلة الاختيارية؛ ويستخدم تعيين صورة Windows الكائن `BinaryImage` الكامل من المحمّل الموجود، بينما يملك نموذج Windows كائنات الضيف ودلالات API. يملك مهايئ Unicorn تنفيذ المعالج والمصدر المعتمد لذاكرة الضيف. لا يستخدم هذا المسار خط الترجمة الأصلية التجريبي ولا يغيّر ملف دعمه.

تُضبط Unicorn مرة واحدة عبر `cmake/NeverDUnicorn.cmake`، وتُشارك مع الاختبارات الدلالية وتتوفر عند `BUILD_TESTING=OFF`. تؤدي الواجهات المجهولة وسلوكيات بيئة المعالج غير المعروفة إلى توقف صريح؛ وتظل حالة الفشل المعادة من برنامج التشغيل متميزة عن المحاكاة غير المكتملة. راجع [محاكاة برامج التشغيل](driver-emulation.md) لمعرفة الحدود والتقارير وعمليات دورة الحياة غير المدعومة.

تظل واجهة C API الأصلية مقتصرة على التهيئة. يستخدم JSON السيناريو محلّلًا صارمًا واحدًا فوق خيارات التنفيذ نفسها، مع إعلان الحقول وأنواع الطلبات في قوائم `.def`. تقع إعادة ضبط العنوان الأساسي المطلوبة وتهيئة security cookie ضمن مسؤولية محمّل التنفيذ. يملك نموذج Windows كائنات IRP/موضع المكدس/الملف ويتحقق من الإكمال المتزامن أو المعلّق بعنصر عمل؛ وتنسّق الجلسة ترتيب الاستدعاءات الراجعة تحت ميزانيات تنفيذ مشتركة. الاستيرادات المجهولة غير المستخدمة روابط مؤجلة؛ وتنفيذها أو قراءة بيانات تصدير غير ممثلة يؤدي إلى توقف صريح.

يخصص سجل الصادرات عناوين ضيف مستقرة للاستيرادات الثابتة وللبحث الديناميكي عن الإجراءات معًا. توافر التصدير مستقل عن تنفيذه: يُحل الغياب الصريح إلى NULL، ويرتبط الإجراء الموجود غير الممثل بمصيدة، ويوقف التوافر الديناميكي غير المحدد التنفيذ. يمتلك نموذج الطلبات هويات ملفات مستقلة وقوائم MDL مملوكة للطلبات، بما فيها صلاحيات التعيين وانتهاء العمر. تقرأ بيئة التنفيذ وسائط الضيف المتغيرة عبر قارئ وسائط Win64 المتحقق منه في الجلسة. تحتفظ أعطال الواجهة الخلفية بسببها الأول بصيغة منظمة؛ ولا تستأنف المشاهدة والتقارير معالجًا متعطلًا ولا تعني معالجة استثناءات Windows.

يدير نموذج Windows أيضًا واصفات MDL مستقلة لمخزن الذاكرة غير القابل للترحيل؛ تحرير الواصف لا يحرر المخزن المؤقت الأصلي. تظل سلاسل MDL وربطها بـIRP غير ممثلة في النموذج. يدير نموذج سجل منفصل شجرة السيناريو الصريحة وصلاحيات المقابض وعمر المفاتيح والقيم، بصورة مستقلة عن قائمة التصدير. يشترك الفحص المسبق والتنفيذ في قواعد التحقق نفسها. يحتفظ التقرير بالقيم النهائية ويفحص التفريغ المقابض التي بقيت مفتوحة.

يدير `KernelScheduler` ترتيب الجاهزية وهوية الاستدعاءات ومواعيد المؤقتات؛ ويملك `KernelDispatcher` كائنات DPC والمؤقتات والأحداث المعتمة وإشاراتها. يدير `KernelModel` تسجيل الانتظار وأعمار العمل/الجهاز وإكمال IRP. يوقف `DriverSession` ويستأنف مكدسات منفصلة وسياقات CPU كاملة، بما فيها وسائط Win64 على المكدس، مع ذاكرة مشتركة. يتقدم الوقت الافتراضي عند حدود المؤقت/الانتظار/الإلغاء فقط عند غياب إطار جاهز؛ وينفذ CPU0 بصورة تعاونية حتمية DPC عند `DISPATCH_LEVEL` والعمل عند `PASSIVE_LEVEL`. لا يشمل ذلك الخيوط/APC/الأقفال الدورانية العامة أو إلغاء WDM/PnP أو تقديم السيناريوهات العامة المتزامن أو PnP/الطاقة الكامل أو العتاد. تأتي حدود IRQL من `KernelAPIIRQL.def` وتتحقق النماذج المسؤولة من الشروط المعتمدة على الوسائط.

يحتفظ `KernelModelDeviceStack` بمالك الجهاز وبرنامج التشغيل وتخصيصه وجيرانه وحالة انتظار الحذف ومراجعه الداخلية في سجل واحد. تختلف قائمة الضيف `NextDevice` عن رسم الإرفاق الذي يملكه النموذج. يحافظ حل الاسم على الجهاز الأدنى المسمى لـ`FILE_OBJECT` والتقارير، ويختار القمة الحالية للإرسال الأول وأعلام READ/WRITE، ويحتفظ بالمسار كله. لا يبطل الفصل أو الحذف جهازًا ما زال طلب أو استدعاء يحتفظ به؛ ويعد `ReferenceCount` المقابض المفتوحة فقط.

يدير `KernelModelIRPStack` المؤشرات المحدودة والإرسال إلى الهدف المحدد وفك الإكمال في الحزمة الأصلية. تبقى كتابات Copy/Skip/SetCompletion المضمنة المرجع المعتمد. تُفصل حالة الإرسال عن تحكم الإكمال و`IoStatus` النهائي، ويمكن نقل pending بعد عودة الإرسال. يحتفظ `STATUS_MORE_PROCESSING_REQUIRED` بـIRP/MDL/المخازن حتى استئناف الفك النهائي، بما فيه الإكمال المتداخل. يحمل `KernelGuestCall` مالك النظام الفرعي ورمزًا محليًا لمنع تصادم استمراريات WDM/WDF؛ ويحفظ `DriverSession` إطارات CPU وIRQL الموروث. يمكن لبرنامج تشغيل ضيف واحد الإرفاق فوق PDO تملكها جهة مستقلة في السيناريو؛ لا تزال IRP التي يخصصها برنامج التشغيل مباشرة غير مدعومة. لا يُدعم إرفاق/تمرير WDF أو توسيع مكدس نشط أو الفصل الوسطي أو تغيير الوظيفة الرئيسية أو الأهداف خارج المسار. يُصفّر كل موضع أدنى مستهلك قبل استدعاء دالة الإكمال العليا.

`DriverPnp.h` و`DeviceLifecycle.def` العام يحددان تعدادات دورة الحياة والعقود التي تتطلب حالة نجاح محددة؛ وتشترك مرحلة التحقق من السيناريو والإكمال النهائي للضيف في `devicePnpFinalStatusError`. يملك `KernelModelPnpDevices` هوية PDO الثابتة وقائمة برنامج تشغيل المزوّد المستقلة وملاحظات AddDevice الفعلية. يربط `KernelModelPnpRequests` معاملات دورة الحياة وهوية الجهاز والملف الثابتة بسجل IRP الموجود، دون اختلاق رفض لعمليات I/O بسبب التوقف أو انتظار الإزالة أو حالة الطاقة. تصل IRP العادية إلى الإرسال الفعلي للضيف، ويقرر برنامج التشغيل ما ينجح أو يفشل أو ينتظر. يدير `KernelModelPnpCompletion` استلام الناقل وإكماله الفعليين والمواعيد الافتراضية، مع إعادة استخدام `KernelModelIRPStack` والاستمراريات الموسومة بمالكها. يثبّت الإكمال العلوي النهائي حالة دورة الحياة؛ ويتطلب نجاح PnP إكمال التمرير إلى المزوّد، بينما قد يُبقي فشل Start/QueryStop/QueryRemove المبكر ملاحظات الناقل null. تتطلب Stop/CancelStop/SurpriseRemoval/CancelRemove/Remove الحالة STATUS_SUCCESS بالضبط. تُرفض STATUS_RESOURCE_REQUIREMENTS_CHANGED ‏(0x119) في QueryStop لأن إعادة الاستعلام عن الموارد غير ممثلة. يظل تقاعد المزوّد وفصل الضيف وحذفه عمليات مستقلة؛ ولا تُنظّف أجهزة الضيف المتسربة صامتًا. تدعم الوظائف الثانوية الثماني الشائعة عقود الناقل الصريحة الموصوفة هنا؛ ويبقى المشغّل العام تسلسليًا، وتتطلب Remove إغلاق الملفات وانتهاء الطلبات السابقة، مع السماح للاستدعاءات بتحرير أقفال الإزالة. لا يوفر ذلك عمليات PnP الأخرى أو العتاد والموارد العامة أو KMDF PnP.

يحدد `DriverPower.def` أسماء أنواع الطاقة وإجراءاتها ومصادر الطلبات، بينما يشترك `DriverPnp.h` في بنية `DriverPowerOperation` واحدة بين حزم السيناريو وطوابير استجابات FIFO الخاصة بكل PDO. يملك `KernelModelPowerRequests` حقائق الحزمة الصريحة والمسارات المحفوظة وحالة الإخطار المستقلة لكل DEVICE_OBJECT؛ ويعيد `PoSetPowerState` قيمة الإخطار الصريحة السابقة لذلك الكائن دون تغيير معاملة دورة الحياة. يملك `KernelModelPowerCompletion` الطلبات الفرعية الفعلية التي ينشئها `PoRequestPowerIrp`، ولكل منها IRP وصف نتيجة وفهرس استجابة مستقل. لا يستهلك إلا رأس طابور PDO المطابق، ولا يستنتج طلبًا أبًا من سياق الاستدعاء ولا يستعير نتيجة الأب. يعيد الإرسال المتداخل واستدعاء الإكمال النهائي ذي المعاملات الخمسة والنتيجة void استخدام الاستمراريات الموسومة بمالكها والمسارات المحتفظ بها ومكدسات الاستدعاء المنفصلة؛ وتبقى لقطة حالة الاستدعاء صالحة أثناء الانتظار حتى عودته. قد يكتمل الطلب الفرعي تزامنيًا قبل أن تعيد الواجهة STATUS_PENDING، وقد يكتمل طلب النظام الأب S0 قبل طلب الجهاز الفرعي D0. يحدد الإكمال العلوي النهائي حالة دورة الحياة المرصودة، وتظل ملاحظات الناقل مستقلة. يتطلب هذا النطاق المحدود للناقل الاصطناعي DO_POWER_PAGABLE دون DO_POWER_INRUSH والإرسال عند PASSIVE_LEVEL، ويدعم Query/Set للحالات D0/D3 وWorking/Sleeping3، ويحفظ SystemContext الصريح ذي 32 بت كقيمة معتمة. لا يوفر سياسة طاقة عامة أو WAIT_WAKE أو إيقاف التشغيل/الإسبات أو العتاد العام أو KMDF PnP أو تقديم سيناريوهات عامة بالتوازي.

`KernelRemoveLocks` هو المرجع الوحيد لتسجيل أقفال الإزالة وملكية DEVICE_OBJECT الدقيقة والحجم وتعدد قيم Tag وإشارة اكتمال التفريغ. وهو مستقل عن معاملات `DeviceLifecycle`؛ فلا تحدد حالة PDO أو هيئة Tag المشابهة لمؤشر IRP المالك. يتحقق `KernelModel` من احتواء التخزين كاملًا داخل امتداد الجهاز ومن حظر الوصول إلى البايتات المعتمة، ويوجه واجهات Ex الأربع ويسجل انتظار RemoveLock قابلًا للاستئناف بنوع محدد. يجعل التحرير الأخير المنتظر جاهزًا قبل عودة الاستدعاء، باستخدام استمراريات CPU القائمة دون اختلاق استدعاء. يتطلب فحص سياق AndWait المحدود مسار REMOVE مرتبطًا واستلام المزوّد الفعلي، ولا يتطلب إكمال الطبقة الأدنى أو بقاء حزمة Tag حية؛ وهو لا ينفذ Driver Verifier كاملًا. يحتفظ قبول REMOVE بقيود إغلاق الملفات وإنهاء الطلبات السابقة، مع السماح للاستدعاءات بتحرير الأقفال. تحتفظ الجلسة بمسار REMOVE خلال إطارات الاستدعاء المتبقية وتتحقق من التفكيك النهائي قبل التخلي عن الملكية. تُفحص حالات حيازة القفل والانتظار قبل تغيير حالة انتظار الحذف؛ ويلغي إنهاء العمر الفعلي لامتداد الجهاز تسجيل القفل. لا يستهلك تفريغ القفل مراجع عناصر العمل أو المسار.

تحدد `DriverResources.h` / `DriverResources.def` و`DriverInterrupts.h` / `DriverInterrupts.def` تخصيصات الذاكرة والمقاطعات الثابتة لـ`register_bank`. يتولى `DriverScenario` الفحص المسبق لـJSON وC++؛ ويسجل `DriverResult` التكوين الابتدائي دون تكرار حالة البنك المرصودة. يملك `KernelResources` وحده التخصيصات الخام والمترجمة المحزّمة وأجيال الموارد والحضور الفيزيائي والطاقة. يملك `KernelMMIO` قيم السجلات المستمرة والتعيينات البديلة المستقلة؛ ويملك `KernelInterrupts` الاتصالات المعتمة والأقفال والنبضات الصريحة. يبني `KernelModelResources` حزم START للقراءة فقط ويدمج إكمال المزوّد الفعلي: ينشر نجاح START الأدنى جيله قبل الاستدعاءات العليا، ويحدّث SET الجهاز الفعلي لدى المزوّد إمكانية الوصول في D0/D3. يُفحص التنظيف بعد فشل START أو STOP/REMOVE قبل الإكمال النهائي لـIRP، بعد إتاحة فك التعيين والاتصال للاستدعاءات العليا، دون تنظيف ضمني. تمنع الإزالة المفاجئة الوصول إلى العتاد فورًا. يتحقق `GuestMemory` و`UnicornBackend` من معاملات CPU/API كاملة قبل آثار MMIO ويحفظان أول خطأ. تحتفظ إعادة التشغيل بالتخصيص الثابت بقيم البنك. تظل RAM الاعتباطية وإعادة توزيع الموارد والمنافذ والمقاطعات المشتركة/المستوية/القائمة على الرسائل وواجهات DMA الأخرى غير مدعومة.

يمتلك `DriverDMA.h` / `DriverDMA.def` القدرات الصريحة لكل PDO والمعاملات الخارجية المستقلة. يسجل `KernelPhysicalMemory` تخصيصات RAM الحية الدقيقة، ويمنح هويات صفحات مشتركة ويثبت نطاقات بايتات؛ وMDL عروض لهذه المرجعية وليست مخازن منسوخة. يتيح `GuestMemory` / `UnicornBackend` الوصول إلى نطاق الذاكرة كاملًا متجاوزًا صلاحيات CPU دون تغييرها، ويرفض MMIO والوصول أثناء التنفيذ أو إعادة الدخول أو بعد خطأ، ويثبت أعطال الخلفية غير المتوقعة. يمتلك `KernelDMA` النطاقات المنطقية المستقلة وهويات الدوال المرتبطة بالمحوّل وتعيينات common/SG وقبول سجلات التعيين ومراجع الاستدعاءات؛ ويحل `KernelDMAEvents` جيل PDO الملتقط عند التسليم الفعلي. يصل `KernelModelPhysicalMemory` و`KernelModelDMA` و`KernelModelDMATransfers` ملكية التخصيص/MDL الأصلية باستدعاءات الضيف غير المباشرة الحقيقية. تتيح موارد SG المتاحة التسليم داخل الاستدعاء، بينما تحجز الاستدعاءات المصطفة الهوية والسعة حتى الترقية بترتيب FIFO. أعمار التعيين والاستدعاء منفصلة: يمكن لـPut تحرير تثبيت البيانات والواصف قبل عودة الاستدعاء، ولا يبقي الاحتفاظ بالاستدعاء IRP مكتملًا حيًا. يسجل `DmaWritable` قصد القفل مستقلًا عن صلاحيات CPU. في اللحظة نفسها، يسبق نشر المزوّد آثار RAM الخاصة بـDMA، ثم تُقيّم أهلية المقاطعات. تظل الأجيال والحضور والطاقة ملكًا حصريًا لـ`KernelResources`؛ ولا يخمّن DMA سجلات المورّد أو يطلق IRQ أو يكمل IRP أو يمتلك دورة حياة ثانية. لا تُعاد العناوين المنطقية، وتحفظ إخفاقات التحقق من المعاملة المشاهدات دون تغيير RAM. تشمل الواجهة المنمذجة common buffer متماسكًا وSG من الإصدار الأول وقنوات DMA لسيد ناقل مع ترجمة العناوين فوق RAM محدودة للنموذج؛ ولا تشمل العتاد العام أو المتحكمات التابعة أو واجهات DMA الأخرى.

يوسع `KernelDMAChannels` موزّع النطاقات نفسه بحجوزات قنوات وطابور FIFO ذي أنواع SG/قناة. يفصل حالة الاستدعاء الراجع عن سجلات التعيين المحتفظ بها وعن التعيين المجمع لكل عملية. تحجز العملية نافذتها المنطقية مرة واحدة وتوسّع تثبيتًا فعليًا واحدًا في موضعه؛ فلا تنسخ استدعاءات MapTransfer المتداخلة زمنيًا RAM أو تخصم السجلات مرتين أو تتراكب مع تعيين آخر. تتحقق خطط النقل والعودة والتفريغ والتحرير الخالية من الآثار من الهويات ودفعات الترقية الكاملة قبل النشر. يفك `KernelModelDMAChannels` ABI غير المباشر ولقطة CurrentIrp الفعلية عند التسجيل، ويتشارك مع SG مساعد عرض MDL. يشترك نوع الجدولة المستقل `DMAAdapterControl` في ترتيب DMA وسعته وحفظ المستدعي الأب أثناء التنفيذ المباشر؛ ولا يفسر إلا نموذج DMA البتات الـ32 الدنيا للإجراء المعاد. يُحمى IRP الملتقط في الطابور قبل فك المكدس النهائي؛ ويحرر دخول الاستدعاء الراجع مرجع الإدخال، فيجوز إكماله داخل جسمه. يزيل التفريغ المجمع البايتات المعيّنة، بينما يزيل FreeMapRegisters المطابق الحجز المستقل. لا يسقط عقد التخزين المتماسك في `KeFlushIoBuffers` أيًا من الالتزامين.

يربط `KernelInterrupts` كل نبضة صريحة برمز الاتصال وجيل الموارد الموجودين عند تقديم طلب المصدر بنجاح. يفحص `KernelModelInterruptEvents` مسبقًا سعة جميع منتجي الأحداث في اللحظة نفسها قبل تقديم الساعة أو تغيير المشاهدات، بما فيها العدد الدقيق لاستدعاءات إلغاء إطار العمل؛ فلا تستهلك المؤقتات أو إكمالات المزوّد أو الإلغاءات المساحة المحجوزة لـISR بصمت. يسبق نشر حالة عتاد المزوّد الفعلية التحقق من أهلية النبضة، وتسبق المقاطعات المقبولة DPC والاستدعاءات عند المستوى السلبي. تظل الجدولة تعاونية: لا يتقدم الوقت الافتراضي إلا عند الخمول، ولا يعني التأخير الصفري استباق التعليمات. يفك `KernelModelInterrupts` ترميز ABI التقليدي ذي الأحد عشر وسيطًا وحقول Ex المحددة، بينما يمنح `KernelGuestCall` استدعاءات المقاطعة مالكًا ورمزًا مستقلين. تمسك ISR واستدعاءات المزامنة القفل غير التكراري نفسه عند DIRQL المخصص؛ وتحفظ إطارات CPU المتداخلة IRQL/CR8 للمستدعي، وتستخدم BOOLEAN البايت AL فقط. تتطلب الأقفال اليدوية هوية التنفيذ نفسها وIRQL المحفوظ؛ ولا يجوز للاستدعاء الرجوع مع قفل لم يُحرر. تبقى النبضات المهيأة بعد انتهاء IRP المصدر؛ ويسجّل انقطاع الاتصال أو غياب جيل متاح أو D3 سببًا صريحًا لعدم التسليم ويوقف التنفيذ، دون إعادة ربط أو اختلاق سلوك سجلات التمكين/الإقرار. يحتوي `DriverResult.Interrupts` مشاهدات مستقلة، لا طلبات IRP اصطناعية ولا إكمالات NTSTATUS.

يتولى `KernelFramework` إدارة روابط KMDF 1.33 وهوية جدول الدوال وكائنات WDF وسياقاتها ومهيئات أجهزة التحكم والطوابير الافتراضية التسلسلية ومقابض الطلبات. تفوّض واجهات الربط ذات الأنواع المحددة للأجهزة والطلبات مسؤولية نطاق أسماء WDM والتخزين وحالة الحزم وتعيين MDL والتحقق من الإكمال إلى `KernelModel`؛ ولا ينشئ أي طرف أجهزة أو حزم IRP مكررة. يحمل توجيه الطابور حالة إرسال يملكها إطار العمل، منفصلة عن عودة استدعاء الضيف ذي النوع `void`. تنفذ استمراريات الإكمال التنظيف وتدمير الكائنات التابعة قبل إنهاء عمر IRP، بينما تحتفظ المراجع الخارجية بسياق WDF فقط. يُرفض حذف الطلبات المعلقة قبل تغيير الكائنات الأسلاف؛ ويظل الإلغاء التلقائي وتفريغ الطلبات أثناء الحذف غير مدعومين. ينفذ `DriverSession` الاستدعاءات المتداخلة ضمن ميزانيات مشتركة. يتحقق `DriverImage` من بيانات CFG؛ ويتولى `GuardControlFlow` الأهداف المعلنة للصورة وواجهات API، ويحفظ محوّل CPU حالة استدعاءات check/dispatch. لا تزال أجهزة PnP والجدولة العامة للطوابير وإلغاؤها وامتدادات الفئات وUMDF غير مدعومة.

يحدد السيناريو مهلة إلغاء افتراضية لكل نقل عبر `cancel_after_100ns`. يملك سجل كل IRP في `KernelModel` المهلة والوقت المطلق الفعلي `cancel_requested_at_100ns`، وتبقى السيناريوهات العامة متسلسلة. يطبق `KernelModel` التأخير الصفري بعد التوجيه وقبل استدعاء الإدخال/الإخراج الراجع للضيف، ويحفظ نتيجة الإكمال السابق ويضم المهل الموجبة إلى تقدم الوقت عند الخمول. يدير `KernelFramework` العلامة وإزالتها وحالتي الاصطفاف والتسليم والمرجع الداخلي حتى عودة استدعاء الإلغاء. الاصطفاف وحده لا يسمح بالإكمال؛ وبعد التسليم يمكن لعنصر عمل تنسيق الإكمال مع الاستدعاء المنتظر. لا يعيد الاحتفاظ بـWDF صلاحية IRP المكتمل. يفصل `KernelScheduler` الإلغاء عن عناصر العمل، ويحفظ النوع عند الإيقاف والاستئناف مع مشاركة حدود السعة والإرسال. تسبق DPC استدعاءات الإلغاء بترتيب FIFO، ثم يأتي العمل العادي واستئناف الانتظارات الجاهزة ذات المستوى السلبي. تعمل استدعاءات الإلغاء عند `PASSIVE_LEVEL`. لا يوفر عقد أجهزة التحكم هذا روتين إلغاء WDM أو مجدولًا عامًا للطوابير.

تستخدم `WdfRequestMarkCancelable` القديمة، عند وجود IRP ملغى مسبقًا، استدعاء `GuestCall` متداخلًا في استكمال الواجهة الحالية. يمكن للإلغاء والتنظيف والتدمير النهائي الانتظار؛ ولا يستأنف المستدعي إلا بعد انتهاء الاستكمال كله. يظل الإلغاء اللاحق للتسجيل عبر المجدول. يملك `KernelFramework` هوية WDF ونتائج getter المحايدة المحددة أثناء الإكمال وبعده. يفوض مضيف دوال الوصول IRP الأصلي وInformation ذي 64 بت وهوية MDL إلى `KernelModel`، الذي يرفض كذلك إكمال IRP يملكه الإطار عبر WDM الضيف. يُنشأ واصف SystemBuffer MDL واحد لكل طلب عند الحاجة؛ وتحتفظ المخازن المباشرة بواصفها ولا ينشئ الاسترجاع وحده تعيينًا. يبطل الإكمال النوعين مع IRP والمخازن، بصورة مستقلة عن مراجع سياق WDF المحتفظ بها.

يمثل `KernelGuestException` ناتج API محدد النوع يحمل حالة من 32 بت، وهو منفصل عن أخطاء النموذج وأعطال الواجهة الخلفية. يحتفظ `DriverImage` ببيانات الاستثناءات الوصفية الموجودة في المحمّل نسبةً إلى عنوان الأساس المفضل. يبني `KernelSEH` منها خطة انتقال صرفة ومحدودة إلى معالج C شامل في x64 بالإصدار الأول، مع التحقق من ترجمة العناوين وقراءات المكدس. يستعيد السجلات العامة غير المتطايرة المحفوظة والمدعومة عبر إطارات الدوال المساعدة العادية، ويختار معالج الضيف الفعلي؛ ويرفض المرشحات/finally التي يصادفها وشخصيات GS/C++ والسلاسل والسجلات الناقصة والمقدمات واستعادة XMM. يطبق `DriverSession` خطة السجلات المتحقق منها فقط عند توقف API سليم، ويبقي نتائج تتبع API بقيمة null، ثم يستأنف المعالج ضمن التنفيذ نفسه. لا يمسح مطلقًا العطل المحتفظ به في الواجهة الخلفية، ولا يفك المكدس نحو مكدس callback آخر. يدعم هذا النطاق ExRaiseStatus/ExRaiseAccessViolation/ExRaiseDatatypeMisalignment؛ أما فحص عناوين المستخدم ومخازنه المثبتة والتعافي من أعطال CPU فتبقى أعمالًا منفصلة.
...
تحتفظ إعلانات الدوال باتفاقية الاستدعاء ضمن هوية التوقيع. تدعم طبقة ABI المشتركة استدعاءات Swift محدودة بمعاملات أعداد صحيحة بعرض 1 أو 2 أو 4 أو 8 بايت ومعاملات مؤشرات؛ وتخصص بنك سجلات الأعداد الصحيحة أولًا ثم حوامل المكدس النسبية إلى SP عند الدخول. يسجل كل حامل ضيق قاعدة التوسيع الدقيقة، وتدعم النتائج حتى كلمتين من الأعداد الصحيحة أو المؤشرات، أو أربع كلمات كاملة بالضبط تعود في x0–x3 على arm64. وتحافظ HighC على `swiftcall` في الإعلانات والتعريفات. على arm64، تستخدم معاملات ونتائج Swift من نوع float وdouble بنك سجلات الفاصلة العائمة المستقل؛ ولا تزال معاملات الفاصلة العائمة على المكدس وتواقيع Swift العائمة على x86_64 غير مدعومة. وقد تعلن جسور قيم Foundation المرصودة من المترجم مؤشر `swift_indirect_result` واحداً ومؤشر `swift_context` واحداً: يستخدم arm64 السجلين x8/x20 ويستخدم x86_64 السجلين RAX/R13، ولا يستهلك أي منهما بنك معاملات الأعداد الصحيحة العادي. وتحافظ HighC على خاصيتي المعاملين. تتطلب واردات بيانات Foundation الوصفية العامة اتفاق مخططات رموز المترجم وIR لاستعلامات فعلية وصادرات SDK الدقيقة عبر ARM64/x86-64 على macOS وMac Catalyst. لا تحدد لاحقة الرمز وحدها ABI. تظل المعاملات العامة أو المخفية غير المصرح بها وأنواع ردود النداء في Swift والحوامل المادية غير المدعومة مرفوضة. لا تثبت إعلانات Mac Catalyst التحقق من التنفيذ على أجهزة iOS.
...
يجوز لمدخل أصلي ذي رمز دالة عند عنوانه الدقيق في الصورة أن يفترض مؤقتًا أن كلمة x0 الكاملة وحدها قابلة للملاحظة. بعد أن يربط استنتاج المصدر ABI الكامل للمدخل، يعيد النشر إثبات LowIR نفسه باستخدام ذلك ABI؛ ولا يمنح هذا الافتراض المؤقت وحده ربطًا للمصدر أو ABI لإرجاع بايت. لا يستخدم استنتاج المعاملات من السجلات المحفوظة مثل هذا الاستدعاء المنطقي إلا بعد إعادة تأهيل موضعه الدقيق في LowIR وفق ABI الحالي للمدخل. ويبقى إثبات بايتات المدخل والاستعادة الكاملة للحالة هو الذي يحدد ما إذا كان السجل المحفوظ المرصود يصبح معاملاً. يمكن لنتيجة أصلية مربوطة من كلمتين توسيع الملاحظة إلى x0/x1 فقط إذا اجتاز الحاملان إثبات الزوج الأصلي؛ ويعيد النشر استنتاج الزوج الكامل قبل قبول الاستدعاء المنطقي.

يجوز لواردات libswiftCore الدقيقة الخاصة ببذرة Hasher وString.hash(into:) وHasher.finalize استعارة نطاق خاص من إطار ARM64 حجمه 72 بايت عبر وسيط Swift ABI المتحقق منه. يُبطل إثبات الحالة كل بايت مستعار بعد الاستدعاء ويرفض التداخل مع السجلات المحفوظة أو تسرّب الإطار؛ ولا يكفي الاسم المطابق دون إثبات الوارد وABI الحاليين.

يُظهر IR لعميل Swift 6.1.2 أيضاً أن وارد libswiftCore الدقيق `_DictionaryStorage.allocate(capacity:)` يعيد مؤشراً ويتلقى سعة صحيحة وبيانات وصفية للقاموس في `swiftself` على ARM64 وx64. تربط واجهة ABI الموثقة هذه الاستدعاء مع الحفاظ على آثار التخصيص؛ ولا تستعيد وحدها المستدعي أو تبعيات القاموس الأخرى.

يعرّف Swift 6.1.2 أيضاً واردَي libswiftCore الدقيقين `_DictionaryStorage.copy(original:)` و`resize(original:capacity:move:)` كاستدعاءين يعيدان مؤشراً مع بيانات القاموس الوصفية الملموسة في `swiftself`. يحمل resize كذلك سعة صحيحة وبايتاً منطقياً. يحافظ الإثبات على الاستدعاءين وآثار تخصيصهما، ويتحقق من الموفّر وABI الكامل، ولا ينشر المستدعين ذوي التبعيات الأخرى غير المحلولة.

يتلقى وارد libswiftCore الدقيق `KEY_TYPE_OF_DICTIONARY_VIOLATES_HASHABLE_REQUIREMENTS` مؤشراً واحداً لبيانات النوع الوصفية ولا يعود أبداً، وفق تصريح Swift 6.1.2. لا يُمنح عقد الإنهاء هذا إلا لموفره وABI الموثقين؛ ويظل الاستدعاء الأصلي والتعليمة الفخ في مسار المصدر. في دالة ARM64 تعود عادةً، يمكن لهذا الاستدعاء المحدد إنهاء فرع استثنائي بلا تابع، بما في ذلك الفرع الذي تتبعه مصيدة مباشرة. وتظل كل عودة عادية بحاجة إلى إثبات كامل للحالة.
