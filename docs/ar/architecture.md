...
`lib/emulation` مكوّن تنفيذ اختياري يُفعّل بواسطة `NEVERD_ENABLE_DRIVER_EMULATION`. تصل إليه CLI المسماة `emulate-driver` عبر واجهة C API العامة. يملك `DriverSession` مسؤولية التهيئة المحدودة لـx64 WDM واستدعاءات create/IOCTL/cleanup/close/unload المتزامنة الاختيارية؛ ويستخدم تعيين صورة Windows الكائن `BinaryImage` الكامل من المحمّل الموجود، بينما يملك نموذج Windows كائنات الضيف ودلالات API. يملك مهايئ Unicorn تنفيذ المعالج والمصدر المعتمد لذاكرة الضيف. لا يستخدم هذا المسار خط الترجمة الأصلية التجريبي ولا يغيّر ملف دعمه.
