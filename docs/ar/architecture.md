...
`DriverPnp.h` و`DeviceLifecycle.def` العام يحددان تعدادات دورة الحياة المشتركة وأنواع بيانات السيناريو والتقرير. يملك `KernelModelPnpDevices` هوية PDO الثابتة وقائمة مستقلة لكائنات برنامج تشغيل المزوّد وملاحظات AddDevice الفعلية. يربط `KernelModelPnpRequests` معاملات دورة الحياة وهوية الجهاز الثابتة بسجل IRP الموجود، ويستخدم قبول عمليات الملفات هذه الهوية. يحتفظ ارتباط FDO بـPDO وهوية دورة حياة الملف بصلاحيتهما بعد الفصل. يدير `KernelModelPnpCompletion` استلام الناقل وإكماله الفعليين والمواعيد الافتراضية المحدودة، ويعيد استخدام `KernelModelIRPStack` والاستمراريات الموسومة بمالكها دون حزمة ثانية أو استدعاء ضيف مصطنع. لا تُثبّت حالة دورة الحياة إلا عند الإكمال العلوي النهائي. يجب أن يصل PnP الناجح إلى المزوّد ويكتمل فيه؛ وتتطلب REMOVE وCANCEL_REMOVE الحالة `STATUS_SUCCESS` (0) بالضبط. يمكن أن تفشل START وQUERY_REMOVE في طبقة أعلى دون ملاحظات للناقل. تقاعد المزوّد وفصل الضيف وحذفه انتقالات ملكية منفصلة؛ لا تُنظّف الأجهزة المتسربة صامتًا. يقتصر النطاق العام على Start/QueryRemove/CancelRemove/Remove بلا موارد، ولا يشمل PnP العام أو الطاقة أو KMDF PnP.
