...
يحدد `DriverResources.h` و`DriverResources.def` تخصيصات `register_bank` الثابتة وصلاحيات السجلات وعروضها وأسماء الحقول والحدود المشتركة. يملك `DriverScenario` التحقق المسبق من JSON وC++؛ ويسجل `DriverResult` التكوين دون تكرار حالة البنك المرصودة. يملك `KernelMMIO` قيم السجلات المستمرة لكل PDO والنطاقات الفيزيائية المترجمة وأجيال الموارد وإمكانية الوصول إلى العتاد وتعييناته المستقلة. يملك `KernelModelResources` قوائم START الخام والمترجمة المحزّمة والمقروءة فقط وربطها بالمزوّد ودورة الحياة. يفعّل نجاح START الأدنى الفعلي جيل الموارد قبل الاستدعاءات العليا؛ ويحدّث SET الجهاز الفعلي لدى المزوّد إمكانية الوصول في D0/D3. يُفحص التنظيف بعد فشل START أو STOP/REMOVE قبل الإكمال النهائي لـIRP؛ ويمكن لاستدعاءات الإكمال العليا إلغاء التعيينات أولًا، دون تحرير ضمني. تمنع الإزالة المفاجئة الوصول إلى العتاد فورًا. يتحقق `GuestMemory` و`UnicornBackend` من عملية CPU/API الأصلية كاملة قبل استدعاءات السجلات، ويشتركان في تثبيت أول خطأ أو توقف كي لا يسبب تقسيم Unicorn آثارًا بعد الفشل. تحافظ إعادة التشغيل بالتخصيص الثابت على القيم؛ ولا تملك التعيينات البديلة نسخًا منفصلة من البايتات. لا ينفذ ناقل سجلات الذاكرة المحدود RAM اعتباطية أو إعادة توزيع الموارد أو المنافذ أو المقاطعات أو DMA.
