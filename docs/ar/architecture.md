...
يدير `KernelModelIRPStack` المؤشرات المحدودة والإرسال إلى الهدف المحدد وفك الإكمال في الحزمة الأصلية. تبقى كتابات Copy/Skip/SetCompletion المضمنة المرجع المعتمد. تُفصل حالة الإرسال عن تحكم الإكمال و`IoStatus` النهائي، ويمكن نقل pending بعد عودة الإرسال. يحتفظ `STATUS_MORE_PROCESSING_REQUIRED` بـIRP/MDL/المخازن حتى استئناف الفك النهائي، بما فيه الإكمال المتداخل. يحمل `KernelGuestCall` مالك النظام الفرعي ورمزًا محليًا لمنع تصادم استمراريات WDM/WDF؛ ويحفظ `DriverSession` إطارات CPU وIRQL الموروث. يوجد حاليًا برنامج تشغيل ضيف واحد، دون PDO أو تنفيذ AddDevice/PnP/الطاقة أو IRP يخصصه برنامج التشغيل. لا يُدعم إرفاق/تمرير WDF أو توسيع مكدس نشط أو الفصل الوسطي أو تغيير الوظيفة الرئيسية أو الأهداف خارج المسار. يُصفّر كل موضع أدنى مستهلك قبل استدعاء دالة الإكمال العليا.
