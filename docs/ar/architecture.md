...
يملك `KernelScheduler` الترتيب الحتمي لعناصر العمل وهوية الاستدعاءات الراجعة، ويدير `KernelModel` عمر العناصر والأجهزة وعقد IRP المعلّق والمكتمل. يفرغ `DriverSession` الطابور عند `PASSIVE_LEVEL` بعد عودة استدعاءات الضيف وقبل تقدم الطلبات المتسلسلة. يحفظ Unicorn سياق المعالج كاملًا مع إبقاء ذاكرة الضيف مشتركة. لا تعني نماذج المؤقتات/DPC الداخلية توفر واجهات ضيف عامة أو دعمًا عامًا للخيوط والانتظار والإلغاء وKMDF وPnP/الطاقة والعتاد.
