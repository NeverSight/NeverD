...
`lib/emulation` مكوّن تنفيذ اختياري يُفعّل بواسطة `NEVERD_ENABLE_DRIVER_EMULATION`. تصل إليه CLI المسماة `emulate-driver` عبر واجهة C API العامة. يملك `DriverSession` مسؤولية التهيئة المحدودة لـx64 WDM واستدعاءات create/IOCTL/read/write/cleanup/close/unload المتزامنة الاختيارية؛ ويستخدم تعيين صورة Windows الكائن `BinaryImage` الكامل من المحمّل الموجود، بينما يملك نموذج Windows كائنات الضيف ودلالات API. يملك مهايئ Unicorn تنفيذ المعالج والمصدر المعتمد لذاكرة الضيف. لا يستخدم هذا المسار خط الترجمة الأصلية التجريبي ولا يغيّر ملف دعمه.
...
تظل واجهة C API الأصلية مقتصرة على التهيئة. يستخدم JSON السيناريو محلّلًا صارمًا واحدًا فوق خيارات التنفيذ نفسها، مع إعلان الحقول وأنواع الطلبات في قوائم `.def`. تقع إعادة ضبط العنوان الأساسي المطلوبة وتهيئة security cookie ضمن مسؤولية محمّل التنفيذ. يملك نموذج Windows كائنات IRP/موضع المكدس/الملف ويتحقق من الإكمال المتزامن؛ وتنسّق الجلسة ترتيب الاستدعاءات الراجعة تحت ميزانيات تنفيذ مشتركة. الاستيرادات المجهولة غير المستخدمة روابط مؤجلة؛ وتنفيذها أو قراءة بيانات تصدير غير ممثلة يؤدي إلى توقف صريح.
