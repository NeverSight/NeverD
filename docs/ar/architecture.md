...
يملك `KernelFramework` ربط KMDF 1.33 وهوية جدول الدوال والبرامج/الكائنات العامة وأعمار السياقات ذات النوع. ينفذ `DriverSession` استدعاءات الضيف المتداخلة ويستأنف عملية إطار العمل المعلقة ضمن الميزانيات المشتركة. يتحقق `DriverImage` من بيانات CFG، ويملك `GuardControlFlow` أهداف الصورة/API المعلنة، ويحفظ محوّل CPU حالة check/dispatch. لا يضيف ذلك أجهزة KMDF أو طوابيره أو طلباته أو امتدادات الفئات أو UMDF.
