...
[Документация](README.md) · [Дорожная карта](roadmap.md) · [Участие](CONTRIBUTING.md)
...
Экспериментальная команда `neverd mobile app.apk -o recovered-app` по умолчанию восстанавливает Java из APK, DEX и smali встроенным движком NeverD с Python 3.10+, без Java и JADX во время работы. Явный `--jadx PATH` выбирает необязательный внешний адаптер; автоматического перехода нет. Эта функция не добавляет поддержку APK и Java в SDK или загрузчик GUI. Настройка, входы, отчёты и ограничения описаны в [руководстве по восстановлению Java для Android](android.md).
