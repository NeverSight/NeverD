...
- Точный Objective-C stub конкретного selector может связать динамический формат, только если вызов заканчивается на фиксированном префиксе параметров. Любой дополнительный аргумент, неточный stub, конфликт объявления или несовпадение физического ABI остаётся неразрешённым.
