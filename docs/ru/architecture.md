...
Проверка логического результата Swift объединяет текущий входной ABI Objective-C, неизменяемые байты прямого вызова, точный сильный импорт среды выполнения и полное доказательство использования LowIR. Допускается одно место сравнения; ABI остальных вызовов заново строятся по текущему каталогу. Нативные предположения, динамическая диспетчеризация и повторные места вызовов отклоняются. Само это доказательство не публикует исходный код и не объявляет ABI с байтовым результатом.
