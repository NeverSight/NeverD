...
`KernelScheduler` определяет детерминированный порядок рабочих элементов и идентичность обратных вызовов; `KernelModel` управляет временем жизни элементов/устройств и контрактом ожидающего/завершённого IRP. `DriverSession` выполняет очередь на `PASSIVE_LEVEL` после возврата гостевых вызовов, прежде чем продолжить последовательные запросы. Unicorn сохраняет полный контекст CPU при общей гостевой памяти. Внутренние модели таймеров/DPC не означают публичных гостевых API, общей поддержки потоков/ожиданий, отмены, KMDF, PnP/питания или оборудования.
