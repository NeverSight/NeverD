...
`KernelFramework` владеет привязкой KMDF 1.33, идентичностью таблицы, драйверами/общими объектами и временем жизни типизированных контекстов. `DriverSession` исполняет вложенные гостевые вызовы и возобновляет операцию фреймворка с общими бюджетами. `DriverImage` проверяет метаданные CFG; `GuardControlFlow` владеет объявленными целями образа/API, адаптер CPU сохраняет состояние check/dispatch. Это не добавляет устройства, очереди, запросы, расширения классов KMDF или UMDF.
