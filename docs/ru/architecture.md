...
`DriverResources.h` и `DriverResources.def` задают фиксированные назначения `register_bank`, доступ и ширину регистров, имена полей и общие ограничения. `DriverScenario` выполняет предварительную проверку JSON/C++; `DriverResult` сохраняет конфигурацию, не дублируя наблюдаемое состояние банка. `KernelMMIO` владеет постоянными значениями регистров каждого PDO, преобразованными физическими диапазонами, поколениями ресурсов, доступностью оборудования и независимыми отображениями одного банка. `KernelModelResources` владеет упакованными исходными/преобразованными списками START только для чтения и связью с провайдером и жизненным циклом. Фактическое успешное нижнее START активирует поколение до верхних callbacks; фактический SET устройства у провайдера меняет доступность D0/D3. Очистка при неудачном START и STOP/REMOVE проверяется до терминального завершения IRP; верхние callbacks завершения могут сначала снять отображения, без неявного освобождения. Внезапное удаление сразу запрещает доступ к оборудованию. `GuestMemory` и `UnicornBackend` проверяют целиком исходные обращения CPU/API до callbacks регистров; общий фиксатор первой ошибки/остановки исключает эффекты после сбоя при дроблении обращения Unicorn. Повторный запуск с фиксированным назначением сохраняет значения, а алиасы не имеют отдельных копий байтов. Эта ограниченная шина регистров памяти не реализует произвольную RAM, перераспределение ресурсов, порты, прерывания или DMA.
