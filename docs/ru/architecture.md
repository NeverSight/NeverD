...
`KernelModelIRPStack` управляет ограниченными курсорами, точной нижней целью и разворачиванием завершения в исходном пакете. Inline-записи Copy/Skip/SetCompletion остаются источником истины. Статус диспетчеризации, управление завершением и окончательный `IoStatus` разделены; pending может передаваться после возврата. `STATUS_MORE_PROCESSING_REQUIRED` удерживает IRP/MDL/буферы до возобновлённого окончательного разворачивания, включая вложенное завершение. `KernelGuestCall` содержит подсистему-владельца и локальный token, исключая коллизии WDM/WDF; `DriverSession` сохраняет CPU-кадры и унаследованный IRQL. Один гостевой драйвер может подключаться над отдельно принадлежащими сценарию PDO; выделенные драйвером IRP и IRP питания не поддерживаются. WDF-присоединение/передача, расширение активного стека, отсоединение среднего слоя, смена основной функции и цели вне маршрута не поддерживаются. Каждая использованная нижняя позиция стека обнуляется перед верхним callback завершения.

`DriverPnp.h` и публичный `DeviceLifecycle.def` задают общие перечисления жизненного цикла и типы сценария/отчёта. `KernelModelPnpDevices` владеет стабильной идентичностью PDO, отдельным списком драйвера-провайдера и реальными наблюдениями AddDevice. `KernelModelPnpRequests` связывает транзакцию состояния и неизменяемую идентичность устройства с существующей IRP; доступ к файлам опирается на эту идентичность. `KernelModelPnpCompletion` управляет фактическими получением/завершением шины и ограниченными виртуальными сроками, повторно используя `KernelModelIRPStack` и продолжения с владельцами. Второй пакет или фиктивный callback не создаётся. Только окончательное верхнее завершение фиксирует состояние. Освобождение провайдера и гостевое отсоединение/удаление — отдельные переходы владения; утечки гостя не очищаются молча. Публичный набор ограничен Start/QueryRemove/CancelRemove/Remove без ресурсов, а не общим PnP, питанием или KMDF PnP. Успех PnP требует фактического завершения провайдера; ранняя верхняя ошибка START/QUERY_REMOVE может оставить наблюдения шины null. Идентичность жизненного цикла устройства/файла сохраняется после отсоединения.
