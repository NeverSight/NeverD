...
Дополнительные тесты проверяют полные контексты CPU (регистры, флаги, SIMD, FPU, CR8), общую память и отказ от контекстов после сбоя или чужого бэкенда. Рабочие фикстуры проверяют помеченные ожидающие IRP, отложенное завершение, ошибки очереди и времени жизни, зависание, общие бюджеты и раздельные статусы диспетчеризации/завершения публичных отчётов. Внутренние тесты таймеров/DPC проверяют только переходы, а не гостевые API или полное асинхронное окружение Windows.
