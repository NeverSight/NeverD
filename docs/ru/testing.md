...
`DriverGuardTests.cpp` и четыре оригинальных варианта `driver_guard.c` проверяют активный/неактивный CFG, перемещение базы, ABI check/dispatch, повреждённые и необъявленные цели. `KernelFrameworkTests.cpp` проверяет привязку и объекты. Оригинальный `driver_kmdf_lifecycle.c` при наличии зависимостей компилируется с настоящими заголовками WDK 1.33 и библиотекой `FxDriverEntry`; задайте `NEVERD_KMDF_FIXTURE` и `NEVERD_KMDF_CFG_FIXTURE` для отдельно собранных обычного образа и образа с активным CFG. Отсутствующие внешние артефакты явно пропускаются. `DriverKMDFLifecycleTests.cpp` и C API/CLI-кейсы в `DriverScenarioPublicTests.cpp` проверяют привязку, контексты, отложенное ссылками уничтожение, реальные гостевые вызовы, выгрузку и документированные ошибки создания. Доказательства получены на Linux и не подтверждают более широкий KMDF.
