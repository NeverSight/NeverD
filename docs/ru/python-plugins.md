**Языки**: [English](../python-plugins.md) | [简体中文](../zh-CN/python-plugins.md) | [繁體中文](../zh-TW/python-plugins.md) | [日本語](../ja/python-plugins.md) | [한국어](../ko/python-plugins.md) | [Français](../fr/python-plugins.md) | [Deutsch](../de/python-plugins.md) | [Español](../es/python-plugins.md) | [Italiano](../it/python-plugins.md) | [Русский](python-plugins.md) | [العربية](../ar/python-plugins.md)
...
### Строгая публикация бинарного sanitizer

`session.sanitize()` выполняет экспериментальную транзакцию «все точки или отказ» `binary-sanitizer-v1` и возвращает неизменяемый `SanitizeResult` только после проверки полного и согласованного аутентифицированного receipt. Хосты не-Darwin отказывают до lifting, генерации guard, создания кандидата или изменения namespace. `PUBLISH_INDETERMINATE` и `PUBLISHED_INCOMPLETE` являются ошибками и означают, что назначение уже может существовать и требует проверки перед использованием или повтором.

Полный Darwin receipt аутентифицирует только удерживаемый во время транзакции объект каталога назначения. Его можно переименовать после открытия, поэтому receipt не гарантирует, что исходный pathname продолжает указывать на объект во время транзакции или после возврата, и не является долговечной независимо проверяемой привязкой пути. При последующем открытии пути вызывающая сторона должна сохранить внешний anchor и снова аутентифицировать объект. Python пока не предоставляет нативного replay всего процесса; `NativeProcessReplayAdapter` — лишь fail-closed C++-граница доступности/factory Phase 0, где все хосты возвращают все возможности false и не предоставляют таблицу операций.
