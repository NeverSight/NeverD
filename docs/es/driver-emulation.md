...
Un subconjunto KMDF PnP sin recursos admite un FDO conectado directamente al PDO. `EvtDriverDeviceAdd` recibe un inicializador propiedad del framework; `WdfDeviceCreate` crea el FDO y `WdfFdoInitWdmGetPhysicalDevice` y `WdfDeviceWdmGetPhysicalDevice` conservan la identidad del PDO. Tras fallar AddDevice o terminar Remove, el framework ejecuta los callbacks de limpieza/destrucción y elimina el dispositivo. Las colas PnP admiten `WdfUseDefault` y `WdfTrue` para la gestión de energía; `WdfFalse` sigue sin gestión automática. `WdfDeviceInitSetPnpPowerEventCallbacks` admite `EvtDevicePrepareHardware`, `EvtDeviceD0Entry`, `EvtDeviceD0Exit` y `EvtDeviceReleaseHardware`. PrepareHardware recibe dos listas de recursos vacías antes de D0Entry; `WdfCmResourceListGetCount` devuelve cero y `WdfCmResourceListGetDescriptor` devuelve NULL. Un fallo en PrepareHardware o D0Entry pasa a ser el estado de START y aun así se ejecuta ReleaseHardware. Al detener o retirar desde D0, D0Exit precede a ReleaseHardware y al final del IRP. Los recursos no vacíos y la política general siguen fuera del alcance. `WdfIoQueuePnpHeld` indica la suspensión de la entrega antes de D0 y tras salir de D0; la parada/reanudación de solicitudes retenidas por el controlador queda fuera del perfil. El fixture WDK `driver_kmdf_pnp.c` usa `NEVERD_KMDF_PNP_FIXTURE` / `NEVERD_KMDF_PNP_CFG_FIXTURE`.
...
`wdm-x64-scheduled-v53`. `nt_status` sigue siendo el resultado de DriverEntry,
