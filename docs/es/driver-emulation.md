...
| Controlador de hardware, DMA, PCI, interrupciones o virtualización | Bancos explícitos, MMIO, interrupciones latched/de nivel exclusivas o compartidas y DMA coherente common/SG/channel | Otros modelos de dispositivos, RAM física arbitraria, PCI, puertos, MSI e interrupciones pasivas, otras interfaces DMA y estado CPU privilegiado |
...
Excepciones anidadas o desenrollados en colisión dentro de filtro/finally, personalidades GS/C++, metadatos encadenados/incompletos, prólogos y operaciones unwind XMM fallan explícitamente. Una excepción API o de usuario no tratada produce `model_error`; los demás fallos de memoria CPU, interrupciones e instrucciones inválidas siguen siendo terminales. En cada retorno del filtro deben permanecer intactos `EXCEPTION_POINTERS`, `EXCEPTION_RECORD` y los campos no admitidos de `CONTEXT`.
