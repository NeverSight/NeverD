...
Un subconjunto KMDF PnP sin recursos admite un FDO conectado directamente al PDO. `EvtDriverDeviceAdd` recibe un inicializador propiedad del framework; `WdfDeviceCreate` crea el FDO y `WdfFdoInitWdmGetPhysicalDevice` y `WdfDeviceWdmGetPhysicalDevice` conservan la identidad del PDO. Tras fallar AddDevice o terminar Remove, el framework ejecuta los callbacks de limpieza/destrucción y elimina el dispositivo. La cola PnP debe definir explícitamente `PowerManaged = WdfFalse`. `WdfDeviceInitSetPnpPowerEventCallbacks` admite `EvtDeviceD0Entry` y `EvtDeviceD0Exit`: D0Entry se ejecuta tras un START correcto del bus y antes de completar el IRP; un fallo se convierte en el estado de START. D0Exit se ejecuta al detener o retirar el dispositivo desde D0. Los callbacks de listas de recursos, las colas con gestión de energía y la política general siguen fuera del alcance. El fixture WDK `driver_kmdf_pnp.c` usa `NEVERD_KMDF_PNP_FIXTURE` / `NEVERD_KMDF_PNP_CFG_FIXTURE`.
...
`wdm-x64-scheduled-v51`. `nt_status` sigue siendo el resultado de DriverEntry,
