...
Este modelo acotado no ofrece todo el comportamiento asíncrono de Windows. No admite esperas alertables o de usuario, APC, cancelación general de solicitudes WDM, IRP simultáneos, API de KMDF para búferes de usuario `METHOD_NEITHER`, UMDF, dispositivos PnP de KMDF y planificación general de colas, PnP/energía completo, hardware general, otras interfaces DMA ni otros modos de interrupción. La inicialización sola ejecuta callbacks explícitamente encolados sin crear solicitudes ni descarga implícita.
...
`wdm-x64-scheduled-v32`. `nt_status` sigue siendo el resultado de DriverEntry,
