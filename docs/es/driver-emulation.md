...
| Controlador de hardware, DMA, PCI, interrupciones o virtualización | Banco de registros de memoria y MMIO declarados explícitamente | Otros modelos de dispositivos, RAM física, PCI, puertos, interrupciones, DMA y estado privilegiado de CPU |
...
Este modelo acotado no ofrece todo el comportamiento asíncrono de Windows. No admite esperas alertables o de usuario, hilos del sistema, APC, cancelación de solicitudes WDM, spinlocks, IRP simultáneos, cambios generales de IRQL, `METHOD_NEITHER`, UMDF, dispositivos PnP de KMDF y planificación general de colas, PnP/energía completo, hardware general, DMA ni interrupciones. La inicialización sola ejecuta callbacks explícitamente encolados sin crear solicitudes ni descarga implícita.
...
`wdm-x64-scheduled-v12`. `nt_status` sigue siendo el resultado de DriverEntry,
