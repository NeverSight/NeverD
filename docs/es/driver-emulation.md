...
| Controlador de hardware, DMA, PCI, interrupciones o virtualización | Bancos de registros, MMIO y callbacks de interrupción latched exclusiva declarados explícitamente | Otros modelos de dispositivos, RAM física, PCI, puertos, interrupciones compartidas/de nivel/MSI, DMA y estado privilegiado de CPU |
...
Este modelo acotado no ofrece todo el comportamiento asíncrono de Windows. No admite esperas alertables o de usuario, hilos del sistema, APC, cancelación de solicitudes WDM, spinlocks, IRP simultáneos, cambios generales de IRQL, `METHOD_NEITHER`, UMDF, dispositivos PnP de KMDF y planificación general de colas, PnP/energía completo, hardware general, DMA ni otros modos de interrupción. La inicialización sola ejecuta callbacks explícitamente encolados sin crear solicitudes ni descarga implícita.
...
`wdm-x64-scheduled-v13`. `nt_status` sigue siendo el resultado de DriverEntry,
