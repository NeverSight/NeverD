...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | MDL de cada solicitud, mappings de sistema e identidades PFN compartidas de solo lectura | Mappings de usuario e interfaces DMA fuera del subconjunto descrito abajo |
| MDL asignados por el controlador | Descriptores independientes o asociados a IRP, MDL parciales, mappings de usuario/sistema compartidos y liberación al completar | Cuotas, MDL manuales, espacios de proceso arbitrarios y reutilización de VA |
...
`MmMapLockedPagesSpecifyCache` acepta `UserMode` y `MmCached` en el contexto de un solicitante activo, incluido el contexto limitado de un worker adjunto, hasta `APC_LEVEL`. Cada llamada crea una vista independiente propiedad del proceso sobre el mismo almacenamiento físico, sin cambiar `MappedSystemVa`. Las vistas nunca son ejecutables (NX); `MdlMappingNoWrite` las hace de solo lectura. El almacenamiento del pool debe ser no paginado y ocupar páginas privadas completas. Volver a bloquear una vista de usuario con `MmProbeAndLockPages` conserva la identidad física original. La dirección solicitada debe conservar el desplazamiento de página del MDL y estar dentro de la arena VA limitada dedicada a las vistas de usuario; su agotamiento genera un `STATUS_INSUFFICIENT_RESOURCES` capturable.

`MmUnmapLockedPages` exige el par exacto dirección de usuario/MDL y el proceso creador (PID). Desasignar una VA original del escenario conserva las vistas MDL independientes y los alias de sistema. La salida de un proceso revoca solo sus propias vistas de usuario; las páginas físicas bloqueadas y los alias de sistema sobreviven. Las vistas activas impiden liberar el descriptor, el bloqueo raíz y el almacenamiento del pool o de la solicitud. Las direcciones de usuario revocadas no se reutilizan. Los espacios de proceso arbitrarios, otras políticas de caché y la reasignación de VA de usuario quedan fuera de este modelo limitado.
