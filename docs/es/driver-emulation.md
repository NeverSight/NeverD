...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | MDL de cada solicitud, mappings de sistema e identidades PFN compartidas de solo lectura | Interfaces DMA fuera del subconjunto descrito abajo |
| MDL asignados por el controlador | Descriptores independientes o asociados a IRP, MDL parciales, mappings de usuario/sistema compartidos y liberación al completar | Cuotas, MDL manuales, espacios de proceso arbitrarios |
...
| WDM `METHOD_NEITHER` | Búferes de usuario separados, sondeo, bloqueo MDL, identidades sintéticas, revocación de VA o salida tras el despacho y cancelación limitada | Sin tablas de páginas ni gestor general de procesos; se admiten vistas MDL acotadas por PID |
...
| Controlador de hardware, DMA, PCI, interrupciones o virtualización | Bancos explícitos, MMIO, interrupciones latched/de nivel exclusivas o compartidas y DMA coherente common/SG/channel | Otros modelos de dispositivos, RAM física arbitraria, PCI, puertos, otras interfaces DMA y estado CPU privilegiado |
...
`register_bank` admite `interrupts` solo o junto con `resources`; al menos una lista debe ser no vacía. `resource_free` rechaza el campo incluso vacío. Cada línea exige `id`, `raw_vector`, `raw_level`, `raw_affinity`, `translated_vector`, `translated_level`, `translated_affinity`, `mode` (`latched` o `level_sensitive`) y `share` (`device_exclusive` o `shared`). Las fuentes de nivel requieren `retrigger_after_100ns` positivo hasta INT64_MAX; las latched lo rechazan. Límites: 8 líneas por PDO y 32 en total, ID ASCII acotados y únicos por PDO, nivel bruto 0–65535 y DIRQL traducido 3–12. Ambas afinidades valen 1 (CPU0/grupo0); los vectores conservan sus 32 bits. Compartir un vector traducido requiere igual nivel, afinidad, modo e intervalo y `share: "shared"` en todos. Los descriptores de memoria preceden a los de interrupción ordenados, con flags de modo y uso compartido correctos. Se modelan líneas sintéticas declaradas, no un controlador PCI. `DriverInterrupts.h` / `DriverInterrupts.def` definen los nombres y límites.

READ/WRITE/IOCTL pueden declarar `interrupt_events` con `after_100ns`, `device_id` e `interrupt_id`. `action` es `pulse` por defecto para latched; las fuentes de nivel exigen `assert` o `deassert` y rechazan `pulse`. Las líneas latched exigen `pulse`. Los demás tipos rechazan el campo incluso vacío. Límites: 64 eventos por solicitud, 1024 declarados en total y demora no negativa hasta INT64_MAX. La admisión captura conexiones y generación del PDO; completar el IRP no cancela los eventos. Se entregan en límites de callback sin preempción de instrucciones; el tiempo avanza al quedar inactivo. La publicación de hardware precede a la admisión y los ISR, ordenados por DIRQL asignado, preceden a DPC/workers. Conexión perdida, generación no disponible o D3 generan `undelivered_reason` sin reasignación implícita.

Un pulso latched compartido llama a todos los ISR capturados por orden de registro, incluso tras TRUE. Una línea de nivel es el OR de estados por PDO/fuente/generación; repetir `assert` es idempotente. Todos los cambios simultáneos se aplican antes de muestrear. Si sigue activa, se vuelve a muestrear al terminar la ronda ISR más `retrigger_after_100ns`. En modo nivel, TRUE detiene la búsqueda de esa ronda, pero no reconoce ni borra ninguna fuente; solo `deassert` la desactiva. Las escrituras de registros no implican reconocimiento. Se admiten hasta 4096 rondas de entrega, con hasta 32 handlers cada una, además de los presupuestos de callbacks e instrucciones. Desconectar o retirar el dispositivo rechaza fuentes aún activas.

`IoConnectInterrupt` usa once argumentos y la asignación traducida exacta. `IoConnectInterruptEx` admite FullySpecified (1), LineBased (2, una línea del PDO explícito) y FullySpecifiedGroup (4, grupo0); desconectar exige versión/contexto coincidentes. Registro y desconexión requieren PASSIVE_LEVEL. Se admite un bloqueo privado o un `KSPIN_LOCK` no paginado inicializado por el llamador; varias conexiones pueden compartirlo con el mismo `SynchronizeIrql`, al menos igual a cada DIRQL asignado. Cero en LineBased elige ese nivel; la prioridad del planificador no cambia. Se mantienen CPU0/grupo0 y la ausencia de guardado flotante. El ISR real recibe el `KINTERRUPT` opaco y ServiceContext y devuelve BOOLEAN en AL. ISR, `KeSynchronizeExecution` y adquisición/liberación manual comparten el bloqueo real no recursivo. Synchronize corre al IRQL de sincronización y restaura IRQL/CR8; se verifica propietario y nivel guardado. Las API de bloqueos ejecutivos no pueden usar un bloqueo conectado. Siguen excluidos preempción de instrucciones. START fallido y STOP/REMOVE correcto exigen desconexión antes del final del IRP; nunca se descarta un DPC en cola de forma silenciosa.

`configuration.pnp_devices[].interrupts` conserva recursos y `configuration.interrupt_events` declaraciones. Las observaciones `interrupts` incluyen `source_request_index`, `event_index`, `device_id`, `interrupt_id`, `action`, `epoch`, `due_at_100ns`, `occurred_at_100ns`, `delivered_at_100ns`, `returned_at_100ns`, `interrupt_object`, `return_value`, `claimed` y `undelivered_reason`. `handlers[]` registra cada ISR realmente ejecutado: objeto, tiempos, BOOLEAN, `claimed` y `delivery_index`. Una aserción puede producir varias rondas; una desaserción o una aserción cancelada simultáneamente no inventa ningún ISR. `occurred_at_100ns` indica cuándo se aplicó el cambio. `claimed` agrega retornos reales; una interrupción no reclamada sigue siendo válida. `delivered_at_100ns` indica la primera entrega; `returned_at_100ns` y `claimed` describen la última ronda, mientras `handlers[]` conserva todo el historial. El [escenario de interrupciones](../examples/driver-interrupt-scenario.json) ejecutable usa el original `driver_wdm_interrupts.c` compilado con WDK auténtico y los artefactos opcionales `NEVERD_WDM_INTERRUPT_FIXTURE` / `NEVERD_WDM_INTERRUPT_CFG_FIXTURE`: siete solicitudes incluyen START retardado, IOCTL pendiente completado por ISR→DPC, limpieza/cierre de archivo y retirada. No cambian la entrada C/Python `scenario_json` ni el diseño de `neverd_driver_options_v1`. La ausencia de artefactos se omite explícitamente; la evidencia de ejecución se limita a Linux.
...
KMDF 1.33 usa exactamente la ABI 1.33.0: 458 entradas de función tienen identidades estables en el invitado y las 99 API siguientes tienen semántica de ejecución. `WdfVersionBind` y `WdfVersionUnbind` gestionan la vinculación del invitado alrededor del wrapper real WDK `FxDriverEntry`. `WdfGetDriver` lee las variables globales públicas del controlador. Los controladores no PnP, objetos genéricos, dispositivos de control, colas y solicitudes entrantes comparten contextos tipados, contadores de referencias y callbacks ejecutados de limpieza/destrucción/descarga. Salvo las DDI de interrupción y las operaciones de solicitudes y objetos con soporte IRQL explícito descritas abajo, las llamadas y callbacks del framework requieren `PASSIVE_LEVEL`; añadir referencias tras finalizar la limpieza sigue fuera de este perfil. Las entradas no modeladas, `WdfLdrQueryInterface`, las extensiones de clase y UMDF detienen explícitamente la ejecución.

API de interrupciones: `WdfInterruptCreate`, `WdfInterruptQueueDpcForIsr`, `WdfInterruptQueueWorkItemForIsr`, `WdfInterruptSynchronize`, `WdfInterruptAcquireLock`, `WdfInterruptReleaseLock`, `WdfInterruptEnable`, `WdfInterruptDisable`, `WdfInterruptWdmGetInterrupt`, `WdfInterruptGetInfo`, `WdfInterruptGetDevice`.

Los objetos de interrupción reutilizan recursos de línea o MSI asignados y bloqueos internos. ISR y callbacks sincronizados se ejecutan en el DIRQL asignado, o en `PASSIVE_LEVEL` para tratamiento pasivo; `WdfInterruptGetInfo` informa ese IRQL. Los DPC usan `DISPATCH_LEVEL` y los elementos de trabajo `PASSIVE_LEVEL`. Las inserciones repetidas se fusionan mientras esperan y la propiedad se conserva durante la suspensión hasta el retorno. Al encender se conecta y habilita tras D0Entry; al apagar se deshabilita, desconecta y drenan los callbacks diferidos antes de D0Exit y de liberar recursos. Habilitar/deshabilitar explícitamente ejecuta callbacks reales, sin inventar pulsos. Siguen excluidos `WDFSPINLOCK`/`WDFWAITLOCK` externos, serialización automática del padre, interrupciones de activación y conexiones inactive retenidas, así como la política de inactividad/activación y la reenumeración automática de dispositivos fallidos.

Las siguientes API de solicitudes y objetos admiten IRQL hasta `DISPATCH_LEVEL`: `WdfObjectDereferenceActual`, `WdfRequestComplete`, `WdfRequestCompleteWithInformation`, `WdfRequestRetrieveInputBuffer`, `WdfRequestRetrieveOutputBuffer`, `WdfRequestRetrieveInputMemory`, `WdfRequestRetrieveOutputMemory`, `WdfRequestRetrieveInputWdmMdl`, `WdfRequestRetrieveOutputWdmMdl`, `WdfRequestGetInformation`, `WdfRequestSetInformation`, `WdfRequestGetIoQueue`, `WdfRequestGetFileObject`, `WdfRequestWdmGetIrp`, `WdfRequestGetParameters`, `WdfRequestGetStatus`, `WdfRequestMarkCancelableEx`, `WdfRequestUnmarkCancelable`, `WdfRequestIsCanceled`, `WdfMemoryGetBuffer`.

`WdfObjectGetTypedContextWorker`, `WdfObjectContextGetObject`, `WdfObjectReferenceActual` también admiten los IRQL de interrupción modelados; acceder al contexto y adquirir referencias no invoca callbacks pasivos.

La finalización de solicitudes o la liberación de la última referencia a un objeto por encima de `PASSIVE_LEVEL` programa la limpieza, destrucción y entrega posterior reales a `PASSIVE_LEVEL`. La API antigua `WdfRequestMarkCancelable` sigue limitada a `PASSIVE_LEVEL` para conservar la ejecución síncrona del callback cuando la solicitud ya está cancelada. Las demás API mantienen sus restricciones IRQL actuales.
...
| `ExRaiseStatus`, `ExRaiseAccessViolation`, `ExRaiseDatatypeMisalignment` | Excepciones C `__except`, filtros reales y `__finally` de desenrollado; sin retorno normal de API |
...
| `IoConnectInterrupt`, `IoDisconnectInterrupt`, `IoConnectInterruptEx`, `IoDisconnectInterruptEx` | Líneas latched/de nivel asignadas, handlers y bloqueos exclusivos/compartidos; ABI heredada y Ex 1/2/4 a PASSIVE_LEVEL |
...
El booleano opcional `user_unmap_after_dispatch` revoca las direcciones de usuario originales de una transferencia WDM neither no vacía después del retorno del despacho y antes del trabajo o la cancelación programados. Las páginas bloqueadas por MDL y sus alias del sistema siguen disponibles hasta desbloquearlas; fallan los punteros de usuario sin bloquear y los bloqueos nuevos. Si se revoca la salida, `output_hex` queda vacío. No se reutilizan las direcciones de las asignaciones de usuario declaradas originalmente por el escenario; tampoco se modela su revocación en instantes arbitrarios.
...
construidos a mano, las asignaciones de usuario fuera del modelo MDL por proceso descrito abajo y el acceso directo mediante
...
El controlador puede modificar `MDL.Next` e `IRP.MdlAddress` para insertar o separar descriptores modelados. La finalización definitiva del IRP valida primero toda la cadena actual, desbloquea los MDL de usuario adjuntos, revoca sus alias y libera todos los descriptores adjuntos. Los descriptores separados y el almacenamiento del pool siguen perteneciendo al controlador. Los ciclos, enlaces desconocidos o liberados, descriptores compartidos entre IRP activos y descriptores privados WDF insertados en cadenas WDM fallan explícitamente. Las dependencias activas de DMA y dispatcher impiden la liberación prematura. Los demás campos MDL modelados y PFN construidos siguen siendo de solo lectura; campos de proceso, PFN no construidos, MDL manuales no se admiten. La descarga exige liberar todos los descriptores restantes del controlador.

`IoBuildPartialMdl` describe un subrango no vacío de un MDL de origen construido; una longitud cero selecciona el resto. El destino debe tener capacidad PFN suficiente. Los descriptores parciales comparten páginas físicas sin añadir otro bloqueo. Heredan un mapping de sistema activo del origen o crean uno independiente con `MmGetSystemAddressForMdlSafe`. Liberar un MDL parcial o prepararlo para reutilización solo libera el mapping que le pertenece. La función inline real del WDK `MmPrepareMdlForReuse` puede preceder entonces a otra llamada a `IoBuildPartialMdl`. Un descriptor de origen no paginado puede liberarse de forma independiente; el bloqueo de usuario raíz o un alias de sistema prestado no pueden retirarse mientras exista un MDL parcial dependiente. La finalización comprueba toda la cadena antes de liberar dependencias, sin depender del orden de los descriptores. El DMA activo impide reconstrucciones y liberaciones prematuras.

`MmMapLockedPagesSpecifyCache` admite `UserMode` en el contexto de un solicitante vivo, incluido un worker adjunto, hasta `APC_LEVEL`. Cada llamada crea una vista del proceso sobre las mismas páginas sin cambiar `MappedSystemVa`. Siempre es NX; `MdlMappingNoWrite` impide escribir. El pool debe ser no paginado y cubrir páginas privadas completas. Bloquear otra vez con `MmProbeAndLockPages` conserva la identidad física. La dirección solicitada debe conservar el desplazamiento de página del MDL y estar dentro del área acotada; el agotamiento genera `STATUS_INSUFFICIENT_RESOURCES` capturable. `MmNonCached`, `MmCached` y `MmWriteCombined` heredan el atributo de caché que ya tienen las páginas; la RAM ordinaria sigue siendo cached. Los valores reservados y los registros físicos incompatibles fallan explícitamente.

`MmUnmapLockedPages` exige el par exacto dirección de usuario/MDL y el PID creador. Retira realmente la vista y libera dirección y presupuesto para reutilización solicitada o automática. Retirar la VA original del escenario conserva las vistas MDL independientes. Los bloqueos físicos y alias de sistema siguen vinculados a sus páginas originales aunque la VA pase a otro respaldo o permisos. Las vistas activas impiden liberar descriptor, bloqueo raíz, pool y almacenamiento de solicitud.

Las vistas se identifican por `(PID, VA)`: distintos procesos pueden tener la misma dirección numérica simultáneamente con páginas y permisos diferentes. El cambio de proceso reemplaza el conjunto activo de forma atómica tras validar todo; volver restaura las páginas y permisos anteriores. La salida o desasignación de un PID no elimina vistas de otro. Un objeto dispatcher opaco vivo en una vista que se retiraría bloquea el cambio hasta su retirada. Este modelo acotado no implementa asignación virtual arbitraria, tablas de páginas ni un gestor general de procesos.
...
`wdm-x64-scheduled-v75`. `nt_status` sigue siendo el resultado de DriverEntry,
...
La entrega usa metadatos unwind x64 V1 y `__C_specific_handler`. Un ámbito constante `EXCEPTION_EXECUTE_HANDLER` selecciona directamente su handler. Ejecuta filtros reales del invitado: cero continúa la búsqueda, un valor positivo elige el handler y uno negativo solicita reanudar. Tras seleccionar el handler, ejecuta los `__finally` de los ámbitos abandonados en orden de desenrollado; no hay limpieza durante la búsqueda ni al reanudar. El código invitado ejecuta los `__finally` normales. Los marcos ordinarios restauran registros generales no volátiles dentro de la pila original. `GetExceptionCode()` conserva el código generado; un handler puede generar otra excepción hacia un ámbito exterior admitido.

Los filtros reciben `EXCEPTION_POINTERS`, registro de excepción y `CONTEXT_INTEGER | CONTEXT_CONTROL` estables en una pila acotada separada, fuera del shadow space Win64. Se conserva el estado CPU original completo, incluidos SIMD/flotantes y flags. Un filtro negativo puede reanudar un fallo CPU de lectura/escritura de usuario tras validar cambios de GPR, RIP/RSP y flags aritméticos/de dirección. Los campos inmutables se validan para cualquier resultado del filtro. No se admite continuar una excepción generada por una API modelada. Identidad de hilo/proceso, PreviousMode y autoridad de acceso se heredan del padre sin conceder acceso a un worker independiente; un worker adjunto mantiene proceso creador de sistema y KernelMode.

Se admiten excepciones anidadas y unwind en colisión, cadenas V1, prólogos parciales, epílogos canónicos y restauración completa XMM6–XMM15. Los registros conservan sus enlaces y no se repite un finally iniciado. C++ y metadatos incompletos siguen rechazándose. Una excepción API o de usuario no tratada produce `model_error`; los demás fallos de memoria CPU, interrupciones e instrucciones inválidas siguen siendo terminales. En cada retorno del filtro deben permanecer intactos `EXCEPTION_POINTERS`, `EXCEPTION_RECORD` y los campos no admitidos de `CONTEXT`.

`__GSHandlerCheck_SEH` comprueba el cookie actual de la imagen antes de buscar y durante el unwind, incluso en marcos sin finally. Los slots fijos o alineados dinámicamente usan desplazamientos con signo comprobados y el puntero original del marco para la codificación. Los indicadores del controlador C son independientes de la comprobación del cookie. Los prólogos y epílogos no leen cookies aún no establecidos. Una discrepancia detiene la ejecución antes del filtro, limpieza o controlador afectado. `__GSHandlerCheck` independiente también comprueba cookies durante búsqueda y unwind sin inventar ámbitos C. Requiere identidad exacta de símbolo o importación; no se infiere código anónimo por patrones de instrucciones. Los wrappers GS/C++ y las personalidades de excepciones C++ siguen sin admitirse. Los marcos originales de `driver_seh_gs.h` también ejecutan el comprobador WDK enlazado.
...
`MmAllocatePagesForMdl`, `MmAllocatePagesForMdlEx` y `MmFreePagesFromMdl` administran páginas independientes con límites físicos, asignación parcial y bloques contiguos. Páginas y descriptor se liberan por separado. `MmProtectMdlSystemAddress` cambia permisos PAGE de la vista de sistema privada sin cambiar PFN, caché ni permisos de usuario. `MmProbeAndLockPages` acepta pools KernelMode: el paginado se bloquea hasta APC_LEVEL y después admite acceso a DISPATCH_LEVEL; no puede liberarse antes de desbloquearlo.

`messages` define explícitamente dirección, datos, vector, nivel, afinidad y polaridad MSI. `message_id` es local al descriptor; el tercer argumento ISR es global al PDO. `CONNECT_MESSAGE_BASED` (3) devuelve la tabla real. `CONNECT_MESSAGE_BASED_PASSIVE` (5) y las líneas pasivas permiten esperas PASSIVE_LEVEL, serializan la misma interrupción y conservan marcos independientes. Llegada y entrega se registran por separado, sin inventar protocolos PCI.

`WdfDeviceInitSetPowerPolicyOwnership` determina el propietario de la política. Se ejecutan Init/Suspend/Restart/Flush/Cleanup y los callbacks D0; D3/D0 conserva los recursos. La política predeterminada vincula Sleeping3/Working con D3/D0 consumiendo `requested_device_power` explícito. Cada IRP hijo tiene `origin: "framework_power_policy"` y `response_index`. S3 espera al hijo; S0 puede terminar después de emitir D0. Inactividad/activación, reenumeración automática siguen excluidos.
