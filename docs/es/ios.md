...
La vinculación del código fuente construye el mapa verificado de identidades de objetos de clase directos solo cuando un argumento puntero tiene una dirección constante. El mapa es local a una operación de vinculación; cada operación posterior vuelve a validar la imagen actual, incluidos los nombres de clase, las banderas de metaclase y los conflictos de identidad.
...
El catálogo C incluye las declaraciones de `sys/mount.h` con identidades de enlace específicas de cada arquitectura: ARM64 usa `getmntinfo` y x86-64 usa `getmntinfo$INODE64`. La salida sigue siendo un puntero a puntero opaco; el modo y el resultado conservan enteros con signo de 32 bits y se comprueban los exports exactos del sistema. No se inventan la disposición de los registros del sistema de archivos ni el contenido del búfer devuelto.
...
Una consulta de diccionario que devuelve `id` sin calificar no adquiere una clase receptora al unirse con una ruta que devuelve una clase conocida mediante `new`. Cada ruta entrante debe conservar la evidencia del receptor; incluso un argumento de puntero con tipo explícito no permite elegir un setter de objetos frente a una declaración incompatible de punto flotante.

En arm64, `+[NSSet setWithObjects:]` conserva la declaración variádica del SDK terminada en nil. El primer caso admitido exige una importación exacta de la clase de plataforma y un stub de selector, junto con cadenas Objective-C inmutables cuyos valores completos de ocho bytes coincidan en todas las rutas entrantes. La recuperación termina en el primer nil demostrado. Si el primer objeto es nil, no se requiere probar los argumentos adicionales de la pila y no se leen posiciones posteriores. La ABI variádica Darwin compartida mantiene tres parámetros fijos y coloca los demás objetos y nil en la pila. La publicación vuelve a validar la declaración, el proveedor, el receptor, el selector y cada argumento frente a la imagen actual. Los objetos dinámicos, escrituras ausentes o parciales, marcos escapados, redefiniciones locales y otras arquitecturas siguen sin admitirse. Cada argumento de pila requiere además una prueba lineal del marco privado en el instante exacto de la carga: las sobrescrituras posteriores no cambian el valor ya leído; escrituras solapadas, llamadas desconocidas, escapes o flujo de control impiden certificarlo.

La exportación de código puede proyectar un getter super BOOL generado por el compilador por cada entrada Objective-C si las pruebas ARM64 actuales y completas de instrucciones, CFG y guardado/restauración certifican la entrada, el cuerpo compartido y el accesor de metadatos (`CMa`). La detección, el enlace y la generación usan un mismo contrato. El helper conserva la llamada CMa real y sus dependencias cerradas, después lee la celda SEL registrada en tiempo de ejecución y llama a `objc_msgSendSuper2` con el tipo certificado; cada selector mantiene su propia celda. El cargador vuelve a verificar el enlace local mediante los símbolos Mach-O y el export trie de LLVM y las correspondencias exactas de segmentos/secciones actuales. Los demás llamadores y la ABI nativa global de la función compartida no cambian; se rechazan las pruebas ausentes o desactualizadas.

Un inicializador Swift once de arm64 puede copiar un objeto entre posiciones estáticas separadas mediante un helper nativo ya tipado. Un único contrato verifica todos los parámetros actuales y las rutas, conservando en orden la lectura del predicado, el `swift_once` opcional, la lectura de origen, la escritura de destino, `objc_retain` y el retorno. Acepta cuatro parámetros de función y una entrada adicional sin uso opcional; solo la inferencia nativa elimina parámetros. El descubrimiento y el enlace revalidan el mismo contrato, los símbolos exactos del callback y del almacenamiento, rangos separados de ocho bytes y el contexto sin uso del callback frente a la imagen actual. ABI, flujo de control, efectos de memoria y comprobaciones de dependencias se mantienen; se rechazan alias, accesos parciales u ordenados, usos adicionales y pruebas obsoletas.

Las declaraciones completas de los SDK iOS para dispositivo y simulador también confirman `UIGraphicsBeginImageContextWithOptions(CGSize, BOOL, CGFloat)` como una llamada C fija que devuelve void. En ARM64, los dos campos del tamaño ocupan `d0` y `d1`, el booleano usa el byte de `w0` y la escala ocupa `d2`. La exportación exacta del enlazador de UIKit autentica al proveedor. El código conserva la llamada real y las actualizaciones de contadores del envoltorio; rechaza otros proveedores, arquitecturas no compatibles y declaraciones alteradas. Los envoltorios nativos siguen requiriendo pruebas independientes del estado y del valor devuelto.

Las mismas pruebas verificadas del SDK de UIKit vinculan `CGSizeFromString(NSString *)` con su resultado `CGSize` de dos double. La ABI compartida conserva tanto `d0` como `d1`, incluidos los resultados separados que siguen utilizándose después de otra llamada. La comprobación del import actual y de la firma rechaza proveedores distintos, sustitutos escalares, parámetros de puntero modificados y portadores de resultado ausentes o intercambiados.

La preservación del marco nativo ARM64 puede leer un argumento entrante en la pila si la firma de entrada inferida describe explícitamente su espacio escalar alineado de ocho bytes. La carga LowIR debe coincidir exactamente con todo el espacio. Los bytes siguen siendo entradas desconocidas y no prueban la identidad de un registro guardado ni una dirección dentro del marco privado de la función llamada. Las escrituras, lecturas parciales, huecos y espacios no declarados no reciben este permiso. El cuerpo fuente, los argumentos de cada llamador y el cierre de dependencias siguen requiriendo validación.

Las declaraciones completas del SDK de UIKit para dispositivo y simulador vinculan `UIAccessibilityPostNotification` como `void(uint32_t, id nullable)`: ARM64 pasa la palabra de notificación sin signo en `w0` y el puntero al objeto en `x1`. `UIAccessibilityAnnouncementNotification` es almacenamiento externo `const uint32_t`. La importación exacta de UIKit acredita su dirección; se conservan la lectura original de cuatro bytes y la llamada real, incluido un argumento nil, sin sustituir el número de notificación. Se rechazan proveedores incorrectos, importaciones débiles y cambios de anchura, signo o retorno; las demás dependencias de almacenamiento diferido requieren su propia prueba.
