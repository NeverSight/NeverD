...
La validación de resultados booleanos de Swift combina el ABI actual de entrada Objective-C, los bytes inmutables de llamada directa, la importación fuerte y exacta y la prueba completa del consumidor LowIR. Admite una comparación y reconstruye los demás ABI de llamada desde el catálogo actual; rechaza conjeturas nativas, despacho dinámico y llamadas duplicadas. Esta evidencia por sí sola no publica código ni declara un ABI de retorno de un byte.
