...
`DriverResources.h` y `DriverResources.def` definen las asignaciones fijas de `register_bank`, el acceso y ancho de los registros, los nombres de campos y los límites compartidos. `DriverScenario` valida JSON y C++ antes de ejecutar; `DriverResult` registra la configuración sin duplicar el estado observado del banco. `KernelMMIO` administra por PDO los valores persistentes, los intervalos físicos traducidos, las generaciones de recursos, la disponibilidad del hardware y los alias independientes. `KernelModelResources` administra las listas START raw/traducidas empaquetadas y de solo lectura y su integración con el proveedor y el ciclo de vida. El éxito real de START inferior habilita la generación antes de los callbacks superiores; el SET de dispositivo real del proveedor actualiza el acceso D0/D3. La limpieza tras START fallido o STOP/REMOVE se comprueba antes de la finalización terminal de la IRP; los callbacks superiores de finalización pueden desasignar primero, sin liberación implícita. La retirada inesperada bloquea inmediatamente el acceso al hardware. `GuestMemory` y `UnicornBackend` validan cada acceso original completo de CPU/API antes de los callbacks de registros y comparten el primer fallo o detención, evitando efectos posteriores si Unicorn divide la operación. El reinicio conserva los valores de la asignación fija; los alias nunca tienen copias independientes de los bytes. Este bus limitado de registros de memoria no implementa RAM arbitraria, redistribución de recursos, puertos, interrupciones ni DMA.
