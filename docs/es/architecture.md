...
`KernelScheduler` controla el orden listo, identidad de callbacks y vencimientos; `KernelDispatcher` posee objetos DPC, temporizador y evento opacos y sus señales. `KernelModel` controla registros de espera, vidas del trabajo/dispositivo y finalización IRP. `DriverSession` suspende y reanuda pilas separadas y contextos CPU completos, incluidos argumentos Win64 en pila, con memoria compartida. El tiempo virtual avanza en límites de temporizador/espera; CPU0 ejecuta cooperativamente DPC a `DISPATCH_LEVEL` y trabajo a `PASSIVE_LEVEL` de forma determinista. No incluye hilos/APC/cancelación/spinlocks generales, IRP simultáneos, PnP/energía completo ni hardware. Los límites IRQL proceden de `KernelAPIIRQL.def`; el modelo propietario comprueba las restricciones por argumento.

`KernelFramework` controla la vinculación KMDF 1.33, identidad de la tabla, controladores/objetos genéricos y vidas de contextos tipados. `DriverSession` ejecuta callbacks invitados anidados y reanuda la operación suspendida con presupuestos compartidos. `DriverImage` valida metadatos CFG; `GuardControlFlow` controla destinos declarados de imagen/API y el adaptador CPU conserva el estado check/dispatch. No añade dispositivos, colas, solicitudes, extensiones de clase KMDF ni UMDF.
