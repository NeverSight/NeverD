...
Unter macOS kompiliert der eigene Objective-C-Runner Original-Fixtures, rekonstruiert `.m` und linkt nur generierten Quelltext mit einem unabhängigen Aufruf-Harness. Er deckt Ganzzahlgrenzen, Verzweigungen, Schleifen, Zeigerzugriffe, versteckte Parameter, float/double-Bitidentität, gemischte und Stackargumente ab. Der Swift-Runner kompiliert generiertes `.swift` mit seinem Harness unabhängig, ohne Original-Dylib, Modul, Brücke oder handgeschriebene Ersatzdeklarationen. Er prüft Skalare/native Aufrufe, Klasseninitialisierung/-speicher, Wert-/mutierende Strukturmethoden sowie Gleitkomma-, Stack-, Zeiger- und Schleifenfälle. Diese strengen Prüfungen können Unterstützungslücken aufdecken; ihre Existenz beweist nicht, dass jeder Fall mit jedem Build besteht.

Beide Skripte akzeptieren `--arch all|arm64|x86_64`, `--fixups both|classic|default`, `--timeout N` und `--work-dir NEW_DIRECTORY`. `--setup-only` prüft nur Originale, keine Rekonstruktion. Nicht ausführbare Architekturen werden, soweit zulässig, ausdrücklich übersprungen; ein Skip ist kein Erfolg. Aufbewahrte Fehlerartefakte unterscheiden fehlenden Quelltext, Compilerfehler und Verhaltensabweichungen. Verifizierte Unterstützung setzt aktuelle Testergebnisse voraus.
