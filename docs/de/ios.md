...
Die Quelltextbindung erstellt die geprüfte Identitätszuordnung direkter Klassenobjekte erst, wenn ein Zeigerargument eine konstante Adresse hat. Die Zuordnung gilt nur innerhalb eines Bindungsvorgangs; jeder spätere Vorgang prüft das aktuelle Abbild erneut, einschließlich Klassennamen, Metaklassen-Flags und widersprüchlicher Identitäten.
...
Ein Wörterbuchzugriff mit unqualifiziertem Rückgabetyp `id` erhält keine Empfängerklasse durch die Zusammenführung mit einem Pfad, der eine bekannte Klasse über `new` liefert. Jeder eingehende Pfad muss einen Empfängernachweis behalten; selbst ein explizit typisiertes Zeigerargument kann keinen objektwertigen Setter gegenüber einer widersprechenden Gleitkommadeklaration auswählen.

Arm64 `+[NSSet setWithObjects:]` behält die nil-terminierte variadische SDK-Deklaration bei. Der erste begrenzte Fall verlangt einen exakten Plattformklassen-Import und Selector-Stub sowie unveränderliche Objective-C-Strings, deren vollständige Achtbytewerte auf allen eingehenden Pfaden übereinstimmen. Die Wiederherstellung endet am ersten nachgewiesenen nil. Ist das erste Objekt nil, ist kein Nachweis weiterer Stackargumente nötig; spätere Slots werden nicht gelesen. Die gemeinsame variadische Darwin-ABI behält drei feste Parameter und legt die übrigen Objekte samt nil auf den Stack. Vor der Ausgabe werden Deklaration, Anbieter, Empfänger, Selector und jedes Argument am aktuellen Image erneut geprüft. Dynamische Objekte, fehlende oder partielle Argument-Stores, entkommene Frames, lokale Überschreibungen und andere Architekturen bleiben ununterstützt. Stackargumente erfordern zusätzlich einen linearen Nachweis im privaten Frame zum jeweiligen Ladezeitpunkt: Späteres Überschreiben ändert bereits gelesene Werte nicht; überlappende Writes, unbekannte Aufrufe, Escapes oder Kontrollfluss verhindern die Bestätigung.

Der Quelltextexport kann einen vom Compiler erzeugten BOOL-Super-Getter pro Objective-C-Einstieg projizieren, wenn vollständige aktuelle ARM64-Instruktions-, CFG- und Sicherungs-/Wiederherstellungsnachweise Einstieg, gemeinsamen Rumpf und Metadaten-Accessor (`CMa`) bestätigen. Erkennung, Bindung und Ausgabe verwenden denselben Vertrag. Der Helper erhält den tatsächlichen CMa-Aufruf samt geschlossenen Abhängigkeiten, liest danach die zur Laufzeit registrierte SEL-Zelle und ruft das typisierte `objc_msgSendSuper2` auf; verschiedene Selektoren behalten getrennte Zellen. Der Loader prüft lokale Bindung erneut anhand der LLVM-Mach-O-Symbole, des Export-Tries und exakt übereinstimmender aktueller Segment-/Section-Abbildungen. Andere Aufrufer und die globale native ABI der gemeinsamen Funktion bleiben unverändert; fehlende oder veraltete Nachweise werden abgewiesen.

Ein Swift-once-Initialisierer für arm64 kann über einen bereits typisierten nativen Helper ein Objekt zwischen getrennten statischen Speicherplätzen kopieren. Ein gemeinsamer Verwendungskontrakt prüft alle aktuellen Parameter und Pfade und erhält die Reihenfolge aus Prädikatlesen, optionalem `swift_once`, Quelllesen, Zielschreiben, `objc_retain` und Rückkehr. Er erlaubt vier Rollenparameter und optional eine ungenutzte Eingabe; nur die native Inferenz entfernt Parameter. Erkennung und Bindung prüfen denselben Kontrakt, die exakten Callback- und Speichersymbole, getrennte Acht-Byte-Bereiche und den ungenutzten Callback-Kontext erneut am aktuellen Image. ABI, Kontrollfluss, Speichereffekte und Abhängigkeitsprüfungen bleiben erhalten; Aliase, partielle oder geordnete Zugriffe, zusätzliche Verwendungen und veraltete Belege werden abgelehnt.

Die vollständigen SDK-Deklarationen für iOS-Geräte und Simulatoren bestätigen auch `UIGraphicsBeginImageContextWithOptions(CGSize, BOOL, CGFloat)` als festen C-Aufruf mit void-Ergebnis. Unter ARM64 belegen die beiden Größenfelder `d0` und `d1`, der boolesche Wert das Byte in `w0` und der Skalierungswert `d2`. Der genaue UIKit-Linkerexport bestätigt die bereitstellende Bibliothek. Der Quelltext erhält den tatsächlichen Aufruf und Zähleränderungen im Wrapper; falsche Anbieter, nicht unterstützte Architekturen und veränderte Deklarationen werden abgelehnt. Native Wrapper benötigen weiterhin unabhängige Nachweise für Zustand und Rückgabewert.

Dieselben geprüften UIKit-SDK-Belege binden `CGSizeFromString(NSString *)` mit seinem `CGSize`-Ergebnis aus zwei double-Werten. Die gemeinsame ABI erhält sowohl `d0` als auch `d1`, einschließlich getrennter Ergebnisse, die über einen weiteren Aufruf hinweg benötigt werden. Die erneute Prüfung des aktuellen Imports und der Signatur lehnt fremde Anbieter, skalare Ersatztypen, geänderte Zeigerparameter sowie fehlende oder vertauschte Ergebnisträger ab.
