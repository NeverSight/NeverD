...
`KernelFramework` besitzt KMDF-1.33-Bindung, Funktionstabellenidentität, Treiber/generische Objekte und typisierte Kontextlebenszeiten. `DriverSession` führt verschachtelte Gast-Callbacks aus und setzt die angehaltene Framework-Operation unter gemeinsamen Budgets fort. `DriverImage` validiert CFG-Metadaten; `GuardControlFlow` besitzt deklarierte Image-/API-Ziele, und der CPU-Adapter erhält den Check-/Dispatch-Aufrufzustand. KMDF-Geräte, Warteschlangen, Anforderungen, Klassenerweiterungen und UMDF werden dadurch nicht ergänzt.
