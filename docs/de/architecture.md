...
`KernelModelIRPStack` verwaltet begrenzte Cursor, zielgenauen unteren Dispatch und Completion-Abwicklung im ursprünglichen Gastpaket. Inline-Schreibzugriffe von Copy/Skip/SetCompletion bleiben maßgeblich. Dispatch-Status, Completion-Steuerung und endgültiger `IoStatus` sind getrennt; pending kann nach Dispatch-Rückkehr weitergegeben werden. `STATUS_MORE_PROCESSING_REQUIRED` hält IRP/MDL/Puffer bis zur fortgesetzten endgültigen Abwicklung, auch bei verschachteltem Abschluss. `KernelGuestCall` trägt Subsystem und lokalen Token gegen WDM/WDF-Kollisionen; `DriverSession` erhält CPU-Rahmen und geerbten IRQL. Ein Gasttreiber kann sich oberhalb separat verwalteter Szenario-PDOs anbinden; vom Treiber angelegte IRPs und Power-IRPs bleiben ununterstützt. WDF-Anbindung/Weiterleitung, aktive Stapelerweiterung, mittleres Trennen, geänderte Major-Funktionen und routenfremde Ziele sind nicht unterstützt. Jede verbrauchte untere Stackposition wird vor dem oberen Completion-Callback geleert.

`DriverPnp.h` und die öffentliche `DeviceLifecycle.def` definieren die gemeinsamen Lebenszyklus-Enums und die typisierten Szenario-/Berichtsdaten. `KernelModelPnpDevices` verwaltet stabile PDO-Identitäten, ein separates Inventar des Providertreibers und tatsächliche AddDevice-Beobachtungen. `KernelModelPnpRequests` verknüpft Lebenszyklustransaktionen und unveränderliche Geräteidentität mit dem bestehenden IRP-Datensatz; die Zulassung von Dateioperationen verwendet diese Identität. FDO/PDO-Zuordnung und Lebenszyklusidentität einer Datei bleiben auch nach dem Trennen erhalten. `KernelModelPnpCompletion` verwaltet tatsächlichen Busempfang und Abschluss sowie begrenzte virtuelle Fristen. Es verwendet `KernelModelIRPStack` und Fortsetzungen mit Eigentümerkennung erneut, ohne ein zweites Paket oder einen künstlichen Gast-Callback. Erst der endgültige obere Abschluss schreibt den Lebenszykluszustand fest. Erfolgreiches PnP muss den Provider erreichen und dort abgeschlossen werden; REMOVE und CANCEL_REMOVE erfordern exakt `STATUS_SUCCESS` (0). START und QUERY_REMOVE dürfen bereits weiter oben fehlschlagen, wobei Busbeobachtungen null bleiben. Providerfreigabe und Trennen/Löschen durch den Gast sind getrennte Eigentumsübergänge; zurückgelassene Gastgeräte werden niemals stillschweigend entfernt. Der öffentliche Umfang umfasst ressourcenfreies Start/QueryRemove/CancelRemove/Remove, kein allgemeines PnP, Power oder KMDF-PnP.
