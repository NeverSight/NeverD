...
`DriverGuardTests.cpp` und vier originale Varianten von `driver_guard.c` prüfen aktives/inaktives CFG, Relokation, Check-/Dispatch-ABI sowie fehlerhafte oder nicht deklarierte Ziele. `KernelFrameworkTests.cpp` prüft Bindungs- und Objektverträge. Das originale `driver_kmdf_lifecycle.c` wird optional mit echten WDK-1.33-Headern kompiliert und über die echte `FxDriverEntry`-Bibliothek gelinkt; `NEVERD_KMDF_FIXTURE` und `NEVERD_KMDF_CFG_FIXTURE` zeigen auf separat erzeugte normale und aktive CFG-Images. Fehlende externe Artefakte werden ausdrücklich übersprungen. `DriverKMDFLifecycleTests.cpp` und die C-API-/CLI-Fälle in `DriverScenarioPublicTests.cpp` prüfen Bindung, typisierte Kontexte, referenzbedingt verzögerte Zerstörung, echte Gast-Callbacks, Unload und dokumentierte Erstellungsfehler. Die Nachweise stammen von Linux-Hosts und belegen keine umfassendere KMDF-Unterstützung.
