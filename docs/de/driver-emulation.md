...
Ein begrenztes ressourcenfreies KMDF-PnP-Profil unterstützt ein direktes FDO/PDO-Paar. `EvtDriverDeviceAdd` erhält einen Framework-Initializer; `WdfDeviceCreate` erzeugt das FDO, und `WdfFdoInitWdmGetPhysicalDevice` sowie `WdfDeviceWdmGetPhysicalDevice` liefern die PDO-Identität. Nach fehlgeschlagenem AddDevice oder abgeschlossenem Remove führt das Framework Cleanup-/Destroy-Callbacks aus und löscht das Gerät. Eine PnP-Queue muss `PowerManaged = WdfFalse` ausdrücklich setzen. `WdfDeviceInitSetPnpPowerEventCallbacks` unterstützt `EvtDevicePrepareHardware`, `EvtDeviceD0Entry`, `EvtDeviceD0Exit` und `EvtDeviceReleaseHardware`. PrepareHardware erhält vor D0Entry zwei unterschiedliche leere Ressourcenlisten; `WdfCmResourceListGetCount` liefert null und `WdfCmResourceListGetDescriptor` NULL. Ein Fehler in PrepareHardware oder D0Entry wird zum START-Status; ReleaseHardware läuft auch nach einem solchen Fehler. Bei STOP oder Entfernen aus D0 läuft D0Exit vor ReleaseHardware und dem IRP-Abschluss. Nichtleere Ressourcen, Power-Managed-Queues und allgemeine Power-Policy fehlen. Das WDK-Fixture `driver_kmdf_pnp.c` nutzt `NEVERD_KMDF_PNP_FIXTURE` / `NEVERD_KMDF_PNP_CFG_FIXTURE`.
...
Profil lautet `wdm-x64-scheduled-v52`. `nt_status` bleibt das
