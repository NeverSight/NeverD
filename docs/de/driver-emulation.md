...
| Hardware-, DMA-, PCI-, Interrupt- oder Virtualisierungstreiber | Explizite Speicherregisterbank, MMIO, exklusive oder geteilte Latched-/Pegelinterrupts und begrenzte kohärente gemeinsame Puffer/SG-/Kanal-DMA unterstützt | Weitere Gerätemodelle, beliebiger physischer RAM, PCI, Ports, MSI und passive Interrupts, weitere DMA-Schnittstellen und privilegierter CPU-Zustand |
...
Profil lautet `wdm-x64-scheduled-v72`. `nt_status` bleibt das
...
Verschachtelte oder kollidierende Ausnahmen in Filter/finally, GS/C++-Persönlichkeiten, verkettete oder unvollständige Metadaten, Prologe und XMM-Unwindoperationen scheitern ausdrücklich. Nicht behandelte API-/Benutzerausnahmen führen zu `model_error`; andere CPU-Speicher-, Interrupt- und ungültige Instruktionsfehler bleiben terminal. Bei jeder Filterrückgabe müssen `EXCEPTION_POINTERS`, `EXCEPTION_RECORD` und nicht unterstützte `CONTEXT`-Felder unverändert bleiben.
