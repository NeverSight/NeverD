...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | Anforderungseigene MDLs, Systemabbildungen, gemeinsame physische Seitenidentitäten und SG-DMA | DMA-Schnittstellen außerhalb der unten beschriebenen Teilmenge |
| Treiberzugewiesene MDLs | Eigenständige oder IRP-zugeordnete Deskriptoren, partielle MDLs, gemeinsame Benutzer-/Systemabbildungen und Freigabe bei Abschluss | Kontingente, handgefertigte MDLs, beliebige Prozessadressräume |
...
| WDM `METHOD_NEITHER` | Getrennte Benutzerpuffer, Zugriffsprüfung, MDL-Sperren, synthetische Prozesskennungen, Entzug von VA oder Prozessende nach Dispatch und begrenzter Abbruch | Keine Seitentabellen und kein allgemeiner Prozessmanager; begrenzte MDL-Sichten pro PID werden unterstützt |
...
| Hardware-, DMA-, PCI-, Interrupt- oder Virtualisierungstreiber | Explizite Speicherregisterbank, MMIO, exklusive oder geteilte Latched-/Pegelinterrupts und begrenzte kohärente gemeinsame Puffer/SG-/Kanal-DMA unterstützt | Weitere Gerätemodelle, beliebiger physischer RAM, PCI, Ports, weitere DMA-Schnittstellen und privilegierter CPU-Zustand |
...
`register_bank` darf `interrupts` allein oder zusammen mit `resources` deklarieren; mindestens eine Liste ist nicht leer. `resource_free` lehnt das Feld auch leer ab. Jede Leitung benötigt `id`, `raw_vector`, `raw_level`, `raw_affinity`, `translated_vector`, `translated_level`, `translated_affinity`, `mode` (`latched` oder `level_sensitive`) und `share` (`device_exclusive` oder `shared`). Pegelquellen benötigen ein positives `retrigger_after_100ns` bis INT64_MAX; latched-Quellen lehnen es ab. Grenzen: 8 Leitungen je PDO, insgesamt 32, begrenzte pro PDO eindeutige ASCII-IDs, Rohpegel 0–65535 und übersetzter DIRQL 3–12. Beide Affinitäten sind 1 (CPU0/Gruppe0); Vektoren behalten alle 32 Bit. Ein gemeinsam genutzter übersetzter Vektor erfordert gleiche Pegel, Affinität, Modus und Abtastperiode sowie überall `share: "shared"`. Speicherdeskriptoren stehen vor geordneten Interruptdeskriptoren mit passenden Sharing- und Modusflags. Dies sind deklarierte synthetische Leitungen, kein PCI-Controller. `DriverInterrupts.h` / `DriverInterrupts.def` definieren Namen und Grenzen.

READ/WRITE/IOCTL dürfen `interrupt_events` mit `after_100ns`, `device_id` und `interrupt_id` deklarieren. `action` ist für latched standardmäßig `pulse`; Pegelquellen verlangen `assert` oder `deassert` und verbieten `pulse`. Latched-Quellen erlauben nur `pulse`. Andere Anforderungsarten lehnen das Feld auch leer ab. Zulässig sind 64 Ereignisse je Anforderung und insgesamt 1024 deklarierte Ereignisse, mit nichtnegativer Verzögerung bis INT64_MAX. Die Einreichung hält aktive Verbindungen und PDO-Epoche fest; IRP-Abschluss löscht Ereignisse nicht. Zustellung erfolgt an Callback-Grenzen ohne Instruktionspräemption; virtuelle Zeit schreitet im Leerlauf fort. Hardwareveröffentlichung geht der Zulassung voraus, ISR laufen nach zugewiesenem DIRQL vor DPCs/Workern. Verlorene Verbindung, ungültige Epoche oder D3 ergeben `undelivered_reason`, ohne stilles Umbinden.

Ein geteilter latched-Impuls ruft alle erfassten ISR in Registrierungsreihenfolge auf, auch nach TRUE. Pegelzustände gelten pro PDO/Quelle/Epoche; die Leitung ist deren ODER. Wiederholtes `assert` ist idempotent. Alle gleichzeitigen Änderungen gelten vor der Abtastung. Bleibt die Leitung aktiv, wird nach Ende der ISR-Runde plus `retrigger_after_100ns` erneut abgetastet. Bei Pegeln beendet TRUE die Suche dieser Runde, quittiert aber keine Quelle; nur `deassert` löscht deren Zustand. Registerschreiben erfinden kein Quittierungsprotokoll. Pro Sitzung gelten höchstens 4096 Zustellrunden mit jeweils bis zu 32 Handlern zusätzlich zu Callback-/Instruktionsbudgets. Trennen und Geräteentfernung lehnen noch aktive Quellen ab.

`IoConnectInterrupt` verwendet elf Argumente und die genaue übersetzte Zuweisung. `IoConnectInterruptEx` unterstützt FullySpecified (1), LineBased (2, eine Leitung des expliziten PDO) und FullySpecifiedGroup (4, Gruppe0); Trennen benötigt passende Version und Kontext. Registrierung und Trennung erfordern PASSIVE_LEVEL. Unterstützt werden private Locks und initialisierte, nicht auslagerbare `KSPIN_LOCK` des Aufrufers. Mehrere Verbindungen dürfen diesen Lock bei gleichem `SynchronizeIrql` teilen, mindestens so hoch wie jeder zugewiesene DIRQL; die Scheduler-Priorität bleibt unverändert. LineBased-Null wählt den zugewiesenen Pegel. CPU0/Gruppe0 und kein Speichern des Gleitkommazustands bleiben erforderlich. Das opake `KINTERRUPT` und ServiceContext gehen an die echte ISR; AL liefert BOOLEAN. ISR, `KeSynchronizeExecution` und manuelles Acquire/Release verwenden denselben nichtrekursiven Lock. Synchronize läuft auf Synchronisierungs-IRQL und stellt IRQL/CR8 wieder her; Besitzer und gespeicherter IRQL werden geprüft. Executive-Spinlock-APIs dürfen verbundene Locks nicht benutzen. Instruktionspräemption bleiben ausgeschlossen. Fehlgeschlagener START und erfolgreicher STOP/REMOVE verlangen Trennung vor endgültigem Abschluss; eingereihte DPCs werden nicht still verworfen.

`configuration.pnp_devices[].interrupts` hält Ressourcen, `configuration.interrupt_events` Deklarationen fest. Beobachtete `interrupts` enthalten `source_request_index`, `event_index`, `device_id`, `interrupt_id`, `action`, `epoch`, `due_at_100ns`, `occurred_at_100ns`, `delivered_at_100ns`, `returned_at_100ns`, `interrupt_object`, `return_value`, `claimed` und `undelivered_reason`. `handlers[]` erfasst jeden tatsächlich aufgerufenen ISR mit Objekt, Zeitpunkten, BOOLEAN, `claimed` und `delivery_index`. Ein Assert kann mehrere Runden auslösen; Deassert oder gleichzeitig aufgehobenes Assert erfinden keinen ISR. `occurred_at_100ns` bezeichnet die angewandte Zustandsänderung. `claimed` fasst tatsächliche Rückgaben zusammen; nicht beanspruchte Interrupts bleiben gültig. `delivered_at_100ns` bezeichnet die erste Zustellung; `returned_at_100ns` und `claimed` beschreiben die letzte Runde, `handlers[]` bewahrt den gesamten Verlauf. Das ausführbare [Interruptszenario](../examples/driver-interrupt-scenario.json) nutzt das originale WDK-Fixture `driver_wdm_interrupts.c` mit optionalem `NEVERD_WDM_INTERRUPT_FIXTURE` / `NEVERD_WDM_INTERRUPT_CFG_FIXTURE`: Sieben Anforderungen umfassen verzögerten START, ein durch ISR→DPC abgeschlossenes wartendes IOCTL, Dateibereinigung/-schließung und Entfernung. Die bestehende C-/Python-Grenze `scenario_json` und das Layout `neverd_driver_options_v1` bleiben unverändert. Fehlende echte Artefakte werden explizit übersprungen; Ausführungsnachweise gelten nur für Linux.
...
Framework-Interruptobjekte nutzen zugewiesene Leitungs- oder MSI-Ressourcen und interne Interruptsperren. ISR und Synchronisationscallbacks laufen auf der zugewiesenen DIRQL, bei passiver Verarbeitung auf `PASSIVE_LEVEL`; `WdfInterruptGetInfo` meldet die passive IRQL. DPCs laufen auf `DISPATCH_LEVEL`, Arbeitsaufträge auf `PASSIVE_LEVEL`. Bereits eingereihte Aufrufe werden zusammengefasst; suspendierte Callbacks behalten ihren Besitz bis zur Rückkehr. Nach D0Entry werden Interrupts verbunden und aktiviert. Beim Abschalten folgen Deaktivierung, Trennung und das Leeren verzögerter Callbacks vor D0Exit und Ressourcenfreigabe. Explizites Aktivieren/Deaktivieren führt Treibercallbacks aus und erfindet keine Hardwareimpulse. Externe `WDFSPINLOCK`/`WDFWAITLOCK`, automatische Elternserialisierung, Wake-Interrupts und beibehaltene inactive-Verbindungen bleiben unmodelliert, ebenso Idle/Wake-Richtlinien und automatische Neuerkennung fehlgeschlagener Geräte.

Die folgenden Anforderungs- und Objekt-APIs unterstützen IRQL bis einschließlich `DISPATCH_LEVEL`: `WdfObjectDereferenceActual`, `WdfRequestComplete`, `WdfRequestCompleteWithInformation`, `WdfRequestRetrieveInputBuffer`, `WdfRequestRetrieveOutputBuffer`, `WdfRequestRetrieveInputMemory`, `WdfRequestRetrieveOutputMemory`, `WdfRequestRetrieveInputWdmMdl`, `WdfRequestRetrieveOutputWdmMdl`, `WdfRequestGetInformation`, `WdfRequestSetInformation`, `WdfRequestGetIoQueue`, `WdfRequestGetFileObject`, `WdfRequestWdmGetIrp`, `WdfRequestGetParameters`, `WdfRequestGetStatus`, `WdfRequestMarkCancelableEx`, `WdfRequestUnmarkCancelable`, `WdfRequestIsCanceled`, `WdfMemoryGetBuffer`.

`WdfObjectGetTypedContextWorker`, `WdfObjectContextGetObject`, `WdfObjectReferenceActual` unterstützen auch die modellierten Interrupt-IRQLs; Kontextzugriff und Referenzaufnahme rufen keine passiven Callbacks auf.

Anforderungsabschluss oder Freigabe der letzten Objektreferenz oberhalb von `PASSIVE_LEVEL` plant tatsächliche Bereinigung, Zerstörung und weitere Framework-Zustellung auf `PASSIVE_LEVEL`. Das ältere `WdfRequestMarkCancelable` bleibt auf `PASSIVE_LEVEL` beschränkt, damit sein Callback bei einer bereits abgebrochenen Anforderung synchron ausgeführt wird. Für andere APIs gelten weiterhin die bisherigen IRQL-Grenzen.
...
Bei gepufferten, direkten und neither KMDF-Anforderungen läuft `WdfDeviceInitSetIoInCallerContextCallback` vor der Warteschlange im Prozess des Antragstellers auf `PASSIVE_LEVEL`. Der Callback muss die Anforderung abschließen oder sie einmal mit `WdfDeviceEnqueueRequest` in die Standardwarteschlange geben. Für `METHOD_NEITHER`-IOCTL und neither READ/WRITE liefern `WdfRequestRetrieveUnsafeUserInputBuffer` und `WdfRequestRetrieveUnsafeUserOutputBuffer` die ursprünglichen Benutzeradressen nur in diesem Callback. `WdfRequestProbeAndLockUserBufferForRead` und `WdfRequestProbeAndLockUserBufferForWrite` prüfen Seitenrechte und sperren anforderungsgebundene Seiten. `WdfMemoryGetBuffer` liefert einen Systemalias, der auch im Queue-Callback außerhalb des Antragstellerkontexts gültig bleibt. Beim Abschluss werden Sperren und Aliasse freigegeben. Eingebettete Zeiger auf explizite `user_buffers` werden unterstützt; beliebige Benutzermappings bleiben unmodelliert.

Neither-Anforderungen können `user_buffers` (`id`, `size`, optional `input`/`access`) und `user_pointers` deklarieren. `source`/`target` enthalten `buffer` (`input`, `output`, `memory`) und `offset`; nur `memory` verwendet eine anforderungslokale `id`. Acht-Byte-Zeigerplätze dürfen sich nicht überlappen; das Ziel liegt im Puffer oder an dessen Ende. Pro Szenario gelten höchstens 64 zusätzliche Puffer, 256 Zeiger und 64 KiB je Puffer innerhalb des Gesamtbudgets von 512 KiB. Gemeinsame Ziele teilen reale Bytes und unterliegen den bestehenden Rechte-, Sperr-, Widerrufs- und Prozessregeln. `configuration.user_memory` erhält die Deklarationen. `user_buffers[].backing_hex` ist eine diagnostische RAM-Aufnahme bei Beendigung, auch nach Widerruf oder Fehler; sie ersetzt weder `output_hex` noch Zugriffsrechte. `id`: 1–64 ASCII, `[A-Za-z0-9][A-Za-z0-9_.-]*`.
...
`WdfDeviceInitSetFileObjectConfig` registriert vor der Geräteerstellung `EvtDeviceFileCreate`, `EvtFileCleanup` und `EvtFileClose` und kopiert optionale Kontextattribute. Dieses Profil unterstützt `WdfFileObjectNotRequired`, `WdfFileObjectWdfCanUseFsContext`, `WdfFileObjectWdfCanUseFsContext2` und `WdfFileObjectWdfCannotUseFsContexts`. Der ausgewählte WDM-Kontextplatz hält das WDF-Handle bis zum fehlgeschlagenen CREATE oder CLOSE und muss anfangs leer sein. Mit `WdfFileObjectCanBeOptional` liefert `WdfRequestGetFileObject` für I/O ohne passendes WDM-Dateiobjekt NULL. CREATE, CLEANUP und CLOSE benötigen weiterhin ein WDM-Dateiobjekt; `WdfFdoInitSetFilter` aktiviert die Standardweiterleitung für Filter; `WdfTrue` aktiviert und `WdfFalse` deaktiviert sie ausdrücklich. Für die Weiterleitung von Dateianforderungen sind eine direkte FDO/PDO-Route und ein explizites `bus_completion` nötig. Ein positives `delay_100ns` ist für die automatische Weiterleitung von CREATE/CLEANUP/CLOSE sowie für synchrone, asynchrone CREATE-Sendungen mit Abschlussroutine und CREATE mit `SEND_AND_FORGET` zulässig. CLEANUP- und CLOSE-Rückrufe laufen vor der Weitergabe an das untere Gerät. Ein CREATE-Rückruf mit `WdfFileObjectNotRequired` kann das geräteeigene lokale Ziel über `WdfDeviceGetIoTarget` beziehen und die ursprüngliche Anforderung mit `WdfRequestSend` ausschließlich mit `WDF_REQUEST_SEND_OPTION_SEND_AND_FORGET` weiterleiten. Das beibehaltene PDO verbraucht die explizite Busantwort und übernimmt den endgültigen Abschluss, auch wenn die untere Antwort verzögert ist. Ein CREATE-Rückruf mit Framework-Dateiobjekt kann `WDF_REQUEST_SEND_OPTION_SYNCHRONOUS` verwenden, das untere Ergebnis mit `WdfRequestGetStatus` lesen und die ursprüngliche Anforderung mit diesem Status abschließen. Für den asynchronen Standardsendeweg wird die Anforderung zuerst mit `WdfRequestFormatRequestUsingCurrentType` formatiert und mit `WdfRequestSetCompletionRoutine` ein Rückruf registriert. Er erhält `WDF_REQUEST_COMPLETION_PARAMS`, liest den unteren Status über `WdfRequestGetCompletionParams` oder `WdfRequestGetStatus` und schließt die ursprüngliche Anforderung mit einem eigenen, vom unteren Status unabhängigen Ergebnis ab. Bei einem Versand mit Abschlussroutine bleibt die Anforderung bis zum virtuellen Termin der verzögerten PDO-Antwort offen; danach wird die Abschlussroutine mit dem endgültigen unteren Status eingereiht. `WDF_REQUEST_SEND_OPTION_TIMEOUT` akzeptiert bei synchronem oder callbackbasiertem asynchronem CREATE ein negatives relatives Intervall oder einen positiven absoluten Termin der virtuellen 100-ns-Uhr. Ein früheres Timeout liefert `STATUS_IO_TIMEOUT`; ein sofortiger unterer Abschluss oder gleicher Termin gewinnt. Ein vergangener absoluter Termin hat keine Restdauer. Null deaktiviert das Timeout; weitere Optionen bleiben ausgeschlossen. WDF-Dateihandle und WDM-`FILE_OBJECT` bleiben getrennte Identitäten, die über `WdfRequestGetFileObject`, `WdfFileObjectGetDevice` und `WdfFileObjectWdmGetFileObject` abfragbar sind. Ein fehlgeschlagenes CREATE löscht das WDF-Dateiobjekt ohne Datei-Cleanup- oder Close-Rückruf; nach erfolgreichem Öffnen laufen CLEANUP und CLOSE vor der Kontextbereinigung und Zerstörung.

Ein verzögertes synchrones `WdfRequestSend` bewahrt den tatsächlichen Gastaufrufrahmen und setzt ihn erst nach Abschluss mit dem booleschen Sendeergebnis fort. Den endgültigen NTSTATUS liefert `WdfRequestGetStatus`. `WDF_REQUEST_SEND_OPTION_TIMEOUT` akzeptiert bei synchronem oder callbackbasiertem asynchronem CREATE ein negatives relatives Intervall oder einen positiven absoluten Termin der virtuellen 100-ns-Uhr. Ein früheres Timeout liefert `STATUS_IO_TIMEOUT`; ein sofortiger unterer Abschluss oder gleicher Termin gewinnt. Ein vergangener absoluter Termin hat keine Restdauer. Null deaktiviert das Timeout; weitere Optionen bleiben ausgeschlossen. Die automatische Weiterleitung ruft die Dateirückrufe vor der unteren Übergabe auf und erhält WDM-IRP und Datei während der für den Abschluss erforderlichen Bereinigungs- und Zerstörungsrückrufe. Externe WDF-Referenzen bewahren den Kontext, verlängern aber nicht die Lebensdauer der WDM-Datei einer abgeschlossenen Anforderung.

An ein konfiguriertes PDO weitergeleitete WDM-Anforderungen CREATE/CLEANUP/CLOSE akzeptieren ebenfalls ein ausdrücklich verzögertes `bus_completion`. Nach dem unteren Termin läuft die tatsächliche Gastabschlussroutine; Pending-Weitergabe und Zuständigkeit für den Endstatus bleiben erhalten.
...
| `ExRaiseStatus`, `ExRaiseAccessViolation`, `ExRaiseDatatypeMisalignment` | Gast-C-`__except`, echte Filter und Unwind-`__finally`; keine normale API-Rückkehr |
...
| `IoConnectInterrupt`, `IoDisconnectInterrupt`, `IoConnectInterruptEx`, `IoDisconnectInterruptEx` | Zugewiesene latched-/Pegel-Leitungen, exklusive/geteilte Handler und Locks; Legacy sowie Ex 1/2/4 bei PASSIVE_LEVEL |
| `KeSynchronizeExecution`, `KeAcquireInterruptSpinLock`, `KeReleaseInterruptSpinLock` | Echter BOOLEAN-Callback unter gemeinsamem nichtrekursivem Lock auf Synchronisierungs-IRQL mindestens gleich den DIRQL; Aufruferkontext wird wiederhergestellt |
...
Das optionale boolesche Feld `user_unmap_after_dispatch` entzieht bei einem nichtleeren WDM-Neither-Transfer den ursprünglichen Benutzeradressen nach der Dispatch-Rückkehr und vor geplanter Arbeit oder einem Abbruch den Zugriff. Gesperrte MDL-Seiten und ihre Systemaliase bleiben bis zum Entsperren nutzbar; rohe Benutzerzeiger und neue Sperren schlagen fehl. Bei entzogenem Ausgabezugriff bleibt `output_hex` leer. Adressen der ursprünglich im Szenario deklarierten Benutzerallokationen werden nicht wiederverwendet; ihr Entzug zu beliebigen Zeitpunkten ist weiterhin nicht modelliert.
...
Prozessfelder, PFNs nicht aufgebauter Deskriptoren, selbst erstellte MDLs, Benutzerabbildungen außerhalb des unten beschriebenen prozesseigenen MDL-Modells und direkter
Zugriff über den rohen UserBuffer werden abgewiesen. Ein direkter Puffer der
Länge null hat eine Null-MDL.

`IoAllocateMdl` reserviert Metadaten für einen nichtleeren, überlauffreien Puffer bis 1 MiB, ohne ihn zu prüfen oder zu sperren. `Irp` darf NULL oder ein gültiger modellierter IRP sein. Ein primärer Deskriptor ersetzt den Kopf der treibereigenen Kette; abgetrennte Deskriptoren bleiben Eigentum des Treibers. `SecondaryBuffer` hängt hinten an oder setzt den Kopf einer leeren Kette. Der ursprüngliche anforderungseigene Direct-I/O-MDL muss erreichbar bleiben und darf weder ersetzt noch vom Treiber freigegeben werden. `ChargeQuota` muss FALSE sein; bei erschöpfter Arena wird NULL geliefert. `MmBuildMdlForNonPagedPool` verlangt den gesamten Bereich in einer lebenden nicht auslagerbaren Poolallokation. Sichere Hilfsfunktion und WDM-Makros verwenden die ursprüngliche Adresse und bestehende Rechte weiter, auch bei neuen Schreib-/Ausführungsverboten. Weitere Systemabbildungen und Aufhebung der Abbildung werden abgelehnt. `IoFreeMdl` gibt nur den benannten Treiberdeskriptor frei, trennt ihn nicht ab und folgt nicht `Next`. Benutzer-MDLs müssen vor manueller Freigabe entsperrt werden. Der Poolpuffer lebt unabhängig; beide Freigabereihenfolgen sind erlaubt, solange freier Speicher nicht weiter benutzt wird.

Treiber dürfen `MDL.Next` und `IRP.MdlAddress` zum Einfügen und Abtrennen modellierter Deskriptoren ändern. Der endgültige IRP-Abschluss prüft zuerst die gesamte aktuelle Kette, entsperrt angehängte Benutzer-MDLs, widerruft deren Aliase und gibt alle angehängten Deskriptoren frei. Abgetrennte Deskriptoren und Poolspeicher bleiben Eigentum des Treibers. Zyklen, unbekannte oder freigegebene Verknüpfungen, gemeinsame Deskriptoren mehrerer lebender IRPs und private WDF-Deskriptoren in WDM-Ketten führen ausdrücklich zum Fehler. Aktive DMA- und Dispatcher-Abhängigkeiten verhindern vorzeitige Freigabe. Andere modellierte MDL-Felder und aufgebaute PFNs bleiben schreibgeschützt; Prozessfelder, unaufgebaute PFNs, handgefertigte MDLs bleiben ununterstützt. Vor dem Entladen müssen alle verbleibenden Treiberdeskriptoren freigegeben sein.

`IoBuildPartialMdl` beschreibt einen nichtleeren Teilbereich einer aufgebauten Quell-MDL; Länge null wählt den Rest. Die Ziel-MDL benötigt genügend PFN-Kapazität. Partielle Deskriptoren teilen physische Seiten ohne zusätzliche Seitensperre. Sie übernehmen eine gültige Systemabbildung der Quelle oder erzeugen mit `MmGetSystemAddressForMdlSafe` eine eigene. Freigabe oder Vorbereitung zur Wiederverwendung einer partiellen MDL löst nur ihre eigene Abbildung. Die echte WDK-Inlinefunktion `MmPrepareMdlForReuse` kann so einem weiteren Aufruf von `IoBuildPartialMdl` vorausgehen. Ein nicht auslagerbarer Quelldeskriptor kann unabhängig freigegeben werden; eine ursprüngliche Benutzersperre oder ein übernommener Systemalias bleibt bestehen, solange eine partielle MDL davon abhängt. Der Abschluss prüft unabhängig von der Deskriptorreihenfolge die gesamte Kette vor dem Freigeben von Abhängigkeiten. Aktives DMA verhindert Neuaufbau und vorzeitige Freigabe.

`MmMapLockedPagesSpecifyCache` akzeptiert `UserMode` im lebenden Anfordererkontext einschließlich eines angehängten Workers bis `APC_LEVEL`. Jeder Aufruf erzeugt eine prozesseigene Sicht auf dieselben Seiten, ohne `MappedSystemVa` zu ändern. Sichten sind immer NX; `MdlMappingNoWrite` verhindert Schreiben. Poolspeicher muss nicht auslagerbar sein und vollständige private Seiten belegen. Erneutes Sperren durch `MmProbeAndLockPages` erhält die physische Identität. Eine gewünschte Adresse muss den MDL-Seitenversatz bewahren und in der begrenzten Arena liegen; Erschöpfung löst abfangbares `STATUS_INSUFFICIENT_RESOURCES` aus. `MmNonCached`, `MmCached` und `MmWriteCombined` übernehmen das bereits vorhandene Seiten-Cacheattribut; gewöhnlicher RAM bleibt cached. Reservierte Werte und widersprüchliche physische Registrierungen scheitern ausdrücklich.

`MmUnmapLockedPages` verlangt das genaue Benutzeradresse/MDL-Paar und den erzeugenden PID. Es entfernt die Sicht tatsächlich und gibt Adresse und Mappingbudget für gewünschte oder automatische Wiederverwendung frei. Das Entfernen der ursprünglichen Szenario-VA lässt unabhängige MDL-Sichten bestehen. Physische Pins und Systemaliase behalten ihre ursprünglichen Seiten, auch wenn dieselbe VA später andere Daten oder Rechte bezeichnet. Lebende Sichten verhindern Freigabe von Deskriptor, Stammsperre, Pool und Anforderungsspeicher.

Sichten sind durch `(PID, VA)` identifiziert: Verschiedene Prozesse können gleichzeitig dieselbe numerische Adresse mit unterschiedlichen Seiten und Rechten besitzen. Ein Prozesswechsel ersetzt den ganzen aktiven Satz nach reiner Vorprüfung atomar; Rückwechsel stellt ursprüngliche Daten und Rechte wieder her. Unmap oder Prozessende eines PID entfernt keine Sicht eines anderen PID. Ein lebendes opakes Dispatcherobjekt in einer zu entfernenden Sicht verhindert den Wechsel bis zur Freigabe. Das begrenzte Modell enthält weder beliebige virtuelle Allokationen noch Seitentabellen oder einen allgemeinen Prozessmanager.
...
Profil lautet `wdm-x64-scheduled-v75`. `nt_status` bleibt das
...
Die Zustellung verwendet x64-V1-Unwindmetadaten und `__C_specific_handler`. Ein konstanter `EXCEPTION_EXECUTE_HANDLER`-Bereich wählt seinen Handler direkt. Tatsächliche Gastfilter werden ausgeführt: Null setzt die Suche fort, ein positiver Wert wählt den Handler, ein negativer fordert Fortsetzung an. Nach Auswahl laufen die `__finally` der verlassenen Bereiche in Abwicklungsreihenfolge vor dem Handler; während Suche oder Fortsetzung läuft keine Bereinigung. Normale `__finally`-Pfade führt der Gastcode aus. Gewöhnliche Frames stellen nichtflüchtige allgemeine Register innerhalb des ursprünglichen Stacks wieder her. `GetExceptionCode()` liefert den ausgelösten Code; Handler dürfen in unterstützte äußere Bereiche erneut auslösen.
...
## Erweiterte Ausführungssemantik

`MmAllocatePagesForMdl`, `MmAllocatePagesForMdlEx` und `MmFreePagesFromMdl` verwalten unabhängige Seiten mit physischen Grenzen, Teilzuweisungen und zusammenhängenden Blöcken. Seiten und Deskriptor werden getrennt freigegeben. `MmProtectMdlSystemAddress` ändert PAGE-Rechte einer privaten Systemabbildung, ohne PFNs, Cache oder Benutzerrechte zu ändern. `MmProbeAndLockPages` unterstützt KernelMode-Pools: ausgelagerten Pool höchstens bei APC_LEVEL sperren, danach Zugriff bei DISPATCH_LEVEL; Freigabe vor dem Entsperren ist verboten.

Explizite `messages` legen MSI-Adresse, Daten, Vektor, Ebene, Affinität und Polarität fest. `message_id` zählt im Deskriptor, das dritte ISR-Argument im gesamten PDO. `CONNECT_MESSAGE_BASED` (3) liefert die echte Tabelle. `CONNECT_MESSAGE_BASED_PASSIVE` (5) und passive Leitungen erlauben PASSIVE_LEVEL-Waits, serialisieren denselben Interrupt und erhalten unabhängige Ausführungskontexte. Ankunft und Zustellung bleiben getrennt; kein PCI-Protokoll wird erfunden.

`WdfDeviceInitSetPowerPolicyOwnership` legt den Richtlinienbesitzer fest. Init/Suspend/Restart/Flush/Cleanup und die D0-Callbacks werden ausgeführt; normales D3/D0 erhält Ressourcen. Die Standardrichtlinie ordnet Sleeping3/Working über explizite `requested_device_power` D3/D0 zu. Jedes Kind-IRP hat `origin: "framework_power_policy"` und `response_index`. S3 wartet auf sein Kind; S0 darf nach Anforderung von D0 abschließen. Idle/Wake und automatische Neuerkennung bleiben unmodelliert.
