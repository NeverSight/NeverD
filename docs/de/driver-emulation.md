...
Das begrenzte Modell ist keine vollständige asynchrone Windows-Umgebung. Alertable-/User-Mode-Warten, Systemthreads, APCs, allgemeiner WDM-Anforderungsabbruch, gleichzeitige öffentliche Szenarioeinreichungen, allgemeine IRQL-Wechsel, KMDF-Aufruferkontext- und Benutzerpuffer-APIs, UMDF, KMDF-PnP-Geräte und allgemeine Warteschlangenplanung, vollständiges PnP/Power, allgemeine Hardwaremodelle, weitere DMA-Schnittstellen und andere Interruptmodi bleiben unmodelliert. Reine Initialisierung führt explizit eingereihte Callbacks aus, erzeugt aber keine Anforderungen oder implizites Entladen.
