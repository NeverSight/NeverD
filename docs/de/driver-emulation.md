...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | Anforderungseigene MDLs, Systemabbildungen, gemeinsame physische Seitenidentitäten und SG-DMA | Benutzerabbildungen und DMA-Schnittstellen außerhalb der unten beschriebenen Teilmenge |
| Treiberzugewiesene MDLs | Eigenständige oder IRP-zugeordnete Deskriptoren, partielle MDLs, gemeinsame Benutzer-/Systemabbildungen und Freigabe bei Abschluss | Kontingente, handgefertigte MDLs, beliebige Prozessadressräume und VA-Wiederverwendung |
...
`MmMapLockedPagesSpecifyCache` akzeptiert `UserMode` und `MmCached` im Kontext eines lebenden Anforderers, einschließlich des begrenzten angehängten Worker-Kontexts, bis einschließlich `APC_LEVEL`. Jeder Aufruf erzeugt eine eigene prozesseigene Sicht auf denselben physischen Speicher, ohne `MappedSystemVa` zu ändern. Sichten sind immer nicht ausführbar (NX); `MdlMappingNoWrite` macht sie schreibgeschützt. Poolspeicher muss nicht auslagerbar sein und vollständige private Seiten belegen. Erneutes Sperren einer Benutzersicht mit `MmProbeAndLockPages` erhält die ursprüngliche physische Identität. Eine gewünschte Adresse muss den MDL-Seitenversatz beibehalten und in der begrenzten VA-Arena für Benutzersichten liegen; Erschöpfung löst ein abfangbares `STATUS_INSUFFICIENT_RESOURCES` aus.

`MmUnmapLockedPages` verlangt das genaue Paar aus Benutzeradresse und MDL sowie den erzeugenden Prozess (PID). Das Aufheben der ursprünglichen Szenario-VA lässt unabhängige MDL-Sichten und Systemaliase bestehen. Prozessende widerruft nur die Benutzersichten dieses Prozesses; gesperrte physische Seiten und Systemaliase bleiben erhalten. Lebende Sichten verhindern die Freigabe von Deskriptor, ursprünglicher Sperre, Pool- und Anforderungsspeicher. Widerrufene Benutzeradressen werden nicht wiederverwendet. Beliebige Prozessadressräume, weitere Cacherichtlinien und erneute Zuordnung von Benutzer-VAs liegen außerhalb dieses begrenzten Modells.
