...
Swift 불리언 결과 검증은 현재 Objective-C 진입 ABI, 변경 불가능한 직접 호출 바이트, 정확한 강한 런타임 가져오기와 완전한 LowIR 사용 증명을 결합합니다. 비교 호출은 한 곳만 허용하고 나머지 호출 ABI는 현재 런타임 목록에서 다시 구성합니다. 네이티브 추측, 동적 디스패치, 중복 호출은 거부합니다. 이 증거만으로 소스를 공개하거나 바이트 반환 ABI를 선언하지 않습니다.
