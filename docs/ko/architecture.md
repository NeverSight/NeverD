...
정확한 진입 주소의 함수 기호가 있는 네이티브 진입은 임시로 전체 x0 워드만 관찰 가능한 결과로 간주할 수 있습니다. 소스 추론이 완전한 진입 ABI를 바인딩하면 공개 단계에서 그 ABI로 동일한 LowIR 증명을 다시 실행합니다. 임시 가정만으로 소스 바인딩이나 바이트 반환 ABI를 제공하지 않습니다.
