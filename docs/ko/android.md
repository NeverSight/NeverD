...
Java 이름 바인딩은 클래스 헤더와 본문을 구분하고 확인된 같은 패키지 내 이름 가림을 처리하지만, 외부 상위 클래스나 인터페이스 선언이 부족해 타입 이름 또는 생성된 Java 보조 코드의 참조 대상을 확정할 수 없으면 명시적으로 거부하며, 선언 없이도 상속 가능한 멤버 타입이 없다고 가정하는 외부 타입은 현재 `java.lang.Object`뿐입니다. Smali 부동소수점 리터럴은 대상 단정밀도 또는 배정밀도로 직접 반올림하여 그 결과의 비트 패턴을 보존합니다.

내장 C++ 엔진은 지원 범위에 있는 클래스·필드·메서드의 `Signature` 메타데이터를 보존하고 검증합니다. 타입 변수, 배열, 와일드카드, 타입 경계, 메서드 수준의 이름 가림을 다루며, 타입 소거 결과는 원본 DEX 선언의 식별 정보와 일치해야 합니다. `Throws`는 버리지 않으며 예외 상속 관계를 입증할 수 없으면 거부합니다. 제네릭 상속이나 멤버 타입 치환, 브리지 메서드 재생성, 제네릭 메서드 호출, 매개변수화된 내부 타입, 생성자의 숨겨진 매개변수 대응은 필요한 근거가 없을 때 계속 명시적으로 지원하지 않습니다.

클래스, 필드, 메서드, 생성자에 붙은 요소가 없고 런타임에 보이는 Java 8 `@java.lang.Deprecated` 마커를 보존합니다. 매개변수 어노테이션, 어노테이션이 붙은 정적 초기화 블록, 다른 가시성 수준, `since`나 `forRemoval` 등의 요소 값은 거부합니다. CI는 재컴파일 후 classfile의 `Deprecated` 속성과 런타임 어노테이션을 각각 비교하며, 어노테이션이 없는 대조 선언과 생성된 보조 메서드도 확인합니다. 보조 메서드는 원본 메서드 수에 포함하지 않습니다.

DEX와 smali는 클래스, 필드, 메서드, 생성자의 플랫폼 어노테이션 `@android.annotation.SuppressLint`도 보존합니다. build 가시성과 문자열 배열 `value` 하나만 허용하며, 빈 배열, 빈 문자열, 중복 값, 순서, 이스케이프된 문자 값을 유지합니다. 매개변수 어노테이션과 어노테이션이 붙은 정적 초기화 블록은 계속 지원하지 않습니다. CI는 실제 Android SDK로 재컴파일한 뒤 CLASS 보존 어노테이션과 프로그램 동작을 확인합니다.

내장 엔진은 지원되는 어노테이션 선언의 런타임에 보이는 `@Retention`, `@Target`, `@Documented`, `@Inherited`도 보존하고 실제 `@interface`를 출력합니다. 이 범위에서는 필드, 메서드, 타입 매개변수와 중첩 선언을 허용하지 않습니다. 이름, 접근 권한과 소속 관계를 확인할 수 있는 최상위 어노테이션 또는 정적 멤버 어노테이션을 지원하며, 지역 클래스나 익명 클래스의 범위는 거부합니다.

클래스, 인터페이스, 어노테이션 선언에 빈 마커를 적용하려면 함께 분석하는 클래스 집합 안에 접근 가능하고 일치하는 마커 정의가 있어야 합니다. 해당 Retention과 Target도 실제 적용을 허용해야 합니다. `SOURCE` 선언은 재구성할 수 있지만 입력에 남아 있는 적용은 거부합니다. `CLASS` 또는 Retention이 없는 적용은 DEX build 가시성(`0`), `RUNTIME`은 runtime 가시성(`1`)이 필요합니다. Retention 누락과 명시적인 `CLASS`, Target 누락과 빈 배열을 구분하며 Target 배열 순서를 보존합니다. Target 값은 Java 8 범위로 제한하여 `MODULE`, `RECORD_COMPONENT` 같은 최신 값은 거부합니다. `@Inherited`를 보존하더라도 상속된 적용을 하위 클래스의 직접 선언으로 복사하지 않습니다.

어노테이션 요소 선언과 기본값, 값이 있는 사용자 정의 어노테이션 적용, 필드·메서드·매개변수의 사용자 정의 마커, 외부 어노테이션 정의, 반복 가능한 어노테이션 컨테이너와 `kotlin.Metadata`는 계속 지원하지 않습니다. 마커 정의는 요소 메서드나 보조 메서드가 없는 클래스 선언으로 출력되며 복구된 메서드 수도 늘리지 않습니다.

CI는 자체 Java 8 픽스처를 D8과 NeverD로 처리한 뒤 생성된 Java 전체를 다시 컴파일하고, 전체 `Signature` 메타데이터와 리플렉션 결과 및 동작을 비교합니다. 이 픽스처 검사는 실제 앱의 완전한 복구를 입증하지 않습니다.
