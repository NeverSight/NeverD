...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | 요청 소유 MDL, 시스템 매핑, 공유 물리 페이지 식별자와 SG DMA | 아래 지원 범위 밖의 사용자 매핑과 DMA 인터페이스 |
| 드라이버 할당 MDL | 독립/IRP 연결 설명자, 부분 MDL, 공유 사용자/시스템 매핑 및 완료 시 해제 | 할당량, 수동 MDL 및 일반 가상 메모리 할당 API |
| READ/WRITE | 순차 buffered/direct/neither I/O와 작업 항목 또는 DPC 완료 | 아래 API 범위만 지원; 동시 요청, 일반적인 WDM 요청 취소 및 암묵적 파일 위치는 미지원 |
| WDM `METHOD_NEITHER` | 별도 사용자 버퍼, 접근 검사, MDL 잠금, 합성 요청자 ID, 디스패치 후 VA 철회 또는 프로세스 종료, 제한된 취소 | 명시되지 않은 일반 프로세스 및 가상 메모리 할당 API |
| KMDF 1.33 비 PnP 드라이버 | 바인딩, 객체/컨텍스트, 이름 있는 제어 장치, 순차 또는 유한 또는 무제한 병렬 기본 큐 및 콜백을 실제 실행하는 버퍼/직접/neither 요청 | PnP 장치, 일반 큐 스케줄링, 클래스 확장 및 UMDF는 지원하지 않음 |
| PnP 버스/기능/필터 드라이버 | 명시적 리소스 없는 PDO 또는 고정 레지스터 뱅크 PDO, 게스트 AddDevice, 여덟 가지 일반 PnP 수명 주기 부 기능 | 기타 PnP 작업, 일반 전원 관리, 기타 하드웨어/리소스 및 일반 KMDF PnP |
...
| 하드웨어, DMA, PCI, 인터럽트 또는 가상화 드라이버 | 명시적 레지스터 뱅크, MMIO, 독점/공유 latched 및 level_sensitive 인터럽트, 제한된 일관성 있는 공통 버퍼/SG/채널 DMA | 기타 장치 모델, 임의 물리 RAM, PCI, 포트, 기타 DMA 인터페이스 및 특권 CPU 상태 |
...
이는 제한된 스케줄링 모델이며 완전한 Windows 비동기 지원은 아닙니다. 경고 가능/사용자 모드 대기, APC, 일반적인 WDM 요청 취소, 동시 공개 시나리오 제출, UMDF, 일반 KMDF PnP 장치 및 일반 큐 스케줄링, 전체 PnP/전원, 일반 하드웨어, 기타 DMA 인터페이스는 지원하지 않습니다. 초기화 전용 호출도 명시적으로 대기열에 넣은 콜백을 실행하지만 요청이나 언로드를 암묵적으로 만들지 않습니다.
...
일반 전원 관리, 드라이버 할당 IRP, 기타 PnP 부 기능, 기타 하드웨어/리소스 모델은 지원하지 않습니다. 직접 FDO/PDO 파일 수명주기 이외의 WDF 대상 전달, 파일이나 콜백이 남은 스택에 연결, 중간 계층 분리, 전달 중 주 기능 변경, 보존 경로 외부 대상으로 전달하면 명시적으로 실패합니다. 실제 WDK의 선택적 `driver_wdm_stack.c`는 `NEVERD_WDM_STACK_FIXTURE`와 `NEVERD_WDM_STACK_CFG_FIXTURE`로 구성합니다. 네이티브와 C API/CLI 검증에는 재배치가 포함되며 산출물 누락 시 명시적으로 건너뜁니다. 실행 근거는 Linux에 한정됩니다.
...
DriverEntry 성공 후 각 구성 PDO에 대해 `AddDevice`를 한 번 실행합니다. 제공자는 별도 `DRIVER_OBJECT`를 소유하며 게스트는 제공자 객체를 삭제하거나 가장할 수 없습니다. PnP IRP는 `KernelMode`, 파일 없음, START 리소스는 구성된 버스에 따름, 초기 `STATUS_NOT_SUPPORTED`입니다. 전달이 해당 PDO에 도달해야 버스 응답을 사용하고, 지연 완료는 공유 가상 시계와 기존 완료 continuation을 사용합니다. 최종 상위 완료가 버스 상태와 별도로 수명 주기 확정/롤백을 결정합니다. Started에서 정상 제거하려면 query 성공, 파일 닫기, 이전 요청 종료, 게스트의 분리/삭제가 필요합니다. 누수 없는 AddDevice 실패는 제공자만 퇴역시키고, 새 게스트 장치 누수는 분리된 경우에도 `model_error`입니다. unload 전에 모든 제공자가 없어야 합니다. 기타 PnP, 일반 전원 관리, 기타 하드웨어/리소스 및 일반 KMDF PnP는 포함하지 않습니다. PnP 성공에는 실제 제공자 완료가 필요하며 START/QUERY_STOP/QUERY_REMOVE의 이른 상위 실패는 버스 관측을 null로 남길 수 있습니다. 장치/파일 수명 주기 식별자는 분리 후에도 유지됩니다.
...
같은 `register_bank` 제공자는 `interrupts`만 선언하거나 `resources`와 함께 선언할 수 있으며 최소 한 목록은 비어 있지 않아야 합니다. `resource_free`는 `[]`를 포함한 명시적 `interrupts`를 거부합니다. 각 인터럽트는 `id`, `raw_vector`, `raw_level`, `raw_affinity`, `translated_vector`, `translated_level`, `translated_affinity`, `mode`, `share`를 제공합니다. `DriverInterrupts.h`/`DriverInterrupts.def`가 타입, 표기, 한도를 관리합니다. PDO당 8개, 전체 32개, PDO 안의 고유한 길이 제한 ASCII ID, raw level 0–65535, translated DIRQL 3–12를 허용합니다. 벡터는 전체 32비트를 유지하며 IRQL과의 관계를 추정하지 않습니다. 양쪽 affinity는 1이어야 하고 CPU0/group0만 모델링합니다. `mode`는 `latched` 또는 `level_sensitive`, `share`는 `device_exclusive` 또는 `shared`입니다. 같은 변환 벡터를 공유하는 리소스는 모두 shared이며 모드, 변환 레벨, affinity, 재트리거 주기가 같아야 합니다. 레벨 모드는 `INT64_MAX` 이하의 양수 `retrigger_after_100ns`를 명시해야 합니다. raw와 translated 값은 독립적입니다. `CM_RESOURCE_LIST`는 메모리 설명자 순서를 유지한 뒤 타입 2 인터럽트 설명자를 배치하며 공유 속성과 트리거 플래그는 선언을 반영합니다.

READ/WRITE/IOCTL은 `interrupt_events`를 선언하며 각 항목에 `after_100ns`, `device_id`, `interrupt_id`와 선택적 `action`을 지정합니다. 다른 요청 종류는 빈 목록도 거부합니다. 지연은 `INT64_MAX` 이하의 음이 아닌 값으로 제출 성공 시점부터 계산하며 당시 연결과 PDO 리소스 세대를 캡처합니다. latched의 기본값은 `action: "pulse"`이며 캡처한 모든 ISR을 호출합니다. level_sensitive는 명시적 `assert`/`deassert`만 허용합니다. PDO/리소스/세대별 소스 상태를 OR하여 라인 레벨을 만들며 반복 assert는 멱등적입니다. 같은 경계의 모든 외부 상태 변경을 적용한 다음 샘플링합니다. 각 ISR 배치 후 현재 가상 시간에 `retrigger_after_100ns`를 더한 시각에 다시 샘플링하고 등록 순서로 TRUE가 나올 때까지 호출합니다. TRUE는 처리를 맡았다는 뜻이며 소스 레벨을 해제하지 않습니다. 원본 IRP 완료는 이벤트를 취소하지 않고 레지스터 쓰기로 enable/status/acknowledgment를 추론하지 않습니다. 시간은 유휴 시 진행하고 지원하는 콜백 경계에서 ISR이 DPC/작업자보다 먼저 실행됩니다. `after_100ns: 0`은 명령어 단위 선점을 보장하지 않습니다. 선언은 요청당 64개, 전체 1024개이고 실제 전달 배치는 별도로 4096개, 배치당 처리기는 32개까지이며 스케줄러 및 명령어 예산도 적용합니다. 연결 손실, 무효 리소스 세대 또는 물리 D3는 `undelivered_reason`을 기록하고 `model_error`로 중단하며 다시 연결하거나 조용히 미루지 않습니다.

`IoConnectInterrupt`는 실제 11개 인수 ABI와 정확히 일치하는 변환 리소스를 사용합니다. `IoConnectInterruptEx`는 FullySpecified(1), LineBased(2, 명시적 PDO의 한 라인), FullySpecifiedGroup(4, group0)을 지원하며 해제에는 일치하는 버전과 컨텍스트가 필요합니다. 연결과 해제는 `PASSIVE_LEVEL`에서 수행합니다. 독점 또는 공유 latched/level_sensitive 라인은 전용 잠금이나 호출자가 초기화한 공유 `KSPIN_LOCK`을 사용할 수 있습니다. 공통 `SynchronizeIrql`은 각 할당 DIRQL 이상이며 LineBased의 0은 할당 레벨을 선택합니다. ISR 전달 우선순위는 할당 DIRQL을 유지하고 실행은 동기화 잠금 레벨을 따릅니다. `KINTERRUPT`는 불투명하며 주소를 재사용하지 않습니다. ISR은 `(Interrupt, ServiceContext)`를 받고 AL의 BOOLEAN으로 처리 여부를 나타내며 FALSE는 NTSTATUS 실패가 아닙니다. `KeSynchronizeExecution`은 같은 잠금으로 실제 단일 인수 콜백을 실행하고 BOOLEAN을 반환한 뒤 호출자 IRQL/CR8을 복원합니다. ISR, 동기화 콜백, 수동 `KeAcquireInterruptSpinLock`/`KeReleaseInterruptSpinLock`은 실제 잠금 소유권을 공유하고 비재귀성, 실행 신원, 저장된 IRQL을 검증하며 잠금을 잡은 채 반환할 수 없습니다. 부동소수점 저장, 명령어 단위 선점은 지원하지 않습니다. START 실패와 STOP/REMOVE 성공 시 최종 완료 전에 연결을 해제해야 하며 상위 완료 콜백이 먼저 정리할 수 있습니다. 해제는 대기 중 DPC를 버리지 않습니다.
...
KMDF 1.33 지원은 정확한 1.33.0 ABI를 사용합니다. 458개 함수 슬롯에 안정적인 게스트 식별자가 있으며 아래 99개 API에 실행 의미가 구현되어 있습니다. `WdfVersionBind`와 `WdfVersionUnbind`는 실제 WDK `FxDriverEntry` 래퍼 전후에 게스트 바인딩을 관리합니다. `WdfGetDriver`는 공용 드라이버 전역 구조를 읽습니다. 비 PnP 드라이버, 일반 객체, 제어 장치, 큐 및 들어오는 요청은 형식화된 컨텍스트, 참조 수와 실제 실행되는 정리/파괴/언로드 콜백을 공유합니다. 인터럽트 DDI와 아래에 지원 IRQL을 명시한 요청 및 객체 작업을 제외한 프레임워크 호출과 콜백은 `PASSIVE_LEVEL`을 요구하며 정리 완료 후 새 참조를 얻는 동작은 이 프로필의 범위 밖입니다. 모델링되지 않은 함수 슬롯, `WdfLdrQueryInterface`, 클래스 확장 및 UMDF는 명시적으로 중단합니다.

인터럽트 API: `WdfInterruptCreate`, `WdfInterruptQueueDpcForIsr`, `WdfInterruptQueueWorkItemForIsr`, `WdfInterruptSynchronize`, `WdfInterruptAcquireLock`, `WdfInterruptReleaseLock`, `WdfInterruptEnable`, `WdfInterruptDisable`, `WdfInterruptWdmGetInterrupt`, `WdfInterruptGetInfo`, `WdfInterruptGetDevice`.

프레임워크 인터럽트 객체는 할당된 라인/MSI 리소스와 내부 인터럽트 잠금을 공유합니다. ISR과 동기화 콜백은 지정된 DIRQL, 수동 처리에서는 `PASSIVE_LEVEL`에서 실행합니다. `WdfInterruptGetInfo`는 수동 IRQL을 보고합니다. DPC는 `DISPATCH_LEVEL`, 작업 항목은 `PASSIVE_LEVEL`에서 실행합니다. 대기열의 중복 등록을 합치고 일시 중단 중에도 콜백 반환까지 소유권을 유지합니다. 전원 진입은 D0Entry 뒤에 연결·활성화하며, 전원 종료는 비활성화·연결 해제와 지연 콜백 배출 뒤에 D0Exit 및 리소스 해제를 진행합니다. 명시적 활성화/비활성화는 실제 드라이버 콜백을 실행하며 하드웨어 인터럽트를 만들지 않습니다. 외부 `WDFSPINLOCK`/`WDFWAITLOCK`, 부모 자동 직렬화, 깨우기 인터럽트와 inactive 연결 유지는 지원하지 않습니다. 유휴/깨우기 정책과 실패 장치 자동 재열거도 범위 밖입니다.

다음 요청 및 객체 API는 `DISPATCH_LEVEL` 이하의 IRQL을 지원합니다: `WdfObjectDereferenceActual`, `WdfRequestComplete`, `WdfRequestCompleteWithInformation`, `WdfRequestRetrieveInputBuffer`, `WdfRequestRetrieveOutputBuffer`, `WdfRequestRetrieveInputMemory`, `WdfRequestRetrieveOutputMemory`, `WdfRequestRetrieveInputWdmMdl`, `WdfRequestRetrieveOutputWdmMdl`, `WdfRequestGetInformation`, `WdfRequestSetInformation`, `WdfRequestGetIoQueue`, `WdfRequestGetFileObject`, `WdfRequestWdmGetIrp`, `WdfRequestGetParameters`, `WdfRequestGetStatus`, `WdfRequestMarkCancelableEx`, `WdfRequestUnmarkCancelable`, `WdfRequestIsCanceled`, `WdfMemoryGetBuffer`.

`WdfObjectGetTypedContextWorker`, `WdfObjectContextGetObject`, `WdfObjectReferenceActual`은 모델링된 인터럽트 IRQL도 지원합니다. 컨텍스트 접근과 참조 획득은 수동 수준 콜백을 호출하지 않습니다.

`PASSIVE_LEVEL`보다 높은 IRQL에서 요청을 완료하거나 객체의 마지막 참조를 해제하면 실제 정리, 파괴 및 후속 프레임워크 전달을 `PASSIVE_LEVEL`에 예약합니다. 기존 `WdfRequestMarkCancelable`은 이미 취소된 요청의 콜백을 동기적으로 실행하기 위해 계속 `PASSIVE_LEVEL`로 제한됩니다. 다른 API에는 기존 IRQL 제한이 적용됩니다.
...
96바이트 `WDF_IO_QUEUE_CONFIG`는 명시적 수동 수준 실행과 프레임워크 동기화 없음으로 설정된 기본 및 비기본 수동·순차·유한 또는 무제한 병렬 큐를 지원합니다. 제어 장치 큐는 전원 관리를 받지 않습니다. 전용 READ/WRITE/IOCTL 콜백이 기본 콜백보다 우선합니다. 수락된 큐 요청은 동기적으로 완료되어도 `STATUS_PENDING`을 반환하며 void 콜백의 반환 레지스터가 요청을 완료시키지 않습니다. 지연 완료에는 기존 스케줄러를 사용합니다. 처리기가 없으면 `STATUS_INVALID_DEVICE_REQUEST`로 완료하고 길이가 0인 READ/WRITE는 전달을 활성화하지 않으면 바로 완료합니다. 기본 파일 패키지는 CREATE/CLEANUP/CLOSE를 성공 상태와 Information=0으로 완료합니다. 기본 큐가 아닌 수동 큐에는 `WdfRequestForwardToIoQueue`로 요청을 전달하고 `WdfIoQueueRetrieveNextRequest`로 FIFO 순서로 가져올 수 있습니다. 가져오기 전에 취소된 요청은 프레임워크가 큐에서 제거하고 `STATUS_CANCELLED`로 완료합니다. 비기본 자동 큐는 전달된 요청을 자체 콜백에 배달하고 수동 기본 큐는 가져올 때까지 수신 요청을 보유합니다. `WdfIoQueueRetrieveNextRequest`는 수동 및 순차 큐에서 동작하며 병렬 큐에서는 `STATUS_INVALID_DEVICE_STATE`를 반환합니다. 일치하는 콜백이 없으면 자동 큐는 전달 슬롯이 비었을 때 요청을 `STATUS_INVALID_DEVICE_REQUEST`로 완료합니다. 더 광범위한 PnP 및 전원 동작은 이 프로필의 범위 밖입니다. `WdfRequestRequeue`는 가져온 요청을 같은 수동 큐의 맨 앞에 다시 넣습니다. `NumberOfPresentedRequests`는 병렬 큐에서 전달 중인 요청 수를 제한하며 나머지는 전달된 요청의 완료 또는 취소를 기다립니다. 기본 순차 큐는 요청 하나를 전달한 상태에서도 추가 요청을 받습니다. 후속 요청은 슬롯이 빌 때까지 FIFO 순서로 대기하며 전달 전에 취소할 수 있습니다. `WdfIoQueueStop`은 요청을 계속 받으면서 전달만 중지합니다. `WdfIoQueueStart`는 대기 중인 요청 전달을 재개하고 `WdfIoQueueGetState`는 대기 및 전달된 요청 수를 보고합니다. 중지 중 가져오기는 `STATUS_WDF_PAUSED`를 반환하며 중지 완료 콜백은 전달된 모든 요청이 완료되거나 큐를 떠난 뒤 지정된 문맥과 함께 실행됩니다. 대기 중인 요청은 이를 지연시키지 않으며, 보류 중인 중복 등록은 거부됩니다.

`WdfIoQueueReadyNotify`는 수동 대기열에 `EvtIoQueueState` 콜백 하나를 등록합니다. 대기 요청 수가 0에서 양수로 바뀌면 드라이버가 이전에 꺼낸 요청을 아직 소유하더라도 `PASSIVE_LEVEL`에서 `(WDFQUEUE, WDFCONTEXT)`를 받습니다. 이미 비어 있지 않은 대기열에 등록하면 즉시 통지할 수 있고, 중지된 대기열은 `WdfIoQueueStart`까지 기다립니다. 중복 등록이나 중지 전 등록 해제는 `STATUS_INVALID_DEVICE_REQUEST`를 반환합니다. `WdfIoQueueStop` 후 NULL을 전달하면 등록을 해제합니다.

`WdfIoQueueFindRequest`는 수동 대기열을 검색하지만 요청 처리 소유권을 넘기지 않습니다. 성공하면 요청 참조가 하나 늘어나므로 드라이버는 `WdfObjectDereference`로 해제해야 합니다. `WdfIoQueueRetrieveFoundRequest`는 아직 대기열에 있는 요청의 소유권을 넘기며, 취소로 제거된 요청에는 `STATUS_NOT_FOUND`를 반환합니다. 선택적 매개변수는 `WdfRequestGetParameters`와 같은 레이아웃을 사용합니다. 유효한 프레임워크 파일 객체로 `WdfIoQueueFindRequest`를 필터링할 수 있습니다. `WdfIoQueueRetrieveRequestByFileObject`는 수동 또는 순차 큐에서 다음 일치 요청을 가져오며, 일치 항목이 없으면 출력 값을 바꾸지 않습니다.

설정된 `EvtIoCanceledOnQueue`는 드라이버에 한 번 전달된 뒤 전달 또는 재대기열에 넣은 요청이나 호출자 컨텍스트 콜백이 명시적으로 대기열에 넣은 요청에 대해서만 `(WDFQUEUE, WDFREQUEST)`를 받습니다. 드라이버에 한 번도 전달되지 않은 대기 요청은 이 콜백 없이 프레임워크가 `STATUS_CANCELLED`로 완료합니다. 통지는 요청 소유권을 드라이버에 돌려주며, 드라이버는 콜백 안이나 나중에 요청을 완료해야 하고 다시 대기열에 넣을 수 없습니다. 제거 작업과 상태 통지는 콜백 반환 및 해당 요청 완료를 기다립니다.

`WdfIoQueueDrain`은 새 요청을 `STATUS_INVALID_DEVICE_STATE`로 거부하고 대기 중인 요청을 계속 전달합니다. 대기 중인 요청과 드라이버 소유 요청이 모두 없어지면 완료 콜백을 실행합니다. 배출 중인 큐로의 전달은 `STATUS_WDF_BUSY`를 반환하며 `WdfIoQueueStart`는 수락을 재개합니다.

`WdfIoQueuePurge`는 아직 전달되지 않은 요청을 `STATUS_CANCELLED`로 취소하고 이미 전달된 취소 가능 요청은 원래 IRP에서 취소합니다. 요청 정리는 IRP 해제 전에 실행됩니다. 상태 콜백은 대기 요청, 드라이버 소유 요청 및 취소 콜백이 모두 끝날 때까지 기다립니다. 취소 가능 표시가 없는 요청은 드라이버가 완료합니다.

`WdfIoQueueStopSynchronously`, `WdfIoQueueDrainSynchronously`, `WdfIoQueuePurgeSynchronously`는 관련 요청이 종료될 때까지 `PASSIVE_LEVEL` 게스트 호출을 중단합니다. 동기 중지는 접수를 유지하고 전달을 멈춘 뒤 전달된 요청을 기다립니다. 동기 드레인은 새 요청을 거절하면서 대기 요청을 전달하고 두 종류의 완료를 기다립니다. 동기 퍼지는 대기 요청과 취소 가능으로 표시된 요청을 취소하고 취소 콜백이 끝날 때까지 기다립니다. 완료 생산자가 없으면 정지된 모델 오류를 보고합니다.

`WdfIoQueueStopAndPurge`와 `WdfIoQueueStopAndPurgeSynchronously`는 호출 전에 대기 중이거나 취소 가능으로 표시된 드라이버 소유 요청을 취소한 뒤, `WdfIoQueueStart` 전까지 전달하지 않고 새 요청은 계속 받습니다. 비동기 상태 콜백과 동기 대기는 기존 요청과 취소 콜백이 끝난 뒤 완료됩니다. 호출 후 들어온 요청은 대기하며 완료 통지를 지연하지 않습니다. 일반 중지와 동기 중지도 이전 드레인 또는 퍼지 상태에서 접수를 다시 켭니다.

요청 매개변수는 40바이트 `WDF_REQUEST_PARAMETERS` 레이아웃을 사용합니다. 입력/출력 접근 함수는 논리 길이를 반환하고 버퍼 별칭 및 기존 직접 I/O MDL 매핑을 유지하며 직접 IOCTL 입력은 계속 버퍼 방식을 사용합니다. 잘못된 방향과 버퍼 부족에는 문서에 정의된 상태를 반환합니다. 완료 처리는 버퍼가 유효한 동안 요청 정리를 실행하고 IRP를 완료해 요청 소유 페이지 잠금을 해제한 뒤 참조가 허용할 때 자식 객체와 요청을 파괴합니다. 완료가 시작되면 새 버퍼 및 매개변수 접근 함수 호출을 거부하지만 이미 얻은 버퍼 포인터는 정리 중에도 사용할 수 있습니다. 외부 객체 참조는 컨텍스트를 유지할 뿐 완료된 IRP 접근을 유지하지 않습니다.
...
`WdfRequestRetrieveInputWdmMdl`과 `WdfRequestRetrieveOutputWdmMdl`은 버퍼 방식 WRITE 입력, READ 출력, IOCTL 입력/출력의 기존 SystemBuffer를 필요할 때 기술합니다. 매번 방향이 유효하고 길이가 0이 아닌지 검사한 뒤 요청의 단일 캐시 설명자를 사용합니다. 첫 성공 검색이 ByteCount를 정하며 반대 방향의 논리 길이가 달라도 유지합니다. 이 설명자에서 `MmGetSystemAddressForMdlSafe`는 원래 VA를 반환하고 추가 매핑, 매핑 해제 및 드라이버의 해제는 거부됩니다. 직접 READ 출력, WRITE 입력 및 IOCTL 출력은 기존 `IRP.MdlAddress`를 반환하며 검색 자체가 매핑하지 않습니다. 직접 IOCTL 입력은 SystemBuffer 캐시를 사용합니다. 설명자, IRP와 버퍼는 완료 시 무효화됩니다. 취소 내부 참조나 외부 참조는 WDF 컨텍스트만 유지하며 완료된 I/O 저장 공간을 유지하지 않습니다. WDF의 `METHOD_NEITHER` MDL 조회 함수는 아직 지원하지 않으며 요청 소유 WDFMEMORY는 별도의 잠긴 페이지 매핑을 사용합니다. 구성된 설명자의 모델 PFN 배열은 읽기 전용이며 미구성 설명자의 PFN 접근은 거부합니다.

버퍼, 직접 및 neither KMDF 요청에서 `WdfDeviceInitSetIoInCallerContextCallback`은 요청 프로세스의 `PASSIVE_LEVEL`에서 큐 이전 콜백을 실행합니다. 요청을 완료하거나 `WdfDeviceEnqueueRequest`를 한 번 호출해 기본 큐로 보내야 합니다. `METHOD_NEITHER` IOCTL과 neither READ/WRITE에서는 `WdfRequestRetrieveUnsafeUserInputBuffer` 및 `WdfRequestRetrieveUnsafeUserOutputBuffer`가 이 콜백 안에서만 원래 사용자 주소를 반환합니다. `WdfRequestProbeAndLockUserBufferForRead` 및 `WdfRequestProbeAndLockUserBufferForWrite`는 페이지 권한을 확인하고 요청 소유 페이지를 잠급니다. `WdfMemoryGetBuffer`는 요청자 컨텍스트를 벗어난 큐 콜백에서도 사용할 수 있는 시스템 별칭을 반환합니다. 완료 시 잠금과 별칭을 해제합니다. 명시한 `user_buffers`를 가리키는 내장 포인터도 지원합니다. 임의 사용자 매핑은 지원하지 않습니다.

neither 요청은 `user_buffers`(`id`, `size`, 선택적 `input`/`access`)와 `user_pointers`를 선언할 수 있습니다. `source`/`target`은 `buffer`(`input`, `output`, `memory`)와 `offset`을 지정하며 `memory`만 요청 내 고유 `id`를 사용합니다. 8바이트 포인터 슬롯의 중복과 범위 초과는 거부하고 대상은 버퍼 내부나 끝일 수 있습니다. 시나리오 전체에서 추가 버퍼 64개, 포인터 256개까지 허용하며 각 버퍼는 최대 64 KiB이고 총 512 KiB 예산에 포함됩니다. 같은 대상은 실제 바이트를 공유하며 기존 권한, 잠금, 해제, 프로세스 종료 규칙을 따릅니다. `configuration.user_memory`는 선언을 보존합니다. `user_buffers[].backing_hex`는 해제나 오류 이후도 포함하는 종료 시점의 진단 스냅샷이며 `output_hex`나 접근 권한을 대신하지 않습니다. `id`: 1–64 ASCII, `[A-Za-z0-9][A-Za-z0-9_.-]*`.

`WdfRequestRetrieveInputMemory`와 `WdfRequestRetrieveOutputMemory`는 기존 버퍼 방식 또는 직접 I/O 버퍼의 요청 소유 WDFMEMORY 뷰를 제공합니다. 같은 방향을 다시 조회해도 핸들이 유지되며 `WdfMemoryGetBuffer`는 원래 버퍼와 논리 길이를 반환합니다. 길이가 0이거나 방향이 맞지 않으면 WDF 상태로 실패하고 neither I/O에는 요청자 컨텍스트의 검사와 페이지 잠금이 필요합니다. 빌린 뷰는 MDL 잠금을 추가하지 않으며 요청 완료 시 무효화됩니다.

KMDF PnP 직접 FDO/PDO 구성을 제한적으로 지원합니다. `EvtDriverDeviceAdd`는 프레임워크 소유 초기화 객체를 받고 `WdfDeviceCreate`는 FDO를 만듭니다. `WdfFdoInitWdmGetPhysicalDevice`와 `WdfDeviceWdmGetPhysicalDevice`는 PDO 식별자를 반환합니다. AddDevice 실패 또는 Remove 완료 후 정리/파괴 콜백을 실행하고 장치를 삭제합니다. PnP 큐는 전원 관리에 `WdfUseDefault` 또는 `WdfTrue`를 사용할 수 있으며 `WdfFalse`는 자동 관리를 비활성화합니다. `WdfDeviceInitSetPnpPowerEventCallbacks`는 `EvtDevicePrepareHardware`, `EvtDeviceD0Entry`, `EvtDeviceD0Exit`, `EvtDeviceReleaseHardware`, `EvtDeviceQueryStop`, `EvtDeviceQueryRemove`, `EvtDeviceSurpriseRemoval`를 지원합니다. QueryStop/QueryRemove는 하위 버스 전달 전에 실제 콜백을 실행합니다. 실패하면 원래 IRP를 거부하고 완료하며 버스에 전달하지 않고 장치 및 전원 수명 주기를 유지합니다. void SurpriseRemoval 콜백도 하위 버스 전달보다 먼저 실행하고 이후 기존 D0 종료 절차를 따릅니다. PrepareHardware는 D0Entry 전에 서로 다른 리소스 목록 두 개를 받으며, 리소스가 없는 장치에서는 목록이 비어 있습니다. 빈 목록에서 `WdfCmResourceListGetCount`는 0을, `WdfCmResourceListGetDescriptor`는 NULL을 반환합니다. PrepareHardware 또는 D0Entry가 실패하면 START도 실패하지만 ReleaseHardware는 실행됩니다. D0에서 중지하거나 제거할 때에는 D0Exit 다음, IRP 완료 전에 ReleaseHardware가 실행됩니다. 구성된 `register_bank` 장치는 ReleaseHardware까지 읽기 전용 `CM_PARTIAL_RESOURCE_DESCRIPTOR` 항목을 제공합니다. 드라이버는 변환된 메모리를 `MmMapIoSpace`로 매핑할 수 있으며 STOP 또는 Remove 완료 전에 해제해야 합니다. 다른 리소스 유형과 일반 전원 정책은 지원하지 않습니다. `WdfIoQueuePnpHeld`는 D0 진입 전과 D0 종료 후의 전달 보류를 나타냅니다. D0에서 나가기 전에 전원 관리 큐에서 드라이버가 보유한 각 요청에 `EvtIoStop`을 호출합니다. 드라이버는 요청을 완료하거나 `WdfRequestStopAcknowledge`를 호출하거나 이미 예약된 작업의 완료를 기다릴 수 있습니다. `EvtIoStop`이 등록되지 않아도 프레임워크는 전달된 모든 요청이 완료되기를 기다립니다. TRUE는 재시작 후 다시 전달하도록 큐에 돌려놓고, FALSE는 드라이버 소유권을 유지하며 D0 재진입 뒤 `EvtIoResume`을 호출합니다. 큐에 남은 요청은 재시작까지 대기하며 완료를 만들 작업이 더는 없을 때에만 대기는 정체된 `model_error`로 끝납니다. 실제 WDK 픽스처 `driver_kmdf_pnp.c`는 `NEVERD_KMDF_PNP_FIXTURE`/`NEVERD_KMDF_PNP_CFG_FIXTURE`로 설정합니다.
...
PnP FDO에서 `WdfDeviceInitSetDeviceType`은 지정한 32비트 유형을 WDM `DEVICE_OBJECT`에 기록합니다. 지정하지 않으면 기본값은 `FILE_DEVICE_UNKNOWN`입니다. 장치 유형에 따른 I/O 우선순위 상승은 모델링하지 않습니다.

`WdfDeviceInitSetExclusive`은 초기화자에서 생성한 WDM 장치에 `DO_EXCLUSIVE`를 설정합니다. 이름이 있는 제어 장치는 첫 파일이 닫힐 때까지 두 번째 독립적인 열기를 거부합니다. PnP FDO의 이 플래그만으로는 이름이 있는 PDO나 전체 스택이 독점 장치가 되지 않습니다. INF로 지정하는 PDO 독점 속성은 이 프로필에서 모델링하지 않습니다.

`WdfDeviceInitSetFileObjectConfig`는 장치 생성 전에 `EvtDeviceFileCreate`, `EvtFileCleanup`, `EvtFileClose`를 등록하고 선택적 파일 개체 컨텍스트 속성을 복사합니다. 이 프로필은 `WdfFileObjectNotRequired`, `WdfFileObjectWdfCanUseFsContext`, `WdfFileObjectWdfCanUseFsContext2`, `WdfFileObjectWdfCannotUseFsContexts`를 지원합니다. 선택한 WDM 컨텍스트 슬롯은 실패한 CREATE 또는 CLOSE까지 WDF 핸들을 보관하며 처음에는 비어 있어야 합니다. `WdfFileObjectCanBeOptional`을 지정하면 일치하는 WDM 파일 객체가 없는 I/O에서 `WdfRequestGetFileObject`가 NULL을 반환합니다. CREATE, CLEANUP, CLOSE에는 여전히 WDM 파일 객체가 필요하며 `WdfFdoInitSetFilter`는 필터의 기본 전달을 활성화하고 `WdfTrue`는 명시적으로 활성화하며 `WdfFalse`는 비활성화합니다. 파일 요청 전달에는 직접 FDO/PDO 경로와 명시적인 `bus_completion`이 필요합니다. 양수 `delay_100ns`는 자동 CREATE/CLEANUP/CLOSE 전달과 동기, 완료 루틴이 있는 비동기 또는 `SEND_AND_FORGET` CREATE 전송에서 허용됩니다. CLEANUP과 CLOSE 콜백은 하위 전달보다 먼저 실행됩니다. `WdfFileObjectNotRequired`를 사용하는 CREATE 콜백은 `WdfDeviceGetIoTarget`으로 장치가 소유한 로컬 대상을 얻고, `WDF_REQUEST_SEND_OPTION_SEND_AND_FORGET`만 지정한 `WdfRequestSend`로 원래 요청을 전달할 수 있습니다. 유지된 PDO가 명시적인 버스 응답을 소비하고, 하위 응답이 지연되더라도 최종 완료를 소유합니다. 프레임워크 파일 개체가 있는 CREATE 콜백은 `WDF_REQUEST_SEND_OPTION_SYNCHRONOUS`를 사용하고 `WdfRequestGetStatus`로 하위 결과를 읽은 뒤 그 상태로 원래 요청을 완료할 수 있습니다. 기본 비동기 전송은 먼저 `WdfRequestFormatRequestUsingCurrentType`로 요청을 형식화하고 `WdfRequestSetCompletionRoutine`으로 완료 콜백을 등록합니다. 콜백은 `WDF_REQUEST_COMPLETION_PARAMS`를 받고 `WdfRequestGetCompletionParams` 또는 `WdfRequestGetStatus`로 하위 결과를 읽고 하위 상태와 독립적으로 선택한 상태로 원래 요청을 완료합니다. 완료 콜백이 있는 전송에서 PDO 응답이 지연되면 요청은 가상 시각의 기한까지 보류되고, 최종 하위 상태와 함께 콜백이 대기열에 들어갑니다. 비동기 CREATE의 `WDF_REQUEST_SEND_OPTION_TIMEOUT`은 음수를 상대 간격, 양수를 합성 가상 시계의 절대 100 ns 기한으로 처리하며 0은 비활성화합니다. 하위 응답보다 먼저 만료되면 `STATUS_IO_TIMEOUT`을 전달합니다. 즉시 완료와 기한이 같은 하위 응답이 우선합니다. 명시되지 않은 다른 전송 옵션은 지원하지 않습니다. WDF 핸들과 WDM `FILE_OBJECT`는 별도 식별자이며, 유효할 때 `WdfRequestGetFileObject`, `WdfFileObjectGetDevice`, `WdfFileObjectWdmGetFileObject`로 조회할 수 있습니다. CREATE 실패 시 파일 CLEANUP/CLOSE 콜백 없이 WDF 개체를 삭제합니다. 성공한 파일은 CLEANUP, CLOSE 콜백 후 컨텍스트 정리와 소멸을 수행합니다.

지연된 동기 `WdfRequestSend`는 실제 게스트 호출 프레임을 보존하여 중단하고, 완료 후 전송 여부를 나타내는 BOOLEAN을 반환하며 재개합니다. 최종 NTSTATUS는 `WdfRequestGetStatus`로 읽습니다. 동기 CREATE의 `WDF_REQUEST_SEND_OPTION_TIMEOUT`도 음수 상대 간격과 양수 절대 가상 100 ns 기한을 지원하며 0은 비활성화합니다. 먼저 만료되면 `STATUS_IO_TIMEOUT`을 반환하고 즉시 완료나 기한이 같은 하위 응답이 우선합니다. 자동 전달에서는 파일 콜백이 하위 전달보다 먼저 실행되며, 완료 과정에 필요한 정리 및 소멸 콜백 동안 WDM IRP와 파일을 유지합니다. 외부 WDF 참조는 컨텍스트를 보존하지만 완료된 요청의 WDM 파일 수명을 연장하지 않습니다.

구성된 PDO로 전달한 WDM CREATE/CLEANUP/CLOSE도 명시적으로 지연된 `bus_completion`을 허용합니다. 하위 기한 이후 실제 게스트 완료 루틴을 실행하며 보류 상태 전파와 최종 상태 소유권 규칙을 유지합니다.
...
선택적 불리언 `user_unmap_after_dispatch`는 비어 있지 않은 WDM neither 전송에서 디스패치 반환 후, 예약된 작업 또는 취소 전에 원래 사용자 주소의 접근을 취소합니다. MDL로 잠근 페이지와 시스템 별칭은 잠금 해제 전까지 사용할 수 있지만 원시 사용자 포인터와 새 잠금은 실패합니다. 출력 주소가 취소되면 `output_hex`는 비어 있습니다. 시나리오의 원래 할당과 명시적으로 선언한 할당의 주소는 재사용하거나 재매핑하지 않으며 임의 시점의 철회도 모델링하지 않습니다. 이 제한은 아래 MDL 사용자 뷰 영역의 주소 재사용에는 적용하지 않습니다.
...
`IoBuildPartialMdl`은 구성된 원본 MDL의 비어 있지 않은 부분 범위를 설명합니다. 길이 0은 나머지 전체를 선택하며, 대상에는 충분한 PFN 용량이 필요합니다. 부분 설명자는 추가 페이지 잠금 없이 물리 페이지를 공유합니다. 원본의 유효한 시스템 매핑을 상속하거나 `MmGetSystemAddressForMdlSafe`로 독립 매핑을 만듭니다. 부분 MDL을 해제하거나 재사용하도록 준비하면 자신이 소유한 매핑만 해제합니다. 실제 WDK 인라인 함수 `MmPrepareMdlForReuse`를 실행한 다음 `IoBuildPartialMdl`을 다시 호출할 수 있습니다. 비페이지 원본 설명자는 독립적으로 해제할 수 있지만, 의존하는 부분 MDL이 남아 있으면 원래 사용자 잠금이나 빌린 시스템 별칭을 해제할 수 없습니다. 완료는 설명자 순서와 무관하게 의존성을 해제하기 전에 전체 체인을 검사합니다. 활성 DMA는 재구성과 조기 해제를 막습니다.

`MmMapLockedPagesSpecifyCache`는 활성 요청자 문맥(지원 범위의 프로세스에 연결한 작업자 포함)에서 `APC_LEVEL` 이하의 `UserMode` 매핑을 지원합니다. `MmNonCached`, `MmCached`, `MmWriteCombined`는 유효한 캐시 인수지만 이미 정해진 물리 페이지의 속성을 상속하며 새 매핑으로 바꾸지 않습니다. 현재 일반 RAM은 Cached이며 예약된 열거값은 거부합니다. CPU 캐시 타이밍은 모사하지 않습니다. 각 호출은 `MappedSystemVa`를 바꾸지 않고 같은 물리 저장소에 프로세스 소유의 독립 뷰를 만듭니다. 뷰는 항상 실행 불가(NX)이며 `MdlMappingNoWrite`는 읽기 전용으로 만듭니다. 풀 저장소는 비페이지 메모리이고 완전한 전용 페이지를 차지해야 합니다. `MmProbeAndLockPages`로 다시 잠가도 원래 PFN을 유지합니다. 지정 주소는 MDL의 페이지 오프셋을 보존하고 사용자 뷰 전용의 제한된 VA 영역 안에 있어야 합니다. 영역 소진은 포착 가능한 `STATUS_INSUFFICIENT_RESOURCES`를 발생시킵니다.

`MmUnmapLockedPages`는 정확한 사용자 주소/MDL 쌍과 뷰를 만든 프로세스를 요구합니다. 별칭을 실제로 제거하고 매핑 예산을 반환하므로 자동 할당과 `RequestedAddress` 모두 해제된 VA를 재사용할 수 있습니다. 새 매핑은 자신의 물리 저장소와 권한을 사용하고 기존 페이지 잠금과 시스템 별칭은 원래 페이지를 유지합니다. 뷰는 `(PID, VA)`로 식별되므로 여러 프로세스가 같은 숫자의 VA에 서로 다른 물리 페이지와 권한을 동시에 보유할 수 있습니다. 프로세스 전환은 CPU에 보이는 바인딩을 트랜잭션으로 교체하며 사전 검증 실패 시 원래 바인딩과 권한을 보존합니다. 프로세스 종료는 해당 프로세스의 뷰만 제거하며 다른 프로세스의 같은 VA에 영향을 주지 않습니다. 시나리오의 원래 VA 철회는 독립 MDL 뷰에 영향을 주지 않습니다. 활성 뷰는 설명자, 루트 잠금, 풀 및 요청 저장소의 조기 해제를 막습니다. 사용자 뷰에 활성 불투명 스케줄러 객체가 있으면 프로세스 전환을 명시적으로 거부하며 프로세스별 커널 객체 식별자를 추정하지 않습니다. 일반 가상 메모리 할당과 모델링하지 않은 프로세스 API는 지원하지 않습니다.
...
WDM READ/WRITE/IOCTL 요청이나 무제한 병렬 KMDF 기본 큐의 요청에 `defer_callback_drain: true`를 지정할 수 있습니다. 디스패치가 `STATUS_PENDING`을 반환하고 IRP가 실제로 보류 중일 때만 콜백을 실행하기 전에 다음 요청을 제출합니다. 이 필드가 없는 다음 요청 후에는 콜백을 처리하고 배치를 마무리합니다. 마지막 요청에 필드가 있으면 시나리오 끝에서 처리합니다. 겹치는 요청은 서로 다른 파일 객체 또는 명시적으로 비동기 방식으로 연 동일 파일 객체를 사용할 수 있습니다. 동기 파일의 중첩, 임의 선점 및 외부 요청 도착은 지원하지 않습니다.
...
`MmAllocatePagesForMdl`, `MmAllocatePagesForMdlEx`, `MmFreePagesFromMdl`은 물리 범위, 부분 할당, 연속 청크 조건을 지키는 독립 페이지를 제공합니다. 페이지와 설명자는 별도로 해제합니다. `MmProtectMdlSystemAddress`는 독립 시스템 매핑의 PAGE 권한을 바꾸며 PFN, 캐시 속성, 사용자 뷰 권한을 유지합니다. `MmProbeAndLockPages`는 KernelMode 풀도 잠급니다. 페이지 풀은 APC_LEVEL 이하에서 잠근 뒤 DISPATCH_LEVEL에서 접근할 수 있고 잠금을 풀기 전에 해제할 수 없습니다.

명시적 `messages`는 MSI 주소, 데이터, 벡터, 수준, 선호도, 극성을 지정합니다. 이벤트의 `message_id`는 설명자 내 번호이며 ISR의 세 번째 인수는 PDO 전체 번호입니다. `CONNECT_MESSAGE_BASED`(3)는 실제 메시지 표를 반환합니다. `CONNECT_MESSAGE_BASED_PASSIVE`(5)와 수동 라인은 PASSIVE_LEVEL 대기를 지원합니다. 같은 인터럽트는 직렬화하고 독립 실행 프레임은 보존합니다. 도착 및 실제 전달 시간을 구분하며 PCI나 장치 프로토콜을 추측하지 않습니다.

`WdfDeviceInitSetPowerPolicyOwnership`는 전원 정책 소유권을 설정합니다. 자체 관리 I/O Init/Suspend/Restart/Flush/Cleanup과 D0 전후 콜백을 실행하고 일반 D3/D0에서 리소스를 유지합니다. 기본 정책은 Sleeping3/Working을 D3/D0에 연결하고 명시적 `requested_device_power`를 소비합니다. 독립 자식 IRP에는 `origin: "framework_power_policy"`와 `response_index`가 있습니다. S3는 자식 완료를 기다리며 S0는 D0 발행 후 완료할 수 있습니다. 유휴/깨우기, 실패 장치 자동 재열거는 지원하지 않습니다.
