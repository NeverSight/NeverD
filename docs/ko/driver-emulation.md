...
`WdfDeviceInitSetFileObjectConfig`는 장치 생성 전에 `EvtDeviceFileCreate`, `EvtFileCleanup`, `EvtFileClose`를 등록하고 선택적 파일 개체 컨텍스트 속성을 복사합니다. 이 프로필은 `WdfFileObjectNotRequired`, `WdfFileObjectWdfCanUseFsContext`, `WdfFileObjectWdfCanUseFsContext2`, `WdfFileObjectWdfCannotUseFsContexts`를 지원합니다. 선택한 WDM 컨텍스트 슬롯은 실패한 CREATE 또는 CLOSE까지 WDF 핸들을 보관하며 처음에는 비어 있어야 합니다. 하위 대상으로 전달하거나 선택적 파일 객체를 사용하는 경우는 지원하지 않습니다. WDF 핸들과 WDM `FILE_OBJECT`는 별도 식별자이며, 유효할 때 `WdfRequestGetFileObject`, `WdfFileObjectGetDevice`, `WdfFileObjectWdmGetFileObject`로 조회할 수 있습니다. CREATE 실패 시 파일 CLEANUP/CLOSE 콜백 없이 WDF 개체를 삭제합니다. 성공한 파일은 CLEANUP, CLOSE 콜백 후 컨텍스트 정리와 소멸을 수행합니다.
