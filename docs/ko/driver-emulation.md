...
`WdfIoQueueFindRequest`는 수동 대기열을 검색하지만 요청 처리 소유권을 넘기지 않습니다. 성공하면 요청 참조가 하나 늘어나므로 드라이버는 `WdfObjectDereference`로 해제해야 합니다. `WdfIoQueueRetrieveFoundRequest`는 아직 대기열에 있는 요청의 소유권을 넘기며, 취소로 제거된 요청에는 `STATUS_NOT_FOUND`를 반환합니다. 선택적 매개변수는 `WdfRequestGetParameters`와 같은 레이아웃을 사용합니다. 프레임워크 파일 객체 필터링은 지원하지 않습니다.
...
`WdfDeviceInitSetFileObjectConfig`는 장치 생성 전에 `EvtDeviceFileCreate`, `EvtFileCleanup`, `EvtFileClose`를 등록하고 선택적 파일 개체 컨텍스트 속성을 복사합니다. 이 프로필은 `WdfFileObjectWdfCannotUseFsContexts`와 `WdfFileObjectNotRequired`를 지원합니다. 하위 대상으로 전달하거나 FsContext를 사용하는 클래스는 명시적으로 중단합니다. WDF 핸들과 WDM `FILE_OBJECT`는 별도 식별자이며, 유효할 때 `WdfRequestGetFileObject`, `WdfFileObjectGetDevice`, `WdfFileObjectWdmGetFileObject`로 조회할 수 있습니다. CREATE 실패 시 파일 CLEANUP/CLOSE 콜백 없이 WDF 개체를 삭제합니다. 성공한 파일은 CLEANUP, CLOSE 콜백 후 컨텍스트 정리와 소멸을 수행합니다.
