...
| 드라이버 할당 MDL | 독립/IRP 연결 설명자, 부분 MDL, 공유 사용자/시스템 매핑 및 완료 시 해제 | 할당량, 수동 MDL, 임의 프로세스 주소 공간과 VA 재사용은 미지원 |
...
이는 제한된 스케줄링 모델이며 완전한 Windows 비동기 지원은 아닙니다. 경고 가능/사용자 모드 대기, APC, 일반적인 WDM 요청 취소, 동시 공개 시나리오 제출, UMDF, 일반 KMDF PnP 장치 및 일반 큐 스케줄링, 전체 PnP/전원, 일반 하드웨어, 기타 DMA 인터페이스와 기타 인터럽트 모드는 지원하지 않습니다. 초기화 전용 호출도 명시적으로 대기열에 넣은 콜백을 실행하지만 요청이나 언로드를 암묵적으로 만들지 않습니다.
...
`MmMapLockedPagesSpecifyCache`는 활성 요청자 문맥에서 `APC_LEVEL` 이하일 때 `UserMode`와 `MmCached`를 지원하며, 제한적으로 지원하는 프로세스 연결 worker 문맥도 포함합니다. 각 호출은 `MappedSystemVa`를 바꾸지 않고 같은 물리 저장소에 프로세스 소유의 독립 뷰를 만듭니다. 뷰는 항상 실행 불가(NX)이며 `MdlMappingNoWrite`는 읽기 전용으로 만듭니다. 풀 저장소는 비페이지 메모리이고 완전한 전용 페이지를 차지해야 합니다. 사용자 뷰를 `MmProbeAndLockPages`로 다시 잠가도 원래 물리 식별자를 유지합니다. 요청한 주소는 MDL의 페이지 오프셋을 보존하고 사용자 뷰 전용의 제한된 VA 영역 안에 있어야 합니다. 영역이 소진되면 포착 가능한 `STATUS_INSUFFICIENT_RESOURCES`가 발생합니다.

`MmUnmapLockedPages`는 정확한 사용자 주소/MDL 쌍과 뷰를 만든 프로세스(PID)를 요구합니다. 시나리오의 원래 VA를 해제해도 독립 MDL 뷰와 시스템 별칭은 유지됩니다. 프로세스 종료는 해당 프로세스의 사용자 뷰만 취소하며 잠긴 물리 페이지와 시스템 별칭은 남습니다. 유효한 뷰는 설명자, 원래 잠금, 풀 및 요청 저장소의 해제를 막습니다. 취소된 사용자 주소는 재사용하지 않습니다. 임의 프로세스 공간, 다른 캐시 정책 및 사용자 VA 재매핑은 이 제한된 모델 밖입니다.
