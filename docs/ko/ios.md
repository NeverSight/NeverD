...
C 카탈로그는 `sys/mount.h` 선언과 아키텍처별 링크 심볼을 포함합니다. ARM64는 `getmntinfo`, x86-64는 `getmntinfo$INODE64`를 사용합니다. 출력 인자는 불투명 포인터에 대한 포인터로 유지하고 모드와 반환값은 부호 있는 32비트 정수로 유지하며 정확한 시스템 내보내기를 검증합니다. 파일 시스템 레코드 배치나 반환 버퍼 내용은 추측하지 않습니다.

외부 데이터 바인딩에는 플랫폼 공통 비 TLS SDK 선언과 정확한 라이브러리 내보내기 근거가 필요합니다. 생성된 C는 실제 심볼 저장소를 참조하고 전역 포인터와 대상 객체의 구별을 포함한 후속 메모리 접근을 보존합니다. 약한 가져오기, 식별 충돌 및 지원하지 않는 저장소는 바인딩하지 않습니다. 데이터 선언은 block 구성이나 소유권을 입증하지 않습니다. 카탈로그에는 CoreData, CoreImage, CoreGraphics, ImageIO, CoreSpotlight 데이터가 포함됩니다. 내장 리터럴 저장소는 모든 대상에서 빈 컬렉션과 불리언 객체를 컴파일해 얻으며, 외부 비 TLS 데이터의 직접 주소만 동일한 내보내기 검사를 거쳐 허용합니다. 카탈로그 생성에는 libclang 외에 `--clang`으로 지정한 Clang 컴파일러가 필요합니다.
...
ARM64 once 콜백은 외부 `_WZ` 심볼과 내부 `_Wz`/`_WZ` 쌍이 정확히 일치하고, 두 콜백 모두 일반 직접 호출자가 없으며, 독립적으로 타입이 검증된 말단 콜백이 컨텍스트를 무시할 때 다른 곳에서 쓰이지 않는 x2를 단일 중첩 `swift_once`에 전달할 수 있습니다. 발견과 투영은 같은 계약을 사용합니다. 투영은 null을 전달하고 모든 문장을 보존하며, void 콜백 ABI에 따라 부작용 없는 레지스터·임시 변수·상수 반환값만 제거합니다. 반환식의 스택 읽기, 로드, 호출 및 해결되지 않은 종속성은 계속 거부됩니다.
...
Arm64의 `+[NSSet setWithObjects:]`는 SDK의 nil 종료 가변 인자 선언을 유지합니다. 첫 지원 범위는 정확한 플랫폼 클래스 가져오기와 selector 스텁, 모든 유입 경로에서 완전한 8바이트 값이 일치하는 불변 Objective-C 문자열 인자입니다. 복원은 처음 확정된 nil에서 멈춥니다. 첫 객체가 nil이면 스택 후속 인자의 증명이 필요 없고 종료 값 뒤의 슬롯을 읽지 않습니다. 공통 Darwin 가변 인자 ABI는 고정 인자 3개를 유지하며 나머지 객체와 nil을 스택에 배치합니다. 게시 단계에서 현재 이미지의 선언, 제공 라이브러리, 수신자, selector 및 각 인자를 다시 검증합니다. 동적 객체, 누락되거나 부분적인 후속 저장, 이탈한 프레임, 로컬 재정의 및 다른 아키텍처는 지원하지 않습니다. 스택 인자는 선형의 비공개 프레임에서 각 실제 로드 시점에도 증명해야 합니다. 이후 덮어쓰기는 이미 읽은 값을 바꾸지 않으며, 겹치는 쓰기, 알 수 없는 호출, 이탈 또는 제어 흐름이 있으면 인증하지 않습니다.

컴파일러가 생성한 BOOL super getter는 현재의 완전한 ARM64 명령, CFG 및 저장/복원 증명이 Objective-C 진입점, 공유 본문과 metadata accessor(`CMa`)를 인증할 때만 진입점별로 소스에 투영할 수 있습니다. 탐색, 바인딩과 출력은 하나의 계약을 사용합니다. helper는 실제 CMa 호출과 닫힌 의존성을 보존한 뒤 런타임에 등록된 SEL 셀을 읽고 인증된 타입으로 `objc_msgSendSuper2`를 호출합니다. 서로 다른 selector는 별도 셀을 유지합니다. loader는 LLVM Mach-O 심볼과 export trie, 현재 segment/section 매핑의 정확한 일치를 통해 로컬 링크 속성을 다시 검증합니다. 다른 호출자나 공유 함수의 전역 native ABI는 변경하지 않으며, 증거가 없거나 오래되었으면 거부합니다.

arm64 Swift once 초기화자는 이미 타입이 확정된 native helper를 통해 별개의 정적 슬롯 사이에서 객체를 복사할 수 있습니다. 하나의 용도 계약이 현재의 모든 매개변수와 경로를 검사하여 predicate 읽기, 선택적 `swift_once`, 원본 읽기, 대상 쓰기, `objc_retain`, 반환 순서를 보존합니다. 네 가지 역할의 매개변수와 선택적으로 사용하지 않는 입력 하나를 허용하며, 매개변수 제거는 native 추론만 담당합니다. 탐색과 바인딩은 현재 image에서 동일한 계약, 정확한 콜백 및 저장소 심볼, 겹치지 않는 8바이트 범위, 콜백의 미사용 context를 다시 검증합니다. ABI, 제어 흐름, 메모리 효과 및 의존성 검사는 유지되며 별칭, 부분 또는 순서 지정 접근, 추가 용도, 오래된 증거는 거부합니다.

완전한 iOS 기기 및 시뮬레이터 SDK 선언은 `UIGraphicsBeginImageContextWithOptions(CGSize, BOOL, CGFloat)`가 void를 반환하는 고정 C 호출임을 확인합니다. ARM64에서 크기의 두 필드는 `d0`과 `d1`, 불리언은 `w0`에 담긴 한 바이트, 배율은 `d2`를 사용합니다. 정확한 UIKit 링커 내보내기로 제공 라이브러리를 인증합니다. 소스는 실제 호출과 래퍼의 카운터 갱신을 유지하며, 다른 제공자, 지원하지 않는 아키텍처, 변경된 선언은 거부합니다. 네이티브 래퍼에는 상태와 반환값에 대한 독립적인 증명이 계속 필요합니다.

동일하게 검증된 UIKit SDK 증거로 `CGSizeFromString(NSString *)`과 두 double로 구성된 `CGSize` 반환값을 바인딩합니다. 공통 ABI는 `d0`와 `d1`을 모두 보존하며, 다른 호출을 거친 뒤에도 사용되는 개별 반환값도 유지합니다. 현재 가져오기와 시그니처 재검증은 다른 제공 라이브러리, 스칼라 대체, 변경된 포인터 매개변수, 누락되거나 교환된 반환 전달 위치를 거부합니다.
...
불변 Swift 문자열의 once 콜백은 6개 명령의 호출자, 12개 명령의 공유 본문, 3개 명령의 주소 접근자가 하나의 정확한 경로를 완전히 입증할 때 소스로 투영할 수 있습니다. 두 번의 8바이트 쓰기 순서와 실제 `swift_bridgeObjectRetain` 호출을 유지합니다. 태그가 있는 문자열을 검증하고 리터럴 섹션 전체의 공유 저장소를 사용해 주소를 재구성하며 대상 심볼의 공유 저장소도 보존합니다. 현재 이미지 바이트, 완전한 파이프라인 검사, 프레임 보존, 강한 런타임 가져오기 및 최종 소스 본문을 다시 검증하며 공유 함수나 주소 접근자에 일반 ABI를 부여하지 않습니다.

ARM64 Objective-C 소스 복원은 호출자에서 증명한 최대 여덟 곳의 Swift 문자열 비교와 원래 한 비트 결과를 지원합니다. 0 확장 전에 현재 진입 ABI, 강한 가져오기, 정확한 호출 위치와 모든 사용을 검증합니다. 생성 C는 런타임을 `swiftcall`의 `_Bool`로 선언하고 결과를 명시적으로 `uint8_t`로 변환하며 기존 마스크와 주변 호출을 유지합니다. 공개 시 LowIR와 파이프라인 감사를 다시 확인하고 중복 평가나 다른 함수의 증거를 거부합니다. 일반 바이트 반환, 네이티브 호출 추측, patch 및 lift 모드는 이 정규화를 사용하지 않습니다. 각 호출 위치는 개별 증명을 통과해야 합니다. 다른 원시 불리언 호출은 입력 위치만 증명하며 결과 바이트가 정의되었다고 선언하지 않습니다. 공개 시 남아 있는 각 평가는 서로 다른 현재 기계 호출 위치에 대응해야 합니다.

수동 Swift String ABI 증거 수집 워크플로는 `String.hasPrefix`용 고정 `prefix` 프로브도 지원합니다. 두 ARM64 iOS SDK의 독립적인 Swift/C 컴파일러 출력을 보관하고 정확한 `swiftcc i1(i64, ptr, i64, ptr)` 호출을 요구합니다. 매니페스트에는 프로브, 심볼, 컴파일러 식별 정보와 소스 해시를 기록합니다. 증거 수집만으로 소스 바인딩을 추가하지 않습니다.

호출별 불리언 증명은 정확한 `String.hasPrefix` 가져오기도 지원합니다. Xcode 26.5 기기 및 시뮬레이터 프로브는 접두사 String의 두 값이 수신자 String의 두 값보다 앞서는 네 인수 `swiftcc i1` 호출을 확인합니다. `_Bool swiftcall` 선언을 다섯 인수 비교 도우미와 구분하고 게시 시 현재 가져오기 종류와 각 호출을 다시 검증합니다. 두 접두사 평가의 원래 분기 순서와 부작용을 유지합니다.
...
HighC는 부호 있는 덧셈과 뺄셈의 오버플로 플래그를 Clang 호환 결과 포인터 내장 함수와 원래 피연산자 폭의 임시 결과로 표현합니다. 중간 지역 변수가 부호 없는 형식이어도 입력 비트 패턴을 부호 있는 값으로 재해석합니다. 따라서 복구된 Swift once 초기화 함수의 생성 C는 경계 동작을 유지하고 Apple Clang에서 컴파일됩니다.

Swift 값 위트니스 호출 인식은 이제 ADRP 페이지와 오프셋을 포함한 인증된 불변 이미지 메타데이터를 정확한 위트니스 테이블 로드까지 추적합니다. 리터럴 메타데이터 바로 앞에는 매핑되고 파일에 기록된 8바이트 슬롯이 있어야 합니다. 숫자만 비슷한 값, 이미지 밖 주소, 잘린 슬롯은 허용하지 않습니다. 이는 간접 호출 ABI만 증명하며, 메서드 복구에는 재배치와 소스 의존성 검증이 여전히 필요합니다.
