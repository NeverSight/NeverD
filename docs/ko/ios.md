...
ARM64 Objective-C 소스 복원은 호출자에서 증명한 한 곳의 Swift 문자열 비교와 원래 한 비트 결과를 지원합니다. 0 확장 전에 현재 진입 ABI, 강한 가져오기, 정확한 호출 위치와 모든 사용을 검증합니다. 생성 C는 런타임을 `swiftcall`의 `_Bool`로 선언하고 결과를 명시적으로 `uint8_t`로 변환하며 기존 마스크와 주변 호출을 유지합니다. 공개 시 LowIR와 파이프라인 감사를 다시 확인하고 중복 평가나 다른 함수의 증거를 거부합니다. 일반 바이트 반환, 네이티브 호출 추측, patch 및 lift 모드는 이 정규화를 사용하지 않습니다.
