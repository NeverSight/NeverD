...
추가 테스트는 전체 CPU 컨텍스트(일반 레지스터, 플래그, SIMD, FPU, CR8) 복원, 공유 메모리 및 장애/다른 백엔드 컨텍스트 거부를 검증합니다. 작업 항목 픽스처는 보류 표시, 지연 완료, 큐와 수명 오류, 정체, 공유 예산, 공개 보고서의 디스패치/완료 상태 분리를 다룹니다. 내부 타이머/DPC 테스트는 상태 전환만 검증하며 게스트 API나 완전한 Windows 비동기 환경 지원을 뜻하지 않습니다.
