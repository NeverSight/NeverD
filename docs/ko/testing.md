...
`DriverGuardTests.cpp`와 네 가지 원본 `driver_guard.c` 변형은 활성/비활성 CFG, 재배치, check/dispatch ABI 및 잘못되거나 미선언된 대상을 검증합니다. `KernelFrameworkTests.cpp`는 바인딩과 객체 계약을 검증합니다. 원본 `driver_kmdf_lifecycle.c`는 실제 WDK 1.33 헤더로 선택적으로 컴파일하고 진짜 `FxDriverEntry` 라이브러리로 링크합니다. 별도로 빌드한 일반 및 활성 CFG 이미지를 `NEVERD_KMDF_FIXTURE`와 `NEVERD_KMDF_CFG_FIXTURE`에 지정합니다. 외부 산출물이 없으면 명시적으로 건너뜁니다. `DriverKMDFLifecycleTests.cpp`와 `DriverScenarioPublicTests.cpp`의 C API/CLI 사례는 바인딩, 형식화된 컨텍스트, 참조로 지연된 파괴, 실제 게스트 콜백, 언로드 및 문서화된 생성 실패를 확인합니다. 이 증거는 Linux 호스트에서 얻었으며 더 넓은 KMDF 지원을 입증하지 않습니다.
