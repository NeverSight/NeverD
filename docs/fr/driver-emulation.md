...
Un sous-ensemble KMDF PnP sans ressources prend en charge un FDO directement lié au PDO. `EvtDriverDeviceAdd` reçoit un initialiseur détenu par le framework; `WdfDeviceCreate` crée le FDO, tandis que `WdfFdoInitWdmGetPhysicalDevice` et `WdfDeviceWdmGetPhysicalDevice` conservent l’identité du PDO. Après un échec AddDevice ou un Remove terminé, le framework exécute les callbacks de nettoyage/destruction et supprime le périphérique. La file PnP doit définir explicitement `PowerManaged = WdfFalse`. `WdfDeviceInitSetPnpPowerEventCallbacks` prend en charge `EvtDevicePrepareHardware`, `EvtDeviceD0Entry`, `EvtDeviceD0Exit` et `EvtDeviceReleaseHardware`. PrepareHardware reçoit deux listes de ressources vides distinctes avant D0Entry ; `WdfCmResourceListGetCount` renvoie zéro et `WdfCmResourceListGetDescriptor` renvoie NULL. Un échec de PrepareHardware ou D0Entry devient le statut de START et déclenche tout de même ReleaseHardware. Lors de l’arrêt ou du retrait depuis D0, D0Exit précède ReleaseHardware et la fin de l’IRP. Les ressources non vides, les files gérées par l’alimentation et la politique générale restent exclues. Le fixture WDK `driver_kmdf_pnp.c` utilise `NEVERD_KMDF_PNP_FIXTURE` / `NEVERD_KMDF_PNP_CFG_FIXTURE`.
...
`wdm-x64-scheduled-v52`. `nt_status` reste le résultat de DriverEntry, tandis
