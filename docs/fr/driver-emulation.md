...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | MDL propres aux requêtes, mappages système, identités physiques partagées et SG DMA | Interfaces DMA hors du sous-ensemble décrit ci-dessous |
| MDL alloués par le pilote | Descripteurs autonomes ou associés à un IRP, MDL partiels, mappages utilisateur/système partagés et libération à la complétion | Quotas, MDL manuels, espaces de processus arbitraires |
...
| WDM `METHOD_NEITHER` | Tampons utilisateur distincts, sondes, MDL verrouillés, identités synthétiques, révocation de VA ou sortie après distribution et annulation limitée | Pas de tables de pages ni de gestionnaire général de processus ; vues MDL bornées par PID prises en charge |
...
| Pilote PnP de bus, de fonction ou de filtre | PDO explicites sans ressources ou à banque de registres fixe, AddDevice invité et huit fonctions mineures courantes du cycle PnP | Autres opérations PnP, politique générale d’alimentation, autres modèles matériels/de ressources et KMDF PnP général |
...
| Pilote matériel, DMA, PCI, d’interruption ou de virtualisation | Banque explicite de registres mémoire, MMIO, interruptions latched/de niveau exclusives ou partagées et DMA cohérent borné par tampons communs/SG/canaux pris en charge | Autres modèles de périphériques, RAM physique arbitraire, PCI, ports, autres interfaces DMA et état CPU privilégié |
...
Les politiques générales d’alimentation, les IRP alloués par le pilote, les autres fonctions mineures PnP et les autres modèles matériels/de ressources restent exclus. Le transfert vers une cible WDF hors du cycle de vie fichier FDO/PDO direct, l’attachement sur une pile ayant des fichiers ou callbacks actifs, le détachement d’une couche intermédiaire, le changement de fonction majeure et les cibles hors route échouent explicitement. Le fixture facultatif `driver_wdm_stack.c`, compilé avec le véritable WDK, utilise `NEVERD_WDM_STACK_FIXTURE` et `NEVERD_WDM_STACK_CFG_FIXTURE`. Les tests natifs et C API/CLI incluent la relocalisation ; les artefacts absents sont explicitement ignorés. Les preuves d’exécution restent limitées à Linux.
...
Le fournisseur `register_bank` accepte `interrupts` seul ou avec `resources` ; au moins une liste doit être non vide. `resource_free` refuse ce champ, même vide. Chaque ligne exige `id`, `raw_vector`, `raw_level`, `raw_affinity`, `translated_vector`, `translated_level`, `translated_affinity`, `mode` (`latched` ou `level_sensitive`) et `share` (`device_exclusive` ou `shared`). Une ligne de niveau exige `retrigger_after_100ns` positif, au plus INT64_MAX ; une ligne latched refuse ce champ. Limites : 8 lignes par PDO, 32 au total, ID ASCII bornés uniques par PDO, niveau brut 0–65535 et DIRQL traduit 3–12. Les vecteurs gardent leurs 32 bits ; les deux affinités valent 1, soit CPU0/groupe0. Un vecteur traduit partagé exige le même mode, niveau, affinité et intervalle, avec `share: "shared"` pour tous. Les descripteurs mémoire précèdent les interruptions ordonnées ; leurs drapeaux décrivent réellement le partage et le mode. Il s’agit de lignes synthétiques déclarées, pas d’un contrôleur PCI. `DriverInterrupts.h` / `DriverInterrupts.def` possèdent les noms et limites.

Les requêtes READ/WRITE/IOCTL peuvent déclarer `interrupt_events` avec `after_100ns`, `device_id` et `interrupt_id`. `action` vaut `pulse` par défaut pour les lignes latched ; les lignes de niveau exigent `assert` ou `deassert` et refusent `pulse`. Les lignes latched exigent `pulse`. Les autres types de requêtes refusent le champ, même vide. Limites : 64 événements par requête, 1024 déclarés au total, délai non négatif jusqu’à INT64_MAX. La soumission capture les connexions actives et la génération du PDO ; achever l’IRP ne supprime pas ses événements. Ils s’exécutent aux frontières de callback, sans préemption d’instruction ; le temps avance à l’inactivité. La publication matérielle précède l’admission, puis les ISR passent avant DPC/workers selon le DIRQL affecté. Connexion perdue, génération indisponible ou D3 produisent `undelivered_reason`, sans réaffectation implicite.

Une impulsion latched partagée appelle toutes les ISR capturées dans l’ordre d’enregistrement, même après TRUE. Une ligne de niveau est le OU des états par PDO/source/génération : une répétition de `assert` est idempotente. Tous les changements de même date sont appliqués avant l’échantillonnage. Si la ligne reste active, le prochain échantillon est prévu à la fin de la ronde ISR plus `retrigger_after_100ns`. Pour un niveau, TRUE arrête la recherche de cette ronde mais ne désactive aucune source ; seul `deassert` le fait. Aucune écriture de registre ne vaut acquittement implicite. Au plus 4096 rondes de livraison, avec 32 handlers par ronde, sont admises en plus des budgets de callbacks/instructions. Déconnexion et retrait refusent une source encore active.

`IoConnectInterrupt` utilise ses onze arguments et l’affectation traduite exacte. `IoConnectInterruptEx` accepte FullySpecified (1), LineBased (2, une ligne du PDO explicite) et FullySpecifiedGroup (4, groupe0) ; la déconnexion exige version/contexte correspondants. Connexion et déconnexion exigent PASSIVE_LEVEL. Le verrou peut être privé ou un `KSPIN_LOCK` non paginé initialisé par l’appelant, partagé entre connexions avec le même `SynchronizeIrql`, au moins égal à chaque DIRQL affecté. Zéro en LineBased choisit ce DIRQL ; la priorité de planification reste celle de l’affectation. CPU0/groupe0 et l’absence de sauvegarde flottante restent requis. Le `KINTERRUPT` opaque est passé à l’ISR réelle avec ServiceContext ; AL contient son BOOLEAN. `KeSynchronizeExecution`, l’ISR et l’acquisition/libération manuelle partagent le véritable verrou non récursif. Synchronize s’exécute à l’IRQL de synchronisation puis restaure IRQL/CR8 ; le retour doit respecter le propriétaire et l’IRQL sauvegardés. Les API de verrou exécutif ne peuvent accéder à un verrou connecté. préemption d’instruction restent exclus. START échoué et STOP/REMOVE réussi exigent la déconnexion avant la complétion finale ; elle ne supprime jamais silencieusement un DPC en attente.

`configuration.pnp_devices[].interrupts` conserve les ressources et `configuration.interrupt_events` les déclarations. Les observations `interrupts` gardent `source_request_index`, `event_index`, `device_id`, `interrupt_id`, `action`, `epoch`, `due_at_100ns`, `occurred_at_100ns`, `delivered_at_100ns`, `returned_at_100ns`, `interrupt_object`, `return_value`, `claimed` et `undelivered_reason`. `handlers[]` décrit chaque ISR réellement appelée : objet, dates, BOOLEAN, `claimed` et `delivery_index`. Une assertion peut produire plusieurs rondes ; une désassertion ou une assertion annulée à la même date ne fabrique aucune ISR. `occurred_at_100ns` indique l’application du changement d’état. `claimed` agrège les retours réels ; une interruption non revendiquée reste valide. `delivered_at_100ns` indique la première livraison ; `returned_at_100ns` et `claimed` décrivent la dernière ronde, tandis que `handlers[]` conserve tout l’historique. Le [scénario d’interruption](../examples/driver-interrupt-scenario.json) exécutable utilise le fixture original compilé avec le vrai WDK `driver_wdm_interrupts.c`, via `NEVERD_WDM_INTERRUPT_FIXTURE` / `NEVERD_WDM_INTERRUPT_CFG_FIXTURE` facultatifs : sept requêtes incluent START différé, IOCTL en attente terminé par ISR→DPC, nettoyage/fermeture de fichier et retrait. La frontière C/Python `scenario_json` et la structure `neverd_driver_options_v1` ne changent pas. Les artefacts absents sont explicitement ignorés ; les preuves d’exécution sont limitées à Linux.
...
KMDF 1.33 utilise exactement l’ABI 1.33.0 : 458 entrées de fonction possèdent une identité invitée stable, et les 99 API ci-dessous ont une sémantique d’exécution. `WdfVersionBind` et `WdfVersionUnbind` gèrent les liaisons invitées autour du véritable wrapper WDK `FxDriverEntry`. `WdfGetDriver` lit les variables globales publiques du pilote. Pilotes non-PnP, objets génériques, périphériques de contrôle, files et requêtes entrantes partagent contextes typés, compteurs de références et callbacks de nettoyage/destruction/déchargement exécutés. Hormis les DDI d’interruption et les opérations sur requêtes et objets dont les IRQL autorisés sont précisés ci-dessous, les appels et callbacks du framework exigent `PASSIVE_LEVEL` ; l’ajout de références après la fin du nettoyage reste hors de ce profil. Les entrées non modélisées, `WdfLdrQueryInterface`, les extensions de classe et UMDF arrêtent explicitement l’exécution.

API d’interruption: `WdfInterruptCreate`, `WdfInterruptQueueDpcForIsr`, `WdfInterruptQueueWorkItemForIsr`, `WdfInterruptSynchronize`, `WdfInterruptAcquireLock`, `WdfInterruptReleaseLock`, `WdfInterruptEnable`, `WdfInterruptDisable`, `WdfInterruptWdmGetInterrupt`, `WdfInterruptGetInfo`, `WdfInterruptGetDevice`.

Les objets d’interruption partagent les ressources de ligne ou MSI attribuées et les verrous internes. ISR et callbacks synchronisés utilisent le DIRQL attribué, ou `PASSIVE_LEVEL` en mode passif ; `WdfInterruptGetInfo` rapporte cet IRQL. Les DPC utilisent `DISPATCH_LEVEL` et les tâches `PASSIVE_LEVEL`. Les insertions répétées sont regroupées en file ; la suspension conserve la propriété jusqu’au retour. La mise sous tension connecte et active après D0Entry ; l’arrêt désactive, déconnecte et attend les callbacks différés avant D0Exit et la libération des ressources. L’activation/désactivation explicite exécute les vrais callbacks sans inventer d’impulsions. Les `WDFSPINLOCK`/`WDFWAITLOCK` externes, la sérialisation automatique du parent, les interruptions de réveil et les connexions inactive conservées restent exclus, comme la politique veille/réveil et la réénumération automatique des périphériques défaillants.

Les API suivantes sur les requêtes et objets acceptent un IRQL jusqu’à `DISPATCH_LEVEL` : `WdfObjectDereferenceActual`, `WdfRequestComplete`, `WdfRequestCompleteWithInformation`, `WdfRequestRetrieveInputBuffer`, `WdfRequestRetrieveOutputBuffer`, `WdfRequestRetrieveInputMemory`, `WdfRequestRetrieveOutputMemory`, `WdfRequestRetrieveInputWdmMdl`, `WdfRequestRetrieveOutputWdmMdl`, `WdfRequestGetInformation`, `WdfRequestSetInformation`, `WdfRequestGetIoQueue`, `WdfRequestGetFileObject`, `WdfRequestWdmGetIrp`, `WdfRequestGetParameters`, `WdfRequestGetStatus`, `WdfRequestMarkCancelableEx`, `WdfRequestUnmarkCancelable`, `WdfRequestIsCanceled`, `WdfMemoryGetBuffer`.

`WdfObjectGetTypedContextWorker`, `WdfObjectContextGetObject`, `WdfObjectReferenceActual` acceptent aussi les IRQL d’interruption modélisés ; l’accès au contexte et l’acquisition de références n’appellent aucun callback passif.

La terminaison d’une requête ou la libération de la dernière référence d’un objet au-dessus de `PASSIVE_LEVEL` programme le nettoyage, la destruction et les livraisons suivantes à `PASSIVE_LEVEL`. L’ancienne API `WdfRequestMarkCancelable` reste limitée à `PASSIVE_LEVEL` pour préserver l’exécution synchrone du callback lorsque la requête est déjà annulée. Les autres API conservent leurs restrictions IRQL existantes.
...
`WdfIoQueuePurge` annule aussi les requêtes non transmises avec `STATUS_CANCELLED` et demande l’annulation des requêtes déjà transmises et marquées annulables sur leur IRP d’origine. Le nettoyage précède la libération de l’IRP. Le callback d’état attend les requêtes en file, celles détenues par le pilote et les callbacks d’annulation actifs. Le pilote reste responsable des requêtes non annulables.

`WdfIoQueueStopSynchronously`, `WdfIoQueueDrainSynchronously` et `WdfIoQueuePurgeSynchronously` suspendent leur appelant invité à `PASSIVE_LEVEL` jusqu’à la fin des requêtes concernées. L’arrêt accepte encore les requêtes mais suspend leur distribution et attend celles déjà remises. Le drainage refuse les nouvelles requêtes, distribue celles en attente et attend leur fin. La purge annule les requêtes en attente et celles marquées annulables, puis attend le retour des callbacks d’annulation. Sans source d’achèvement, l’attente produit une erreur de modèle bloqué.

`WdfIoQueueStopAndPurge` et `WdfIoQueueStopAndPurgeSynchronously` annulent les requêtes déjà en file et celles que le pilote possède et a marquées annulables, puis acceptent de nouvelles requêtes sans les distribuer avant `WdfIoQueueStart`. Le callback d’état asynchrone et l’attente synchrone se terminent après les anciennes requêtes et leurs callbacks d’annulation ; les nouvelles requêtes restent en file sans retarder la notification. Les deux formes de Stop rétablissent aussi l’acceptation après un drainage ou une purge.
...
Pour les requêtes KMDF tamponnées, directes et neither, `WdfDeviceInitSetIoInCallerContextCallback` s’exécute avant la file dans le processus demandeur à `PASSIVE_LEVEL`. Le callback doit terminer la requête ou appeler une fois `WdfDeviceEnqueueRequest` pour la file par défaut. Pour un IOCTL `METHOD_NEITHER` ou une lecture/écriture neither, `WdfRequestRetrieveUnsafeUserInputBuffer` et `WdfRequestRetrieveUnsafeUserOutputBuffer` donnent les adresses utilisateur d’origine uniquement dans ce callback. `WdfRequestProbeAndLockUserBufferForRead` et `WdfRequestProbeAndLockUserBufferForWrite` vérifient les droits des pages et les verrouillent pour la requête ; `WdfMemoryGetBuffer` fournit un alias système utilisable dans le callback de file hors du contexte demandeur. L’achèvement libère les pages et alias. Les pointeurs intégrés vers des `user_buffers` explicites sont pris en charge ; les mappages arbitraires restent exclus.

Les requêtes neither peuvent déclarer `user_buffers` (`id`, `size`, `input`/`access` facultatifs) et `user_pointers`. Les références `source`/`target` indiquent `buffer` (`input`, `output`, `memory`) et `offset` ; seul `memory` utilise un `id` local à la requête. Les emplacements de pointeur de huit octets ne doivent pas se chevaucher ; la cible peut être dans le tampon ou à sa fin. Le scénario accepte 64 tampons supplémentaires et 256 pointeurs, avec 64 KiB par tampon dans le budget total de 512 KiB. Les cibles identiques partagent les octets réels et les règles existantes de droits, verrouillage, révocation et sortie du processus. `configuration.user_memory` conserve les déclarations. `user_buffers[].backing_hex` est un instantané RAM de diagnostic à l’arrêt, même après révocation ou faute ; il ne remplace ni `output_hex` ni les droits d’accès. `id`: 1–64 ASCII, `[A-Za-z0-9][A-Za-z0-9_.-]*`.

`WdfRequestRetrieveInputMemory` et `WdfRequestRetrieveOutputMemory` exposent des vues WDFMEMORY appartenant à la requête sur les tampons existants des E/S tamponnées ou directes. Une nouvelle récupération dans la même direction conserve la poignée ; `WdfMemoryGetBuffer` renvoie le tampon initial et sa longueur logique. Une longueur nulle ou une direction incorrecte échoue avec un statut WDF ; les E/S neither exigent toujours la vérification et le verrouillage dans le contexte de l’appelant. Ces vues empruntées ne créent pas de verrou MDL et expirent à la fin de la requête.

Un sous-ensemble KMDF PnP limité prend en charge un FDO directement lié au PDO. `EvtDriverDeviceAdd` reçoit un initialiseur détenu par le framework; `WdfDeviceCreate` crée le FDO, tandis que `WdfFdoInitWdmGetPhysicalDevice` et `WdfDeviceWdmGetPhysicalDevice` conservent l’identité du PDO. Après un échec AddDevice ou un Remove terminé, le framework exécute les callbacks de nettoyage/destruction et supprime le périphérique. Les files PnP acceptent `WdfUseDefault` et `WdfTrue` pour la gestion de l’alimentation ; `WdfFalse` reste sans gestion automatique. `WdfDeviceInitSetPnpPowerEventCallbacks` prend en charge `EvtDevicePrepareHardware`, `EvtDeviceD0Entry`, `EvtDeviceD0Exit`, `EvtDeviceReleaseHardware`, `EvtDeviceQueryStop`, `EvtDeviceQueryRemove` et `EvtDeviceSurpriseRemoval`. `EvtDeviceQueryStop` et `EvtDeviceQueryRemove` s’exécutent avant le bus et peuvent attendre ou refuser : le refus termine l’IRP avec leur statut sans observation du bus ni transition d’alimentation. `STATUS_PENDING` et `STATUS_NOT_SUPPORTED` sont des retours invalides. `EvtDeviceSurpriseRemoval` est une notification void avant transfert au bus ; son registre de retour est ignoré. PrepareHardware reçoit deux listes de ressources distinctes avant D0Entry, vides pour les périphériques sans ressources ; Pour les listes vides, `WdfCmResourceListGetCount` renvoie zéro et `WdfCmResourceListGetDescriptor` renvoie NULL. Un échec de PrepareHardware ou D0Entry devient le statut de START et déclenche tout de même ReleaseHardware. Lors de l’arrêt ou du retrait depuis D0, D0Exit précède ReleaseHardware et la fin de l’IRP. Les périphériques `register_bank` configurés exposent des entrées `CM_PARTIAL_RESOURCE_DESCRIPTOR` en lecture seule jusqu’à ReleaseHardware ; le pilote peut mapper la mémoire traduite avec `MmMapIoSpace` et doit la démapper avant la fin de STOP ou Remove. Les autres types de ressources et la politique générale restent exclus. `WdfIoQueuePnpHeld` signale la suspension des livraisons avant D0 et après sa sortie ; avant de quitter D0, `EvtIoStop` reçoit chaque requête détenue par le pilote dans une file gérée par l’alimentation ; le pilote peut la terminer, appeler `WdfRequestStopAcknowledge` ou laisser une source déjà programmée la terminer. Sans `EvtIoStop` enregistré, le framework attend aussi la fin de toutes les requêtes livrées. TRUE la remet en file pour livraison après redémarrage ; FALSE en conserve la propriété et appelle `EvtIoResume` après le retour en D0. Les requêtes déjà en file attendent le redémarrage. Si aucune source de terminaison ne reste disponible, l’attente se termine par un `model_error` de blocage. Le fixture WDK `driver_kmdf_pnp.c` utilise `NEVERD_KMDF_PNP_FIXTURE` / `NEVERD_KMDF_PNP_CFG_FIXTURE`.
...
Pour un FDO PnP, `WdfDeviceInitSetDeviceType` enregistre le type 32 bits fourni dans le `DEVICE_OBJECT` WDM ; sans appel, la valeur par défaut reste `FILE_DEVICE_UNKNOWN`. Les augmentations de priorité des E/S liées au type ne sont pas modélisées.
...
`WdfDeviceInitSetFileObjectConfig` enregistre `EvtDeviceFileCreate`, `EvtFileCleanup` et `EvtFileClose` avant la création du périphérique et copie les attributs de contexte facultatifs. Ce profil accepte `WdfFileObjectNotRequired`, `WdfFileObjectWdfCanUseFsContext`, `WdfFileObjectWdfCanUseFsContext2` et `WdfFileObjectWdfCannotUseFsContexts`. Le champ de contexte WDM choisi contient la poignée WDF jusqu’à l’échec de CREATE ou à CLOSE et doit être initialement vide. Avec `WdfFileObjectCanBeOptional`, `WdfRequestGetFileObject` renvoie NULL pour une requête d’E/S sans objet fichier WDM correspondant. CREATE, CLEANUP et CLOSE exigent toujours un objet fichier WDM ; `WdfFdoInitSetFilter` active le transfert par défaut des filtres ; `WdfTrue` l’active explicitement et `WdfFalse` le désactive. Le transfert d’une requête fichier exige une route FDO/PDO directe et un `bus_completion` explicite. Un `delay_100ns` positif est accepté pour le transfert automatique de CREATE/CLEANUP/CLOSE et pour CREATE synchrone, asynchrone avec routine de complétion ou avec `SEND_AND_FORGET`. Les rappels CLEANUP et CLOSE précèdent l’envoi inférieur. Un rappel CREATE configuré avec `WdfFileObjectNotRequired` peut obtenir la cible locale appartenant au périphérique via `WdfDeviceGetIoTarget` et transférer la requête d’origine avec `WdfRequestSend` en utilisant uniquement `WDF_REQUEST_SEND_OPTION_SEND_AND_FORGET`. Le PDO conservé consomme la réponse explicite du bus et assure la complétion finale, même lorsque la réponse inférieure est différée. Un rappel CREATE avec un objet fichier du framework peut utiliser `WDF_REQUEST_SEND_OPTION_SYNCHRONOUS`, lire le résultat inférieur avec `WdfRequestGetStatus`, puis achever la requête d’origine avec cet état. L’envoi asynchrone par défaut formate d’abord la requête avec `WdfRequestFormatRequestUsingCurrentType` et enregistre `WdfRequestSetCompletionRoutine`. Son rappel reçoit `WDF_REQUEST_COMPLETION_PARAMS`, peut lire le résultat inférieur avec `WdfRequestGetCompletionParams` ou `WdfRequestGetStatus`, puis achève la requête d’origine avec un état choisi indépendamment de celui du pilote inférieur. Pour un envoi doté d’une routine de complétion, la réponse différée du PDO laisse la requête en attente jusqu’à l’échéance virtuelle, puis place la routine en file avec l’état final inférieur. `WDF_REQUEST_SEND_OPTION_TIMEOUT` accepte un intervalle relatif négatif ou une échéance absolue positive sur l’horloge virtuelle de 100 ns, pour CREATE synchrone ou asynchrone avec routine de complétion. Une expiration antérieure produit `STATUS_IO_TIMEOUT` ; la réponse inférieure immédiate ou à échéance égale prévaut. Une échéance absolue passée n’a plus de délai restant. Zéro désactive le délai ; les autres options restent exclues. La poignée WDF et le `FILE_OBJECT` WDM gardent des identités distinctes, consultables avec `WdfRequestGetFileObject`, `WdfFileObjectGetDevice` et `WdfFileObjectWdmGetFileObject`. Un CREATE en échec supprime l’objet WDF sans rappels de nettoyage ou de fermeture ; après une ouverture réussie, CLEANUP et CLOSE précèdent le nettoyage et la destruction du contexte.

Un `WdfRequestSend` synchrone retardé conserve et suspend le véritable cadre d’appel invité, puis le reprend avec le résultat booléen d’envoi après la complétion. Le NTSTATUS final se lit avec `WdfRequestGetStatus`. `WDF_REQUEST_SEND_OPTION_TIMEOUT` accepte un intervalle relatif négatif ou une échéance absolue positive sur l’horloge virtuelle de 100 ns, pour CREATE synchrone ou asynchrone avec routine de complétion. Une expiration antérieure produit `STATUS_IO_TIMEOUT` ; la réponse inférieure immédiate ou à échéance égale prévaut. Une échéance absolue passée n’a plus de délai restant. Zéro désactive le délai ; les autres options restent exclues. Le transfert automatique exécute les rappels de fichier avant l’envoi inférieur et conserve l’IRP et le fichier WDM pendant les rappels de nettoyage et de destruction nécessaires à la complétion. Les références WDF externes préservent le contexte, mais ne prolongent pas la durée de vie du fichier WDM d’une requête terminée.

Les CREATE/CLEANUP/CLOSE WDM transmis à un PDO configuré acceptent aussi un `bus_completion` explicitement retardé. La véritable routine de complétion invitée s’exécute après l’échéance inférieure, en conservant la propagation de l’état en attente et la responsabilité du statut final.
...
| `ExRaiseStatus`, `ExRaiseAccessViolation`, `ExRaiseDatatypeMisalignment` | Exceptions invitées C `__except`, filtres réels et `__finally` de déroulement ; aucun retour API normal |
...
| `IoConnectInterrupt`, `IoDisconnectInterrupt`, `IoConnectInterruptEx`, `IoDisconnectInterruptEx` | Lignes latched/de niveau affectées, handlers et verrous exclusifs/partagés ; ABI héritée et Ex 1/2/4 à PASSIVE_LEVEL |
| `KeSynchronizeExecution`, `KeAcquireInterruptSpinLock`, `KeReleaseInterruptSpinLock` | Véritable callback BOOLEAN sous le verrou commun non récursif, à un IRQL de synchronisation au moins égal aux DIRQL ; restauration du contexte appelant |
...
les mappages utilisateur hors du modèle MDL par processus décrit ci-dessous et l’accès direct via le UserBuffer brut sont rejetés.
Un tampon direct de longueur nulle possède un MDL nul.

`IoAllocateMdl` alloue les métadonnées d’un tampon non vide, sans débordement et limité à 1 MiB, sans le sonder ni le verrouiller. `Irp` peut être NULL ou un IRP modélisé actif. Un descripteur principal remplace la tête de la chaîne du pilote ; les descripteurs détachés restent sa propriété. `SecondaryBuffer` ajoute en fin de chaîne ou devient la tête si elle est vide. Le MDL direct-I/O original appartenant à la requête doit rester accessible et ne peut être remplacé ni libéré par le pilote. `ChargeQuota` doit être FALSE ; l’épuisement de l’arène renvoie NULL. `MmBuildMdlForNonPagedPool` exige toute la plage dans une allocation active du pool non paginé. La fonction sûre et les macros WDM réutilisent l’adresse et les permissions originales, même avec de nouveaux drapeaux interdisant écriture/exécution. Les mappings système supplémentaires et leur suppression sont refusés. `IoFreeMdl` libère uniquement le descripteur du pilote indiqué, sans le détacher ni suivre `Next`. Un MDL utilisateur doit être déverrouillé avant sa libération manuelle. Le tampon du pool a une durée indépendante ; les deux ordres de libération sont permis sans accès ultérieur au stockage libéré.

Le pilote peut modifier `MDL.Next` et `IRP.MdlAddress` pour insérer ou détacher des descripteurs modélisés. La complétion finale de l’IRP valide d’abord toute la chaîne actuelle, déverrouille les MDL utilisateur attachés, révoque leurs alias et libère tous les descripteurs attachés. Les descripteurs détachés et le stockage du pool restent au pilote. Les cycles, liens inconnus ou libérés, descripteurs partagés entre IRP actifs et descripteurs WDF privés insérés dans une chaîne WDM échouent explicitement. Les dépendances DMA et dispatcher actives empêchent une libération prématurée. Les autres champs MDL modélisés et les PFN construits restent en lecture seule ; champs de processus, PFN non construits, MDL manuels restent exclus. Le déchargement exige la libération de tous les descripteurs du pilote restants.

`IoBuildPartialMdl` décrit une sous-plage non vide d’un MDL source construit ; une longueur nulle sélectionne le reste. La cible doit avoir une capacité PFN suffisante. Les descripteurs partiels partagent les pages physiques sans verrouillage supplémentaire. Ils héritent d’un mappage système actif de la source ou créent un mappage indépendant avec `MmGetSystemAddressForMdlSafe`. Libérer un MDL partiel ou le préparer à la réutilisation ne libère que le mappage qui lui appartient. La véritable fonction inline WDK `MmPrepareMdlForReuse` peut alors précéder un nouvel appel à `IoBuildPartialMdl`. Un descripteur source non paginé peut être libéré indépendamment ; un verrou utilisateur racine ou un alias système emprunté ne peut disparaître tant qu’un MDL partiel en dépend. La complétion vérifie toute la chaîne avant de libérer les dépendances, quel que soit l’ordre des descripteurs. Un DMA actif interdit la reconstruction et la libération prématurée.

`MmMapLockedPagesSpecifyCache` accepte `UserMode` dans un contexte de demandeur actif, y compris un worker attaché, jusqu’à `APC_LEVEL`. Chaque appel crée une vue propre au processus sur les mêmes pages sans modifier `MappedSystemVa`. Les vues sont toujours NX ; `MdlMappingNoWrite` interdit l’écriture. Le pool doit être non paginé et couvrir des pages privées complètes. Un nouveau verrouillage par `MmProbeAndLockPages` conserve l’identité physique. L’adresse demandée doit garder le décalage de page du MDL et rester dans l’arène bornée ; l’épuisement lève `STATUS_INSUFFICIENT_RESOURCES`, interceptable. `MmNonCached`, `MmCached` et `MmWriteCombined` héritent de l’attribut de cache existant des pages : la RAM ordinaire reste cached. Les valeurs réservées et les enregistrements physiques incompatibles échouent explicitement.

`MmUnmapLockedPages` exige la paire adresse utilisateur/MDL exacte et le PID créateur. Il retire réellement la vue et rend son adresse ainsi que son budget réutilisables, par adresse demandée ou allocation automatique. Retirer la VA originale du scénario préserve les vues MDL indépendantes. Les verrous physiques et alias système conservent leurs pages initiales, même si la VA désigne ensuite un autre stockage ou d’autres droits. Les vues actives empêchent la libération du descripteur, du verrou racine, du pool et du stockage de requête.

Les vues sont identifiées par `(PID, VA)` : plusieurs processus peuvent posséder simultanément la même adresse numérique avec des pages et permissions distinctes. Le changement de contexte remplace atomiquement l’ensemble des vues après vérification préalable ; revenir restaure le stockage et les droits d’origine. La sortie ou le retrait d’une vue d’un PID ne modifie pas celle d’un autre PID. Un objet dispatcher opaque actif dans une vue à retirer interdit le changement jusqu’à sa destruction. Ce modèle borné ne fournit ni allocation virtuelle arbitraire, ni tables de pages, ni gestionnaire général de processus.
...
`wdm-x64-scheduled-v75`. `nt_status` reste le résultat de DriverEntry, tandis
...
La livraison utilise les métadonnées unwind x64 V1 et `__C_specific_handler`. Une portée constante `EXCEPTION_EXECUTE_HANDLER` choisit directement son handler. Elle exécute les véritables filtres invités : zéro poursuit la recherche, une valeur positive choisit le handler, une valeur négative demande la reprise. Après sélection, les `__finally` des portées quittées s’exécutent dans l’ordre de déroulement avant le handler ; aucun nettoyage ne s’exécute pendant la recherche ou la reprise. Le chemin normal de `__finally` est exécuté par le code invité. Les frames ordinaires restaurent les registres généraux non volatils dans les limites de la pile d’origine. `GetExceptionCode()` garde le code levé ; un handler peut lever vers une portée englobante prise en charge.
...
`MmAllocatePagesForMdl`, `MmAllocatePagesForMdlEx` et `MmFreePagesFromMdl` gèrent des pages indépendantes avec contraintes physiques, allocations partielles et blocs contigus. Pages et descripteur sont libérés séparément. `MmProtectMdlSystemAddress` modifie les permissions PAGE d’une vue système privée sans changer PFN, cache ou permissions des vues utilisateur. `MmProbeAndLockPages` accepte aussi les pools KernelMode : verrouillage du pool paginé au plus à APC_LEVEL, accès ultérieur à DISPATCH_LEVEL, libération interdite tant que les pages sont verrouillées.

Les `messages` explicites définissent adresse, données, vecteur, niveau, affinité et polarité MSI. `message_id` est local au descripteur ; le troisième argument ISR est global au PDO. `CONNECT_MESSAGE_BASED` (3) retourne la table réelle. `CONNECT_MESSAGE_BASED_PASSIVE` (5) et les lignes passives permettent les attentes à PASSIVE_LEVEL, sérialisent une même interruption et préservent les cadres indépendants. Arrivée et livraison sont observées séparément, sans protocole PCI inventé.

`WdfDeviceInitSetPowerPolicyOwnership` définit le propriétaire de la politique. Les callbacks Init/Suspend/Restart/Flush/Cleanup et ceux entourant D0 sont exécutés ; D3/D0 conserve les ressources. La politique par défaut associe Sleeping3/Working à D3/D0 via `requested_device_power` explicite. Chaque IRP enfant porte `origin: "framework_power_policy"` et `response_index`. S3 attend son enfant ; S0 peut finir après émission de D0. Veille inactive/réveil, réénumération automatique restent exclus.
