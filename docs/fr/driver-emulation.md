...
| Pilote matériel, DMA, PCI, d’interruption ou de virtualisation | Banque explicite de registres mémoire, MMIO, interruptions latched exclusives et tampons communs cohérents/SG DMA pris en charge | Autres modèles de périphériques, RAM physique arbitraire, PCI, ports, autres modes d’interruption, autres interfaces DMA et état CPU privilégié |
...
Ce modèle borné n’offre pas tout l’asynchronisme Windows. Attentes alertables ou utilisateur, threads système, APC, annulation des requêtes WDM, spinlocks généraux, soumissions concurrentes de scénarios publics, changements généraux d’IRQL, `METHOD_NEITHER`, UMDF, périphériques PnP KMDF et ordonnancement général des files, PnP/alimentation complet, matériel général, autres interfaces DMA et autres modes d’interruption restent non pris en charge. L’initialisation seule exécute les callbacks explicitement mis en file sans créer de requêtes ni déchargement implicites.
...
`wdm-x64-scheduled-v14`. `nt_status` reste le résultat de DriverEntry, tandis
