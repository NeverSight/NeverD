...
| `METHOD_IN_DIRECT`, `METHOD_OUT_DIRECT` | MDL propres aux requêtes, mappages système, identités physiques partagées et SG DMA | Mappages utilisateur et interfaces DMA hors du sous-ensemble décrit ci-dessous |
| MDL alloués par le pilote | Descripteurs autonomes ou associés à un IRP, MDL partiels, mappages utilisateur/système partagés et libération à la complétion | Quotas, MDL manuels, espaces de processus arbitraires et réutilisation des VA |
...
`MmMapLockedPagesSpecifyCache` accepte `UserMode` et `MmCached` dans le contexte d’un demandeur actif, y compris le contexte limité d’un worker attaché, à un niveau inférieur ou égal à `APC_LEVEL`. Chaque appel crée une vue indépendante appartenant au processus sur le même stockage physique, sans modifier `MappedSystemVa`. Les vues sont toujours non exécutables (NX) ; `MdlMappingNoWrite` les rend accessibles uniquement en lecture. Le stockage du pool doit être non paginé et occuper des pages privées complètes. Reverrouiller une vue utilisateur avec `MmProbeAndLockPages` conserve l’identité physique d’origine. L’adresse demandée doit conserver le décalage de page du MDL et appartenir à l’arène VA bornée réservée aux vues utilisateur ; son épuisement lève un `STATUS_INSUFFICIENT_RESOURCES` interceptable.

`MmUnmapLockedPages` exige la paire exacte adresse utilisateur/MDL et le processus créateur (PID). Le démappage d’une VA d’origine du scénario préserve les vues MDL indépendantes et les alias système. La sortie d’un processus ne révoque que ses propres vues utilisateur ; les pages physiques verrouillées et les alias système subsistent. Les vues actives empêchent la libération du descripteur, du verrou racine et du stockage du pool ou de la requête. Les adresses utilisateur révoquées ne sont pas réutilisées. Les espaces de processus arbitraires, les autres politiques de cache et le remappage des VA utilisateur restent hors de ce modèle borné.
