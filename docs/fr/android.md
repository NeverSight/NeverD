...
Le marqueur Java 8 `@java.lang.Deprecated`, sans éléments et visible à l’exécution, est conservé sur les classes, champs, méthodes et constructeurs. Les annotations de paramètres, les initialiseurs statiques annotés, les autres niveaux de visibilité et les valeurs d’éléments tels que `since` ou `forRemoval` sont refusés. Après recompilation, la CI compare séparément l’attribut `Deprecated` du fichier de classe et les annotations visibles à l’exécution, y compris les déclarations témoins sans annotation et les méthodes auxiliaires générées ; ces dernières ne sont pas comptées parmi les méthodes d’origine.

DEX et smali conservent aussi l’annotation de plateforme `@android.annotation.SuppressLint` sur les classes, champs, méthodes et constructeurs. Elle exige la visibilité build et un unique tableau de chaînes `value` ; les tableaux et chaînes vides, répétitions, ordre et caractères échappés sont conservés. Les annotations de paramètres et d’initialiseurs statiques restent non prises en charge. La CI utilise le SDK Android pour vérifier les annotations conservées avec CLASS après recompilation et comparer le comportement du programme.

Le moteur intégré conserve aussi `@Retention`, `@Target`, `@Documented` et `@Inherited`, visibles à l’exécution, sur les déclarations d’annotation prises en charge, et produit un véritable `@interface`. Ce sous-ensemble ne permet aucun champ, méthode, paramètre de type ou déclaration imbriquée. Les annotations de premier niveau et les annotations membres statiques sont admises si leurs noms, leur accessibilité et leur relation avec le type englobant sont établis ; les portées locales ou anonymes sont refusées.

L’application d’un marqueur vide à une déclaration de classe, d’interface ou d’annotation exige une définition correspondante et accessible dans le même ensemble de classes analysées. Retention et Target doivent autoriser cette application. Une déclaration `SOURCE` peut être reconstruite, mais son application persistante dans l’entrée est refusée ; `CLASS` ou l’absence de Retention exige la visibilité DEX build (`0`), et `RUNTIME` exige la visibilité runtime (`1`). L’absence de Retention reste distincte d’un `CLASS` explicite, comme l’absence de Target reste distincte d’un tableau vide ; l’ordre du tableau Target est conservé. Les valeurs Target sont limitées à Java 8 ; les valeurs plus récentes, telles que `MODULE` et `RECORD_COMPONENT`, sont refusées. `@Inherited` est conservé sans copier les applications héritées comme annotations directes des sous-classes.

Les déclarations d’éléments d’annotation et leurs valeurs par défaut, les applications personnalisées non vides, les marqueurs personnalisés sur les champs/méthodes/paramètres, les définitions d’annotation externes, les conteneurs d’annotations répétables et `kotlin.Metadata` restent non pris en charge. La définition d’un marqueur est émise comme une déclaration de classe sans méthodes d’éléments ni méthodes auxiliaires, et n’augmente pas le nombre de méthodes récupérées.
