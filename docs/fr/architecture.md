...
`DriverPnp.h` et le fichier public `DeviceLifecycle.def` définissent les énumérations communes du cycle de vie et les données typées des scénarios et rapports. `KernelModelPnpDevices` gère l’identité stable des PDO, un inventaire distinct du pilote fournisseur et les observations réelles d’AddDevice. `KernelModelPnpRequests` relie les transactions du cycle de vie et l’identité immuable du périphérique à l’enregistrement IRP existant ; l’admission des opérations de fichier utilise cette identité. L’association FDO/PDO et l’identité du cycle de vie du fichier subsistent après détachement. `KernelModelPnpCompletion` gère la réception et la fin réelles sur le bus ainsi que les échéances virtuelles bornées. Il réutilise `KernelModelIRPStack` et les continuations identifiées par propriétaire, sans deuxième paquet ni callback invité artificiel. Seule la fin supérieure définitive valide l’état du cycle de vie. Une opération PnP réussie doit atteindre le fournisseur et y être terminée ; REMOVE et CANCEL_REMOVE exigent exactement `STATUS_SUCCESS` (0). START et QUERY_REMOVE peuvent échouer dans une couche supérieure, avec des observations du bus nulles. Le retrait du fournisseur et le détachement/la suppression par l’invité sont des transitions de propriété distinctes ; les périphériques invités non libérés ne sont jamais supprimés silencieusement. Le périmètre public se limite à Start/QueryRemove/CancelRemove/Remove sans ressources, sans PnP général, alimentation ni KMDF PnP.
