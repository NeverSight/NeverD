...
`DriverResources.h` et `DriverResources.def` définissent les affectations fixes `register_bank`, les droits des registres, largeurs, noms de champs et limites communes. `DriverScenario` possède la validation préalable JSON/native ; `DriverResult` enregistre la configuration sans dupliquer l’état observé de la banque. `KernelMMIO` possède les valeurs persistantes de chaque PDO, les intervalles physiques traduits, les générations de ressources, l’accessibilité matérielle et les alias de mappage indépendants. `KernelModelResources` gère les listes START brutes/traduites compactes en lecture seule et l’intégration fournisseur/cycle de vie. Le START inférieur réellement réussi publie une génération avant les callbacks supérieurs ; le SET de périphérique du fournisseur met à jour l’accès D 0/D 3. Les callbacks d’achèvement supérieurs peuvent d’abord appeler unmap. Le nettoyage après START échoué ou STOP/REMOVE est vérifié avant l’achèvement final de l’IRP, sans libération implicite. Surprise-removal refuse immédiatement l’accès matériel. `GuestMemory` et `UnicornBackend` valident les accès CPU/API originaux complets avant les callbacks de registres, avec un verrou commun du premier défaut/arrêt pour empêcher les effets après échec malgré le découpage Unicorn. Le redémarrage à affectation fixe conserve les valeurs ; les alias ne possèdent pas d’octets distincts. Ce bus mémoire borné ne fournit ni RAM arbitraire, rééquilibrage des ressources, ports, interruptions ni DMA.
