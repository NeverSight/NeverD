...
La qualification des résultats booléens Swift combine l’ABI Objective-C d’entrée actuel, les octets immuables des appels directs, l’import fort exact et la preuve complète des consommateurs LowIR. Elle accepte une seule comparaison et reconstruit les autres ABI d’appel depuis le catalogue actuel ; les suppositions natives, la répartition dynamique et les occurrences dupliquées sont rejetées. Cette preuve seule ne publie aucun code source et ne déclare pas un ABI de retour d’un octet.
