...
Les recettes de références de types Swift AArch64 acceptent aussi le descripteur nominal exact `_ContiguousArrayStorage` exporté par `libswiftCore`, attesté par les exports SDK conservés pour appareil et simulateur. Elles exigent un import fort sans addend dans un stockage immuable unique et les preuves existantes du cache, de la référence et du nom encodé. Le C généré conserve l’identité du descripteur, les références relatives et le cache partagé modifiable, sans copier les octets du descripteur. Les autres descripteurs de la bibliothèque standard restent non pris en charge.
