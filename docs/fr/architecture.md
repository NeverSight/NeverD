...
fichier et valide l’achèvement synchrone ou en attente par élément de travail ; la session ordonne les callbacks
...
ne reprennent pas une CPU en défaut et ne traitent pas ces défauts backend par les exceptions
Windows.

Le modèle Windows gère aussi les MDL autonomes de pool non paginé : libérer le descripteur ne libère pas le tampon sous-jacent. Les chaînes de MDL et l’association aux IRP restent non modélisées. Un modèle de registre distinct gère l’arbre explicite du scénario, les droits des handles et la durée de vie des clés et valeurs, indépendamment de l’inventaire des exports. La préparation et l’exécution partagent les mêmes règles de validation. Le rapport conserve les valeurs finales et le déchargement vérifie les handles encore ouverts.

`KernelScheduler` gère l’ordre des files prêtes, l’identité des callbacks et les échéances ; `KernelDispatcher` possède les objets opaques DPC, timer et événement et leurs signaux. `KernelModel` gère les attentes, les durées de vie travail/périphérique et l’achèvement IRP. `DriverSession` suspend et reprend des piles séparées et des contextes CPU complets, y compris les arguments Win64 sur pile, avec mémoire partagée. Le temps virtuel progresse aux échéances de timer, d’attente ou d’annulation uniquement en l’absence de contexte prêt ; sur CPU0, l’ordonnancement coopératif déterministe exécute les DPC à `DISPATCH_LEVEL` et le travail à `PASSIVE_LEVEL`. Threads/APC/spinlocks généraux, annulation WDM/PnP, soumissions concurrentes de scénarios publics, PnP/alimentation complet et matériel général restent hors périmètre. Les plafonds IRQL proviennent de `KernelAPIIRQL.def` ; le modèle propriétaire vérifie les restrictions dépendant des arguments.

`KernelModelDeviceStack` conserve dans un seul enregistrement le propriétaire pilote, l’allocation, les voisins, la suppression différée et les références internes de chaque périphérique. La liste invitée `NextDevice` et le graphe d’attachement détenu par l’hôte sont distincts. La résolution de nom garde le périphérique inférieur nommé pour `FILE_OBJECT` et les rapports, choisit le sommet courant pour le dispatch initial et les indicateurs READ/WRITE, puis conserve la route entière. Détachement/suppression n’invalident pas les périphériques encore détenus par une requête ou un callback ; `ReferenceCount` ne compte que les handles ouverts.

`KernelModelIRPStack` gère les curseurs bornés, le dispatch vers la cible exacte et le déroulement de fin dans le paquet original. Les écritures inline Copy/Skip/SetCompletion restent la référence. Statut de dispatch, contrôle de fin et `IoStatus` final sont distincts ; pending peut se propager après le retour du dispatch. `STATUS_MORE_PROCESSING_REQUIRED` conserve IRP/MDL/tampons jusqu’au déroulement final repris, y compris pour les fins imbriquées. `KernelGuestCall` porte un propriétaire de sous-système et un jeton local pour éviter les collisions WDM/WDF ; `DriverSession` conserve cadres CPU et IRQL hérité. Un pilote invité peut se rattacher au-dessus de PDO du scénario appartenant à un propriétaire distinct ; les IRP alloués directement par le pilote restent non pris en charge. Attachement/transfert WDF, extension de pile active, détachement intermédiaire, changement de fonction majeure et cibles hors route restent exclus. Chaque emplacement inférieur consommé est remis à zéro avant le callback de fin supérieur.

`DriverPnp.h` et le fichier public `DeviceLifecycle.def` définissent les énumérations du cycle de vie et les contrats imposant un statut de succès exact ; la validation préalable du scénario et la fin définitive côté invité partagent `devicePnpFinalStatusError`. `KernelModelPnpDevices` gère l’identité stable des PDO, l’inventaire distinct du pilote fournisseur et les observations réelles d’AddDevice. `KernelModelPnpRequests` relie les transactions du cycle de vie et l’identité immuable du périphérique/fichier à l’enregistrement IRP existant, sans inventer de rejet d’E/S à partir d’un état arrêté, en attente de suppression ou d’alimentation. Les IRP ordinaires atteignent le véritable dispatch invité ; le pilote décide quelles opérations réussissent, échouent ou attendent. `KernelModelPnpCompletion` gère la réception et la fin réelles sur le bus ainsi que les échéances virtuelles, en réutilisant `KernelModelIRPStack` et les continuations identifiées par propriétaire. La fin supérieure définitive valide l’état du cycle de vie ; un succès PnP exige un transfert achevé vers le fournisseur, tandis qu’un échec précoce de Start/QueryStop/QueryRemove peut laisser les observations du bus nulles. Stop/CancelStop/SurpriseRemoval/CancelRemove/Remove exigent exactement STATUS_SUCCESS. QueryStop avec STATUS_RESOURCE_REQUIREMENTS_CHANGED (0x119) est rejeté, car la nouvelle interrogation des ressources n’est pas modélisée. Le retrait du fournisseur et le détachement/la suppression par l’invité restent distincts ; les périphériques invités non libérés ne sont jamais supprimés silencieusement. Les huit fonctions mineures courantes utilisent des profils de bus explicites ; l’exécuteur public reste séquentiel et Remove exige des fichiers fermés ainsi que des requêtes antérieures terminées, tout en autorisant les callbacks qui libèrent les verrous de suppression. Les autres opérations PnP, les autres modèles matériels/de ressources et KMDF PnP ne sont pas fournis.

`DriverPower.def` déclare les noms des types et actions d’alimentation ainsi que les origines des requêtes, tandis que `DriverPnp.h` partage un même `DriverPowerOperation` entre les paquets de scénario et les FIFO de réponses propres à chaque PDO. `KernelModelPowerRequests` gère les données explicites des paquets, les routes capturées et l’état de notification de chaque DEVICE_OBJECT ; `PoSetPowerState` renvoie la précédente valeur explicite de notification de cet objet sans modifier la transaction du cycle de vie. `KernelModelPowerCompletion` gère les véritables requêtes enfants de `PoRequestPowerIrp`, chacune avec son propre IRP, sa ligne de résultat et son indice de réponse. Il ne consomme que la tête correspondante de la FIFO du PDO, ne déduit jamais un parent du contexte du callback et n’emprunte pas son résultat. Le dispatch imbriqué et les callbacks terminaux void à cinq arguments réutilisent les continuations identifiées par propriétaire, les routes conservées et des piles séparées ; l’instantané de statut reste valide pendant les attentes jusqu’au retour du callback. La fin synchrone d’un enfant peut précéder le retour STATUS_PENDING de l’API, et un parent système S0 peut finir avant son enfant périphérique D0. La fin supérieure définitive détermine l’état observé du cycle de vie ; les observations du bus restent indépendantes. Ce profil borné exige DO_POWER_PAGABLE sans DO_POWER_INRUSH et un dispatch à PASSIVE_LEVEL, prend en charge Query/Set pour D0/D3 et Working/Sleeping3 et conserve le SystemContext explicite de 32 bits comme donnée opaque. Il ne fournit ni politique générale d’alimentation, ni WAIT_WAKE, arrêt/hibernation, matériel général, KMDF PnP ou soumission simultanée de scénarios publics.

`KernelRemoveLocks` est l’unique autorité pour l’enregistrement des verrous de suppression, leur appartenance exacte à un DEVICE_OBJECT, leur taille, la multiplicité des Tags et le signal mémorisé de fin de vidage. Il est distinct des transactions `DeviceLifecycle` ; ni l’état du PDO ni un Tag ressemblant à un IRP ne désignent le propriétaire. `KernelModel` valide le stockage complet dans l’extension et les limites d’accès opaques, dirige les quatre exports Ex et enregistre des attentes RemoveLock typées et reprenables. La dernière libération rend l’attente prête avant le retour du callback, à l’aide des continuations CPU existantes sans callback artificiel. La vérification bornée du contexte AndWait exige une route REMOVE associée et la réception effective du fournisseur, sans imposer la fin de la couche inférieure ni un paquet Tag encore valide ; elle ne reproduit pas intégralement Driver Verifier. L’admission REMOVE conserve les exigences de fichiers fermés et de requêtes antérieures terminées, tout en autorisant les callbacks qui libèrent les verrous. La session conserve la route REMOVE pendant les cadres restants et valide le démontage final avant de libérer sa propriété. Les vérifications du stockage des acquisitions et attentes précèdent le passage en suppression différée ; le retrait physique de l’extension désenregistre le verrou. Le vidage ne consomme aucune référence de work item ou de route.

`DriverResources.h` / `DriverResources.def` et `DriverInterrupts.h` / `DriverInterrupts.def` définissent les affectations fixes de mémoire et d’interruptions de `register_bank`. `DriverScenario` possède la validation préalable JSON/native ; `DriverResult` enregistre la configuration initiale sans dupliquer l’état observé de la banque. Seul `KernelResources` possède les affectations brutes/traduites compactes, les générations de ressources, la présence physique et l’alimentation. `KernelMMIO` possède les valeurs persistantes des registres et les alias de mappage indépendants ; `KernelInterrupts` possède les connexions opaques, les verrous et les impulsions explicites. `KernelModelResources` crée les paquets START en lecture seule et intègre l’achèvement réel du fournisseur : le START inférieur réussi publie sa génération avant les callbacks supérieurs, et le SET de périphérique effectivement achevé par le fournisseur met à jour l’accès D0/D3. Le nettoyage après START échoué ou STOP/REMOVE est vérifié avant l’achèvement final de l’IRP, après que les callbacks supérieurs ont pu démapper et déconnecter, sans nettoyage implicite. Le retrait surprise refuse immédiatement l’accès matériel. `GuestMemory` et `UnicornBackend` valident les transactions CPU/API entières avant les effets MMIO et conservent le premier défaut. Le redémarrage avec affectation fixe conserve les valeurs. RAM arbitraire, rééquilibrage des ressources, ports, interruptions partagées/de niveau/par message et autres interfaces DMA restent non pris en charge.

`KernelInterrupts` lie chaque impulsion explicite au jeton de connexion et à la génération de ressources présents lors de la soumission réussie de la requête source. `KernelModelInterruptEvents` vérifie les capacités de tous les producteurs simultanés avant de faire avancer l’horloge ou de modifier les observations, y compris le nombre exact de callbacks d’annulation du framework ; timers, achèvements du fournisseur et annulations ne peuvent pas consommer silencieusement la place réservée à une ISR. La publication réelle de l’état matériel par le fournisseur précède l’éligibilité de l’impulsion ; les interruptions admises précèdent DPC et callbacks passifs. L’ordonnancement reste coopératif : le temps virtuel n’avance qu’à vide, et un délai nul n’implique pas de préemption d’instruction. `KernelModelInterrupts` décode l’ancienne ABI à onze arguments et les champs Ex sélectionnés ; `KernelGuestCall` donne aux callbacks d’interruption un propriétaire et un jeton distincts. ISR et callbacks de synchronisation détiennent le même verrou non récursif au DIRQL affecté ; les contextes CPU imbriqués préservent IRQL/CR8 de l’appelant, et BOOLEAN n’utilise que AL. Les verrous manuels exigent la même identité d’exécution et l’IRQL sauvegardé ; un callback ne peut pas retourner avec un verrou encore détenu. Les impulsions armées survivent à leur IRP source. Une connexion perdue, une génération indisponible ou D3 lors de la livraison produit un motif explicite de non-livraison et un arrêt, sans nouvelle liaison ni comportement inventé de registre enable/ack. `DriverResult.Interrupts` contient des observations indépendantes, jamais d’IRP synthétiques ni d’achèvements NTSTATUS.

`DriverDMA.h` / `DriverDMA.def` définissent les capacités explicites par PDO et les transactions externes indépendantes. `KernelPhysicalMemory` enregistre les allocations RAM actives exactes, attribue des identités de pages partagées et retient les plages d’octets ; les MDL sont des vues de cette autorité, pas des copies de tampons. `GuestMemory` / `UnicornBackend` offrent un accès à la plage sous-jacente entière en contournant les permissions CPU sans les changer. Ils refusent MMIO, accès pendant l’exécution, réentrance ou état défaillant et mémorisent les défaillances inattendues du backend. `KernelDMA` gère domaines logiques indépendants, identités de méthodes liées à l’adaptateur, mappages communs/SG, admission des registres de mappage et références de callbacks. `KernelDMAEvents` résout les époques PDO capturées lors de la livraison réelle. `KernelModelPhysicalMemory`, `KernelModelDMA` et `KernelModelDMATransfers` relient la propriété originale des allocations/MDL aux véritables callbacks invités indirects. Les ressources SG disponibles permettent une livraison immédiate ; les callbacks en attente réservent identité et capacité jusqu’à leur promotion FIFO. Les durées de vie du mappage et du callback sont distinctes : Put peut libérer les références aux données/descripteur avant le retour du callback, et retenir un callback ne maintient pas un IRP achevé en vie. `DmaWritable` enregistre l’intention de verrouillage indépendamment des permissions CPU. Au même instant, la publication du fournisseur précède les effets DMA sur la RAM, puis l’éligibilité des interruptions. Époques, présence et alimentation restent exclusivement dans `KernelResources` ; DMA ne déduit aucun registre constructeur, ne déclenche aucun IRQ, n’achève aucun IRP et ne possède pas de second cycle de vie. Les adresses logiques ne sont jamais recyclées et les échecs de validation d’une transaction conservent les observations sans changer la RAM. L’interface modélisée fournit des tampons communs cohérents, du SG DMA version 1 et des canaux DMA traduits de maître de bus sur une RAM modélisée bornée. Matériel général, contrôleurs subordonnés et autres interfaces DMA restent non pris en charge.

`KernelDMAChannels` étend le même allocateur de domaine avec des réservations de canal et une FIFO typée SG/canal. Il sépare l’état du callback, les registres de mappage conservés et le mappage global de chaque opération. Une opération de canal réserve une seule fois sa plage logique et agrandit sur place un seul maintien physique. Les appels MapTransfer entrelacés ne copient donc pas la RAM, ne facturent pas deux fois les registres et ne chevauchent aucun autre mappage. Des plans purs de transfert, retour, vidage et libération valident les identités et les lots complets de promotions avant publication. `KernelModelDMAChannels` décode l’ABI indirecte et la véritable capture de CurrentIrp à l’enregistrement ; il partage avec SG l’outil de vue MDL. Le type distinct `DMAAdapterControl` de l’ordonnanceur partage l’ordre DMA, la capacité et la préservation du parent des callbacks imbriqués. Seul le modèle DMA interprète les 32 bits bas de l’action de retour. Un IRP capturé en attente est protégé avant le déroulement terminal de sa pile. L’entrée du callback libère cette rétention d’entrée, permettant l’achèvement depuis son corps. Le vidage global retire les octets mappés ; FreeMapRegisters exact retire la réservation indépendante. Le contrat de cache cohérent de `KeFlushIoBuffers` ne satisfait aucune de ces deux obligations.

`KernelFramework` gère les liaisons KMDF 1.33, l’identité de la table, les objets et contextes WDF, les initialisateurs de périphériques de contrôle, les files par défaut séquentielles et les handles de requête. Ses interfaces typées pour les périphériques et les requêtes délèguent à `KernelModel` l’espace de noms WDM, le stockage, l’état des paquets, le mappage MDL et la validation de l’achèvement ; aucun des deux composants ne crée de périphériques ou d’IRP en double. Le routage des files distingue le statut de dispatch géré par le framework du retour du callback invité de type `void`. Les continuations d’achèvement exécutent le nettoyage et la destruction des objets enfants avant de libérer l’IRP ; les références externes ne conservent que le contexte WDF. La suppression des requêtes en attente est refusée avant toute modification des ancêtres ; l’annulation automatique et la vidange lors de la suppression restent exclues. `DriverSession` exécute les callbacks imbriqués avec des budgets partagés. `DriverImage` valide les métadonnées CFG ; `GuardControlFlow` gère les cibles image/API déclarées, et l’adaptateur CPU préserve l’état des appels check/dispatch. Les périphériques PnP, l’ordonnancement général des files et leur annulation, les extensions de classe et UMDF restent non pris en charge.

Le scénario définit une échéance virtuelle par transfert avec `cancel_after_100ns`. Le registre de chaque IRP dans `KernelModel` possède cette échéance et l’instant absolu effectif `cancel_requested_at_100ns` ; les scénarios publics restent sériels. `KernelModel` applique le délai nul après routage et avant le callback d’E/S invité, préserve un achèvement antérieur et inclut les délais positifs dans l’avancement à vide. `KernelFramework` gère marquage/retrait, mise en file/livraison et référence interne jusqu’au retour du callback d’annulation. La simple mise en file n’autorise pas l’achèvement ; après livraison, un élément de travail peut l’effectuer en coordination avec le callback qui attend. Conserver un objet WDF ne réactive pas son IRP terminé. `KernelScheduler` sépare annulations et travail, préserve leur type pendant suspension/reprise et partage les budgets de capacité et de dispatch. Les DPC précèdent les annulations FIFO, puis le travail ordinaire et la reprise des attentes passives prêtes. Les callbacks d’annulation s’exécutent à `PASSIVE_LEVEL`. Ce contrat limité aux périphériques de contrôle ne fournit ni routine d’annulation WDM ni ordonnanceur général de files.

L’ancien `WdfRequestMarkCancelable` utilise un `GuestCall` imbriqué dans la continuation de l’API pour un IRP déjà annulé. Annulation, nettoyage et destruction finale peuvent attendre ; l’appelant ne reprend qu’à la fin de la continuation. L’annulation après enregistrement utilise toujours le scheduler. `KernelFramework` possède l’identité des handles WDF et les résultats neutres définis pendant/après l’achèvement. Son hôte d’accesseurs délègue l’IRP original, Information sur 64 bits et l’identité MDL à `KernelModel`, qui refuse aussi l’achèvement WDM invité d’un IRP possédé par WDF. Un seul MDL SystemBuffer par requête est créé à la demande ; les tampons directs gardent leur descripteur sans mappage à la simple récupération. L’achèvement invalide les deux types avec l’IRP et les tampons, indépendamment des références au contexte WDF.

`KernelGuestException` est un résultat API typé contenant un statut de 32 bits, distinct des erreurs de modèle et des défauts backend. `DriverImage` conserve les métadonnées d’exception existantes du loader à leur base préférée. `KernelSEH` prépare sur ces données un plan pur et borné de transfert vers un catch-all C x64 version 1, avec traduction d’adresses et lectures de pile vérifiées. Il restaure les sauvegardes GPR non volatiles prises en charge à travers les frames auxiliaires ordinaires et sélectionne le véritable handler invité. Il refuse les filtres/finally, personnalités GS/C++, chaînes, enregistrements incomplets, prologues et restaurations XMM rencontrés. `DriverSession` applique le plan de registres validé uniquement à un arrêt API sain, garde les résultats API de trace à null et reprend le handler dans la même exécution. Il n’efface jamais le défaut backend conservé et ne déroule pas une autre pile de callback. Cette frontière prend en charge ExRaiseStatus/ExRaiseAccessViolation/ExRaiseDatatypeMisalignment ; les probes utilisateur, tampons utilisateur verrouillés et reprises après défaut CPU restent des travaux distincts.
...
HighIR peut réduire une variable locale source de 64 à 32 bits avec les mêmes garanties que pour un support de 128 bits : toutes les définitions doivent partager les mêmes largeurs de support et de préfixe, et chaque lecture doit sélectionner explicitement le préfixe bas. Les écritures de largeur complète, les échappements, les lectures des octets hauts ou les effets de bord de la partie haute empêchent cette réduction. Le remplissage des paramètres source reste inconnu.
Les copies exactes de variables entières peuvent partager cette preuve dans un graphe borné si une définition constructive établit la largeur du préfixe. Chaque destination doit elle-même pouvoir être réduite ; un consommateur de largeur complète invalide toutes les exceptions en amont. Les cycles sans largeur établie, les conflits de largeur et les budgets épuisés conservent les valeurs originales.
Les conversions entières, tranches à décalage nul et extensions bornées peuvent transmettre cette preuve si chaque largeur intermédiaire préserve le préfixe. Chaque usage est vérifié sous la racine de sa propre instruction. Deux passes bornées peuvent révéler un préfixe plus étroit après suppression du remplissage vectoriel.
...
LowIR porte l’identité exacte de chaque appel : adresse d’instruction, séquence d’opération, opcode et cible statique. Les preuves de l’état natif et du résultat source partagent cette identité tout en conservant des autorisations distinctes. Une preuve de résultat ARM64 sur un seul bit vérifie chaque chemin accessible avant de considérer la mise à zéro des bits 63:1 comme inobservable ; une altération de registre autorisée pour un appel ne prouve pas que l’appelé a écrit ces bits. Si une différence atteint un appel, chaque octet volatil des registres généraux, vectoriels et des indicateurs peut ensuite différer ; les octets préservés conservent leurs faits précédents. Les appels ordinaires exigent toujours des ABI complètes établies indépendamment. Le candidat de comparaison Swift authentifié séparément décrit le résultat brut sur un bit du compilateur et le fournisseur exact de l’import ; à lui seul, il ne publie aucune déclaration de retour sur un octet et n’autorise aucune projection source.

HighC émet les écritures mémoire ordinaires avec des fonctions de copie d’octets respectant la largeur exacte de la valeur. Une adresse machine ne prouve ni l’alignement ni le type effectif en C. Instructions d’écriture, expressions d’écriture et affectations en mémoire utilisent le même chemin, qui évalue chaque adresse et valeur une seule fois et renvoie la valeur écrite comme résultat d’expression.

La qualification booléenne Swift combine l’ABI Objective-C d’entrée actuel, les appels directs immuables, l’import fort exact et la preuve complète des consommateurs LowIR. Les autres appels exigent un ABI du catalogue actuel, une preuve complète d’accesseur de classe à huit instructions ou un super `init` importé fortement avec deux arguments pointeurs et un résultat pointeur. La publication vérifie toujours les dépendances natives, le récepteur super et son cadre. Les appels natifs ou dynamiques non prouvés et les occurrences dupliquées sont rejetés ; ces faits seuls ne publient aucun source et ne déclarent pas un ABI de retour d’un octet.

Une entrée native dotée d’un symbole de fonction à son adresse exacte dans l’image peut provisoirement considérer le mot x0 entier comme observable. Après la liaison de son ABI d’entrée complet par l’inférence de source, la publication relance la même preuve LowIR avec cet ABI ; cette hypothèse provisoire ne fournit à elle seule ni liaison source ni ABI de retour sur un octet. L’inférence des registres préservés ne peut utiliser un tel appel booléen qu’après avoir requalifié son occurrence LowIR exacte avec l’ABI d’entrée courante. Les preuves des octets d’entrée et de la restauration complète de l’état déterminent toujours si un registre préservé observé devient un paramètre. Un résultat natif lié sur deux mots ne peut étendre l’observation à x0/x1 que si les deux supports passent la preuve native de la paire ; la publication réinfère la paire complète avant d’accepter l’appel booléen.

Les imports libswiftCore exacts de la graine de Hasher, de String.hash(into:) et de Hasher.finalize peuvent emprunter une zone privée de 72 octets de la pile ARM64 via leur argument ABI Swift vérifié. La preuve d’état invalide chaque octet emprunté après l’appel et rejette le chevauchement avec les registres sauvegardés ou une fuite du cadre ; un nom correspondant sans preuve actuelle de l’import et de l’ABI ne suffit pas.
...
Swift 6.1.2 définit aussi les imports libswiftCore exacts `_DictionaryStorage.copy(original:)` et `resize(original:capacity:move:)` comme des appels renvoyant un pointeur avec les métadonnées concrètes du dictionnaire dans `swiftself`. Resize transporte également une capacité entière et un octet booléen. La preuve conserve les appels et leurs effets d’allocation, vérifie le fournisseur et l’ABI complet et ne publie pas les appelants qui ont d’autres dépendances non résolues.

L’import libswiftCore exact `KEY_TYPE_OF_DICTIONARY_VIOLATES_HASHABLE_REQUIREMENTS` reçoit un pointeur vers les métadonnées du type et ne retourne jamais, selon Swift 6.1.2. Seuls son fournisseur et son ABI authentifiés reçoivent ce contrat de terminaison ; l’appel et le piège d’origine restent dans le chemin source. Dans une fonction ARM64 qui retourne normalement, cet appel exact peut terminer une branche exceptionnelle sans successeur, même avec un piège immédiat. Chaque retour normal exige toujours une preuve complète de l’état.
...
La preuve du résultat booléen suit les bits de différence à travers les décalages entiers constants à gauche, à droite logique et à droite arithmétique dans la largeur de l’opérande, ainsi que les résultats bit à bit tronqués. Elle couvre les tests de bits ARM64 sans déclarer définis les bits de remplissage non observés. Un décalage variable ou SELECT n’est admis que si toutes ses entrées sont identiques dans les deux exécutions ; les décalages variables avec entrées différentes, les décalages constants hors plage et les bits de remplissage atteignant une branche, un argument, une écriture ou un retour interdisent toujours la normalisation.

Un retour entier natif déduit de 64 bits peut être limité aux 32 bits bas si tous les retours permettent cette projection et si au moins un contient explicitement un remplissage haut indéfini. Le flux source complet et toutes les définitions locales doivent concorder. Les octets bas inconnus, définitions cycliques, branches manquantes et expressions hautes avec effets restent rejetés. Le candidat est relifté avec la nouvelle ABI source. Les appelants qui lisent le mot haut supprimé conservent des valeurs non résolues et ne peuvent publier de source récupérée.

Les tableaux et dictionnaires Objective-C constants peuvent conserver comme éléments les singletons booléens CoreFoundation exactement importés. Chaque arête exige une liaison SDK forte sans décalage, dans un stockage fichier unique et immuable, sans relocalisations superposées. Les helpers renvoient l’adresse de l’objet importé et préservent l’identité des éléments répétés sans copier leur représentation. L’adresse du slot ne désigne pas l’objet chargé ; les booléens ne deviennent pas des clés texte. La publication revérifie tout le graphe.

Les recettes de références de types Swift AArch64 acceptent aussi le descripteur nominal exact `_ContiguousArrayStorage` exporté par `libswiftCore`, attesté par les exports SDK conservés pour appareil et simulateur. Elles exigent un import fort sans addend dans un stockage immuable unique et les preuves existantes du cache, de la référence et du nom encodé. Le C généré conserve l’identité du descripteur, les références relatives et le cache partagé modifiable, sans copier les octets du descripteur. Le descripteur exact `_DictionaryStorage` est également accepté uniquement si l’image liée prouve une liaison forte, sans addend, à `libswiftCore`. Les autres descripteurs de la bibliothèque standard restent non pris en charge.

Les adresses exactes des pointeurs globaux immuables autoréférents partagent le même stockage reconstruit, de taille pointeur, que les lectures de leur valeur. Le pointeur initial désigne ce stockage lui-même et conserve l’identité de clé opaque et le contenu du pointeur. La publication des adresses directes revérifie l’unicité du stockage, l’autorelocalisation locale et l’immutabilité ; les stockages modifiables, superposés, tronqués ou conflictuels restent non résolus.

La récupération de source peut projeter une fonction feuille locale AArch64 bornée ne contenant que des copies de registres pleine largeur et `RET x30`. Le chargeur authentifie les octets immuables du Mach-O lié, la liaison locale et l’occurrence BL originale exacte ; les opérandes des registres de plateforme, de cadre, de liaison et zéro, les effets mémoire et les autres instructions sont rejetés. Les copies séquentielles sont normalisées vers les valeurs d’entrée de la feuille ; chaque consommateur lit toutes les sources avant d’écrire les destinations. La conversion MedIR, les faits de récepteur et de cadre Objective-C et la restauration de l’état natif partagent ce transfert tout en conservant l’écriture réelle du registre de liaison par BL. Le LowIR original et les comportements génériques de lifting et de patch restent inchangés. MedIR et HighIR conservent des preuves concordantes, revalidées avec les octets actuels et les frontières d’instructions de l’appelant avant publication. Toute preuve absente, dupliquée, contradictoire ou périmée reste non résolue ; les auxiliaires extraits ayant des effets mémoire exigent une preuve distincte. Une feuille qui écrit dans des registres que l’appelé doit préserver ne peut pas non plus être déclarée comme fonction C ordinaire.

Cette projection de feuille réservée au source admet aussi `ADRP` suivi d’un `ADD` 64 bits sans décalage pour les adresses complètes d’objets chaîne constants. Les valeurs de registre distinguent explicitement entrées et adresses d’objets. Les pages restent internes ; une page encore présente au retour, un débordement ou un objet non vérifié rejette toute la projection. Le calcul utilise le PC de l’instruction appelée. `readObjCConstantString` authentifie chaque objet et son contenu, conservés dans la preuve pour comparaison ultérieure. MedIR marque l’objet complet `DataAddress`, propriétaire de lui-même ; la publication exige toujours la liaison source normale. Les écritures constantes invalident les faits de récepteur, de registre d’entrée et d’octet de cadre qui se chevauchent, sans modifier les autres registres ni la mémoire. Ces mêmes effets régissent l’inférence des entrées natives et le rejet des sorties privées.

Une preuve distincte d’appel ordinaire couvre le getter de classe local `ADRP x8; LDR x0,[x8,#imm]; RET x30`. La preuve commune du chargeur authentifie le BL initial, la feuille complète, l’emplacement immuable d’import de classe et l’appartenance exacte classe/bibliothèque du SDK. Le transfert des faits Objective-C utilise le comportement sans entrée prouvé pour préserver le cadre privé et retrouver le récepteur de classe, sans effacer un échappement antérieur. Le CALL reste présent et exige toujours une signature native liée indépendamment et des dépendances source complètes. MedIR et HighIR conservent des preuves identiques ; la publication revérifie les octets actuels, l’identité d’import et l’unique appel ordinaire. Seule la vérification dédiée autorise les métadonnées de classe correspondantes ; les règles de lecture ordinaires restent inchangées.

Une feuille projetée peut aussi exécuter exactement un `STR Xn,[SP,#0]` contenant une adresse de chaîne constante fraîchement authentifiée. La preuve conserve l’instruction originale et la valeur au moment du stockage indépendamment des registres finaux. Propagation et préservation exigent un SP courant connu, aligné sur 16 octets, et huit octets entièrement dans le cadre privé alloué ; les faits écrasés et la zone des arguments sortants sont invalidés. Un cadre échappé ne redevient pas privé. Toute fonction publiée avec cet effet, y compris Objective-C ou native déjà typée, doit satisfaire la preuve complète du cadre avec des ABI d’entrée Med/High explicites et concordantes. Aucun repli sans cadre ni ABI autonome ordinaire du helper n’est autorisé.

Le même stockage SP unique peut recevoir une valeur normalisée vers un registre à l’entrée de la feuille, sauvegardée avant les écritures finales. Tout octet dérivé du cadre est refusé et tous les faits écrasés sont remplacés ; une écriture ne définit pas des bits inconnus. Seul un argument sortant déclaré consommant huit octets d’entrée complets et contigus prouve un usage du registre ; une sauvegarde inutilisée ne crée aucun paramètre. Objective-C conserve seulement les faits scalaires, récepteurs ou paramètres prouvés et refuse les identités de blocs copiés connues. Les contrôles complets de définition, d’ABI, de cadre et de fermeture des dépendances restent obligatoires.

Un appel natif direct opaque ne peut participer à la normalisation booléenne que si tous les registres physiques et indicateurs ont les mêmes bits dans les deux exécutions à cet endroit. La preuve rejette toujours les observations mémoire différentes, conserve les différences des temporaires et répète le contrôle sur les retours de boucle. Elle ne fournit ni ABI, ni définition du résultat, ni liaison source pour la fonction appelée. La publication exige toujours des liaisons indépendantes pour l’appel original et ses dépendances. Les trampolines d’import reconnaissables sans ABI courante et les liaisons existantes invalides restent rejetés.

Les getters paresseux d’objets Swift acceptent aussi un `swift_retain` authentifié séparément, suivi de `objc_autoreleaseReturnValue`. Les deux appels et leur chaîne réelle de résultats sont conservés. Une étiquette vide facultative avant l’initialisation ne doit contenir aucune expression, instruction imbriquée ou effet mémoire. La suppression du contexte once accessoire exige toujours une preuve indépendante que l’initialiseur ne l’utilise pas, vérifiée de nouveau à la publication.

Le candidat d’égalité Swift `NSObject` utilise l’ABI `swiftcc i1(ptr, ptr, ptr swiftself)`, vérifiée indépendamment sur appareil et simulateur : objets en x0/x1, métadonnées en x20. Il exige l’import fort exact de `libswiftObjectiveC`, un stockage immuable et la preuve complète existante de normalisation chez l’appelant. HighC dérive le prototype `_Bool` et le paramètre `swift_context` du même contrat canonique d’entrée ; la recherche seule ne publie jamais une ABI de retour sur un octet.

Un getter d’objet Objective-C fixe sans argument ne peut participer à cette preuve de normalisation que comme appel opaque à état identique. Les 20 octets immuables du selector stub courant, la référence du selector, l’import fort `objc_msgSend` et l’ABI pointeur exacte du SDK doivent tous concorder ; un échec de preuve `__objc_stubs` ne peut pas revenir à un appel natif inconnu. Aucun fait de clobber, résultat ou liaison source n’est accordé : tous les registres physiques, indicateurs et observations mémoire doivent déjà être identiques au point d’appel.
