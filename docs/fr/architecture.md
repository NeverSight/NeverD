...
`KernelFramework` possède la liaison KMDF 1.33, l’identité de la table, les pilotes/objets génériques et la durée de vie des contextes typés. `DriverSession` exécute les callbacks invités imbriqués puis reprend l’opération du framework sous les budgets partagés. `DriverImage` valide les métadonnées CFG ; `GuardControlFlow` possède les cibles image/API déclarées, et l’adaptateur CPU préserve l’état check/dispatch. Cela n’ajoute ni périphériques, files, requêtes ou extensions de classe KMDF, ni UMDF.
