...
L’export source peut projeter un getter super BOOL généré par le compilateur pour chaque entrée Objective-C si les preuves ARM64 actuelles et complètes des instructions, du CFG et des sauvegardes/restaurations certifient l’entrée, le corps partagé et l’accesseur de métadonnées (`CMa`). Découverte, liaison et rendu utilisent un même contrat. Le helper conserve l’appel CMa réel et ses dépendances closes, puis lit la cellule SEL enregistrée à l’exécution et appelle `objc_msgSendSuper2` avec le type certifié ; chaque sélecteur garde sa propre cellule. Le chargeur revérifie la liaison locale à partir des symboles Mach-O et de l’export trie LLVM, avec une correspondance exacte des segments/sections actuels. Les autres appelants et l’ABI native globale de la fonction partagée restent inchangés ; toute preuve absente ou périmée est refusée.

Un initialiseur Swift once arm64 peut copier un objet entre des emplacements statiques distincts via un helper natif déjà typé. Un contrat unique vérifie tous les paramètres actuels et tous les chemins, en conservant dans cet ordre la lecture du prédicat, le `swift_once` éventuel, la lecture source, l’écriture destination, `objc_retain` et le retour. Il accepte quatre paramètres de rôle, avec éventuellement une entrée inutilisée ; seule l’inférence native supprime des paramètres. La découverte et la liaison revalident ce contrat, les symboles exacts du callback et du stockage, les étendues disjointes de huit octets et le contexte inutilisé du callback dans l’image actuelle. ABI, contrôle, effets mémoire et vérifications des dépendances restent intacts ; alias, accès partiels ou ordonnés, usages supplémentaires et preuves périmées sont rejetés.
