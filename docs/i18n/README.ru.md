...
[Документация](../README.ru.md) · [Дорожная карта](../roadmap/README.ru.md) · [Участие](#участие)
...
NeverD — движок анализа и декомпиляции нативных бинарников вокруг **1:1-поднятия на уровне инструкций**. Он загружает **PE**, **ELF** и **Mach-O**, декодирует через [Capstone](https://www.capstone-engine.org/) и поднимает четырёхступенчатым IR-конвейером с **рукописной семантикой** — не приближённый перевод. Цель — **100% семантическая верность**: поддерживаемые инструкции сохраняют полное наблюдаемое поведение в **LLVM IR**, **структурированном C** или **перезаписанном бинарнике**.
...
> Интеграционное покрытие Mach-O i386 использует релокируемые `thin`-объекты и тесты бэкенда перезаписи исполняемых файлов; текущий хост macOS не может линковать устаревшие исполняемые файлы i386.
...
Стиль следует соглашениям LLVM (`.clang-format`).

Разработка ведётся в ветке **`dev`** (ветка по умолчанию на GitHub).
