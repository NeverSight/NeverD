...
| Hardware, DMA, PCI, interrupt or virtualization driver | Explicit register banks, MMIO and exclusive latched interrupt callbacks | Other device models, physical RAM, PCI, ports, shared/level/MSI interrupts, DMA and privileged CPU state |
...
UMDF, KMDF PnP devices and general queue scheduling, full PnP/power, general hardware, DMA and other interrupt modes remain unsupported.
...
The profile is `wdm-x64-scheduled-v13`. `nt_status` remains the DriverEntry
