...
| KMDF 1.33 non-PnP driver | Binding, objects/contexts, named control devices, sequential or parallel default queues with finite or unlimited presentation, nondefault manual queues and buffered/direct/neither requests with executed callbacks | No PnP devices, automatic nondefault queues, class extensions or UMDF |
...
The 96-byte `WDF_IO_QUEUE_CONFIG` supports sequential and parallel default queues with finite or unlimited presentation plus nondefault manual queues, all with explicit passive execution and no framework synchronization. A parallel queue uses `Settings.Parallel.NumberOfPresentedRequests = -1` for unlimited delivery or a positive finite limit. Additional requests wait in FIFO order until a presented request completes or is canceled; a queued cancellation retires the original IRP without calling the driver. Manual default queues and automatic nondefault queues remain unsupported. Control-device queues are not power managed. Specific READ/WRITE/IOCTL callbacks take precedence over the default callback. Accepted requests return `STATUS_PENDING` even when completed synchronously; a void callback's return register does not complete its request. `WdfRequestForwardToIoQueue` transfers an owned request to a manual queue on the same device, freeing its sequential source to deliver another request. The framework owns the request while queued; `WdfIoQueueRetrieveNextRequest` transfers it back to the driver in FIFO order and returns `STATUS_NO_MORE_ENTRIES` with a null output when empty. `WdfRequestRequeue` returns a retrieved, unmarked request to the head of the same manual queue. If cancellation wins before retrieval, including before forwarding, the framework removes and completes the queued request with `STATUS_CANCELLED`, running its cleanup callbacks before retiring the IRP; a later retrieval observes an empty queue. Deferred completion uses the existing scheduler, including explicit `defer_callback_drain` batches on asynchronous files or separate files after forwarding. Without a handler the request completes with `STATUS_INVALID_DEVICE_REQUEST`; zero-length READ/WRITE completes without delivery unless enabled. The default file package completes CREATE/CLEANUP/CLOSE successfully with Information=0. File callbacks, PnP devices and full PnP/power remain unsupported.
...
The profile is `wdm-x64-scheduled-v37`. `nt_status` remains the DriverEntry
