...
| KMDF 1.33 PnP FDO | `EvtDriverDeviceAdd`, FDO/PDO identity, default or explicit power-managed I/O queues, common PnP minors, hardware/D0 and request stop/resume callbacks, and framework cleanup | Unmodeled resource types, other power callbacks and general KMDF PnP |
...
`WdfIoQueueFindRequest` scans a manual queue without transferring request ownership and adds one request reference on success. The driver releases that reference with `WdfObjectDereference`; `WdfIoQueueRetrieveFoundRequest` transfers ownership of a still-queued request, while a request removed by cancellation returns `STATUS_NOT_FOUND`. Optional request parameters use the same layout as `WdfRequestGetParameters`. Framework file-object filtering remains unsupported.
...
`WdfDeviceInitSetFileObjectConfig` registers `EvtDeviceFileCreate`, `EvtFileCleanup` and `EvtFileClose` and copies optional file-object context attributes before device creation. This profile supports `WdfFileObjectWdfCannotUseFsContexts` and `WdfFileObjectNotRequired`; forwarding to a lower target and FsContext-backed classes stop explicitly. A configured file object keeps the WDM `FILE_OBJECT` and WDF handle distinct. `WdfRequestGetFileObject`, `WdfFileObjectGetDevice` and `WdfFileObjectWdmGetFileObject` expose that identity while it is live. Failed CREATE deletes the WDF file object without file cleanup or close callbacks; successful CLEANUP and CLOSE run their callbacks before context cleanup and destruction.
...
The profile is `wdm-x64-scheduled-v59`. `nt_status` remains the DriverEntry
