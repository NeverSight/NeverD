...
| KMDF 1.33 resource-free PnP FDO | `EvtDriverDeviceAdd`, FDO/PDO identity, explicit non-power-managed I/O queues, common PnP minors and framework cleanup on failed AddDevice or Remove | Resource-bearing FDOs, queue power transitions, device power callbacks and general KMDF PnP |
...
The profile is `wdm-x64-scheduled-v49`. `nt_status` remains the DriverEntry
