...
| Hardware, DMA, PCI, interrupt or virtualization driver | Explicit register banks, MMIO, exclusive/shared latched/level interrupts and bounded coherent common/SG/channel DMA | Other device models, host physical RAM, PCI, ports, MSI/passive interrupts, other DMA interfaces and privileged CPU state |
...
level 0–65535 and translated DIRQL 3–12. Both affinity masks must be 1, making
...
APIs or outlive their storage. Interrupt waits, MSI, passive interrupts and
instruction-level preemption remain unsupported. Failed START and successful
...
Nested or collided exceptions raised from a filter/finally, GS/C++
personalities, chained or incomplete metadata, prologue unwinding and XMM
unwind operations fail explicitly. An uncaught API or supported user-memory
