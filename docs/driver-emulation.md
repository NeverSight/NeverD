...
For buffered and direct KMDF transfers, `WdfDeviceInitSetIoInCallerContextCallback` registers a prequeue callback before device creation. READ, WRITE and IOCTL reach that callback in the requestor process at `PASSIVE_LEVEL`; it must complete the request or call `WdfDeviceEnqueueRequest` once. An enqueued request then reaches the sequential default queue. This bounded path does not provide KMDF `METHOD_NEITHER` unsafe-buffer retrieval, probing or locking.
...
The profile is `wdm-x64-scheduled-v32`. `nt_status` remains the DriverEntry
