...
| Hardware, DMA, PCI, interrupt or virtualization driver | Explicit memory register banks and bounded MMIO mapping | Other device models, physical RAM, PCI, ports, interrupts, DMA and privileged CPU state |
...
UMDF, KMDF PnP devices and general queue scheduling, full PnP/power, general hardware, DMA and interrupts remain unsupported.
...
The profile is `wdm-x64-scheduled-v12`. `nt_status` remains the DriverEntry
