...
Swift の真偽値結果の検証は、現在の Objective-C エントリ ABI、不変な直接呼び出し命令、正確な強いランタイムインポート、完全な LowIR 利用側証明を統合します。比較呼び出しは一か所に限定し、ほかの呼び出し ABI は現在のランタイムカタログから再構築します。ネイティブ ABI の推測、動的ディスパッチ、重複する呼び出しは拒否します。この証拠だけではソース公開やバイト戻り値 ABI の宣言を認めません。
