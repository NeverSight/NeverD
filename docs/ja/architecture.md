...
正確な libswiftCore の `KEY_TYPE_OF_DICTIONARY_VIOLATES_HASHABLE_REQUIREMENTS` インポートは、Swift 6.1.2 の宣言どおり型メタデータポインタを一つ受け取り、戻りません。この終了契約は認証済みの提供元と ABI にのみ適用され、元の呼び出しとトラップはソース経路に残ります。
