...
`KernelScheduler` は実行待ちキューの順序、コールバック識別、タイマー期限を管理し、`KernelDispatcher` は不透明な DPC・タイマー・イベントとシグナルを管理します。`KernelModel` は待機登録、ワーク項目／デバイスの寿命、IRP 完了を管理します。`DriverSession` は Win64 スタック引数を含む独立スタックと完全な CPU コンテキストを中断・復元し、ゲストメモリを共有します。仮想時間はタイマー／待機境界で進み、CPU0 の決定的な協調スケジューリングで DPC は `DISPATCH_LEVEL`、ワーク項目は `PASSIVE_LEVEL` となります。一般のスレッド／APC／キャンセル／スピンロック、並行 IRP、完全な PnP／電源、ハードウェアは含みません。 API の IRQL 上限は `KernelAPIIRQL.def` にあり、引数依存の制約は担当モデルが検査します。

`KernelFramework` が KMDF 1.33 バインド、関数表の識別、ドライバー／汎用オブジェクト、型付きコンテキストの寿命を管理します。`DriverSession` は共通実行予算内で入れ子のゲストコールバックを実行し、中断したフレームワーク操作を再開します。`DriverImage` が CFG メタデータを検証し、`GuardControlFlow` が宣言済みイメージ／API ターゲットを管理し、CPU アダプターが check/dispatch の呼び出し状態を維持します。KMDF デバイス、キュー、要求、クラス拡張、UMDF は含みません。
