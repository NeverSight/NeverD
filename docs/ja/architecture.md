...
`DriverResources.h` と `DriverResources.def` は固定 `register_bank` 割り当て、レジスター権限、幅、フィールド名、共通上限を定義します。`DriverScenario` が JSON／ネイティブ事前検証、`DriverResult` が実際のバンク状態を複製しない設定記録を担当します。`KernelMMIO` は PDO ごとの永続値、変換後物理区間、リソース世代、ハードウェア可用性、独立マッピング別名を所有します。`KernelModelResources` は packed 配置の読み取り専用 raw／変換後 START リストとプロバイダー／ライフサイクル連携を担当します。実際の下位 START 成功は上位コールバック前に世代を公開し、プロバイダーのデバイス SET は D0／D3 可用性を更新します。上位完了コールバックは先に unmap できます。START 失敗や STOP／REMOVE の解放確認は最終 IRP 完了前に行い、暗黙には解放しません。突然の取り外しは直ちにハードウェアアクセスを禁止します。`GuestMemory` と `UnicornBackend` は元の CPU／API アクセス全体をレジスターコールバック前に検証し、最初の障害／停止ラッチを共有して Unicorn の分割後の効果を防ぎます。固定割り当ての再開は値を保持し、別名に独立したバイト列はありません。この限定メモリレジスターバスは任意 RAM、リソース再配分、ポート、割り込み、DMA を実装しません。
