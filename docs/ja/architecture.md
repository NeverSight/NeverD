...
`KernelScheduler` は決定的なワーク項目の順序とコールバック識別を、`KernelModel` は項目・デバイスの寿命と IRP の保留・完了契約を管理します。`DriverSession` はゲスト呼び出しが戻る境界で `PASSIVE_LEVEL` のキューを処理してから逐次リクエストを進めます。Unicorn は完全な CPU コンテキストを保存し、ゲストメモリは共有します。内部のタイマー／DPC 状態機械は、公開ゲスト API、スレッド／待機、キャンセル、KMDF、PnP／電源、ハードウェアの対応を意味しません。
