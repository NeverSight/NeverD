...
追加テストは完全な CPU コンテキスト（汎用レジスタ、フラグ、SIMD、FPU、CR8）の復元、共有メモリ、障害後や別バックエンドのコンテキストの拒否を検証します。ワーク項目フィクスチャは保留マーク、遅延完了、キューと寿命の誤り、停滞、共通予算、公開レポートのディスパッチ・完了状態の分離を扱います。内部タイマー／DPC テストは状態遷移のみで、ゲスト API や完全な Windows 非同期環境への対応を示しません。
