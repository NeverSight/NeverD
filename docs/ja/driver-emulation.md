...
リソースを使用しない KMDF PnP の直接 FDO/PDO 構成を限定的にサポートします。`EvtDriverDeviceAdd` はフレームワーク所有の初期化子を受け取り、`WdfDeviceCreate` は FDO を作成します。`WdfFdoInitWdmGetPhysicalDevice` と `WdfDeviceWdmGetPhysicalDevice` は PDO 識別子を返します。AddDevice の失敗と Remove の完了後にはクリーンアップ／破棄コールバックを実行してデバイスを削除します。PnP キューは `PowerManaged = WdfFalse` を明示的に指定する必要があります。`WdfDeviceInitSetPnpPowerEventCallbacks` では `EvtDevicePrepareHardware`、`EvtDeviceD0Entry`、`EvtDeviceD0Exit`、`EvtDeviceReleaseHardware` をサポートします。PrepareHardware は D0Entry より先に、別々の空のリソース一覧を受け取ります。`WdfCmResourceListGetCount` は 0、`WdfCmResourceListGetDescriptor` は NULL を返します。PrepareHardware または D0Entry が失敗すると START が失敗し、その場合も ReleaseHardware を呼び出します。D0 からの停止・削除では、D0Exit の後、IRP 完了前に ReleaseHardware を実行します。空でないリソース、電源管理キュー、一般的な電源ポリシーは未対応です。実 WDK フィクスチャ `driver_kmdf_pnp.c` には `NEVERD_KMDF_PNP_FIXTURE`／`NEVERD_KMDF_PNP_CFG_FIXTURE` を使用します。
