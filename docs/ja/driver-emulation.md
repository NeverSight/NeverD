...
| ドライバー割り当て MDL | 独立／IRP 関連の記述子、部分 MDL、共有ユーザー／システムマッピング、完了時解放 | クォータ、手動構築 MDL、任意のプロセスアドレス空間、VA 再利用は未対応 |
...
`MmMapLockedPagesSpecifyCache` は、有効な要求元のコンテキスト（対応範囲内のプロセスにアタッチしたワーカーを含む）で、`APC_LEVEL` 以下の `UserMode` と `MmCached` を受け付けます。各呼び出しは、`MappedSystemVa` を変更せず、同じ物理領域にプロセス所有の独立したビューを作成します。ビューは常に実行不可（NX）で、`MdlMappingNoWrite` は読み取り専用にします。プール領域は非ページ領域で、完全な専用ページを占有する必要があります。ユーザービューを `MmProbeAndLockPages` で再ロックしても、元の物理ページ識別子を保持します。指定アドレスは MDL のページ内オフセットを維持し、ユーザービュー専用の有限 VA 領域内になければなりません。領域の枯渇は捕捉可能な `STATUS_INSUFFICIENT_RESOURCES` を発生させます。

`MmUnmapLockedPages` には、正確なユーザーアドレスと MDL の組、およびビューを作成したプロセス（PID）が必要です。シナリオの元の VA をアンマップしても、独立した MDL ビューとシステムエイリアスは維持されます。プロセス終了はそのプロセスのユーザービューだけを無効にし、ロック済み物理ページとシステムエイリアスは存続します。有効なビューがある間、記述子、元のロック、プール領域、要求ストレージは解放できません。無効化したユーザーアドレスは再利用しません。任意のプロセス空間、他のキャッシュポリシー、ユーザー VA の再マッピングは、この限定モデルの対象外です。
