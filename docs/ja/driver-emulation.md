...
|----------------------------|----------------|--------------|
| 下記 API を使用する x64 ソフトウェア WDM ドライバー | 初期化、逐次ファイルライフサイクル、有界のワーク項目コールバック | 追加で実行される API ごとに明確なモデルが必要 |
| `METHOD_BUFFERED` IOCTL | 独立したファイル識別子、交互の逐次リクエストとワーク項目による完了 | その他の非同期完了元は未対応 |
...
| READ/WRITE | デバイスフラグに応じたバッファードまたはダイレクト転送。ワーク項目による完了に対応 | Neither I/O、暗黙のファイル位置、その他の非同期完了元 |
...
| ワーク項目を使うドライバー | `PASSIVE_LEVEL` で決定的に実行する `DelayedWorkQueue` コールバック | ワーカースレッド、タイマー、DPC、APC、待機、キャンセルは未対応 |
...
キューに登録したワーク項目は、DriverEntry、リクエストのディスパッチ、別のワーク項目コールバックなどのドライバー呼び出しが戻った時点で決定的に実行します。リクエストは逐次処理され、保留中のリクエストが完了するまで次を開始しません。ディスパッチは IRP を保留としてマークし、`STATUS_PENDING` を返す必要があります。その後、キュー内のワーク項目が `PASSIVE_LEVEL` で完了できます。完了を発生させる実行可能な処理がなく保留のままなら、停滞した `model_error` として停止します。コールバックにも共通の命令、メモリ、イベント、時間予算を適用します。

完全な Windows カーネル、KMDF、PnP／電源ライフサイクル、neither IOCTL、割り込み、一般の待機、スレッド、キャンセル、ゲストのタイマー／DPC／APC API は引き続き未実装です。内部スケジューラーの状態機械テストは、これらの公開機能への対応を意味しません。初期化のみの呼び出しでも DriverEntry が明示的に登録したワーク項目を実行しますが、シナリオのリクエストやアンロードを暗黙には生成しません。
