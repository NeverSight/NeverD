...
| ハードウェア、DMA、PCI、割り込み、仮想化を扱うドライバー | 明示的なレジスターバンク、MMIO、排他的／共有 latched と level_sensitive 割り込み、有界コヒーレント共通バッファー／SG／チャネル DMA | その他のデバイスモデル、任意物理 RAM、PCI、ポート、MSI／パッシブレベル割り込み、その他の DMA インターフェース、特権 CPU 状態 |
...
これは限定的なスケジューリングモデルであり、完全な Windows 非同期対応ではありません。アラート可能／ユーザーモード待機、APC、一般的な WDM 要求キャンセル、公開シナリオの並行送信、UMDF、一般的な KMDF PnP デバイスと一般のキュースケジューリング、完全な PnP／電源、一般のハードウェア、その他の DMA インターフェース、MSI／パッシブレベル割り込みは未対応です。初期化のみの呼び出しも明示的に登録したコールバックを実行しますが、要求やアンロードを暗黙には生成しません。
