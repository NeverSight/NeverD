...
リソースを使用しない KMDF PnP の直接 FDO/PDO 構成を限定的にサポートします。`EvtDriverDeviceAdd` はフレームワーク所有の初期化子を受け取り、`WdfDeviceCreate` は FDO を作成します。`WdfFdoInitWdmGetPhysicalDevice` と `WdfDeviceWdmGetPhysicalDevice` は PDO 識別子を返します。AddDevice の失敗と Remove の完了後にはクリーンアップ／破棄コールバックを実行してデバイスを削除します。PnP キューは `PowerManaged = WdfFalse` を明示的に指定する必要があります。`WdfDeviceInitSetPnpPowerEventCallbacks` では `EvtDeviceD0Entry` と `EvtDeviceD0Exit` をサポートします。D0Entry はバスの START 成功後、IRP 完了前に実行され、失敗時は START の状態になります。D0Exit は D0 からの停止または削除時に実行されます。リソース一覧コールバック、電源管理キュー、一般的な電源管理は未対応です。実 WDK フィクスチャ `driver_kmdf_pnp.c` には `NEVERD_KMDF_PNP_FIXTURE`／`NEVERD_KMDF_PNP_CFG_FIXTURE` を使用します。
