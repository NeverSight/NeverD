...
| `METHOD_IN_DIRECT`、`METHOD_OUT_DIRECT` | 要求所有 MDL、システムマッピング、共有物理ページ識別子、SG DMA | 以下の対応範囲外のユーザーマッピングと DMA インターフェース |
| ドライバー割り当て MDL | 独立／IRP 関連の記述子、部分 MDL、共有ユーザー／システムマッピング、完了時解放 | クォータ、手動構築 MDL、一般的な仮想メモリー割り当て API |
| READ/WRITE | 逐次 buffered/direct/neither I/O、ワーク項目または DPC による完了 | 以下の API 部分集合のみ。公開シナリオの並行送信、一般的な WDM 要求キャンセル、暗黙のファイル位置は未対応 |
| WDM `METHOD_NEITHER` | 独立ユーザーバッファー、プローブ、MDL ロック、合成要求元 ID、ディスパッチ後の VA 取り消しまたはプロセス終了、限定的なキャンセル | 未記載の一般的なプロセス API と仮想メモリー割り当て API |
...
| ハードウェア、DMA、PCI、割り込み、仮想化を扱うドライバー | 明示的なレジスターバンク、MMIO、排他的／共有 latched と level_sensitive 割り込み、有界コヒーレント共通バッファー／SG／チャネル DMA | その他のデバイスモデル、任意物理 RAM、PCI、ポート、その他の DMA インターフェース、特権 CPU 状態 |
...
これは限定的なスケジューリングモデルであり、完全な Windows 非同期対応ではありません。アラート可能／ユーザーモード待機、APC、一般的な WDM 要求キャンセル、公開シナリオの並行送信、UMDF、一般的な KMDF PnP デバイスと一般のキュースケジューリング、完全な PnP／電源、一般のハードウェア、その他の DMA インターフェースは未対応です。初期化のみの呼び出しも明示的に登録したコールバックを実行しますが、要求やアンロードを暗黙には生成しません。
...
一般的な電源管理、ドライバー割り当て IRP、その他の PnP マイナー機能、その他のハードウェア／リソースモデルは未対応です。直接 FDO/PDO のファイルライフサイクル以外の WDF ターゲット転送、ファイルやコールバックが残るスタックへの接続、中間層の切断、転送時のメジャー機能変更、保持経路外への転送は明示的に失敗します。実 WDK の任意フィクスチャ `driver_wdm_stack.c` は `NEVERD_WDM_STACK_FIXTURE` と `NEVERD_WDM_STACK_CFG_FIXTURE` で指定します。ネイティブと C API／CLI の検証には再配置も含み、成果物がなければ明示的にスキップします。実行証拠は Linux のみです。
...
同じ `register_bank` プロバイダーは `interrupts` を単独で、または `resources` とともに宣言でき、少なくとも一方は空でない必要があります。`resource_free` は `[]` を含む明示的な `interrupts` を拒否します。各割り込みは `id`、`raw_vector`、`raw_level`、`raw_affinity`、`translated_vector`、`translated_level`、`translated_affinity`、`mode`、`share` を持ちます。型、綴り、上限は `DriverInterrupts.h`／`DriverInterrupts.def` に集約しています。PDO ごとに 8 件、合計 32 件、PDO 内で一意の長さ制限付き ASCII ID、raw level 0–65535、translated DIRQL 3–12 に対応します。ベクトルは完全な 32 ビットを保持し、IRQL との関係を推測しません。両 affinity は 1 とし、CPU0／group0 のみを扱います。`mode` は `latched` または `level_sensitive`、`share` は `device_exclusive` または `shared` です。同じ変換済みベクトルを共有するリソースはすべて shared とし、モード、変換済みレベル、affinity、再トリガー間隔を一致させます。レベル方式は `INT64_MAX` 以下の正の `retrigger_after_100ns` を明示します。raw と translated は独立です。`CM_RESOURCE_LIST` はメモリー記述子の順序を維持してから型 2 の割り込み記述子を配置し、共有属性とトリガーフラグは宣言を反映します。

READ／WRITE／IOCTL は `interrupt_events` を宣言でき、各要素に `after_100ns`、`device_id`、`interrupt_id`、任意の `action` を指定します。他の要求種別では空の一覧も拒否します。遅延は `INT64_MAX` 以下の非負値で、送信成功を起点として、その時点の接続と PDO リソース世代を捕捉します。latched の既定は `action: "pulse"` で、捕捉した全 ISR を呼びます。level_sensitive は明示的な `assert`／`deassert` のみを受け付けます。PDO／リソース／世代ごとのソース状態を OR してラインレベルを決定し、繰り返した assert は冪等です。同じ境界の外部状態変更をすべて適用してからサンプリングします。各 ISR バッチの後、現在時刻に `retrigger_after_100ns` を加えた時刻に再サンプリングし、登録順に TRUE が返るまで呼びます。TRUE は受理を示すだけでソースを解除しません。発生元 IRP の完了はイベントを取り消さず、レジスター書き込みから enable、status、acknowledgment を推測しません。時間はアイドル時に進み、対応するコールバック境界で ISR が DPC／ワーカーより先に実行されます。`after_100ns: 0` は命令単位のプリエンプションを保証しません。宣言は要求ごとに 64 件、合計 1024 件まで、実際の配送バッチは別途 4096 回、各バッチは 32 ハンドラーまでで、スケジューラーと命令の予算も適用します。接続喪失、無効なリソース世代、物理 D3 は `undelivered_reason` を記録して `model_error` で停止し、再結合や黙った延期を行いません。

`IoConnectInterrupt` は実際の 11 引数 ABI と厳密に一致する変換済みリソースを使います。`IoConnectInterruptEx` は FullySpecified（1）、LineBased（2、明示 PDO の 1 ライン）、FullySpecifiedGroup（4、group0）に対応し、切断には一致するバージョンとコンテキストが必要です。接続と切断は `PASSIVE_LEVEL` で行います。排他的または共有の latched／level_sensitive ラインは専用ロック、または呼び出し側が初期化した共有 `KSPIN_LOCK` を使用できます。共通の `SynchronizeIrql` は各割り当て DIRQL 以上で、LineBased のゼロは割り当てレベルを選びます。ISR の配送優先順位は割り当て DIRQL のままで、実行時は同期ロックのレベルに従います。`KINTERRUPT` は不透明でアドレスを再利用しません。ISR は `(Interrupt, ServiceContext)` を受け、AL の BOOLEAN で受理を示します。FALSE は NTSTATUS の失敗ではありません。`KeSynchronizeExecution` は同じロック内で実際の単一引数コールバックを実行し、BOOLEAN を返して呼び出し元の IRQL／CR8 を復元します。ISR、同期コールバック、手動の `KeAcquireInterruptSpinLock`／`KeReleaseInterruptSpinLock` は実際のロック所有権を共有し、非再帰、実行識別子、保存 IRQL を検証します。保持したまま戻れません。浮動小数点保存、命令単位のプリエンプションは未対応です。START 失敗と STOP／REMOVE 成功は最終完了前の切断を要求し、上位完了コールバックで先に処理できます。切断は待機済み DPC を破棄しません。
...
`WdfIoQueueFindRequest` は手動キューを検索しますが、要求の処理権は移しません。成功時には要求の参照が 1 つ増えるため、ドライバーは `WdfObjectDereference` で解放します。`WdfIoQueueRetrieveFoundRequest` はキューに残る要求の処理権を移し、キャンセルで取り除かれた要求には `STATUS_NOT_FOUND` を返します。省略可能なパラメーターは `WdfRequestGetParameters` と同じ配置です。有効なフレームワークファイルオブジェクトで `WdfIoQueueFindRequest` を絞り込めます。`WdfIoQueueRetrieveRequestByFileObject` は手動または順次キューから次の一致する要求を取得し、一致しなければ出力を書き換えません。
...
`WdfDeviceInitSetFileObjectConfig` はデバイス作成前に `EvtDeviceFileCreate`、`EvtFileCleanup`、`EvtFileClose` を登録し、任意のファイルオブジェクト用コンテキスト属性をコピーします。このプロファイルは `WdfFileObjectNotRequired`、`WdfFileObjectWdfCanUseFsContext`、`WdfFileObjectWdfCanUseFsContext2`、`WdfFileObjectWdfCannotUseFsContexts` に対応します。選択した WDM コンテキスト欄は、失敗した CREATE または CLOSE まで WDF ハンドルを保持し、作成前には空でなければなりません。`WdfFileObjectCanBeOptional` を指定すると、対応する WDM ファイルオブジェクトがない I/O では `WdfRequestGetFileObject` が NULL を返します。CREATE、CLEANUP、CLOSE には引き続き WDM ファイルオブジェクトが必要で、`WdfFdoInitSetFilter` はフィルターの既定転送を有効にし、`WdfTrue` は明示的に有効、`WdfFalse` は無効にします。ファイル要求の転送には直接 FDO/PDO 経路と明示的な `bus_completion` が必要です。正の `delay_100ns` は、自動 CREATE/CLEANUP/CLOSE 転送、および同期・完了ルーチン付き非同期・`SEND_AND_FORGET` の CREATE 送信で指定できます。CLEANUP と CLOSE のコールバックは下位への送信より先に実行します。`WdfFileObjectNotRequired` を使う CREATE コールバックは、`WdfDeviceGetIoTarget` でデバイス所有のローカルターゲットを取得し、`WDF_REQUEST_SEND_OPTION_SEND_AND_FORGET` のみを指定した `WdfRequestSend` で元の要求を転送できます。保持された PDO が明示的なバス応答を消費し、下位応答が遅延する場合も最終完了を所有します。フレームワークのファイルオブジェクトを持つ CREATE コールバックは `WDF_REQUEST_SEND_OPTION_SYNCHRONOUS` を使用し、`WdfRequestGetStatus` で下位の結果を読み、その状態で元の要求を完了できます。既定の非同期送信では、まず `WdfRequestFormatRequestUsingCurrentType` で要求を整形し、`WdfRequestSetCompletionRoutine` で完了コールバックを登録します。コールバックは `WDF_REQUEST_COMPLETION_PARAMS` を受け取り、`WdfRequestGetCompletionParams` または `WdfRequestGetStatus` で下位の結果を読み、元の要求を下位とは独立して選んだ状態で完了します。完了コールバックを使う送信で PDO の応答が遅延する場合、要求は仮想時刻の期限まで保留され、最終的な下位ステータスとともにコールバックがキューに入ります。非同期 CREATE の `WDF_REQUEST_SEND_OPTION_TIMEOUT` は、負値を相対間隔、正値を合成仮想時計上の絶対 100 ns 期限として扱い、ゼロで無効になります。期限が下位応答より早ければ `STATUS_IO_TIMEOUT` を通知します。即時完了と期限と同時の下位応答を優先します。その他の未記載の送信オプションは未対応です。WDF ハンドルと WDM `FILE_OBJECT` は別の識別子であり、有効な間は `WdfRequestGetFileObject`、`WdfFileObjectGetDevice`、`WdfFileObjectWdmGetFileObject` で参照できます。CREATE の失敗時はファイルの CLEANUP／CLOSE コールバックなしで WDF オブジェクトを削除し、成功後は CLEANUP、CLOSE の順に呼んでからコンテキストを掃除・破棄します。

遅延する同期 `WdfRequestSend` は実際のゲスト呼び出しフレームを保存して中断し、完了後に送信済みを示す BOOLEAN を返して再開します。最終 NTSTATUS は `WdfRequestGetStatus` で取得します。同期 CREATE の `WDF_REQUEST_SEND_OPTION_TIMEOUT` も負値の相対間隔と正値の絶対仮想 100 ns 期限に対応し、ゼロで無効になります。早い期限は `STATUS_IO_TIMEOUT` を返し、即時完了または同時刻の下位応答を優先します。自動転送ではファイルコールバックを下位送信より先に実行し、完了処理に必要なクリーンアップ・破棄コールバックの間は WDM IRP とファイルを保持します。外部 WDF 参照はコンテキストを保持しますが、完了した要求の WDM ファイルの寿命は延長しません。

構成済み PDO に転送する WDM CREATE/CLEANUP/CLOSE も、明示した遅延 `bus_completion` を受け付けます。下位期限の後に実際のゲスト完了ルーチンを実行し、保留状態の伝播と最終ステータスの所有権規則を維持します。
...
省略可能な真偽値 `user_unmap_after_dispatch` は、空でない WDM neither 転送について、ディスパッチ復帰後かつ予定されたワークやキャンセルの前に元のユーザーアドレスのアクセス権を取り消します。MDL でロック済みのページとシステムエイリアスは解除まで使えますが、生のユーザーポインターと新たなロックは失敗します。出力アドレスを取り消した場合、`output_hex` は空です。シナリオの元の割り当てと明示的に宣言した割り当てのアドレスは再利用・再マッピングせず、任意時刻の無効化もモデル化しません。この制限は、後述する MDL ユーザービュー領域のアドレス再利用には適用しません。
...
`IoBuildPartialMdl` は構築済みソース MDL の空でない部分範囲を記述します。長さゼロは残り全体を選択し、ターゲットには十分な PFN 容量が必要です。部分記述子は追加のページロックを取得せず、同じ物理ページを共有します。ソースの有効なシステムマッピングを継承するか、`MmGetSystemAddressForMdlSafe` で独立したマッピングを作成します。部分 MDL の解放や再利用準備は、その MDL 自身が所有するマッピングだけを解放します。実際の WDK インライン関数 `MmPrepareMdlForReuse` を実行してから、再び `IoBuildPartialMdl` を呼び出せます。非ページソースの記述子は独立して解放できますが、依存する部分 MDL が残る間、元のユーザーロックや借用システムエイリアスは解放できません。完了処理は記述子の順序によらず、依存関係を解放する前にチェーン全体を検証します。有効な DMA は再構築と早期解放を阻止します。

`MmMapLockedPagesSpecifyCache` は、有効な要求元コンテキスト（対応範囲内のプロセスにアタッチしたワーカーを含む）で、`APC_LEVEL` 以下の `UserMode` マッピングに対応します。`MmNonCached`、`MmCached`、`MmWriteCombined` は有効なキャッシュ引数ですが、既存の物理ページ属性を継承し、新しいマッピングで変更しません。現在の通常 RAM は Cached です。予約済み列挙値は拒否し、CPU キャッシュのタイミングは模擬しません。各呼び出しは `MappedSystemVa` を変更せず、同じ物理領域にプロセス所有の独立したビューを作ります。ビューは常に実行不可（NX）で、`MdlMappingNoWrite` は読み取り専用にします。プール領域は非ページ領域で、完全な専用ページを占有する必要があります。`MmProbeAndLockPages` で再ロックしても元の PFN を保持します。指定アドレスは MDL のページ内オフセットを維持し、ユーザービュー専用の有限 VA 領域内に置きます。枯渇時は捕捉可能な `STATUS_INSUFFICIENT_RESOURCES` が発生します。

`MmUnmapLockedPages` は、正確なユーザーアドレスと MDL の組、およびビューを作成したプロセスを要求します。エイリアスを実際に除去してマッピング予算を返すため、自動割り当てと `RequestedAddress` は解除済み VA を再利用できます。新しいマッピングは独自の物理領域と権限を持ち、既存のページロックとシステムエイリアスは元のページを保持します。ビューは `(PID, VA)` で識別し、複数プロセスが同じ数値の VA に別の物理ページと権限を同時に保持できます。プロセス切り替えは CPU に見える結合を一括で置き換え、事前検証の失敗では元の結合と権限を維持します。プロセス終了は自身のビューだけを除去し、他のプロセスの同一 VA に影響しません。シナリオの元の VA を無効化しても独立 MDL ビューは維持されます。有効なビューがある間、記述子、元のロック、プール、要求ストレージは解放できません。ユーザービュー内に有効な不透明スケジューラーオブジェクトがある場合、プロセス切り替えは明示的に失敗し、プロセス別のカーネルオブジェクト識別子を推測しません。一般的な仮想メモリー割り当てや未モデル化のプロセス API は未対応です。
...
`MmAllocatePagesForMdl`、`MmAllocatePagesForMdlEx`、`MmFreePagesFromMdl` は独立ページを扱い、物理範囲、部分割り当て、連続チャンクの制約を守ります。ページと記述子は別々に解放します。`MmProtectMdlSystemAddress` は独立システムマッピングの PAGE 権限を変更し、PFN、キャッシュ属性、ユーザービューの権限を保ちます。`MmProbeAndLockPages` は KernelMode プールもロックできます。ページプールは APC_LEVEL 以下でロックした後、DISPATCH_LEVEL でアクセスでき、解除前の解放は禁止されます。
