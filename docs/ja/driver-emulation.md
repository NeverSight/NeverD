...
| ドライバー割り当て MDL | 非ページプールまたは単一ユーザー割り当ての独立／IRP 関連 MDL、ゲストチェーンリンクと完了時解放 | クォータ、手動／部分 MDL、任意のユーザーマッピングは未対応 |
