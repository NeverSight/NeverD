...
これは限定的なスケジューリングモデルであり、完全な Windows 非同期対応ではありません。アラート可能／ユーザーモード待機、APC、一般的な WDM 要求キャンセル、公開シナリオの並行送信、KMDF の `METHOD_NEITHER` ユーザーバッファー API、UMDF、KMDF PnP デバイスと一般のキュースケジューリング、完全な PnP／電源、一般のハードウェア、その他の DMA インターフェース、その他の割り込みモードは未対応です。初期化のみの呼び出しも明示的に登録したコールバックを実行しますが、要求やアンロードを暗黙には生成しません。
...
バッファー方式／直接方式の KMDF 要求では、`WdfDeviceInitSetIoInCallerContextCallback` で登録したコールバックを要求元プロセスで実行します。要求を完了するか `WdfDeviceEnqueueRequest` を一度呼んで順次処理の既定キューへ渡す必要があります。`METHOD_NEITHER` のユーザーバッファー取得・検査・ロックは未対応です。
