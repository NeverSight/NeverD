...
新增測試驗證完整 CPU 內容還原（暫存器、旗標、SIMD、FPU、CR8）、共用記憶體及故障／跨後端內容拒絕。工作項目範例涵蓋標記後的待處理請求、延遲完成、佇列與生命週期錯誤、停滯請求、共用預算，以及公開報告中分開的派送／完成狀態。內部計時器／DPC 測試僅驗證狀態轉換，不表示支援客體 API 或完整 Windows 非同步環境。
