...
單一直線 ARM64 輔助函式在唯一呼叫是重新驗證的不返回 Swift 執行階段匯入時，可以保留未被覆寫且完整觀測到的入口上下文。同一套位元組身分與堆疊框架逃逸證明檢查此前的每個操作，並要求所有傳出的純量堆疊參數佔用已完整寫入的私有位元組。分支、返回、例外邊與未知呼叫仍不支援。僅副作用的入口位元組需求允許獨立證明的終止控制流程；預設的無用輸入證明仍要求觀測到返回。這只產生候選簽章，原始碼函式主體與相依閉包仍須驗證。
