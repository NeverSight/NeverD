...
Swift 布林結果的聯合驗證同時檢查目前的 Objective-C 入口 ABI、不可變的直接呼叫指令、精確的強執行階段匯入，以及完整的 LowIR 使用端證明。只允許一個比較呼叫點，其他呼叫的 ABI 全部從目前的執行階段目錄重新建立；拒絕原生簽章猜測、動態分派與重複呼叫點。此證據本身不會發布原始碼，也不會宣告位元組回傳 ABI。
