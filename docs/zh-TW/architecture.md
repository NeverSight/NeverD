...
`DriverResources.h` 與 `DriverResources.def` 定義固定 `register_bank` 配置、暫存器存取權限與寬度、欄位拼法及共用限制。`DriverScenario` 負責 JSON／C++ 預檢；`DriverResult` 記錄設定，不重複保存觀測到的暫存器組狀態。`KernelMMIO` 擁有每個 PDO 的持久暫存器值、轉譯後實體區間、資源世代、硬體可存取性及獨立映射別名。`KernelModelResources` 擁有緊密排列、唯讀的原始／轉譯 START 清單及提供者／生命週期整合；實際成功的下層 START 先於上層回呼啟用資源世代，提供者實際裝置 SET 則更新 D0／D3 可存取性。失敗 START 或 STOP／REMOVE 的清理會在 IRP 最終完成前檢查；上層完成回呼可先解除映射，不會隱式釋放。意外移除立即禁止硬體存取。`GuestMemory` 與 `UnicornBackend` 在暫存器回呼前驗證完整的原始 CPU／API 存取，共用首次錯誤／停止狀態，避免 Unicorn 拆分操作後仍產生效果。固定配置重新啟動保留數值，映射別名不擁有獨立位元組。此有限記憶體暫存器匯流排不實作任意 RAM、資源重新平衡、連接埠、中斷或 DMA。
