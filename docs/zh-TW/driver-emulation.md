...
| KMDF 1.33 非 PnP 驅動程式 | 版本繫結、物件／內容、具名控制裝置、循序或無限並行預設佇列、非預設手動佇列，以及實際執行回呼的緩衝／直接／neither 請求 | 不支援 PnP 裝置、一般佇列排程、類別擴充或 UMDF |
