...
目前支援無資源 KMDF PnP 的直接 FDO/PDO 子集：`EvtDriverDeviceAdd` 接收架構持有的初始化器，`WdfDeviceCreate` 建立 FDO，`WdfFdoInitWdmGetPhysicalDevice` 與 `WdfDeviceWdmGetPhysicalDevice` 保留 PDO 身分；`WdfDeviceWdmGetAttachedDevice` 傳回下層 WDM 物件，`WdfWdmDeviceGetWdfDeviceHandle` 將本驅動程式建立的 WDM 物件映回 WDF 控制代碼，`WdfDeviceGetDriver` 傳回所屬 WDFDRIVER。AddDevice 失敗及 Remove 完成後，架構執行清理／銷毀回呼並刪除裝置。`WdfDeviceInitSetPnpPowerEventCallbacks` 接受精確的 KMDF 1.33 結構，支援 `EvtDevicePrepareHardware`、`EvtDeviceD0Entry`、`EvtDeviceD0Exit` 與 `EvtDeviceReleaseHardware`，其他非空回呼會明確報錯。匯流排成功完成 START 後，PrepareHardware 先收到兩份不同的空資源清單，再於原 IRP 完成前執行 D0Entry；`WdfCmResourceListGetCount` 傳回零，`WdfCmResourceListGetDescriptor` 傳回 NULL。PrepareHardware 或 D0Entry 失敗時，其失敗狀態成為 START 結果，仍執行 ReleaseHardware，且不呼叫 D0Exit。STOP、意外移除或從 D0 正常移除前依序執行 D0Exit 與 ReleaseHardware；再次 START 會重新準備硬體並進入 D0。START 失敗後須在情境中明確移除裝置。PnP 佇列可用 `WdfUseDefault` 或 `WdfTrue` 啟用電源管理，`WdfFalse` 則停用自動管理。佇列在裝置進入 D0 前及離開 D0 時設定 `WdfIoQueuePnpHeld`，進入 D0 後恢復派送。仍由驅動程式持有要求時的停止／恢復回呼、非空硬體資源與一般電源策略尚未實作。真正的 WDK 範例 `driver_kmdf_pnp.c` 可透過 `NEVERD_KMDF_PNP_FIXTURE`／`NEVERD_KMDF_PNP_CFG_FIXTURE` 驗證一般及 active-CFG 映像。
