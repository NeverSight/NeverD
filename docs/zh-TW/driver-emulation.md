...
這是有界排程模型，不代表完整 Windows 非同步支援。可警示或使用者模式等待、APC、一般 WDM 請求取消、並行公開情境提交、UMDF、一般 KMDF PnP 裝置及一般佇列排程、完整 PnP／電源、一般硬體、其他 DMA 介面與其他中斷模式仍不支援。僅初始化呼叫會執行明確排入佇列的回呼，不會隱含產生請求或卸載。
