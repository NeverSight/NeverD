...
`WdfDeviceInitSetFileObjectConfig` 在建立裝置前註冊 `EvtDeviceFileCreate`、`EvtFileCleanup`、`EvtFileClose`，並複製可選的檔案物件內容屬性。目前支援 `WdfFileObjectNotRequired`、`WdfFileObjectWdfCanUseFsContext`、`WdfFileObjectWdfCanUseFsContext2` 與 `WdfFileObjectWdfCannotUseFsContexts`。選定的 WDM 內容槽在 CREATE 失敗或 CLOSE 前保存 WDF 控制代碼，建立前必須為空。設定 `WdfFileObjectCanBeOptional` 後，若 I/O 要求沒有相符的 WDM 檔案物件，`WdfRequestGetFileObject` 會傳回 NULL。CREATE、CLEANUP 與 CLOSE 仍需要 WDM 檔案物件；`WdfFdoInitSetFilter` 啟用篩選驅動的預設轉送；`WdfTrue` 明確啟用，`WdfFalse` 停用。檔案要求轉送需要直接 FDO/PDO 路徑和明確的 `bus_completion`；只有具備完成回呼的預設非同步 CREATE 傳送允許正數 `delay_100ns`，其餘模式要求即時回應；CLEANUP 與 CLOSE 回呼先於下層派送。使用 `WdfFileObjectNotRequired` 的 CREATE 回呼可透過 `WdfDeviceGetIoTarget` 取得裝置持有的本機目標，並以唯一支援的 `WDF_REQUEST_SEND_OPTION_SEND_AND_FORGET` 選項呼叫 `WdfRequestSend` 轉送原始要求。保留的 PDO 消耗明確的同步匯流排回應並負責最終完成。持有框架檔案物件的 CREATE 回呼可使用 `WDF_REQUEST_SEND_OPTION_SYNCHRONOUS`，透過 `WdfRequestGetStatus` 讀取下層結果，再以該狀態完成原始要求。預設非同步傳送須先以 `WdfRequestFormatRequestUsingCurrentType` 格式化要求，再註冊 `WdfRequestSetCompletionRoutine`。完成回呼會收到 `WDF_REQUEST_COMPLETION_PARAMS`，可透過 `WdfRequestGetCompletionParams` 或 `WdfRequestGetStatus` 讀取下層結果，並以可獨立於下層結果選擇的狀態完成原始要求。延遲的 PDO 回應讓非同步要求保持未完成，直到虛擬時間到達期限，再以下層最終狀態排入完成回呼。非同步 CREATE 傳送可設定相對 `WDF_REQUEST_SEND_OPTION_TIMEOUT`：逾時先發生時向回呼交付 `STATUS_IO_TIMEOUT`，與下層回應同時到期時優先採用下層結果。零表示不逾時；絕對逾時及其他傳送選項仍不支援。WDF 檔案控制代碼與 WDM `FILE_OBJECT` 保持不同身分，存活期間可由 `WdfRequestGetFileObject`、`WdfFileObjectGetDevice` 及 `WdfFileObjectWdmGetFileObject` 查詢。CREATE 失敗只刪除 WDF 檔案物件，不呼叫檔案清理或關閉回呼；成功開啟後的 CLEANUP、CLOSE 回呼先於內容清理與銷毀執行。
