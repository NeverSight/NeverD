...
| 驅動程式自行配置的 MDL | 獨立或 IRP 關聯描述元、部分 MDL、共用使用者／系統對映及完成時釋放 | 配額、手工 MDL 及一般虛擬記憶體配置 API |
| READ/WRITE | 循序緩衝／直接／neither I/O，可由工作項目或 DPC 完成 | 僅支援下列 API 子集；不支援並行公開情境提交、一般 WDM 請求取消或隱含檔案位置 |
| WDM `METHOD_NEITHER` | 獨立使用者緩衝區、存取探測、MDL 鎖頁、合成要求行程身分、派送後 VA 撤銷或行程結束及有限取消 | 未列出的一般行程及虛擬記憶體配置 API |
| KMDF 1.33 非 PnP 驅動程式 | 版本繫結、物件／內容、具名控制裝置、手動、循序或有限或無限並行預設及非預設佇列，以及實際執行回呼的緩衝／直接／neither 請求 | 不支援 PnP 裝置、一般佇列排程、類別擴充或 UMDF |
| PnP 匯流排／功能／篩選驅動程式 | 明確的無資源或暫存器組 PDO、客體 AddDevice 與八種常見 PnP 生命週期次要功能 | 其他 PnP 操作、一般電源管理、其他硬體／資源及 一般 KMDF PnP |
| 儲存、網路、顯示、檔案系統及迷你篩選驅動程式 | 不支援相關子系統契約 | 連接埠／類別／迷你連接埠框架、NDIS/WFP、圖形或檔案系統服務 |
| 工作項目、計時器、DPC、事件與等待 | 目前執行 IRQL 在派送與工作項目中為 `PASSIVE_LEVEL`，在 DPC 中為 `DISPATCH_LEVEL` | 僅支援下列 API 子集；不支援並行公開情境提交 或 一般 WDM 請求取消 |
| 使用處理程序／執行緒回呼、控制代碼、登錄／檔案操作或核心模組探索的驅動程式 | 支援配置的登錄；其他行為限於下列 API | 物件管理員、系統狀態及回呼／事件產生機制 |
| 硬體、DMA、PCI、中斷或虛擬化驅動程式 | 明確暫存器組、MMIO、獨佔／共用 latched 與 level_sensitive 中斷，以及有界一致性 common／SG／channel DMA | 其他裝置模型、任意實體 RAM、PCI、連接埠、其他 DMA 介面及特權 CPU 狀態 |
...
這是有界排程模型，不代表完整 Windows 非同步支援。可警示或使用者模式等待、APC、一般 WDM 請求取消、並行公開情境提交、UMDF、一般 KMDF PnP 裝置及一般佇列排程、完整 PnP／電源、一般硬體、其他 DMA 介面仍不支援。僅初始化呼叫會執行明確排入佇列的回呼，不會隱含產生請求或卸載。
...
同一 `register_bank` 提供者可單獨宣告 `interrupts`，或與 `resources` 一起宣告；至少一份清單非空。`resource_free` 拒絕明確的 `interrupts`，包含 `[]`。每個中斷提供 `id`、`raw_vector`、`raw_level`、`raw_affinity`、`translated_vector`、`translated_level`、`translated_affinity`、`mode` 與 `share`。`DriverInterrupts.h`／`DriverInterrupts.def` 統一型別、拼法及限制：每 PDO 最多 8 個、總計 32 個；PDO 內 ID 是唯一的有界 ASCII 字串；raw level 為 0–65535，translated DIRQL 為 3–12。向量保留完整 32 位元，不推測其與 IRQL 的關係；兩份 affinity 必須為 1，僅模擬 CPU0／group0。`mode` 支援 `latched` 與 `level_sensitive`，`share` 支援 `device_exclusive` 與 `shared`。共用同一轉譯向量的資源都須宣告 shared，且模式、轉譯層級、affinity 及重觸發週期一致。電位觸發模式須明確提供不超過 `INT64_MAX` 的正數 `retrigger_after_100ns`。raw 與 translated 值獨立；`CM_RESOURCE_LIST` 保留記憶體描述元順序，再放置型別 2 中斷描述元，其共用屬性與觸發旗標反映宣告。

READ／WRITE／IOCTL 可宣告 `interrupt_events`，每項指定 `after_100ns`、`device_id`、`interrupt_id` 與選用 `action`；其他要求種類連空清單也拒絕。延遲是至多 `INT64_MAX` 的非負值，從要求成功提交起算，並擷取當時的連線及 PDO 資源世代。latched 使用預設 `action: "pulse"`，每個脈衝呼叫全部已擷取 ISR；level_sensitive 僅接受明確的 `assert`／`deassert`。各 PDO／資源／世代的來源狀態以 OR 合成電位，重複 assert 具冪等性；同一邊界先套用全部外部狀態變更再取樣。每輪 ISR 後，在目前虛擬時間加 `retrigger_after_100ns` 時再次取樣，按註冊順序呼叫至 ISR 傳回 TRUE；TRUE 僅表示認領，不撤銷來源電位。來源 IRP 完成不取消事件，也不從暫存器寫入推測 enable、status 或 acknowledgment。時間只於閒置時推進；ISR 在支援的回呼邊界先於 DPC／工作項執行，`after_100ns: 0` 不保證指令級搶占。每要求最多 64 個、總計 1024 個宣告事件；實際遞送批次另限 4096，每批最多 32 個處理常式，並受排程與指令預算約束。失去連線、無效資源世代或實體 D3 會記錄 `undelivered_reason` 並以 `model_error` 停止，不重新繫結或默默延後。

`IoConnectInterrupt` 使用真正十一引數 ABI 並精確比對轉譯資源；`IoConnectInterruptEx` 支援 FullySpecified（1）、LineBased（2，明確 PDO 的一條線）及 FullySpecifiedGroup（4，group0），斷開要求相符版本與內容。連接與斷開要求 `PASSIVE_LEVEL`。獨佔及共用 latched／level_sensitive 線路可使用私有鎖，或呼叫者已初始化的共用 `KSPIN_LOCK`；共同 `SynchronizeIrql` 不低於各自配置 DIRQL，LineBased 的零值選用配置層級。ISR 排程優先順序仍使用配置 DIRQL，執行遵循同步鎖層級。`KINTERRUPT` 不透明且位址不重用。ISR 接收 `(Interrupt, ServiceContext)`，AL 的 BOOLEAN 表示是否認領，FALSE 不是 NTSTATUS 失敗。`KeSynchronizeExecution` 在同一鎖下執行真正單引數回呼，傳回 BOOLEAN 並恢復呼叫者 IRQL／CR8。ISR、同步回呼與手動 `KeAcquireInterruptSpinLock`／`KeReleaseInterruptSpinLock` 共用實際鎖所有權，驗證不可遞迴、執行身分及儲存的 IRQL，不能持鎖返回。仍不支援浮點保存及指令級搶占。失敗 START 與成功 STOP／REMOVE 必須在最終完成前斷開，上層完成回呼可先清理；斷開不會丟棄已排入佇列的 DPC。
...
KMDF 1.33 支援使用精確的 1.33.0 ABI：458 個函式槽具有穩定的客體識別，下列 88 個 API 實作了執行語義。`WdfVersionBind` 與 `WdfVersionUnbind` 在真實 WDK `FxDriverEntry` 包裝函式前後管理客體繫結。`WdfGetDriver` 讀取公用驅動程式全域結構。非 PnP 驅動程式、一般物件、控制裝置、佇列和傳入請求共用具型別內容、參考計數，以及實際執行的清理／銷毀／卸載回呼。所有已建模的框架呼叫與回呼目前都要求 `PASSIVE_LEVEL`；清理完成後新增參考仍不在此設定的支援範圍內。未建模的函式槽、`WdfLdrQueryInterface`、類別擴充和 UMDF 會明確停止。
...
`WdfIoQueueStopSynchronously`、`WdfIoQueueDrainSynchronously` 與 `WdfIoQueuePurgeSynchronously` 會暫停客體的 `PASSIVE_LEVEL` 呼叫框架，直到相關請求結束。同步停止繼續接受但暫停遞送，並等待已遞送請求；同步排空拒絕新請求、繼續遞送排隊請求，並等待兩類請求完成；同步清除取消排隊及已標記可取消的請求，等待取消回呼返回。沒有完成來源的等待會回報停滯模型錯誤。

`WdfIoQueueStopAndPurge` 與 `WdfIoQueueStopAndPurgeSynchronously` 取消呼叫前已排隊及已標記可取消的驅動程式持有請求，之後繼續接受新請求，但直到 `WdfIoQueueStart` 才重新遞送。非同步狀態回呼與同步等待在原有請求及其取消回呼結束後完成；呼叫後進入佇列的新請求維持排隊，不延遲完成通知。一般與同步停止也會從先前的排空或清除狀態恢復接受。

請求參數使用 40 位元組的 `WDF_REQUEST_PARAMETERS` 配置。輸入／輸出存取函式傳回邏輯長度，保留緩衝區別名及現有直接 I/O 的 MDL 對映；直接 IOCTL 的輸入仍使用緩衝區。方向錯誤或緩衝區不足會傳回文件規定的狀態。完成操作先在緩衝區仍有效時執行請求清理，再完成 IRP 並釋放請求所屬鎖頁；參考允許時才銷毀子物件和請求。一旦開始完成操作，就拒絕新的緩衝區與參數存取函式呼叫；已取得的緩衝區指標在清理期間仍可使用。外部物件參考保留內容，但不保留對已完成 IRP 的存取權。
...
`WdfRequestRetrieveInputWdmMdl` 與 `WdfRequestRetrieveOutputWdmMdl` 視需要為緩衝 WRITE 輸入、READ 輸出及 IOCTL 輸入／輸出描述現有 SystemBuffer。每次擷取均先驗證方向有效且長度非零，再使用該請求唯一的快取描述元；首次成功擷取決定 ByteCount，即使另一方向的邏輯長度不同也保留此值。對此描述元，`MmGetSystemAddressForMdlSafe` 傳回原 VA；額外對映、解除對映及驅動程式釋放均遭拒絕。直接 READ 輸出、WRITE 輸入及 IOCTL 輸出傳回現有 `IRP.MdlAddress`，擷取本身不建立對映；直接 IOCTL 輸入使用 SystemBuffer 快取。描述元、IRP 和緩衝區在完成時一同失效。取消內部參考或外部參考只保留 WDF 內容，不保留已完成的 I/O 儲存空間。WDF 對 `METHOD_NEITHER` 的 MDL 擷取仍不支援；未建立 MDL 的 PFN 存取也會拒絕。請求所屬的 WDFMEMORY 使用獨立的鎖頁對映。

對緩衝、直接與 neither KMDF 要求，`WdfDeviceInitSetIoInCallerContextCallback` 在要求端處理序的 `PASSIVE_LEVEL` 執行預佇列回呼。回呼必須完成要求，或恰好呼叫一次 `WdfDeviceEnqueueRequest` 後進入預設佇列。對 `METHOD_NEITHER` IOCTL 與 neither READ／WRITE，`WdfRequestRetrieveUnsafeUserInputBuffer` 和 `WdfRequestRetrieveUnsafeUserOutputBuffer` 僅在此回呼傳回原始使用者位址。`WdfRequestProbeAndLockUserBufferForRead` 和 `WdfRequestProbeAndLockUserBufferForWrite` 檢查頁面權限並鎖定要求所屬頁面；`WdfMemoryGetBuffer` 傳回系統別名，在離開要求端內容後的佇列回呼仍可使用。完成要求時釋放鎖頁與別名。亦支援透過內嵌指標存取明確宣告的 `user_buffers`；任意使用者對映仍不支援。

neither 要求可宣告 `user_buffers`（`id`、`size`、選用 `input`／`access`）與 `user_pointers`。指標的 `source`／`target` 指定 `buffer`（`input`、`output` 或 `memory`）與 `offset`；只有 `memory` 使用要求內唯一的 `id`。八位元組指標欄位不得重疊或越界，目標可在緩衝區內或末尾。整個情境最多 64 個附加緩衝區、256 個指標，每區最多 64 KiB，計入 512 KiB 總預算。重複目標共用真實位元組；頁面權限、鎖頁、撤銷與處理序退出沿用現有機制。`configuration.user_memory` 保留宣告，`user_buffers[].backing_hex` 是停止時的診斷快照，包含撤銷或故障後資料，不取代 `output_hex` 或恢復存取權限。 `id`: 1–64 ASCII, `[A-Za-z0-9][A-Za-z0-9_.-]*`.

`WdfRequestRetrieveInputMemory` 與 `WdfRequestRetrieveOutputMemory` 為既有緩衝或直接 I/O 緩衝區提供要求所屬的 WDFMEMORY 檢視。重複取得同一方向會保留控制代碼；`WdfMemoryGetBuffer` 傳回原緩衝區與邏輯長度。長度為零或方向不符會傳回 WDF 狀態；neither I/O 仍須在呼叫端內容中探測並鎖頁。借用檢視不額外鎖頁，要求完成時失效。

目前支援 KMDF PnP 的直接 FDO/PDO 子集：`EvtDriverDeviceAdd` 接收架構持有的初始化器，`WdfDeviceCreate` 建立 FDO，`WdfFdoInitWdmGetPhysicalDevice` 與 `WdfDeviceWdmGetPhysicalDevice` 保留 PDO 身分；`WdfDeviceWdmGetAttachedDevice` 傳回下層 WDM 物件，`WdfWdmDeviceGetWdfDeviceHandle` 將本驅動程式建立的 WDM 物件映回 WDF 控制代碼，`WdfDeviceGetDriver` 傳回所屬 WDFDRIVER。AddDevice 失敗及 Remove 完成後，架構執行清理／銷毀回呼並刪除裝置。`WdfDeviceInitSetPnpPowerEventCallbacks` 接受精確的 KMDF 1.33 結構，支援 `EvtDevicePrepareHardware`、`EvtDeviceD0Entry`、`EvtDeviceD0Exit`、`EvtDeviceReleaseHardware`、`EvtDeviceQueryStop`、`EvtDeviceQueryRemove` 與 `EvtDeviceSurpriseRemoval`，其他非空回呼會明確報錯。QueryStop／QueryRemove 在下層匯流排派送前執行真正回呼；失敗會否決並完成原 IRP，不進入匯流排，也不改變裝置及電源生命週期。SurpriseRemoval 的 void 回呼同樣先於下層匯流排派送，再按既有流程退出 D0。 匯流排成功完成 START 後，PrepareHardware 先收到兩份不同的資源清單（無資源裝置的清單為空），再於原 IRP 完成前執行 D0Entry；空清單的 `WdfCmResourceListGetCount` 傳回零，`WdfCmResourceListGetDescriptor` 傳回 NULL。PrepareHardware 或 D0Entry 失敗時，其失敗狀態成為 START 結果，仍執行 ReleaseHardware，且不呼叫 D0Exit。STOP、意外移除或從 D0 正常移除前依序執行 D0Exit 與 ReleaseHardware；再次 START 會重新準備硬體並進入 D0。START 失敗後須在情境中明確移除裝置。PnP 佇列可用 `WdfUseDefault` 或 `WdfTrue` 啟用電源管理，`WdfFalse` 則停用自動管理。佇列在裝置進入 D0 前及離開 D0 時設定 `WdfIoQueuePnpHeld`，進入 D0 後恢復派送。設定 `register_bank` 的裝置提供唯讀 `CM_PARTIAL_RESOURCE_DESCRIPTOR` 項目，指標有效期持續至 ReleaseHardware；驅動程式可用 `MmMapIoSpace` 對應轉譯後的記憶體，但須在 STOP 或 Remove 完成前解除對應。離開 D0 前，電源管理佇列會對每個由驅動程式持有的要求呼叫 `EvtIoStop`；驅動程式可完成要求、呼叫 `WdfRequestStopAcknowledge`，或等待已排定的工作完成要求。未註冊 `EvtIoStop` 時，框架同樣等待所有已遞送要求完成。TRUE 會將要求重新入列，於重新啟動後再次遞送；FALSE 保留驅動程式的所有權，並在重新進入 D0 後呼叫 `EvtIoResume`。佇列中的要求在 STOP 期間保持等待；只有沒有任何完成來源時，等待才以停滯的 `model_error` 結束。其他資源類型與一般電源策略尚未實作。真正的 WDK 範例 `driver_kmdf_pnp.c` 可透過 `NEVERD_KMDF_PNP_FIXTURE`／`NEVERD_KMDF_PNP_CFG_FIXTURE` 驗證一般及 active-CFG 映像。
...
對於 PnP FDO，`WdfDeviceInitSetDeviceType` 會將指定的 32 位元類型寫入 WDM `DEVICE_OBJECT`；未呼叫時預設為 `FILE_DEVICE_UNKNOWN`。目前不模擬依賴裝置類型的 I/O 優先權提升。
...
`WdfDeviceInitSetFileObjectConfig` 在建立裝置前註冊 `EvtDeviceFileCreate`、`EvtFileCleanup`、`EvtFileClose`，並複製可選的檔案物件內容屬性。目前支援 `WdfFileObjectNotRequired`、`WdfFileObjectWdfCanUseFsContext`、`WdfFileObjectWdfCanUseFsContext2` 與 `WdfFileObjectWdfCannotUseFsContexts`。選定的 WDM 內容槽在 CREATE 失敗或 CLOSE 前保存 WDF 控制代碼，建立前必須為空。設定 `WdfFileObjectCanBeOptional` 後，若 I/O 要求沒有相符的 WDM 檔案物件，`WdfRequestGetFileObject` 會傳回 NULL。CREATE、CLEANUP 與 CLOSE 仍需要 WDM 檔案物件；`WdfFdoInitSetFilter` 啟用篩選驅動的預設轉送；`WdfTrue` 明確啟用，`WdfFalse` 停用。檔案要求轉送需要直接 FDO/PDO 路徑和明確的 `bus_completion`；自動轉送 CREATE/CLEANUP/CLOSE，以及同步、具備完成回呼的非同步或 `SEND_AND_FORGET` CREATE 傳送均允許正數 `delay_100ns`；CLEANUP 與 CLOSE 回呼先於下層派送。使用 `WdfFileObjectNotRequired` 的 CREATE 回呼可透過 `WdfDeviceGetIoTarget` 取得裝置持有的本機目標，並以唯一支援的 `WDF_REQUEST_SEND_OPTION_SEND_AND_FORGET` 選項呼叫 `WdfRequestSend` 轉送原始要求。保留的 PDO 消耗明確的匯流排回應並負責最終完成，下層回應延遲時也是如此。持有框架檔案物件的 CREATE 回呼可使用 `WDF_REQUEST_SEND_OPTION_SYNCHRONOUS`，透過 `WdfRequestGetStatus` 讀取下層結果，再以該狀態完成原始要求。預設非同步傳送須先以 `WdfRequestFormatRequestUsingCurrentType` 格式化要求，再註冊 `WdfRequestSetCompletionRoutine`。完成回呼會收到 `WDF_REQUEST_COMPLETION_PARAMS`，可透過 `WdfRequestGetCompletionParams` 或 `WdfRequestGetStatus` 讀取下層結果，並以可獨立於下層結果選擇的狀態完成原始要求。對於具備完成回呼的傳送，延遲的 PDO 回應讓要求保持未完成，直到虛擬時間到達期限，再以下層最終狀態排入回呼。非同步 CREATE 傳送可設定 `WDF_REQUEST_SEND_OPTION_TIMEOUT`：負值為相對間隔，正值為合成虛擬時鐘上的絕對 100 ns 期限，零停用逾時。逾時早於下層回應時交付 `STATUS_IO_TIMEOUT`；立即完成及與逾時同刻的下層回應優先。其他未列出的傳送選項仍不支援。WDF 檔案控制代碼與 WDM `FILE_OBJECT` 保持不同身分，存活期間可由 `WdfRequestGetFileObject`、`WdfFileObjectGetDevice` 及 `WdfFileObjectWdmGetFileObject` 查詢。CREATE 失敗只刪除 WDF 檔案物件，不呼叫檔案清理或關閉回呼；成功開啟後的 CLEANUP、CLOSE 回呼先於內容清理與銷毀執行。

延遲的同步 `WdfRequestSend` 保留並暫停真實來賓呼叫框架，完成後恢復並傳回表示已傳送的 BOOLEAN；最終 NTSTATUS 由 `WdfRequestGetStatus` 提供。同步 CREATE 同樣支援負值相對、正值絕對虛擬 100 ns 期限的 `WDF_REQUEST_SEND_OPTION_TIMEOUT`，零停用逾時。較早的逾時傳回 `STATUS_IO_TIMEOUT`，立即完成或同刻的下層回應優先。自動轉送仍先執行檔案回呼，再派送至下層；WDM IRP 和檔案在完成流程所需的清理、銷毀回呼期間保持有效。外部 WDF 參考保留內容，但不延長已完成請求對應 WDM 檔案的生命週期。

轉送到已配置 PDO 的 WDM CREATE/CLEANUP/CLOSE 也接受明確延遲的 `bus_completion`。真實來賓完成例程在下層期限後執行，保留待處理狀態傳播和最終狀態歸屬規則。
...
| `IoConnectInterrupt`, `IoDisconnectInterrupt`, `IoConnectInterruptEx`, `IoDisconnectInterruptEx` | 精確配置的獨佔／共用 latched 或 level_sensitive 線路；PASSIVE_LEVEL 下傳統 ABI 與 Ex 1／2／4、嚴格連線世代及鎖所有權 |
| `KeSynchronizeExecution`, `KeAcquireInterruptSpinLock`, `KeReleaseInterruptSpinLock` | 真正 BOOLEAN 同步回呼，在不低於配置 DIRQL 的同步 IRQL 下持有同一把不可遞迴鎖；恢復原始呼叫者 IRQL 與所有權 |
...
選用布林欄位 `user_unmap_after_dispatch` 適用於非空的 WDM neither 傳輸：派送傳回後、排程工作或取消執行前撤銷原使用者虛擬位址的存取權。已鎖定的 MDL 頁面及其系統別名仍可使用，直到驅動程式解除鎖定；原始使用者指標及新的鎖頁操作會失敗。若輸出使用者位址已撤銷，`output_hex` 為空。原始及明確宣告的情境配置位址不重用，也不重新對映這些位址或模擬任意時刻撤銷；下文 MDL 使用者檢視區的位址重用不受此限制。
...
`MmMapLockedPagesSpecifyCache` 在有效要求者情境（含有限支援的行程附加工作項）中，以不高於 `APC_LEVEL` 的 IRQL 支援 `UserMode` 檢視。`MmNonCached`、`MmCached` 與 `MmWriteCombined` 都是有效快取參數；已有快取屬性的實體頁面繼承原屬性，不會被新對映改變。目前一般 RAM 使用 Cached；保留的列舉值遭拒絕，也不模擬 CPU 快取時序。每次呼叫為相同實體儲存建立獨立、隸屬行程的檢視，不改變 `MappedSystemVa`。檢視一律不可執行（NX），`MdlMappingNoWrite` 將其設為唯讀。集區儲存必須為非分頁記憶體，且占用完整的專用頁面。以 `MmProbeAndLockPages` 再次鎖定檢視仍保留原 PFN。指定的位址必須保留 MDL 頁面內偏移，且位於使用者檢視專用的有限 VA 區域；區域耗盡會引發可捕捉的 `STATUS_INSUFFICIENT_RESOURCES`。

`MmUnmapLockedPages` 要求確切的使用者位址／MDL 配對，並由建立檢視的行程呼叫。它實際移除別名並歸還對映預算；自動配置與 `RequestedAddress` 都可重用解除對映後的 VA，新對映使用自己的實體儲存與權限，既有鎖頁與系統別名仍指向原實體頁面。檢視以 `(PID, VA)` 識別，不同行程可同時保留數值相同但實體頁面與權限不同的位址；行程切換以交易方式替換 CPU 可見的繫結，預檢失敗保留原繫結與權限。行程結束只撤銷其自身檢視，不影響其他行程的同址檢視。撤銷情境原始 VA 不影響獨立 MDL 檢視；有效檢視仍阻止描述元、根鎖定、集區及要求儲存空間提前釋放。使用者檢視中有活動的不透明排程物件時，行程切換會明確失敗，不會虛構各行程的核心物件身分。一般虛擬記憶體配置及其他未建模行程 API 仍不支援。
...
`__GSHandlerCheck_SEH` 在搜尋處理常式前及展開階段分別檢查映像目前的安全 cookie，包含沒有 finally 的堆疊框架。支援固定與動態對齊的位置、帶正負號的框架偏移及原始框架指標編碼；包裝層的 cookie 檢查與 C 處理常式旗標分別生效。前置程式碼及尾聲展開不會讀取尚未建立的 cookie。不匹配會在相關篩選函式、清理或處理常式執行前停止。獨立的 `__GSHandlerCheck` 及 GS/C++ 包裝仍不支援。`driver_seh_gs.h` 定義原創框架並實際呼叫連結的 WDK cookie 檢查函式。
...
WDM READ/WRITE/IOCTL 要求或無限並行 KMDF 預設佇列中的要求可設定 `defer_callback_drain: true`。只有派送傳回 `STATUS_PENDING` 且 IRP 仍待完成時，執行器才會先提交下一項要求；下一項未設定此欄位的要求之後，會排空回呼並完成整批要求。重疊要求可使用不同的檔案物件，或使用明確非同步開啟的相同檔案物件；若最後一項要求設定此欄位，場景結尾會排空回呼。同步檔案物件的重疊要求、任意執行緒搶佔及外部要求到達仍不支援。

## 非同步檔案物件

只有 CREATE 要求可設定布林欄位 `asynchronous_file: true`；省略或 false 維持同步開啟。非同步開啟會清除來賓 `FILE_OBJECT` 的 `FO_SYNCHRONOUS_IO`，後續檔案 IRP 不設定 `IRP_SYNCHRONOUS_API`。同一非同步檔案的 READ/WRITE/IOCTL 只有在明確延後回呼排空時才能重疊；CLEANUP/CLOSE 仍須等待所有先前傳輸完成並最終化。非同步檔案不維護隱含檔案位置；`byte_offset` 仍逐要求指定，預設為零。非 CREATE 要求即使設為 false 也拒絕此欄位。

## 本版補齊的執行語義

獨立頁支援 `MmAllocatePagesForMdl`、`MmAllocatePagesForMdlEx` 與 `MmFreePagesFromMdl`，遵守實體範圍、部分配置及連續區塊限制；頁面與描述元分別釋放。`MmProtectMdlSystemAddress` 修改獨立系統映射的 PAGE 權限，保留 PFN、快取屬性與使用者映射權限。`MmProbeAndLockPages` 可鎖定 KernelMode 集區；分頁集區在 APC_LEVEL 以下鎖定後可於 DISPATCH_LEVEL 存取，解鎖前不可釋放。
...
`WdfDeviceInitSetPowerPolicyOwnership` 控制電源策略所有權。自管理 I/O 的 Init/Suspend/Restart/Flush/Cleanup 與 D0 前後回呼實際執行，普通 D3/D0 保留硬體資源。預設策略將 Sleeping3/Working 對應至 D3/D0，消費明確的 `requested_device_power`；獨立子 IRP 報告 `origin: "framework_power_policy"` 與 `response_index`。S3 等待子請求，S0 可於發出 D0 後完成。閒置／喚醒、WDF 中斷物件與失敗裝置自動重新列舉仍未建模。
