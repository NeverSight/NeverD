...
ARM64 Objective-C 原始碼復原支援由呼叫端證明的單一 Swift 字串比較及其原始單一位元結果。零擴展前會驗證目前的入口 ABI、強匯入、精確呼叫點與所有使用端。產生的 C 以 `swiftcall` 的 `_Bool` 宣告執行階段函式，再明確轉換為 `uint8_t`，保留原有遮罩與周圍呼叫。發布時重新檢查 LowIR 與管線稽核，拒絕重複求值及來自其他函式的證據。一般位元組回傳、原生呼叫猜測以及 patch、lift 模式不使用此正規化。
