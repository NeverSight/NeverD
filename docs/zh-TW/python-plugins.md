**語言**: [English](../python-plugins.md) | [简体中文](../zh-CN/python-plugins.md) | [繁體中文](python-plugins.md) | [日本語](../ja/python-plugins.md) | [한국어](../ko/python-plugins.md) | [Français](../fr/python-plugins.md) | [Deutsch](../de/python-plugins.md) | [Español](../es/python-plugins.md) | [Italiano](../it/python-plugins.md) | [Русский](../ru/python-plugins.md) | [العربية](../ar/python-plugins.md)
...
`session.lowir_concolic` 從明確指定的入口暫存器位元組範圍沿一條原生 LowIR 軌跡執行，只傳回由求解器產生、並經全新重播在同一控制決策出現位置驗證的候選：
...
暫存器偏移是 NeverD 暫存器檔案中的位元組偏移，不是原生指標或暫存器編號。報告始終是非窮舉的；UNSAT、求解器預算耗盡、投影拒絕和重播拒絕均保留為具型別的翻轉結果，而不是例外。
...
`session.sanitize()` 執行實驗性的全點位或拒絕 `binary-sanitizer-v1` 交易；只有完整且一致的驗證 receipt 通過檢查後才回傳凍結的 `SanitizeResult`。非 Darwin 主機會在 lifting、guard 產生、候選建立或命名空間變更前拒絕。`PUBLISH_INDETERMINATE` 與 `PUBLISHED_INCOMPLETE` 都是失敗，且表示目的可能已存在，使用或重試前必須檢查。

Darwin 的完整 receipt 只驗證交易期間持有的目的目錄物件。該目錄可在開啟後重新命名，因此 receipt 不保證原始路徑在交易中或回傳後仍指向該物件，也不是持久且可獨立複核的路徑綁定；稍後重新開啟路徑的程式必須保留外部錨點並重新驗證物件。Python 目前沒有原生全處理程序重播方法；`NativeProcessReplayAdapter` 只是故障關閉的 Phase 0 C++ 可用性／工廠邊界，所有主機都回報能力全 false 且沒有操作表。
