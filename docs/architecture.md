...
Swift Boolean qualification combines the current Objective-C entry ABI, immutable direct-call bytes, exact strong runtime import and complete LowIR consumer proof. It admits one comparison occurrence and rebuilds every other call ABI from the current runtime catalog; native guesses, dynamic dispatch and duplicate occurrences are rejected. This evidence alone neither publishes source nor declares a byte-return ABI.
