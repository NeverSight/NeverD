...
thread/APC/spinlock scheduling, WDM or PnP cancellation, concurrent public scenario requests, full
PnP/power or general hardware. API IRQL ceilings come from `KernelAPIIRQL.def`, with
...
`DriverResources.h` and `DriverResources.def` define fixed `register_bank` assignments, register access, widths, field spellings and shared limits. `DriverScenario` owns JSON/native preflight; `DriverResult` records configuration without duplicating observed bank state. `KernelMMIO` owns each PDO's persistent register values, translated physical intervals, resource epochs, hardware accessibility and independent mapping aliases. `KernelModelResources` owns packed read-only raw/translated START lists and provider/lifecycle integration; actual successful lower START publishes an epoch before upper callbacks, and actual provider device-SET updates D0/D3 accessibility. Failed START or STOP/REMOVE teardown is checked before terminal IRP completion, after upper completion callbacks have had the opportunity to unmap, with no implicit release. Surprise removal immediately denies hardware access. `GuestMemory` and `UnicornBackend` validate whole original CPU/API accesses before register callbacks, sharing the first fault/stop latch so Unicorn splitting cannot cause effects after failure. Fixed assignment restart preserves values, and mapping aliases never own separate bytes. This bounded memory-register bus does not implement arbitrary RAM, resource rebalance, ports, interrupts or DMA.
