...
An opaque direct native call may participate in Boolean normalization only when every physical register and flag has identical bits in both executions at that occurrence. The proof already rejects differing memory observations, retains temporary differences, and repeats this check on loop backedges. This grants no callee ABI, result definition or source binding; publication still requires the original call and dependencies to be independently bound. Recognizable import veneers without a current ABI and invalid existing bindings remain rejected.
