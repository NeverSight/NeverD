...
`KernelModelIRPStack` owns bounded stack cursors, exact-target lower dispatch and completion unwinding over the original guest packet. Inline Copy/Skip/SetCompletion writes remain authoritative. Dispatch status, completion callback control and final `IoStatus` are distinct; pending propagation may occur after dispatch returns. `STATUS_MORE_PROCESSING_REQUIRED` retains packet/MDL/buffer storage until resumed terminal unwinding, including nested completion. `KernelGuestCall` carries a subsystem owner and local token so WDM and WDF nested continuations cannot collide; `DriverSession` retains CPU frames and inherited IRQL. One guest driver may attach above separately owned scenario PDOs; driver-allocated IRPs and power IRPs remain unsupported. WDF attachment/forwarding, live-stack attachment, intermediate detach, changed forwarded majors and targets outside the captured route remain unsupported. Each consumed lower stack location is cleared before the upper completion callback runs.

`DriverPnp.h` and public `DeviceLifecycle.def` own the shared lifecycle enums and typed scenario/report data. `KernelModelPnpDevices` owns stable PDO identity, a separate provider driver inventory and actual AddDevice observations. `KernelModelPnpRequests` ties lifecycle transactions and immutable device identity to the existing IRP record; file admission uses that identity. `KernelModelPnpCompletion` owns actual bus receipt/completion and bounded virtual deadlines. It reuses `KernelModelIRPStack` and owner-tagged continuations without a second packet or synthetic guest callback. Only final upper completion commits lifecycle state. Provider retirement and guest detach/delete remain separate ownership transitions; leaked guest devices are never silently cleaned up. The public subset is resource-free Start/QueryRemove/CancelRemove/Remove, not general PnP, power or KMDF PnP. PnP success requires actual provider completion; an early START/QUERY_REMOVE failure may retain null bus observations. Device/file lifecycle identity persists after detach.
