...
A file-private top-level Swift scalar with a complete private-name mangling, a standard-library scalar type, and one exact writable symbol can use shared rebuilt storage. An address kept in a local remains the same pointer for `swift_beginAccess` and associated-object key uses; direct key arguments use that storage too. The symbol's encoded width, initial bytes, and relocation checks are revalidated before publication. Untyped globals and address fragments remain unbound.
...
Tagged literal words may retain a full-width image address from an immutable C-string pool under the exact high-bit tag, including when a Swift call gives the word a pointer-typed carrier. Source binding relocates only that proven address into the existing permanent shared pool and preserves the integer OR, tag and interior offset. Scalar or partial address lookalikes, conflicting provenance, mutable or relocated pools and direct memory-address uses remain rejected; this does not infer a Swift String object layout.

An exported static `String` addressor can retain its address when the storage is a complete read-only, pointer-free 16-byte small ASCII String. Generated source uses one aligned immutable cell per proven symbol and preserves its bytes and shared address. Dynamic, large, mutable, relocated, or non-exported String storage is not inferred from the addressor name.
...
ARM64 Objective-C source recovery supports up to eight caller-proven Swift string comparisons with a raw one-bit result. It verifies the current entry ABI, strong import, exact call occurrence and every consumer before zero-extending the normalized value. Generated C declares the runtime as `_Bool` with `swiftcall` and explicitly converts its result to `uint8_t`; the original masks and surrounding calls remain. Publication repeats the LowIR and pipeline-audit checks and rejects duplicate evaluations or foreign-function evidence. A direct native callee with a current explicit source ABI may use its declared inputs and complete result carriers in this proof; its source body and dependency closure must still validate. Ordinary byte returns, native-call guesses, patch mode and lift mode do not gain this normalization. Each occurrence must pass its own proof; other raw Boolean calls establish input locations only, without defining any result bytes. Publication matches each surviving evaluation to a distinct current machine occurrence.
...
Swift value-witness call recognition now traces authenticated immutable image metadata, including an ADRP page plus offset, through the exact witness-table load. Literal metadata requires a mapped, file-backed eight-byte slot immediately before it; numeric lookalikes, out-of-image addresses and truncated slots do not qualify. This proves the indirect call ABI only. Relocation and source-closure checks still determine whether the method can be recovered.
