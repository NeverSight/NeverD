//===- SBFCPI.cpp - Cross-program invocation ABI and decoding -------------===//
//
// NeverD Decompiler
//
//===----------------------------------------------------------------------===//
...
#define SBF_CPI_FIELD(ABI_ID, FIELD, FIELD_OFFSET, FIELD_FORM)                 \
  Built[static_cast<size_t>(CPIABI::ABI_ID)]                                   \
      ...
      cpiFieldName(CPIField::FIELD), FIELD_OFFSET, CPIFieldForm::FIELD_FORM};
  ...
  for (const CPIABIInfo &Info : cpiABIInfos()) {
    const auto Check = [&](llvm::StringRef Structure, const CPIFieldInfo &Field,
      ...
      if (Field.Offset + Field.size() > Limit)
        return abiError(
            "the " + Info.Name + " " + Structure + " field '" + Field.Name +
            "' ends at " + llvm::Twine(Field.Offset + Field.size()) +
            " but the structure is only " + llvm::Twine(Limit) + " bytes");
    ...
    for (const CPIFieldInfo &Field : Info.AccountMetaFields)
      if (llvm::Error E =
              Check("account reference", Field, Info.AccountMetaSize))
        return E;
...
llvm::Error validateProgramInstructionTables() {
  ...
  for (const KnownAddressInfo &Address : knownAddressInfos()) {
    ...
    if (!First)
      continue;
    llvm::DenseSet<uint32_t> Tags;
    for (const InstructionSetProgramInfo &Entry :
         instructionSetProgramInfos()) {
