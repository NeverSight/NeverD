//===- RuntimeProfile.cpp - Which runtime the answer is about -----------===//
//
// NeverD Decompiler
//
//===----------------------------------------------------------------------===//
...
#define SBF_LOADER(ID, NAME, KNOWN_ADDRESS, ACCOUNT_ABI, DEPLOYS, EXECUTES,    \
                   SUMMARY)                                                    \
  LoaderInfo{Loader::ID,          NAME,      KnownAddress::KNOWN_ADDRESS,      \
             AccountABI::ACCOUNT_ABI, (DEPLOYS), (EXECUTES),                   \
             SUMMARY},
...
llvm::Error validateRuntimeProfileTables() {
  ...
  for (const RuntimeFeatureInfo &Info : runtimeFeatureInfos()) {
    ...
    if (Info.Address.empty())
      return profileError("gate '" + Info.Name + "' has no account, so no "
