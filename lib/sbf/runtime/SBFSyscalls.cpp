//===- SBFSyscalls.cpp - Solana runtime syscall metadata ------------------===//
//
// NeverD Decompiler
//
//===----------------------------------------------------------------------===//
...
#define SBF_SYSCALL_MEMORY_OPAQUE(SYSCALL, ARGUMENT, ACCESS)                   \
  opaqueWindow(Syscall::SYSCALL, SyscallArgument::ARGUMENT,                    \
               SyscallMemoryAccess::ACCESS),
#include "neverd/sbf/runtime/SBFSyscallMemory.def"
  };
  return Table;
}
...
llvm::ArrayRef<SyscallMemoryInfo> getSyscallMemory(Syscall ID) {
  ...
  if (First == Table.end())
    return {};
  const auto Last =
      std::find_if(First, Table.end(), [&](const SyscallMemoryInfo &Info) {
        return Info.ID != ID;
      });
