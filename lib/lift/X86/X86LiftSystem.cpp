...
bool liftSystem(X86Lifter &L, X86Lifter::LiftState &S, const cs_insn *Insn,
  ...
  case X86_INS_INT: {
    ...
    if (InsnId == X86_INS_INT && X86.op_count >= 1 &&
      ...
      if (Vector == 0x2D && L.targetArch() == Arch::X64) {
        // x64 Windows debug service (DebugPrint, DebugPrompt, ...): EAX holds
        // the service code, RCX/RDX/R8/R9 its arguments; the status comes
        // back in RAX.  The input order is x86DebugServiceRegisters().
        S.emitIntrinsic(Intrinsic::DebugService, NdVar::reg(x86reg::RAX, 8),
                        {NdVar::reg(x86reg::RAX, 8), NdVar::reg(x86reg::RCX, 8),
