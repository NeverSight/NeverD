...
KernelFramework::callControl(llvm::StringRef Name, Binding &B,
  ...
  if (Name == api::WdfFdoInitSetFilter) {
    auto I = DeviceInits.find(A[1]);
    if (I == DeviceInits.end() || I->second.Binding != B.Globals ||
        I->second.Kind != DeviceInitKind::Pnp)
      return controlError(
          "filter registration requires a live FDO initializer");
  ...
  if (Name == api::WdfDeviceCreate) {
    ...
    const bool ForwardsFiles = I->second.Files.forwards(I->second.Filter);
    if (ForwardsFiles && I->second.Files.Create &&
        I->second.Files.Class != FileObjectNotRequired)
      return controlError(
          "forwarded file-create callbacks with framework file objects "
          "require completion ownership beyond this profile");
  ...
  if (Name != api::WdfDeviceCreateSymbolicLink &&
      ...
      Name != api::WdfDeviceGetDriver && Name != api::WdfDeviceGetIoTarget)
    return Result{};
  auto O = Objects.find(A[1]);
  auto D = Devices.find(A[1]);
  ...
  if (Name == api::WdfDeviceGetIoTarget) {
    ...
    if (!D->second.LocalTarget) {
      ...
      D->second.LocalTarget = *Target;
    }
    return Result{D->second.LocalTarget};
  }
  if (O->second.Deleting)
