...
public:
  enum class ExportKind { ModuleExport, FrameworkFunction };
  ...
  struct Export {
    ...
    /// The framework's guest globals identity, never a host pointer. Zero for
    /// ordinary PE imports. The framework model owns binding lifetime checks.
    uint64_t Binding = 0;
  };
