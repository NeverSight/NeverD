...
///
/// \file
/// Emitter that translates MedIR functions to LLVM IR modules.
///
...
private:
  ...
  mutable std::map<SlotKey, bool> SlotAddressEscapesCache;
  mutable std::map<SlotKey, bool> SlotMatchingKeyLoadCache;
  mutable std::map<SlotKey, bool> SlotReloadUsedLocallyCache;
  ...
  struct PendingDispatchStore {
    ...
    std::vector<std::pair<int, MedVar>>
        Preds; ///< (predecessor block id, index)
  };
  ...
  std::map<uint64_t, std::vector<SubRegAllocInfo>> SubRegPropMap;
