//===- ARM32_SwitchTests.cpp - Switch / jump-table tests (ARM32) ----------===//
//
// NeverD Decompiler
//
//===----------------------------------------------------------------------===//
...
class ARM32_SwitchTest : public NeverDLiftTest {};

static fs::path switchObj() {
  return fs::path(TEST_OBJ_DIR) / "test_switch_arm.o";
}

TEST_F(ARM32_SwitchTest, LowIRProduced) {
  auto R = liftToLowIR(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
  EXPECT_FALSE(R.out.empty());
}

TEST_F(ARM32_SwitchTest, MedIRProduced) {
  auto R = liftToMedIR(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
  EXPECT_FALSE(R.out.empty());
}

TEST_F(ARM32_SwitchTest, HighIRProduced) {
  auto R = liftToHighIR(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
  EXPECT_FALSE(R.out.empty());
}

TEST_F(ARM32_SwitchTest, LLVMIRUnoptProduced) {
  auto R = liftToLLVMIRUnopt(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
  EXPECT_FALSE(R.out.empty());
}

TEST_F(ARM32_SwitchTest, SwitchFunctionsDetected) {
  auto R = liftToLowIR(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
...
TEST_F(ARM32_SwitchTest, DecompileProducesOutput) {
...
TEST_F(ARM32_SwitchTest, LLVMIRUnoptNoVerifierErrors) {
  auto R = liftToLLVMIRUnopt(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
...
TEST_F(ARM32_SwitchTest, MultipleFunctionsLifted) {
  auto R = liftToLowIR(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
  int FuncCount = 0;
...
TEST_F(ARM32_SwitchTest, HighIRContainsSwitchKeyword) {
  auto R = liftToHighIR(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
...
TEST_F(ARM32_SwitchTest, PatchProducesOutput) {
  auto ElfExe = fs::path(TEST_OBJ_DIR) / "test_switch_arm_elf";
  if (!fs::exists(ElfExe))
    GTEST_SKIP() << "ELF executable not built (ld.lld not available)";
  auto R = patchBinary(ElfExe);
  ASSERT_EQ(R.exitCode, 0) << "ARM32 switch patch failed: " << R.err;
...
TEST_F(ARM32_SwitchTest, LowIRHasIndirectOrConditional) {
  auto R = liftToLowIR(switchObj());
  ASSERT_EQ(R.exitCode, 0) << R.err;
