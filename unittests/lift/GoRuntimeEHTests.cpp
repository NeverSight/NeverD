...
/// `FUNCDATA_OpenCodedDeferInfo`: two unsigned varints, the distance below
/// varp of the defer bitmask byte and of the first closure slot.
std::vector<uint8_t> buildOpenCodedDeferInfo(uint32_t DeferBits,
                                             uint32_t Slots) {
  ByteBuilder B;
  B.varint(DeferBits);
  B.varint(Slots);
...
TEST(GoLegacyPclnTab, ReadsOpenCodedDeferInfoFromItsPreGo116Index) {
  GoTestImage T;
  const va_t DeferInfoVA = T.addPayload(buildOpenCodedDeferInfo(0x18, 0x10));
  GoFuncSpec Work = makeDeferringFunc("main.work", kTextVA + 0x100);
